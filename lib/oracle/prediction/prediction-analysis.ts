...
function calculateProjectedRatingChange(
  trend: TrendProfile
): number {
  ...
  const skillWeight =
    trend.skillTrends.length > 0
      ? average(
          trend.skillTrends.map((skill) => skill.change)
        ) * 0.4
      : 0;
...
function calculateProjectedWinChange(
  trend: TrendProfile
): number {
  if (trend.sampleSize < 2) {
    return 0;
...
function buildPredictedSkills(
  trend: TrendProfile
): PredictedSkill[] {
  return trend.skillTrends.map((skill) => {
    const expectedChange = Math.round(skill.change * 0.75);
    const predicted = clampScore(
      skill.latestValue + expectedChange
    );
...
function calculateSessionsToNextTier(
  ...
  const expectedGainPerSession = Math.max(
    1,
    momentumScore / 3
  );
...
function calculatePlateauRisk(
  ...
  if (
    ...
    trend.momentum === "strong_positive"
  ) {
    return "low";
  }
