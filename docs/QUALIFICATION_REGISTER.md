...
| 2 | Candidate Freeze and Package Reconciliation | **R5 failed with zero residue; R6 prepared and one attempt authorised** | R5 authority is consumed and immutable; R6 binds the same corrected product candidate under unique package version `0.1.4.0` and hash-binds the R5 failure | Execute only the exact R6 governed wrapper; reconcile and accept evidence before downstream preparation |
