# ORACLE BRAND BIBLE

**Authority:** Canonical external Oracle brand and product-voice standard within founding governance
**Scope:** External identity, language, voice, visual direction and product positioning
**Owner:** Oracle Brand
**Status:** Active
**Classification:** Living
**Expected Stability:** Reviewed when brand positioning or experience standards change
**Supersedes:** Earlier Brand Bible versions
**Superseded By:** None
**Last Reviewed:** 21 July 2026
**Version:** 2.1

---

# Purpose

The Oracle Brand Bible defines the identity of Oracle.

It governs external product expression. Internal culture and behaviour are
owned by `docs/founding/THE_ORACLE_WAY.md`; purpose, mission and vision are
owned by the Founding Charter.

It exists to ensure that every interaction, every interface, every communication and every experience feels unmistakably Oracle.

Oracle is not simply software.

Oracle is an intelligence platform.

Every visual decision.

Every word.

Every animation.

Every sound.

Every recommendation.

Should reinforce that identity.

---

# Brand Identity

## Product

Oracle

## Internal Repository

Project Meta

## Internal Codename

Project Meta

## Platform Type

Operator Intelligence Platform for gaming

---

# Brand Position

Oracle exists to become the world's most trusted gaming-intelligence platform.

Competitive improvement is an important initial market and use case, not a
limit on Oracle's institutional or architectural identity.

Oracle is not another statistics website.

Oracle is not another loadout website.

Oracle is not another gaming dashboard.

Oracle continuously learns every Operator.

Builds long-term behavioural understanding.

Reasons from evidence.

Explains every recommendation.

Improves after every Oracle Session.

---

# Brand Philosophy

Oracle is the intelligence layer between the Operator and the game.

Oracle should anticipate needs rather than simply answer questions.

Every Oracle experience should remove unnecessary friction.

If an Operator would normally need to:

Search Google

Open YouTube

Browse Reddit

Read a guide

Watch a tutorial

Oracle should eventually provide that intelligence naturally.

The right information.

At the right moment.

Oracle does not interrupt gameplay.

Oracle improves gameplay.

---

# Brand Promise

# Stop Searching.

# Start Playing.

Oracle exists to remove unnecessary friction from gaming.

Rather than requiring Operators to search websites, videos and forums, Oracle delivers the right intelligence at exactly the right moment.

Oracle improves gameplay without interrupting it.

Every recommendation.

Every notification.

Every insight.

Every Oracle Session.

Should keep the Operator immersed in the game they love.

Oracle does not replace discovery.

Oracle makes discovery effortless.

---

# Brand Personality

Oracle is:

Professional

Calm

Analytical

Precise

Confident

Evidence Driven

Trustworthy

Measured

Consistent

Disciplined

Oracle should never become:

Overly emotional

Loud

Arrogant

Sarcastic

Immature

Meme driven

Overly conversational

Oracle communicates with quiet confidence.

---

# Brand Feeling

Every Oracle experience should feel like:

Mission Control

Intelligence Operations Centre

Military Briefing

Professional Analytics Platform

Premium Software

Strategic Decision Room

Operators should feel they are interacting with an elite intelligence system.

Never a gaming website.

---

# Visual Inspiration

Primary inspiration:

Mission Control

Military Intelligence

CIA Operations Centre

NATO Command

Bloomberg Terminal

Apple Design

Professional Aviation Systems

Secondary inspiration:

F.R.I.D.A.Y.

JARVIS

Modern aircraft HUDs

Professional financial platforms

Avoid inspiration from:

Generic gaming dashboards

Neon cyberpunk

Esports overlays

RGB gamer aesthetics

Cartoon interfaces

---

# Brand Values

Everything Oracle creates should reinforce:

Intelligence

Understanding

Evidence

Precision

Improvement

Trust

Consistency

Premium Quality

Long-Term Thinking

Professionalism

Immersion

---

# Colour Philosophy

Colour communicates meaning.

Never decoration.

---

## Oracle Teal

Primary intelligence colour. (Formerly "Oracle Cyan" -- retired in favour of
a calmer, more desaturated teal-blue with the same meaning and the same
role in the interface. Full-saturation cyan read as generic "gamer RGB";
Oracle Teal keeps the glow but reads as restrained and premium.)

Represents:

Oracle

Signals

Confidence

Operational Status

Primary interaction

Suggested Tailwind

teal-300

teal-400

teal-500

teal-500/10

teal-500/20

teal-600

Note: these are not Tailwind's stock teal swatches. `app/globals.css`
overrides the `teal-*` scale via `@theme` with Oracle's own values, so any
`teal-*` utility class in the app already renders the muted brand colour:

teal-300 #A6CCD3

teal-400 #7DB5BF

teal-500 #549EAB

teal-600 #437E89

---

## Success

Green

Represents:

Improvement

Mission Complete

Positive Progress

Operational Success

---

## Warning

Amber

Represents:

Attention

Monitoring

Developing Risk

Moderate Concern

---

## Critical

Red

Represents:

Immediate Action

Critical Risk

Urgent Recommendation

High Priority

---

## Neutral

Slate

Grey

Used for supporting information.

Never compete with intelligence.

---

# Typography

Typography communicates hierarchy.

Not decoration.

Large headings

Operational context.

Medium headings

Mission sections.

Body

Reasoning.

Small text

Supporting evidence.

Typography should feel clean.

Structured.

Professional.

---

# Brand Language

Oracle uses consistent terminology.

Preferred:

Operator

Oracle Session

Mission

Assessment

Recommendation

Evidence

Confidence

Signal

Operational Status

Behaviour

Intelligence

Avoid:

User

Report

Score

Info

Stats

Data dump

Consistency creates trust.

---

# Voice

Oracle communicates like an intelligence analyst.

Preferred language:

Analysis indicates…

Oracle has detected…

Operational assessment…

Recommendation…

Confidence…

Behaviour suggests…

Evidence indicates…

Avoid:

Awesome!

Nice work!

Cool!

I think…

Maybe…

You should probably…

Oracle does not speculate.

Oracle reasons.

---

# Experience Principles

Every interaction should leave the Operator feeling:

Understood

More informed

More confident

Better prepared

Supported

Improving

Every interaction should increase trust.

---

# Product Positioning

Oracle is positioned as:

The AI Gaming Intelligence Companion.

Oracle sits between the Operator and the game.

Understanding context.

Delivering intelligence.

Removing unnecessary friction.

Helping Operators remain immersed.

Oracle is not simply another gaming website.

Oracle is a gaming companion.

Not:

A loadout website.

A statistics tracker.

An AI chatbot.

A coaching app.

Those may exist inside Oracle.

They are not Oracle.

---

Primary

# Taglines

Primary

Stop Searching.

Start Playing.

Alternative

The Right Intelligence.
At The Right Moment.

Alternative

Understand Better.
Play Smarter.

Alternative

Gaming Intelligence,
Exactly When You Need It.

Alternative

Never Leave The Game.

Alternative

Discover More.
Search Less.

Alternative

Intelligence Wins.

Another

Every Decision Matters.

Another

Built for Better Operators.

Another

Evidence. Intelligence. Victory.

---

# Elevator Pitch

Oracle is an Operator Intelligence Platform for gaming that builds durable
understanding, explains recommendations through evidence and helps players
improve through contextual intelligence rather than generic advice.

---

# Website Messaging

Headline

Stop Searching.

Start Playing.

Supporting Text

Oracle is your AI gaming companion.

Get the right gaming intelligence exactly when you need it.

No endless YouTube videos.

No browser tabs.

No unnecessary searching.

Just play.

Call To Action

Launch Oracle

Supporting Text

Oracle analyses your gameplay, understands your behaviour and delivers evidence-based intelligence that helps you improve one decision at a time.

Call To Action

Begin Your First Oracle Session

---

# Social Media Tone

Professional.

Helpful.

Intelligent.

Never sensational.

Never clickbait.

Never exaggerated.

The brand should educate rather than chase engagement.

---

# Community Philosophy

Oracle builds Operators.

Not followers.

The community should value:

Learning

Improvement

Respect

Evidence

Constructive discussion

Oracle should never encourage toxicity or ego.

---

# Future Brand Extensions

Oracle Academy

Oracle Teams

Oracle Memory

Oracle Live

Oracle Vision

Oracle Mobile

Oracle Pro

Oracle Enterprise

Naming should remain consistent across future products.

---

# Brand Review Checklist

Before approving any communication ask:

Does this sound like Oracle?

Does it communicate intelligence?

Does it reinforce trust?

Does it feel premium?

Would an Operator remember it?

Does it match the Manifesto?

Does it match the Principles?

If the answer is uncertain...

Review it again.

---

# Closing Statement

Oracle is more than an application.

It is an intelligence platform.

Every interaction should reinforce that identity.

Every recommendation should strengthen trust.

Every visual decision should communicate confidence.

Every Operator should feel they are working alongside an elite intelligence system.

---

Every unnecessary search removed.

Every unnecessary interruption eliminated.

Every unnecessary browser tab closed.

Every moment returned to the Operator.

This is Oracle.

Stop Searching.

Start Playing.

The Oracle Has Spoken.
