# Sprint 30.5 Stage 3 Qualification R5 Pre-Execution Gate

**Status:** R5 final preparation authorised; execution blocked
**Programme identity:** `Sprint 30.5 Stage 3 Qualification R5`
...
The gate proves that the R5 preparation system, still bound exclusively to
the accepted Stage 2 R2 candidate, is ready to receive a separate Founder
execution decision. Founder authority in the R5 final-preparation
mission permits one create-only R5 transfer on the already
...
| Transfer | Authorised for construction | Create one R5 sibling on the approved USB and bind its exact manifest and custody record |
...
Under the Founder-approved R5 final-preparation authority:

```powershell
npm.cmd run sprint-30-5:stage-3:r5:prepare-transfer -- `
  --founder-authority FOUNDER-AUTHORISED-STAGE3-R5-TRANSFER `
  --transfer-id <transfer-stage3-r5-YYYYMMDDTHHMMSSmmmZ-xxxxxxxx> `
...
powershell.exe -NoProfile -ExecutionPolicy Bypass `
  -File <verified-local-transfer-copy>\payload\Get-OracleStage3R5HostContinuity.ps1 `
  -OutputPath <existing-isolated-root>\Oracle.Stage3R5HostContinuity.json
```
...
powershell.exe -NoProfile -NonInteractive -ExecutionPolicy Bypass `
  -File <verified-local-transfer-copy>\payload\Invoke-OracleStage3R5PreAuthorityPreflight.ps1 `
...
```powershell
powershell.exe -NoProfile -ExecutionPolicy Bypass `
  -File <verified-local-transfer-copy>\payload\Invoke-OracleStage3R5Qualification.ps1 `
  -FounderAuthorityToken FOUNDER-AUTHORISED-STAGE3-R5-EXECUTION `
  -AuthorityId <authority-stage3-r5-YYYYMMDDTHHMMSSmmmZ-xxxxxxxx> `
  -AttemptId <stage3-r5-YYYYMMDDTHHMMSSmmmZ-xxxxxxxx> `
...
1. the exact committed Stage 3 R5 preparation revision;
2. the R5 transfer root, transfer identity, manifest hash and custody hash;
