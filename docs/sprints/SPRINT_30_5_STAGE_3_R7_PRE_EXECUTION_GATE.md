# Sprint 30.5 Stage 3 Qualification R7 Pre-Execution Gate

**Status:** R7 launch-correction preparation authorised; transfer and execution blocked
**Programme identity:** `Sprint 30.5 Stage 3 Qualification R7`
...
The gate defines the review boundary for the R7 preparation system, still
...
immutable. R7 uses an elevated process and exact machine-scoped
...
That R6 assertion is retired. R7 uses
...
The current R7 name belongs to the attempt-scoped Stage 3 qualification
...
| --- | --- | --- |
| Governance | Conditional ready | Founder must separately authorise one R7 transfer and later one execution |
...
| Transfer | Not authorised | Founder must authorise one create-only R7 transfer and bind its exact manifest and custody record |
...
```powershell
npm.cmd run sprint-30-5:stage-3:r7:prepare-transfer -- `
  --founder-authority FOUNDER-AUTHORISED-STAGE3-R7-TRANSFER `
  --transfer-id <transfer-stage3-r7-YYYYMMDDTHHMMSSmmmZ-xxxxxxxx> `
...
powershell.exe -NoProfile -ExecutionPolicy Bypass `
  -File <verified-local-transfer-copy>\payload\Get-OracleStage3R7HostContinuity.ps1 `
  -OutputPath <existing-isolated-root>\Oracle.Stage3R7HostContinuity.json
```
...
powershell.exe -NoProfile -NonInteractive -ExecutionPolicy Bypass `
  -File <verified-local-transfer-copy>\payload\Invoke-OracleStage3R7PreAuthorityPreflight.ps1 `
...
```powershell
powershell.exe -NoProfile -ExecutionPolicy Bypass `
  -File <verified-local-transfer-copy>\payload\Invoke-OracleStage3R7Qualification.ps1 `
  -FounderAuthorityToken FOUNDER-AUTHORISED-STAGE3-R7-EXECUTION `
  -AuthorityId <authority-stage3-r7-YYYYMMDDTHHMMSSmmmZ-xxxxxxxx> `
  -AttemptId <stage3-r7-YYYYMMDDTHHMMSSmmmZ-xxxxxxxx> `
...
```powershell
npm.cmd run sprint-30-5:stage-3:r7:verify-return -- `
...
1. the exact committed Stage 3 R7 preparation revision;
2. the R7 transfer root, transfer identity, manifest hash and custody hash;
