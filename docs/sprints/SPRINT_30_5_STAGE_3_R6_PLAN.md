# Sprint 30.5 Stage 3 Qualification R6 - Preparation and Execution Plan
...
historical R1–R5 preparations, transfers, authorities, attempts, failure
evidence or reconciliation. It does not repair or reuse the accepted failed
R5 attempt. It qualifies the exact accepted Stage 2 R2 package on a separately
admitted clean Windows host. Preparation authority does not authorise transfer,
trust, installation or execution. R6 transfer construction requires a
...
## R6 AppX trust-scope correction

The accepted R5 attempt passed package identity, untrusted-package rejection,
certificate extraction, exact certificate import and strict current-user
Authenticode validation, then failed closed when Windows AppX deployment
rejected the package with HRESULT `0x800B0109` under activity ID
`a549c0c7-1d3c-0004-cd5a-b7a63c1ddd01`.

R5 established only `CurrentUser\Root`. That state is sufficient for
`Get-AuthenticodeSignature` in the invoking user process but is not the
machine-scoped package trust required by the Windows AppX deployment provider.
Oracle's preserved earlier revision evidence independently records the same
store/HRESULT relationship.

R6 requires an elevated 64-bit Windows PowerShell 5.1 process during the
read-only pre-authority gate. It uses deterministic, shell-free
`certutil.exe -addstore TrustedPeople <exact-cer>` without `-user` or `-f`.
It verifies exactly one physical certificate at
`LocalMachine\TrustedPeople` using the corresponding machine registry store.
An inherited `CurrentUser\TrustedPeople` view is classified as a logical
projection, not a second physical trust entry.

Every unexpected physical store, logical store, duplicate, subject,
thumbprint, raw-byte or private-key state fails closed. Strict Authenticode
`Status = Valid` remains mandatory. Teardown uses only
`certutil.exe -delstore TrustedPeople <exact-thumbprint>` and then requires
zero physical and logical residue. The governed process envelope records every
CertUtil invocation, and partial import failure enters the same exact teardown
path. No accepted package byte changes.

This design follows the Windows deployment trust boundary rather than treating
the invoking user's Authenticode result as package-deployment admission.
Microsoft's
[MSIX troubleshooting guidance](https://learn.microsoft.com/en-us/windows/msix/msix-troubleshooting-guide)
identifies `0x800B0109` as an untrusted-root failure and directs testing
certificates to the local-computer Trusted People store. Microsoft's
[package-signing certificate guidance](https://learn.microsoft.com/en-gb/windows/msix/package/create-certificate-package-signing)
likewise imports the certificate into Local Machine / Trusted People.
[CertUtil](https://learn.microsoft.com/en-us/windows-server/administration/windows-commands/certutil)
provides the noninteractive machine-store add and exact-certificate delete
operations used by the governed process envelope.

## Revision lineage and historical retirement

`Sprint 30.5 Stage 3 Qualification R6` is the sixth revision in the current
...
That legacy design remains historical and non-operational; the R6 contract
records this distinction explicitly.

Repository operational entry points for R1 through R5 transfer construction
...
- authority `authority-stage3-r6-YYYYMMDDTHHMMSSmmmZ-xxxxxxxx`;
- attempt `stage3-r6-YYYYMMDDTHHMMSSmmmZ-xxxxxxxx`;
...
and Stage 3 R6 kit. It creates a unique destination, copies bytes without replacement,
...
hardware serial `5F10110403558`, volume serial `783A-2CD4`) may host R6 only
...
R6 transfer root, identity, manifest and custody hashes; the presence of a
...
Admission copies the approved R6 directory create-only into a new isolated
...
- the exact R6 transfer on the approved medium or another separately approved
  isolated method;
