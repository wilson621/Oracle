# Sprint 30.5 Stage 3 Qualification R2 Pre-Execution Gate

**Status:** R2 corrective preparation authorised; execution blocked
**Programme identity:** `Sprint 30.5 Stage 3 Qualification R2`
...
The gate proves that the R2-bound preparation system is ready to receive a
separate Founder execution decision. Founder authority in the R2 corrective
preparation mission permits one create-only R2 transfer on the already
approved USB. It does not authorise certificate trust, package installation,
Stage 3 execution or qualification evidence.

R2 corrects only the confirmed Windows computer-name comparison defect.
`FOUNDER-QA-01` and `Founder-QA-01` compare equal using ordinal
case-insensitive equality, while the raw observed value remains unchanged in
evidence. A materially different name remains a stop condition.
...
| --- | --- | --- |
| Governance | Ready | One R2 transfer is authorised; Founder must separately authorise execution |
...
| Transfer | Authorised for construction | Create one R2 sibling on the approved USB and bind its exact manifest |
...
```powershell
npm.cmd run sprint-30-5:stage-3:r2:validate
...
Under the Founder-approved R2 corrective-preparation authority:

```powershell
npm.cmd run sprint-30-5:stage-3:r2:prepare-transfer -- `
  --founder-authority FOUNDER-AUTHORISED-STAGE3-R2-TRANSFER `
  --transfer-id <transfer-stage3-r2-YYYYMMDDTHHMMSSmmmZ-xxxxxxxx> `
...
powershell.exe -NoProfile -ExecutionPolicy Bypass `
  -File <transfer>\payload\Get-OracleStage3R2HostContinuity.ps1 `
  -OutputPath <existing-isolated-root>\Oracle.Stage3R2HostContinuity.json
```
...
```powershell
powershell.exe -NoProfile -ExecutionPolicy Bypass `
  -File <transfer>\payload\Invoke-OracleStage3R2Qualification.ps1 `
  -FounderAuthorityToken FOUNDER-AUTHORISED-STAGE3-R2-EXECUTION `
  -AuthorityId <authority-stage3-r2-YYYYMMDDTHHMMSSmmmZ-xxxxxxxx> `
  -AttemptId <stage3-r2-YYYYMMDDTHHMMSSmmmZ-xxxxxxxx> `
...
```powershell
npm.cmd run sprint-30-5:stage-3:r2:verify-return -- `
...
1. the exact committed Stage 3 R2 preparation revision;
2. the R2 transfer root, transfer identity and manifest hash;
