# Sprint 30.5 Stage 3 Qualification R9 Pre-Execution Gate

**Status:** R9 observation/teardown correction preparation authorised; transfer and execution blocked
...
The gate defines the review boundary for the R9 preparation system, still
...
R9 additionally requires its transfer-bound observation policy to prove at
least 60,000 monotonic milliseconds from the first valid captured sample to a
final valid captured sample. Sample count and nominal deadlines are not
duration evidence. Its window policy must pass Windows PowerShell 5.1
discovery-array and teardown-race fixtures and load the Windows AppModel
process-identity API before authority creation. Runtime observation and
teardown bind each live governed Oracle PID to the installed package-family
name; only a typed OpenProcess failure followed immediately by proof that the
PID is absent is a safe exit race.
...
immutable. R9 uses an elevated process and exact machine-scoped
...
R6 proved that machine trust and package installation succeed. Windows then
recorded registered application activation and Desktop AppX container
creation, but the harness failed because it treated the exit status of
`explorer.exe shell:AppsFolder...` as authoritative activation evidence.
That R6 assertion is retired. R9 uses
`IApplicationActivationManager.ActivateApplication` through
`CLSCTX_LOCAL_SERVER`; it requires `S_OK` and a non-zero process ID, then
retains exact `Oracle.WindowDiscovery` and `Oracle.WindowObserver` runtime
proof. The pre-authority gate probes COM availability without launching an
application.

The current R9 name belongs to the attempt-scoped Stage 3 qualification
...
entry points for R1 through R6 are retired and fail closed; historical
...
| --- | --- | --- |
| Governance | Conditional ready | Founder must separately authorise one R9 transfer and later one execution |
| Candidate | Ready | Exact R2 hashes must revalidate |
| Harness | Ready | Founder must approve the committed preparation revision |
| Application activation | Ready | Shared direct activation policy requires `S_OK` and non-zero PID; native window observation remains mandatory |
| Windows executable resolution | Ready | Only CertUtil and ReAgentC remain System32-bound; Explorer is excluded from the governed process path |
...
| Transfer | Not authorised | Founder must authorise one create-only R9 transfer and bind its exact manifest and custody record |
...
```powershell
npm.cmd run sprint-30-5:stage-3:r9:prepare-transfer -- `
  --founder-authority FOUNDER-AUTHORISED-STAGE3-R9-TRANSFER `
  --transfer-id <transfer-stage3-r9-YYYYMMDDTHHMMSSmmmZ-xxxxxxxx> `
...
powershell.exe -NoProfile -ExecutionPolicy Bypass `
  -File <verified-local-transfer-copy>\payload\Get-OracleStage3R9HostContinuity.ps1 `
  -OutputPath <existing-isolated-root>\Oracle.Stage3R9HostContinuity.json
```
...
powershell.exe -NoProfile -NonInteractive -ExecutionPolicy Bypass `
  -File <verified-local-transfer-copy>\payload\Invoke-OracleStage3R9PreAuthorityPreflight.ps1 `
...
```powershell
powershell.exe -NoProfile -ExecutionPolicy Bypass `
  -File <verified-local-transfer-copy>\payload\Invoke-OracleStage3R9Qualification.ps1 `
  -FounderAuthorityToken FOUNDER-AUTHORISED-STAGE3-R9-EXECUTION `
  -AuthorityId <authority-stage3-r9-YYYYMMDDTHHMMSSmmmZ-xxxxxxxx> `
  -AttemptId <stage3-r9-YYYYMMDDTHHMMSSmmmZ-xxxxxxxx> `
...
```powershell
npm.cmd run sprint-30-5:stage-3:r9:verify-return -- `
...
1. the exact committed Stage 3 R9 preparation revision;
2. the R9 transfer root, transfer identity, manifest hash and custody hash;
