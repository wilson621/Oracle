# Sprint 30.5 Stage 3 Qualification R2 - Preparation and Execution Plan
...
historical Stage 3 plans, R1 preparation, transfers, attempts, evidence or
reconciliation. It qualifies
...
R2 corrects the R1 Windows computer-name comparison defect confirmed on
`Founder-QA-01`. Windows computer-name identity uses explicit ordinal
case-insensitive equality, so the raw observed value `FOUNDER-QA-01` matches
the governed identity `Founder-QA-01`. The collector and qualification
harness share one transferred identity-policy implementation. Evidence keeps
the raw observed casing unchanged. Any materially different computer name
...
- authority `authority-stage3-r2-YYYYMMDDTHHMMSSmmmZ-xxxxxxxx`;
- attempt `stage3-r2-YYYYMMDDTHHMMSSmmmZ-xxxxxxxx`;
...
and Stage 3 R2 kit. It creates a unique destination, copies bytes without replacement,
...
hardware serial `5F10110403558`, volume serial `783A-2CD4`) may host R2 in a
new create-only sibling directory. The approved R1 transfer remains immutable.
Admission must bind the exact R2 transfer root, identity and manifest hash;
the presence of R1 does not authorise its use. Any different physical or
...
- the exact R2 transfer on the approved medium or another separately approved
  isolated method;
