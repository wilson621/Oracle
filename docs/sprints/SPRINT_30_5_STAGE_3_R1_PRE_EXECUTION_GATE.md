# Sprint 30.5 Stage 3 Qualification R1 Pre-Execution Gate

**Status:** Complete - ready for a separate Founder execution decision
...
separate Founder decision. It does not authorise transfer construction,
certificate trust, package installation, Stage 3 execution or qualification
evidence.
...
| --- | --- | --- |
| Governance | Ready | Founder must separately authorise transfer method and one execution |
...
| Transfer | Mechanically ready | Founder must approve one replacement medium or isolated method |
...
Only after the Founder approves the transfer method:

```powershell
npm.cmd run sprint-30-5:stage-3:r1:prepare-transfer -- `
  --founder-authority FOUNDER-AUTHORISED-STAGE3-R1-TRANSFER `
  --transfer-id <transfer-stage3-r1-YYYYMMDDTHHMMSSmmmZ-xxxxxxxx> `
  --timestamp-utc <YYYY-MM-DDTHH:mm:ss.mmmZ> `
  --approved-root <existing-approved-create-only-root> `
  --method <founder-approved-method> `
...
powershell.exe -NoProfile -ExecutionPolicy Bypass `
  -File <transfer>\payload\Get-OracleStage3R1HostContinuity.ps1 `
  -OutputPath <existing-isolated-root>\Oracle.Stage3R1HostContinuity.json
```
...
```powershell
powershell.exe -NoProfile -ExecutionPolicy Bypass `
  -File <transfer>\payload\Invoke-OracleStage3R1Qualification.ps1 `
  -FounderAuthorityToken FOUNDER-AUTHORISED-STAGE3-R1-EXECUTION `
  -AuthorityId <authority-stage3-r1-YYYYMMDDTHHMMSSmmmZ-xxxxxxxx> `
  -AttemptId <stage3-r1-YYYYMMDDTHHMMSSmmmZ-xxxxxxxx> `
...
The Founder must explicitly decide:

1. the replacement physical medium or isolated logical transfer method;
2. the exact committed Stage 3 preparation revision;
3. the transfer root, transfer identity and manifest hash;
4. the fresh `Founder-QA-01` continuity record and hash;
5. the evidence-return root and custody path;
6. one authority and one attempt identity;
7. temporary exact-certificate Root trust;
8. exact R2 package installation, repair/reset and removal; and
9. one execution beginning before `2026-08-26T20:35:39Z`.
