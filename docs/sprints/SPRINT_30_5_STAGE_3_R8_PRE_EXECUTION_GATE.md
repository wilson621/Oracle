# Sprint 30.5 Stage 3 Qualification R8 Pre-Execution Gate

**Status:** R8 launch-correction preparation authorised; transfer and execution blocked
**Programme identity:** `Sprint 30.5 Stage 3 Qualification R8`
...
The gate defines the review boundary for the R8 preparation system, still
...
R8 additionally requires its transfer-bound window policy to pass Windows
PowerShell 5.1 discovery-array fixtures and to load the Windows AppModel
process-identity API before authority creation. Runtime observation and
teardown both bind each governed Oracle PID to the installed package-family
name; filesystem naming alone is never ownership evidence.
...
immutable. R8 uses an elevated process and exact machine-scoped
...
That R6 assertion is retired. R8 uses
...
The current R8 name belongs to the attempt-scoped Stage 3 qualification
...
| --- | --- | --- |
| Governance | Conditional ready | Founder must separately authorise one R8 transfer and later one execution |
...
| Transfer | Not authorised | Founder must authorise one create-only R8 transfer and bind its exact manifest and custody record |
...
```powershell
npm.cmd run sprint-30-5:stage-3:r8:prepare-transfer -- `
  --founder-authority FOUNDER-AUTHORISED-STAGE3-R8-TRANSFER `
  --transfer-id <transfer-stage3-r8-YYYYMMDDTHHMMSSmmmZ-xxxxxxxx> `
...
powershell.exe -NoProfile -ExecutionPolicy Bypass `
  -File <verified-local-transfer-copy>\payload\Get-OracleStage3R8HostContinuity.ps1 `
  -OutputPath <existing-isolated-root>\Oracle.Stage3R8HostContinuity.json
```
...
powershell.exe -NoProfile -NonInteractive -ExecutionPolicy Bypass `
  -File <verified-local-transfer-copy>\payload\Invoke-OracleStage3R8PreAuthorityPreflight.ps1 `
...
```powershell
powershell.exe -NoProfile -ExecutionPolicy Bypass `
  -File <verified-local-transfer-copy>\payload\Invoke-OracleStage3R8Qualification.ps1 `
  -FounderAuthorityToken FOUNDER-AUTHORISED-STAGE3-R8-EXECUTION `
  -AuthorityId <authority-stage3-r8-YYYYMMDDTHHMMSSmmmZ-xxxxxxxx> `
  -AttemptId <stage3-r8-YYYYMMDDTHHMMSSmmmZ-xxxxxxxx> `
...
1. the exact committed Stage 3 R8 preparation revision;
2. the R8 transfer root, transfer identity, manifest hash and custody hash;
