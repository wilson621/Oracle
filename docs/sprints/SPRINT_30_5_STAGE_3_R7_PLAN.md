# Sprint 30.5 Stage 3 Qualification R7 - Preparation and Execution Plan
...
trust, installation or execution. R7 transfer construction requires a
...
R7 removes Explorer from the activation path. One shared policy invokes
...
`Sprint 30.5 Stage 3 Qualification R7` is the seventh revision in the current
...
That legacy design remains historical and non-operational; the R7 contract
records this distinction explicitly.

Repository operational entry points for R1 through R6 transfer construction
...
- authority `authority-stage3-r7-YYYYMMDDTHHMMSSmmmZ-xxxxxxxx`;
- attempt `stage3-r7-YYYYMMDDTHHMMSSmmmZ-xxxxxxxx`;
...
and Stage 3 R7 kit. It creates a unique destination, copies bytes without replacement,
...
hardware serial `5F10110403558`, volume serial `783A-2CD4`) may host R7 only
...
R7 transfer root, identity, manifest and custody hashes; the presence of a
...
Admission copies the approved R7 directory create-only into a new isolated
...
- the exact R7 transfer on the approved medium or another separately approved
  isolated method;
