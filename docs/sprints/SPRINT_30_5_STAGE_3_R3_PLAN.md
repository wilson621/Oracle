# Sprint 30.5 Stage 3 Qualification R3 - Preparation and Execution Plan
...
historical Stage 3 plans, R1 preparation, the failed R2 preparation, transfers,
attempt identities, evidence or reconciliation. It qualifies
...
or execution. The R3 corrective-preparation authority separately authorises
construction of one create-only R3 transfer on the already approved USB; it
...
R3 preserves the R2 Windows computer-name correction and fixes the
deterministic harness self-path defect exposed by failed identity
...
Under the exclusively governed `powershell.exe -File` invocation, R3 captures
the executing harness path once at script scope, derives `scriptRoot` from
that path and uses the captured path inside `Assert-IdentityAndTransfer`.
...
- authority `authority-stage3-r3-YYYYMMDDTHHMMSSmmmZ-xxxxxxxx`;
- attempt `stage3-r3-YYYYMMDDTHHMMSSmmmZ-xxxxxxxx`;
...
and Stage 3 R3 kit. It creates a unique destination, copies bytes without replacement,
...
hardware serial `5F10110403558`, volume serial `783A-2CD4`) may host R3 in a
new create-only sibling directory. The R1 and failed R2 transfers remain
immutable. Admission must bind the exact R3 transfer root, identity, manifest
and custody hashes; the presence of R1 or R2 does not authorise their use. Any
...
- the exact R3 transfer on the approved medium or another separately approved
  isolated method;
