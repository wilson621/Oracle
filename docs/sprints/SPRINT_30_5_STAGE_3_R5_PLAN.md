# Sprint 30.5 Stage 3 Qualification R5 - Preparation and Execution Plan
...
historical Stage 3 plans, R1 preparation, and failed R2, R3 and R4 preparation,
...
or execution. The R5 final-preparation authority separately authorises
construction of one create-only R5 transfer on the already approved USB; it
...
- authority `authority-stage3-r5-YYYYMMDDTHHMMSSmmmZ-xxxxxxxx`;
- attempt `stage3-r5-YYYYMMDDTHHMMSSmmmZ-xxxxxxxx`;
...
attempt creation, the governed read-only preflight must verify 64-bit Windows
PowerShell 5.1, required commands and parameters, registry shapes, installed
...
and Stage 3 R5 kit. It creates a unique destination, copies bytes without replacement,
...
hardware serial `5F10110403558`, volume serial `783A-2CD4`) may host R5 in a
new create-only sibling directory. The R1 and failed R2, R3 and R4 transfers remain
immutable. Admission must bind the exact R5 transfer root, identity, manifest
and custody hashes; the presence of R1, R2, R3 or R4 does not authorise their use. Any
...
Admission copies the approved R5 directory create-only into a new isolated
...
- the exact R5 transfer on the approved medium or another separately approved
  isolated method;
