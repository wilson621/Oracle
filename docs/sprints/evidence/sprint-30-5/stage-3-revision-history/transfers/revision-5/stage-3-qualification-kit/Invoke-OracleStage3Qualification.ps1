...
$networkIsolationRuleName = "Oracle.Stage3.R5.OutboundIsolation"
$networkIsolationRuleDisplayName = `
  "Oracle Stage 3 Revision 5 temporary outbound isolation"
...
function Invoke-InstallAndStartupContinuation {
  ...
  $diagnosticName = "03a-install-and-startup-r5-diagnostic.json"
  ...
  if (
    ...
    throw "Revision 5 diagnostic evidence already exists. Refusing overwrite."
  }
  if (
  ...
  $diagnostic = [ordered]@{
    schemaVersion = 1
    contract = "oracle.sprint-30-5.stage-3-install-and-startup-r5-diagnostic"
    revision = 5
    result = "failed"
    recordedAtUtc = [DateTime]::UtcNow.ToString("o")
  ...
  try {
    ...
    Assert-EvidenceHash `
      -Filename ([string]$contract.requalification.revision4NegativePathEvidence) `
      -ExpectedSha256 (
    ...
    if ($packages.Count -ne 1) {
      throw "Revision 5 requires exactly one existing installed Oracle package."
    }
    $package = $packages[0]
    if (
    ...
    if ($alreadyRunning.Count -ne 0) {
      throw "Oracle is already running before Revision 5 activation."
    }
    ...
    if ($diagnosticFinalized) {
      ...
      $evidenceWriteFailureName = `
        "03b-install-and-startup-r5-finalisation-failure.json"
      Write-Evidence $evidenceWriteFailureName ([ordered]@{
        schemaVersion = 1
        contract = `
          "oracle.sprint-30-5.stage-3-install-and-startup-r5-finalisation-failure"
        revision = 5
