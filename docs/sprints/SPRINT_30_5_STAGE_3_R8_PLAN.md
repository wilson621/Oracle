# Sprint 30.5 Stage 3 Qualification R8 - Preparation and Execution Plan
...
historical R1-R6 preparations, transfers, authorities, attempts, failure
evidence or reconciliation. It does not repair or reuse the accepted failed
R6 attempt. It qualifies the exact accepted Stage 2 R2 package on a separately
admitted clean Windows host. Preparation authority does not authorise transfer,
trust, installation or execution. R8 transfer construction requires a
...
R7 removed Explorer from the activation path. R8 preserves that shared policy,
which invokes
...
## R7 discovery and teardown failure; R8 correction
...
attempted to cast it to `System.Int64`. R8 requires an array root, explicitly
...
AppModel associated PID `9808` with the governed package. R8 uses
...
`Sprint 30.5 Stage 3 Qualification R8` is the eighth revision in the current
...
That legacy design remains historical and non-operational; the R8 contract
records this distinction explicitly.

Repository operational entry points for R1 through R7 transfer construction
...
- authority `authority-stage3-r8-YYYYMMDDTHHMMSSmmmZ-xxxxxxxx`;
- attempt `stage3-r8-YYYYMMDDTHHMMSSmmmZ-xxxxxxxx`;
...
and Stage 3 R8 kit. It creates a unique destination, copies bytes without replacement,
...
hardware serial `5F10110403558`, volume serial `783A-2CD4`) may host R8 only
...
R8 transfer root, identity, manifest and custody hashes; the presence of a
...
Admission copies the approved R8 directory create-only into a new isolated
...
- the exact R8 transfer on the approved medium or another separately approved
  isolated method;
