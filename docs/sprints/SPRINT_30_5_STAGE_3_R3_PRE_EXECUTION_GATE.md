# Sprint 30.5 Stage 3 Qualification R3 Pre-Execution Gate

**Status:** R3 corrective preparation authorised; execution blocked
**Programme identity:** `Sprint 30.5 Stage 3 Qualification R3`
...
The gate proves that the R3 preparation system, still bound exclusively to
the accepted Stage 2 R2 candidate, is ready to receive a separate Founder
execution decision. Founder authority in the R3 corrective preparation
mission permits one create-only R3 transfer on the already
approved USB. It does not authorise certificate trust, package installation,
Stage 3 execution or qualification evidence.

R3 preserves the R2 Windows computer-name correction and fixes only the
...
immutable and retired. R3 captures the exact executing harness path at script
...
| --- | --- | --- |
| Governance | Ready | One R3 transfer is authorised; Founder must separately authorise execution |
...
| Transfer | Authorised for construction | Create one R3 sibling on the approved USB and bind its exact manifest and custody record |
...
Under the Founder-approved R3 corrective-preparation authority:

```powershell
npm.cmd run sprint-30-5:stage-3:r3:prepare-transfer -- `
  --founder-authority FOUNDER-AUTHORISED-STAGE3-R3-TRANSFER `
  --transfer-id <transfer-stage3-r3-YYYYMMDDTHHMMSSmmmZ-xxxxxxxx> `
...
powershell.exe -NoProfile -ExecutionPolicy Bypass `
  -File <transfer>\payload\Get-OracleStage3R3HostContinuity.ps1 `
  -OutputPath <existing-isolated-root>\Oracle.Stage3R3HostContinuity.json
```
...
```powershell
powershell.exe -NoProfile -ExecutionPolicy Bypass `
  -File <transfer>\payload\Invoke-OracleStage3R3Qualification.ps1 `
  -FounderAuthorityToken FOUNDER-AUTHORISED-STAGE3-R3-EXECUTION `
  -AuthorityId <authority-stage3-r3-YYYYMMDDTHHMMSSmmmZ-xxxxxxxx> `
  -AttemptId <stage3-r3-YYYYMMDDTHHMMSSmmmZ-xxxxxxxx> `
...
1. the exact committed Stage 3 R3 preparation revision;
2. the R3 transfer root, transfer identity, manifest hash and custody hash;
