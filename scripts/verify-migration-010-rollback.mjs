...
const evidenceFile = process.env.SPRINT18_ROLLBACK_EVIDENCE_FILE;
...
async function main() {
  ...
  const countsBefore = await preservationCounts();
  ...
  const countsAfter = await preservationCounts();
  assert.equal(after, before, "Independent catalog differs after rollback");
  assert.equal(countsAfter, countsBefore, "Protected rows changed during rollback");
  ...
  const evidence = {
    ...
    migration010Sha256: migrationHash,
    catalogSha256Before: crypto.createHash("sha256").update(before).digest("hex"),
    catalogSha256After: crypto.createHash("sha256").update(after).digest("hex"),
    catalogIdentical: true,
    preservedRows: JSON.parse(countsAfter.trim()),
    result: "pass",
  };
  if (evidenceFile) {
  ...
  process.stdout.write(
    "Migration 010 rollback and independent catalog verification passed.\n"
  );
}
