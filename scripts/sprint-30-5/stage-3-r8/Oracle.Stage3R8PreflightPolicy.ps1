...
function Get-OracleStage3R8RequiredPropertyValue {
  [CmdletBinding()]
  param(
...
function Assert-OracleStage3R8CommandSurface {
  [CmdletBinding()]
  param()
...
function Get-OracleStage3R8AcceptedPublicCertificate {
  [CmdletBinding()]
  param(
...
function Invoke-OracleStage3R8ReadOnlyProcess {
  [CmdletBinding()]
  param(
...
function Assert-OracleStage3R8NoReparseTraversal([string]$Path) {
...
function Get-OracleStage3R8PreAuthorityObservation {
  [CmdletBinding()]
  param(
    [Parameter(Mandatory = $true)][object]$Contract,
    [Parameter(Mandatory = $true)][string]$TransferRoot,
  ...
  if (
    ...
    -not (Test-OracleStage3R8ProcessIsElevated)
  ) {
  ...
  foreach ($path in @($TransferRoot, $EvidenceReturnRoot, $HostContinuityPath)) {
    Assert-OracleStage3R8NoReparseTraversal $path
  }
  if (
  ...
  $recovery = Invoke-OracleStage3R8ReadOnlyProcess `
  ...
  $manufacturer = [string](Get-OracleStage3R8RequiredPropertyValue `
    $computerSystem "Manufacturer" "Win32_ComputerSystem")
  $model = [string](Get-OracleStage3R8RequiredPropertyValue `
  ...
  foreach ($member in @("TpmPresent", "TpmReady")) {
    [void](Get-OracleStage3R8RequiredPropertyValue $tpm $member "Get-Tpm")
  }
  foreach ($member in @("AntivirusEnabled", "RealTimeProtectionEnabled")) {
    [void](Get-OracleStage3R8RequiredPropertyValue $defender $member "Get-MpComputerStatus")
  }
  if (
    -not $secureBoot -or
    -not [bool](Get-OracleStage3R8RequiredPropertyValue $tpm "TpmPresent" "Get-Tpm") -or
    -not [bool](Get-OracleStage3R8RequiredPropertyValue $tpm "TpmReady" "Get-Tpm") -or
    -not [bool](Get-OracleStage3R8RequiredPropertyValue (
      $defender
  ...
  $activation = @(Get-CimInstance SoftwareLicensingProduct -ErrorAction Stop |
    Where-Object {
      $partialKey = Get-OracleStage3R8RequiredPropertyValue `
        $_ "PartialProductKey" "SoftwareLicensingProduct"
      $licenseStatus = Get-OracleStage3R8RequiredPropertyValue `
  ...
  $installedSoftware = @(Get-OracleStage3R8InstalledSoftwareInventory)
  ...
  $signerCertificate = Get-OracleStage3R8AcceptedPublicCertificate `
    -Contract $Contract -TransferRoot $TransferRoot
  $observedUntrustedSigner = Get-OracleStage3R8RequiredPropertyValue `
    $untrustedSignature "SignerCertificate" "Get-AuthenticodeSignature"
  if (
    [string](Get-OracleStage3R8RequiredPropertyValue (
      $untrustedSignature
  ...
  foreach ($member in @(
    ...
    [void](Get-OracleStage3R8RequiredPropertyValue $continuity $member "host continuity")
  }
  foreach ($member in @("deviceName", "manufacturer", "model")) {
    [void](Get-OracleStage3R8RequiredPropertyValue $continuity.host $member "host continuity host")
  }
  ...
  if (
    [string]$continuity.contract -cne
      "oracle.sprint-30-5.stage-3-r8-host-continuity" -or
  ...
  [pscustomobject][ordered]@{
    ...
    security = [ordered]@{
      ...
      tpmPresent = [bool](Get-OracleStage3R8RequiredPropertyValue (
        $tpm
      ) "TpmPresent" "Get-Tpm")
      tpmReady = [bool](Get-OracleStage3R8RequiredPropertyValue (
        $tpm
      ) "TpmReady" "Get-Tpm")
      defenderAntivirusEnabled = [bool](Get-OracleStage3R8RequiredPropertyValue (
        $defender
      ) "AntivirusEnabled" "Get-MpComputerStatus")
      defenderRealTimeProtectionEnabled = [bool](
        Get-OracleStage3R8RequiredPropertyValue (
          $defender
        ) "RealTimeProtectionEnabled" "Get-MpComputerStatus"
      )
