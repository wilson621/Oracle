...
function Get-OracleStage3R8OptionalPropertyValue {
  [CmdletBinding()]
  param(
...
function ConvertTo-OracleStage3R8InstalledSoftwareInventory {
  [CmdletBinding()]
  param(
  ...
  foreach ($entry in $RegistryEntries) {
    ...
    $displayNameValue = Get-OracleStage3R8OptionalPropertyValue `
    ...
    $displayVersionValue = Get-OracleStage3R8OptionalPropertyValue `
      -InputObject $entry -Name "DisplayVersion"
    $publisherValue = Get-OracleStage3R8OptionalPropertyValue `
...
function Get-OracleStage3R8InstalledSoftwareInventory {
  [CmdletBinding()]
  param(
  ...
  ConvertTo-OracleStage3R8InstalledSoftwareInventory -RegistryEntries $entries
}
