...
function Assert-OracleStage3R8CertificateTrustContract {
...
function Assert-OracleStage3R8Thumbprint {
...
function Get-OracleStage3R8CertificateStoreDefinitions {
  @(
...
function Get-OracleStage3R8PhysicalCertificateMatches {
  param([Parameter(Mandatory = $true)][string]$Thumbprint)

  Assert-OracleStage3R8Thumbprint -Thumbprint $Thumbprint
  $matches = @()
  foreach ($definition in @(Get-OracleStage3R8CertificateStoreDefinitions)) {
...
function Get-OracleStage3R8LogicalCertificateViews {
  param([Parameter(Mandatory = $true)][string]$Thumbprint)
...
function Test-OracleStage3R8CertificateIdentity {
  param(
...
function Assert-OracleStage3R8NoCertificateResidue {
  param(
...
function Assert-OracleStage3R8TemporaryTrustState {
  param(
    [Parameter(Mandatory = $true)]
    [AllowEmptyCollection()][object[]]$PhysicalMatches,
    [Parameter(Mandatory = $true)]
  ...
  if (
    ...
    -not (Test-OracleStage3R8CertificateIdentity `
...
function Assert-OracleStage3R8ExactRemovalTarget {
  param(
    [Parameter(Mandatory = $true)]
    [AllowEmptyCollection()][object[]]$PhysicalMatches,
    [Parameter(Mandatory = $true)]
...
function Test-OracleStage3R8ProcessIsElevated {
