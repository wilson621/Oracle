...
const attemptPattern = new RegExp(`^stage3-r8-${compactTime}-${suffix}$`, "u");
const authorityPattern = new RegExp(
  `^authority-stage3-r8-${compactTime}-${suffix}$`,
  "u"
);
const transferPattern = new RegExp(`^transfer-stage3-r8-${compactTime}-${suffix}$`, "u");
...
function validateScopedId(value, timestampUtc, pattern, label) {
  ...
  if (!match) {
    throw new Error(`${label} has an invalid Stage 3 R8 format.`);
  }
...
export function validateAcceptedBindings(candidate) {
  ...
  for (const field of [
    ...
    if (candidate[field] !== expected[field]) {
      throw new Error(`Accepted Stage 2 R2 binding mismatch: ${field}`);
    }
  }
  for (const field of [
    "finalEvidenceManifestSha256",
