...
function Assert-OracleStage3R8ApplicationActivationContract {
  [CmdletBinding()]
...
function Initialize-OracleStage3R8ActivationApi {
  if ($null -ne ("Oracle.Stage3R8.ApplicationActivation" -as [type])) {
    return
  }
...
function Test-OracleStage3R8ApplicationActivationApi {
  [CmdletBinding()]
  ...
  try {
    $native = if ($null -ne $ProbeRunner) {
      ...
      Initialize-OracleStage3R8ActivationApi
      [Oracle.Stage3R8.ApplicationActivation]::Probe()
    }
    if (
      $null -eq $native -or
...
function Invoke-OracleStage3R8ApplicationActivation {
  [CmdletBinding()]
  param(
  ...
  try {
    $native = if ($null -ne $ActivationRunner) {
      ...
      Initialize-OracleStage3R8ActivationApi
      [Oracle.Stage3R8.ApplicationActivation]::Activate(
        $AppUserModelId,
        $Arguments,
        [uint32]2
      )
...
function Assert-OracleStage3R8ApplicationActivationSucceeded {
  [CmdletBinding()]
