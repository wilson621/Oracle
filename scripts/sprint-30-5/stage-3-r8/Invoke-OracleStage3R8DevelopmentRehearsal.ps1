...
. (Join-Path $PSScriptRoot "Oracle.Stage3R8LifecyclePolicy.ps1")
. (Join-Path $PSScriptRoot "Oracle.Stage3R8IdentityPolicy.ps1")
. (Join-Path $PSScriptRoot "Oracle.Stage3R8ActivationPolicy.ps1")
. (Join-Path $PSScriptRoot "Oracle.Stage3R8CertificateTrustPolicy.ps1")
. (Join-Path $PSScriptRoot "Oracle.Stage3R8InstalledSoftwarePolicy.ps1")
. (Join-Path $PSScriptRoot "Oracle.Stage3R8ProcessPolicy.ps1")
...
function Invoke-SimulatedPhase(
  ...
  [void](Move-OracleStage3R8Lifecycle -State $State -Phase $Phase)
}

function Invoke-SimulatedLifecycle([string]$Root, [string]$InjectedFailure) {
  $state = New-OracleStage3R8LifecycleState
  $failure = $null
  try {
    foreach ($phase in @(Get-OracleStage3R8LifecyclePhases)) {
  ...
  $teardown = Get-OracleStage3R8TeardownObligations `
...
$parent = Join-Path $repositoryRoot ".tmp-stage3-r8-development-rehearsal"
...
try {
  $software = @(ConvertTo-OracleStage3R8InstalledSoftwareInventory -RegistryEntries @(
  ...
  foreach ($name in @("certutil.exe", "reagentc.exe")) {
    ...
    [void](Get-OracleStage3R8WindowsExecutablePath `
      -Name $name -SystemDirectory $fixtureSystem32)
  }
  $fixtureActivation = Invoke-OracleStage3R8ApplicationActivation `
  ...
  $fixtureWindows = @(
    ConvertFrom-OracleStage3R8WindowDiscoveryJson -Json (
  ...
  if (
    ...
    -not (Test-OracleStage3R8QualifyingWindow $fixtureWindows[0])
  ...
  $processCounts = @{}
  $processResult = Invoke-OracleStage3R8GovernedProcess `
