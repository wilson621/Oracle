...
function Get-OracleStage3R8RequiredProcessMember {
  [CmdletBinding()]
  param(
...
function Invoke-OracleStage3R8GovernedProcess {
  [CmdletBinding()]
  param(
    [Parameter(Mandatory = $true)][string]$Name,
  ...
  foreach ($member in @(
    ...
    [void](Get-OracleStage3R8RequiredProcessMember $record $member)
  }
  ...
  $processError = Get-OracleStage3R8RequiredProcessMember $record "processError"
  $signal = Get-OracleStage3R8RequiredProcessMember $record "signal"
  $exitCode = Get-OracleStage3R8RequiredProcessMember $record "exitCode"
