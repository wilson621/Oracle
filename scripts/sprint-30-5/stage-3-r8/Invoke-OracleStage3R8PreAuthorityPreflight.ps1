...
if (
  ...
  [string]$manifest.programmeIdentity -cne
    "Sprint 30.5 Stage 3 Qualification R8" -or
...
$runningScriptRelativePath = "payload/Invoke-OracleStage3R8PreAuthorityPreflight.ps1"
$runningScriptEntry = @($payloadEntries | Where-Object {
  [string]$_.path -ceq $runningScriptRelativePath
})
$contractRelativePath = "payload/Oracle.Stage3R8Contract.json"
...
$getCertificateMatches = {
  @(
    @(Get-OracleStage3R8PhysicalCertificateMatches `
      -Thumbprint $contract.stage2.certificateThumbprint)
    @(Get-OracleStage3R8LogicalCertificateViews `
      -Thumbprint $contract.stage2.certificateThumbprint)
  )
}

$result = Get-OracleStage3R8PreAuthorityObservation `
  -Contract $contract `
