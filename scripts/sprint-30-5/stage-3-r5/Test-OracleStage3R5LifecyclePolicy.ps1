...
. (Join-Path $PSScriptRoot "Oracle.Stage3R5LifecyclePolicy.ps1")
...
$success = New-OracleStage3R5LifecycleState
foreach ($phase in $phases) {
  [void](Move-OracleStage3R5Lifecycle -State $success -Phase $phase)
}
...
$skipRejected = $false
try {
  $state = New-OracleStage3R5LifecycleState
  [void](Move-OracleStage3R5Lifecycle -State $state -Phase $phases[1])
...
try {
  $state = New-OracleStage3R5LifecycleState
  [void](Move-OracleStage3R5Lifecycle -State $state -Phase $phases[0])
  [void](Move-OracleStage3R5Lifecycle -State $state -Phase $phases[0])
...
for ($failureIndex = 0; $failureIndex -lt $phases.Count; $failureIndex++) {
  ...
  for ($index = 0; $index -lt $failureIndex; $index++) {
    [void](Move-OracleStage3R5Lifecycle -State $state -Phase $phases[$index])
  }
  $obligations = Get-OracleStage3R5TeardownObligations `
  ...
  Assert-True (
    $obligations.removeExactRootTrust -eq $expectedTrust
