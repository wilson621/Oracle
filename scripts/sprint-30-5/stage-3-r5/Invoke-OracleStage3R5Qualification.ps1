...
throw "STAGE3_R5_ENTRY_POINT_RETIRED: historical qualification execution is permanently disabled."
...
$bootstrapManifestPath = Join-Path $TransferRoot (
  "Oracle.Stage3R5TransferManifest.json"
)
if (
...
$bootstrapRequiredFiles = @(
  "Invoke-OracleStage3R5Qualification.ps1",
  "Oracle.Stage3R5Contract.json",
...
if (
  -not [StringComparer]::OrdinalIgnoreCase.Equals(
    ...
    [IO.Path]::GetFullPath((
      Join-Path $TransferRoot "payload/Invoke-OracleStage3R5Qualification.ps1"
    ))
  )
...
. (Join-Path $scriptRoot "Oracle.Stage3R5PackageInventoryPolicy.ps1")
...
$manifestPath = Join-Path $TransferRoot "Oracle.Stage3R5TransferManifest.json"
$custodyPath = Join-Path $TransferRoot "Oracle.Stage3R5TransferCustody.json"
...
$identityPolicyPath = Join-Path $payloadRoot "Oracle.Stage3R5IdentityPolicy.ps1"
...
$lifecycleState = New-OracleStage3R5LifecycleState
...
function Write-Lifecycle([string]$Phase, $Details) {
  ...
  Assert-OracleStage3R5NextLifecyclePhase -State $lifecycleState -Phase $Phase
  ...
  [void](Move-OracleStage3R5Lifecycle -State $lifecycleState -Phase $Phase)
}
...
function Invoke-GovernedProcess(
  ...
  Invoke-OracleStage3R5GovernedProcess `
    -Name $Name `
...
function Assert-IdentityAndTransfer {
  ...
  $attempt = [regex]::Match($AttemptId, '^stage3-r5-(\d{8}T\d{9}Z)-([0-9a-f]{8})$')
  $authority = [regex]::Match($AuthorityId, '^authority-stage3-r5-(\d{8}T\d{9}Z)-([0-9a-f]{8})$')
  ...
  $expectedTransferRootEntries = @(
    "Oracle.Stage3R5TransferCustody.json",
    "Oracle.Stage3R5TransferCustody.json.sha256.txt",
    "Oracle.Stage3R5TransferManifest.json",
    "Oracle.Stage3R5TransferManifest.json.sha256.txt",
    "payload"
  ) | Sort-Object
  ...
  if ($manifest.transferId -cnotmatch '^transfer-stage3-r5-\d{8}T\d{9}Z-[0-9a-f]{8}$') {
  ...
  if (
    ...
    [string]$custody.contract -cne
      "oracle.sprint-30-5.stage-3-r5-transfer-custody" -or
  ...
  $expectedPayload = @(
    "Get-OracleStage3R5HostContinuity.ps1",
    "Invoke-OracleStage3R5PreAuthorityPreflight.ps1",
    "Invoke-OracleStage3R5Qualification.ps1",
  ...
  $runningHarnessEntry = @($manifest.payload | Where-Object {
    [string]$_.path -ceq "payload/Invoke-OracleStage3R5Qualification.ps1"
  })
  $runningContractEntry = @($manifest.payload | Where-Object {
    [string]$_.path -ceq "payload/Oracle.Stage3R5Contract.json"
  })
  $runningIdentityPolicyEntry = @($manifest.payload | Where-Object {
    [string]$_.path -ceq "payload/Oracle.Stage3R5IdentityPolicy.ps1"
  })
  $runningPackageInventoryPolicyEntry = @($manifest.payload | Where-Object {
    [string]$_.path -ceq "payload/Oracle.Stage3R5PackageInventoryPolicy.ps1"
  })
  $runningInstalledSoftwarePolicyEntry = @($manifest.payload | Where-Object {
    [string]$_.path -ceq "payload/Oracle.Stage3R5InstalledSoftwarePolicy.ps1"
  })
  $runningLifecyclePolicyEntry = @($manifest.payload | Where-Object {
    [string]$_.path -ceq "payload/Oracle.Stage3R5LifecyclePolicy.ps1"
  })
  $runningPreflightPolicyEntry = @($manifest.payload | Where-Object {
    [string]$_.path -ceq "payload/Oracle.Stage3R5PreflightPolicy.ps1"
  })
  ...
  $phaseAuditPath = Join-Path $payloadRoot "Oracle.Stage3R5PhaseAudit.json"
  ...
  if (
    ...
    @($auditedPhases).Count -ne
      @(Get-OracleStage3R5LifecyclePhases).Count
...
function Assert-PackageContent {
  ...
  foreach ($entry in @($packageInventory.entries)) {
    $canonicalExpectedPath =
      ConvertTo-OracleStage3R5CanonicalPackagePath ([string]$entry.path)
    if (
      $canonicalExpectedPath -cne [string]$entry.path -or
      (Test-OracleStage3R5ReservedPackageMetadata $canonicalExpectedPath) -or
  ...
  $zipEntries = @(Get-OracleStage3R5PackageZipInventory $msixPath)
...
try {
  ...
  $preflight = Get-OracleStage3R5PreAuthorityObservation `
    -Contract $contract `
  ...
  Write-CreateOnlyJson $authorityPath ([ordered]@{
    contract = "oracle.sprint-30-5.stage-3-r5-authority"
    authorityId = $AuthorityId
    attemptId = $AttemptId
  ...
  $certificate = Get-OracleStage3R5AcceptedPublicCertificate `
  ...
  if ($authorityConsumed) {
    foreach ($operation in @(
      ...
      @{ name = "remove-exact-root-trust"; action = {
        Remove-ExactRootTrust
      }},
      ...
      @{ name = "verify-zero-certificate-residue"; action = {
        if (@(Get-ExactCertificateMatches).Count -ne 0) {
          throw "Certificate residue remains after failure teardown."
        }
      }},
