...
function Get-OracleStage3R5RequiredPropertyValue {
  [CmdletBinding()]
  param(
...
function Assert-OracleStage3R5CommandSurface {
  [CmdletBinding()]
  param()
...
function Get-OracleStage3R5AcceptedPublicCertificate {
  [CmdletBinding()]
  param(
...
function Invoke-OracleStage3R5ReadOnlyProcess {
  [CmdletBinding()]
  param(
...
function Assert-OracleStage3R5NoReparseTraversal([string]$Path) {
...
function Get-OracleStage3R5PreAuthorityObservation {
  [CmdletBinding()]
  param(
    [Parameter(Mandatory = $true)][object]$Contract,
    [Parameter(Mandatory = $true)][string]$TransferRoot,
  ...
  foreach ($path in @($TransferRoot, $EvidenceReturnRoot, $HostContinuityPath)) {
    Assert-OracleStage3R5NoReparseTraversal $path
  }
  if (
  ...
  $manufacturer = [string](Get-OracleStage3R5RequiredPropertyValue `
    $computerSystem "Manufacturer" "Win32_ComputerSystem")
  $model = [string](Get-OracleStage3R5RequiredPropertyValue `
  ...
  foreach ($member in @("TpmPresent", "TpmReady")) {
    [void](Get-OracleStage3R5RequiredPropertyValue $tpm $member "Get-Tpm")
  }
  foreach ($member in @("AntivirusEnabled", "RealTimeProtectionEnabled")) {
    [void](Get-OracleStage3R5RequiredPropertyValue $defender $member "Get-MpComputerStatus")
  }
  if (
    -not $secureBoot -or
    -not [bool](Get-OracleStage3R5RequiredPropertyValue $tpm "TpmPresent" "Get-Tpm") -or
    -not [bool](Get-OracleStage3R5RequiredPropertyValue $tpm "TpmReady" "Get-Tpm") -or
    -not [bool](Get-OracleStage3R5RequiredPropertyValue (
      $defender
  ...
  $activation = @(Get-CimInstance SoftwareLicensingProduct -ErrorAction Stop |
    Where-Object {
      $partialKey = Get-OracleStage3R5RequiredPropertyValue `
        $_ "PartialProductKey" "SoftwareLicensingProduct"
      $licenseStatus = Get-OracleStage3R5RequiredPropertyValue `
  ...
  $installedSoftware = @(Get-OracleStage3R5InstalledSoftwareInventory)
  ...
  $signerCertificate = Get-OracleStage3R5AcceptedPublicCertificate `
    -Contract $Contract -TransferRoot $TransferRoot
  $observedUntrustedSigner = Get-OracleStage3R5RequiredPropertyValue `
    $untrustedSignature "SignerCertificate" "Get-AuthenticodeSignature"
  if (
    [string](Get-OracleStage3R5RequiredPropertyValue (
      $untrustedSignature
  ...
  foreach ($member in @(
    ...
    [void](Get-OracleStage3R5RequiredPropertyValue $continuity $member "host continuity")
  }
  foreach ($member in @("deviceName", "manufacturer", "model")) {
    [void](Get-OracleStage3R5RequiredPropertyValue $continuity.host $member "host continuity host")
  }
  ...
  if (
    [string]$continuity.contract -cne
      "oracle.sprint-30-5.stage-3-r5-host-continuity" -or
  ...
  [pscustomobject][ordered]@{
    ...
    security = [ordered]@{
      ...
      tpmPresent = [bool](Get-OracleStage3R5RequiredPropertyValue (
        $tpm
      ) "TpmPresent" "Get-Tpm")
      tpmReady = [bool](Get-OracleStage3R5RequiredPropertyValue (
        $tpm
      ) "TpmReady" "Get-Tpm")
      defenderAntivirusEnabled = [bool](Get-OracleStage3R5RequiredPropertyValue (
        $defender
      ) "AntivirusEnabled" "Get-MpComputerStatus")
      defenderRealTimeProtectionEnabled = [bool](
        Get-OracleStage3R5RequiredPropertyValue (
          $defender
        ) "RealTimeProtectionEnabled" "Get-MpComputerStatus"
      )
