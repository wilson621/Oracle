...
if (
  ...
  [string]$manifest.contract -cne
    "oracle.sprint-30-5.stage-3-r5-transfer" -or
  [string]$manifest.revision -cne "R5" -or
...
$runningScriptRelativePath = "payload/Invoke-OracleStage3R5PreAuthorityPreflight.ps1"
$runningScriptEntry = @($payloadEntries | Where-Object {
  [string]$_.path -ceq $runningScriptRelativePath
})
$contractRelativePath = "payload/Oracle.Stage3R5Contract.json"
...
$getCertificateMatches = {
  ...
  foreach ($store in $stores) {
    $certificate = Get-Item -LiteralPath (
      "Cert:\$($store[0])\$($store[1])\$($contract.stage2.certificateThumbprint)"
    ) -ErrorAction SilentlyContinue
    if ($null -ne $certificate) { $matches += $certificate }
  }
  @($matches)
}

$result = Get-OracleStage3R5PreAuthorityObservation `
  -Contract $contract `
