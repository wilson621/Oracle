...
function Get-OracleStage3R5RequiredProcessMember {
  [CmdletBinding()]
  param(
...
function Invoke-OracleStage3R5GovernedProcess {
  [CmdletBinding()]
  param(
    [Parameter(Mandatory = $true)][string]$Name,
  ...
  foreach ($member in @(
    ...
    [void](Get-OracleStage3R5RequiredProcessMember $record $member)
  }
  ...
  $processError = Get-OracleStage3R5RequiredProcessMember $record "processError"
  $signal = Get-OracleStage3R5RequiredProcessMember $record "signal"
  $exitCode = Get-OracleStage3R5RequiredProcessMember $record "exitCode"
