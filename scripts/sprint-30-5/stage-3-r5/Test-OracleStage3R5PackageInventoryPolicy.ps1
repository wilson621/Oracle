...
$fixtureRoot = Join-Path (
  [IO.Path]::GetTempPath()
) "oracle-stage3-r5-package-inventory-$([Guid]::NewGuid().ToString('N'))"
...
try {
  ...
  $first = @(Get-OracleStage3R5PackageZipInventory $validZip)
  $second = @(Get-OracleStage3R5PackageZipInventory $validZip)
  if (
    $first.Count -ne 3 -or
  ...
  try {
    Get-OracleStage3R5PackageZipInventory $duplicateZip | Out-Null
  ...
  try {
    Get-OracleStage3R5PackageZipInventory $traversalZip | Out-Null
  ...
  $contentMismatch = @(
    Get-OracleStage3R5PackageZipInventory $contentMismatchZip |
  ...
  $metadataMismatch = @(
    Get-OracleStage3R5PackageZipInventory $metadataMismatchZip |
  ...
  foreach ($entry in @($acceptedInventory.entries)) {
    ...
    if (
      ...
      (Test-OracleStage3R5ReservedPackageMetadata $canonical) -or
  ...
  $acceptedZip = @(
    Get-OracleStage3R5PackageZipInventory $AcceptedPackagePath
  )
