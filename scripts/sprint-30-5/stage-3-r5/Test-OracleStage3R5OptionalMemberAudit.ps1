...
function Get-Classification(
  ...
  if (
    ...
    return [ordered]@{
      ...
      testCoverage = "Test-OracleStage3R5OptionalMemberAudit.ps1 and verify-preparation.mjs"
      disposition = "fail-closed-if-missing"
    }
  }
  if (
    $File -ceq "Oracle.Stage3R5PreflightPolicy.ps1" -and
    ...
    return [ordered]@{
      ...
      testCoverage = "Test-OracleStage3R5HostShapeFixtures.ps1"
      disposition = "fail-closed-if-missing"
    }
  }
  if ($Expression -match '\.PSObject\.Properties\[') {
    return [ordered]@{
      ...
      testCoverage = "Test-OracleStage3R5InstalledSoftwarePolicy.ps1"
      disposition = "safe"
    }
  }
  if ($File -ceq "Oracle.Stage3R5InstalledSoftwarePolicy.ps1") {
    return [ordered]@{
  ...
  if ($File -ceq "Oracle.Stage3R5LifecyclePolicy.ps1") {
    return [ordered]@{
      classification = "structurally-guaranteed-by-construction"
      dataSource = "policy-owned lifecycle state"
      mandatory = $true
      protection = "state shape is created only by New-OracleStage3R5LifecycleState"
      testCoverage = "Test-OracleStage3R5LifecyclePolicy.ps1"
      disposition = "safe"
    }
  }
  if ($File -ceq "Oracle.Stage3R5PreflightPolicy.ps1") {
    return [ordered]@{
      ...
      mandatory = $true
      protection = "typed producer or Get-OracleStage3R5RequiredPropertyValue"
      testCoverage = "host-shape fixtures and Founder-QA-01 pre-authority probe"
      disposition = "fail-closed-if-unavailable"
    }
  }
  if ($File -ceq "Oracle.Stage3R5ProcessPolicy.ps1") {
    return [ordered]@{
      ...
      mandatory = $true
      protection = "Get-OracleStage3R5RequiredProcessMember checks every member before use"
      testCoverage = "Test-OracleStage3R5ProcessPolicy.ps1"
      disposition = "fail-closed-if-missing"
    }
  }
  if ($File -ceq "Oracle.Stage3R5PackageInventoryPolicy.ps1") {
    return [ordered]@{
      ...
      testCoverage = "Test-OracleStage3R5PackageInventoryPolicy.ps1"
      disposition = "safe"
    }
  }
  if ($File -ceq "Oracle.Stage3R5IdentityPolicy.ps1") {
    return [ordered]@{
      classification = "structurally-guaranteed-by-platform-type"
      dataSource = "System.StringComparer"
      mandatory = $true
      protection = "typed .NET ordinal comparison"
      testCoverage = "Test-OracleStage3R5IdentityPolicy.ps1"
      disposition = "safe"
    }
  }
  ...
  if ($Expression -match '^\$contract(?:\.|$)') {
    return [ordered]@{
      ...
      dataSource = "Oracle.Stage3R5Contract.json"
      mandatory = $true
...
$report = [ordered]@{
  schemaVersion = "1.0.0"
  contract = "oracle.sprint-30-5.stage-3-r5-optional-member-audit"
