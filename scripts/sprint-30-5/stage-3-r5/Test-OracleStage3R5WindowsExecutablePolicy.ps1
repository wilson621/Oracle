...
$root = Join-Path $env:TEMP (
  "oracle-stage3-r5-windows-executable-" + [Guid]::NewGuid().ToString("N")
)
...
try {
  ...
  $explorer = Get-OracleStage3R5WindowsExecutablePath `
    -Name "explorer.exe" `
    -WindowsDirectory $windowsDirectory `
    -SystemDirectory $systemDirectory
  $certutil = Get-OracleStage3R5WindowsExecutablePath `
    -Name "certutil.exe" `
    -WindowsDirectory $windowsDirectory `
    -SystemDirectory $systemDirectory
  $reagentc = Get-OracleStage3R5WindowsExecutablePath `
    -Name "reagentc.exe" `
    -WindowsDirectory $windowsDirectory `
    -SystemDirectory $systemDirectory
  ...
  try {
    [void](Get-OracleStage3R5WindowsExecutablePath `
      -Name "explorer.exe" `
      -WindowsDirectory $windowsDirectory `
      -SystemDirectory $systemDirectory)
  } catch {
