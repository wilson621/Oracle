...
function Invoke-SimulatedPhase(
  ...
  [void](Move-OracleStage3R5Lifecycle -State $State -Phase $Phase)
}
...
function Invoke-SimulatedLifecycle([string]$Root, [string]$InjectedFailure) {
  ...
  $failure = $null
  try {
    foreach ($phase in @(Get-OracleStage3R5LifecyclePhases)) {
  ...
  $teardown = Get-OracleStage3R5TeardownObligations `
...
$parent = Join-Path $repositoryRoot ".tmp-stage3-r5-development-rehearsal"
...
try {
  $software = @(ConvertTo-OracleStage3R5InstalledSoftwareInventory -RegistryEntries @(
  ...
  $processCounts = @{}
  $processResult = Invoke-OracleStage3R5GovernedProcess `
