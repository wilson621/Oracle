...
$requiredAuthority = "FOUNDER-AUTHORISED-STAGE-2-R6-SINGLE-ATTEMPT"
if ($FounderAuthority -cne $requiredAuthority) {
  throw "The exact R6 Founder execution authority token is required."
}

$repositoryRoot = [IO.Path]::GetFullPath((Join-Path $PSScriptRoot "..\..\.."))
$contractPath = Join-Path $PSScriptRoot "Oracle.Stage2RequalificationR6Contract.json"
$executorPath = Join-Path $PSScriptRoot "execute-attempt.mjs"
if (-not (Test-Path -LiteralPath $contractPath -PathType Leaf)) { throw "R6 contract is missing." }
if (-not (Test-Path -LiteralPath $executorPath -PathType Leaf)) { throw "R6 executor is missing." }
...
try {
  ...
  $identity = New-OracleStage2R6ExecutionIdentity
  if ($identity.suffix -eq "00000000") { throw "The prohibited fixed suffix was generated." }
  $outputRoot = ".artifacts/sprint-30-5/stage-2-requalification-r6/$($identity.attemptId)"
  $authorityPath = ".artifacts/sprint-30-5/stage-2-requalification-r6/authorities/$($identity.authorityId).json"
  ...
  [pscustomobject]@{
    contract = "oracle.sprint-30-5.stage-2-requalification-r6-governed-invocation"
  ...
  if ($null -eq $executorExitCode) { throw "R6 executor returned a null exit code." }
