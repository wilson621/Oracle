...
try {
  ...
  writeJsonAtomicCreateOnly(
    join(directories.evidence, "stage-2-requalification-r6-certification.json"),
    certification
  );
  const evidenceIndex = createEvidenceIndex();
  writeJsonAtomicCreateOnly(
    join(directories.evidence, "stage-2-requalification-r6-evidence-index.json"),
    evidenceIndex
  );
  ...
  writeJsonAtomicCreateOnly(
    join(
      ...
      "stage-2-requalification-r6-completion.json"
    ),
    {
      schemaVersion: "1.0.0",
      contract:
        "oracle.sprint-30-5.stage-2-requalification-r6-execution-completion",
      ...identity(),
  ...
  writeFileAtomicCreateOnly(
    join(directories.evidence, "Oracle.Stage2RequalificationR6FinalEvidenceHash.sha256.txt"),
...
function assertGovernedWrapperInvocation() {
  const environmentName = "ORACLE_STAGE2_R6_GOVERNED_WRAPPER";
...
function publishFailureOutcome(error, failureRecord, teardownError) {
  const attemptRecordPath = join(
    attemptRoot,
    "Oracle.Stage2RequalificationR6Attempt.json"
  );
  ...
  writeJsonAtomicCreateOnly(
    join(directories.evidence, "stage-2-requalification-r6-failure-outcome.json"),
    {
      schemaVersion: "1.0.0",
      contract:
        "oracle.sprint-30-5.stage-2-requalification-r6-failure-outcome",
      ...identity(),
...
function assertExecutionContract() {
  assert.equal(contract.programmeIdentity, "Sprint 30.5 Stage 2 Requalification R6");
  assert.equal(contract.revision, "R6");
  assert.equal(contract.package.identity, "Oracle.Platform.LocalCertification");
  assert.equal(contract.package.version, "0.1.4.0");
  assert.equal(contract.package.architecture, "x64");
  assert.equal(contract.package.semanticVersion, "0.1.4");
  ...
  assert.equal(
    ...
    "Migration 011 does not match the fixed R6 candidate binding."
  );
  assert.equal(
    sha256File(join(repositoryRoot, "database", "012_operator_identity_lifecycle.sql")),
    contract.candidate.migration012Sha256,
    "Migration 012 does not match the fixed R6 candidate binding."
  );
}
...
function createQualificationCandidate() {
  ...
  return {
    schemaVersion: "1.0.0",
    contract:
      "oracle.sprint-30-5.stage-2-requalification-r6-qualification-candidate",
    ...identity(),
...
function runSourceBaseline() {
  ...
  writeJsonAtomicCreateOnly(
    ...
    {
      schemaVersion: "1.0.0",
      contract: "oracle.sprint-30-5.stage-2-requalification-r6-build-secrecy",
      ...identity(),
...
function assertRepositoryAndCandidateUnchanged(candidate) {
  if (git(["branch", "--show-current"]) !== contract.requiredBranch) {
    throw new Error("Branch changed during R6 execution.");
  }
  if (git(["rev-parse", "HEAD"]) !== input.harnessCommit) {
    throw new Error("HEAD changed during R6 execution.");
  }
  if (git(["status", "--porcelain=v1", "--untracked-files=all"]) !== "") {
    throw new Error("Repository changed during R6 build.");
  }
...
async function constructPackageLayout() {
  ...
  writeJsonAtomicCreateOnly(join(stage, "package.json"), {
    name: "oracle-stage-2-requalification-r6-local-qualification",
    productName: "Oracle Stage 2 Requalification R6 Local Qualification",
...
function createTemporaryCertificate(layout) {
  pfxPath = join(directories.signing, "r6-local-test-signing.pfx");
  ...
  const script = [
    ...
    "$cert=[Security.Cryptography.X509Certificates.X509Certificate2]::new($env:ORACLE_R6_PFX,$env:ORACLE_R6_PASSWORD,$flags)",
  ...
  const result = JSON.parse(
    runLogged(
      ...
      [],
      { ORACLE_R6_PFX: pfxPath, ORACLE_R6_PASSWORD: password }
    ).stdout.trim()
  );
...
function recoverCertificateIdentityFromStoreDelta() {
  ...
  if (
    ...
    throw new Error("New certificate-store entries do not match the R6 identity.");
  }
...
function recoverCertificateIdentity() {
  ...
  const result = spawnSync(
    ...
    ["-NoProfile", "-NonInteractive", "-Command", script],
    {
      ...
      env: {
        ...process.env,
        ORACLE_R6_PFX: pfxPath,
        ORACLE_R6_PASSWORD: password,
      },
      encoding: "utf8",
      shell: false,
      windowsHide: true,
...
function buildSignedRelease(layout, candidate) {
  ...
  const sbomPath = join(directories.release, "oracle-0.1.4.cdx.json");
  ...
  writeJsonAtomicCreateOnly(
    ...
    {
      schemaVersion: "1.0.0",
      contract: "oracle.sprint-30-5.stage-2-requalification-r6-release-build",
      ...identity(),
...
function verifyRelease(release, candidate) {
  ...
  writeJsonAtomicCreateOnly(
    join(directories.release, "package-content-inventory.json"),
    {
      schemaVersion: "1.0.0",
      contract:
        "oracle.sprint-30-5.stage-2-requalification-r6-package-content-inventory",
      ...identity(),
      packageSha256: sha256File(release.packagePath),
  ...
  try {
    ...
    if (exactStoreMatches.length === 0) {
      writeJsonAtomicCreateOnly(cleanupPath, {
        schemaVersion: "1.0.0",
        contract:
          "oracle.sprint-30-5.stage-2-requalification-r6-certificate-cleanup",
        programmeIdentity: contract.programmeIdentity,
        completedAt: new Date().toISOString(),
        status: "passed",
...
function createCertification(release, candidate, verification) {
  return {
    schemaVersion: "1.0.0",
    contract: "oracle.sprint-30-5.stage-2-requalification-r6-certification",
    ...identity(),
...
function createEvidenceIndex() {
  ...
  return {
    schemaVersion: "1.0.0",
    contract: "oracle.sprint-30-5.stage-2-requalification-r6-evidence-index",
    ...identity(),
    createdAt: new Date().toISOString(),
    status: "complete-awaiting-freeze",
    files: files.filter(
      (entry) => !entry.path.endsWith("stage-2-requalification-r6-evidence-index.json")
    ),
  };
}
...
function freezeEvidence(release, certification, evidenceIndex) {
  ...
  writeJsonAtomicCreateOnly(
    join(directories.evidence, "stage-2-requalification-r6-frozen-evidence.json"),
    {
      schemaVersion: "1.0.0",
      contract: "oracle.sprint-30-5.stage-2-requalification-r6-frozen-evidence",
      ...identity(),
      frozenAt: new Date().toISOString(),
...
function createFinalEvidenceManifest(archive) {
  return {
    schemaVersion: "1.0.0",
    contract: "oracle.sprint-30-5.stage-2-requalification-r6-final-evidence",
    ...identity(),
    createdAt: new Date().toISOString(),
    status: "execution-passed-awaiting-founder-review",
    archive: {
...
function publishRepositoryEvidence() {
  ...
  for (const name of [
    ...
    "single-attempt-authority.json",
    "stage-2-requalification-r6-certification.json",
    "stage-2-requalification-r6-evidence-index.json",
    "stage-2-requalification-r6-frozen-evidence.json",
    "stage-2-requalification-r6-completion.json",
...
function publishFinalRepositoryEvidence(target) {
  for (const name of [
    ...
    "Oracle.Stage2RequalificationR6EvidenceManifest.json",
    "Oracle.Stage2RequalificationR6FinalEvidenceHash.sha256.txt",
  ]) {
    writeFileAtomicCreateOnly(
      join(target, name),
...
function createFinalMachineCheckpoint() {
  ...
  if (git(["diff", "--cached", "--name-only"]) !== "") {
    throw new Error("The repository index changed during R6 execution.");
  }
  const machineCheckpoint = {
    schemaVersion: "1.0.0",
    contract:
      "oracle.sprint-30-5.stage-2-requalification-r6-machine-checkpoint",
    ...identity(),
    recordedAt: new Date().toISOString(),
...
function createFinalRepositoryCheckpoint(repositoryEvidenceTarget) {
  ...
  writeJsonAtomicCreateOnly(
    join(directories.evidence, "final-repository-checkpoint.json"),
    {
      schemaVersion: "1.0.0",
      contract:
        "oracle.sprint-30-5.stage-2-requalification-r6-repository-checkpoint",
      ...identity(),
      recordedAt: new Date().toISOString(),
      branch: git(["branch", "--show-current"]),
...
function createProvenance(packagePath, candidate) {
  return {
    ...
    predicate: {
      ...
      runDetails: {
        builder: {
          id: "oracle.local/sprint-30-5-stage-2-requalification-r6",
        },
...
function runLogged(label, command, args, redactions = [], environment = {}) {
  ...
  const record = {
    schemaVersion: "1.0.0",
    contract: "oracle.sprint-30-5.stage-2-requalification-r6-command",
    ...identity(),
    label,
    command,
