...
const fixtureAttemptId = "r6-20260728T131415678Z-deadbeef";
...
const fixtureBinding = {
  ...
  wrapperProtocol: "oracle-stage2-r6-governed-wrapper-v1",
...
assert.deepEqual(
  validateGovernedWrapperInvocation({
    observed: "oracle-stage2-r6-governed-wrapper-v1:4242",
    parentProcessId: 4242,
  }),
  {
    protocol: "oracle-stage2-r6-governed-wrapper-v1",
    parentProcessId: 4242,
  }
);
assert.throws(
...
assert.throws(
  () =>
    validateGovernedWrapperInvocation({
      observed: "oracle-stage2-r6-governed-wrapper-v1:4243",
      parentProcessId: 4242,
    }),
  /must be invoked by the governed/
);
assert.throws(
  () =>
    validateGovernedWrapperInvocation({
      observed: "oracle-stage2-r6-governed-wrapper-v1:0",
      parentProcessId: 0,
    }),
...
assert.throws(
  () =>
    validateAttemptIdentity({
      attemptId: "r6-20260728T131415678Z-00000000",
      timestampUtc: fixtureTimestamp,
    }),
...
assert.equal(contract.candidate.commit, "ee8fbeb7a8d18d393cc9a3e92d622250eb2165ff");
assert.equal(contract.candidate.tree, "8455a05780989a9d5f6c6d527f7d427d94526b04");
assert.equal(
...
].map((name) => `scripts/sprint-30-5/stage-2-requalification-r6/${name}`).sort();
assert.deepEqual(
...
const attemptCreationRecord = createAttemptRecord({
  ...
  attemptId: fixtureAttemptId,
  timestampUtc: fixtureTimestamp,
  outputRoot: join(".artifacts", "sprint-30-5", "stage-2-requalification-r6", fixtureAttemptId),
});
...
const r6ValidatorSource = readFileSync(import.meta.filename, "utf8");
...
const r6FixtureLabels = new Set(
  [...r6ValidatorSource.matchAll(fixtureLabelPattern)].map((match) => match[1])
);
assert.ok(r2FixtureLabels.length > 0, "R2 regression labels were not discovered.");
for (const label of r2FixtureLabels) {
  assert.ok(r6FixtureLabels.has(label), `R6 lost inherited R2 fixture: ${label}`);
}
assert.throws(
...
assert.equal(
  ...
  false,
  "Segment-aware protection must distinguish the R6 and historical roots."
);
...
assert.doesNotThrow(() => assertR6ArtifactPath(governedRoot, fixtureAttemptId));
assert.doesNotThrow(() =>
  assertNoReparseTraversal(
    join(".artifacts", "sprint-30-5", "stage-2-requalification-r6")
  )
);
assert.throws(
  () =>
...
assert.throws(
  () => assertR6ArtifactPath(`${governedRoot}-other`, fixtureAttemptId),
...
for (const [base, prefix] of [
  ...
  for (const entry of readdirSync(base, { withFileTypes: true })) {
    if (
      ...
      entry.name !== "stage-2-requalification-r6"
    ) {
...
const temporaryRoot = mkdtempSync(join(os.tmpdir(), "oracle-stage2-r6-static-"));
try {
...
assert.doesNotThrow(() =>
  validateFinalIdentity({
    ...certificateBound,
    evidence: {
      manifest: "Oracle.Stage2RequalificationR6EvidenceManifest.json",
...
assert.match(
  cleanupScript,
  /exact generated R6 certificate thumbprint was not found/u
);
assert.match(
  cleanupScript,
...
assert.match(
  cleanupScript,
  /ORACLE_R6_ROOT_REMOVAL_PROCESS=[\s\S]*ConvertTo-Json -Compress -Depth 5/u
);
assert.match(
  cleanupScript,
...
const teardownExpectedSubject =
  "CN=Oracle Stage 2 Requalification R6 Local Test Signing - NOT PRODUCTION";
const syntheticMy = {
...
const r6PackageEntryPoints = Object.keys(packageJson.scripts).filter((name) =>
  name.startsWith("sprint-30-5:stage-2:r6:")
);
assert.deepEqual(
  r6PackageEntryPoints,
  [],
  "R6 must not modify the candidate package.json or expose independently runnable phases."
);
assert.equal(
...
assert.ok(
  ...
  "R6 must not expose a standalone attempt-preparation entry point."
);
...
assert.throws(
  () =>
    validateAuthorityIdentity({
      authorityId: "authority-r6-other",
      attemptId: fixtureAttemptId,
    }),
  /must exactly equal/
);
const previousAttemptId = "r6-20260728T120000000Z-01234567";
...
assert.match(executorSource, /stage-2-requalification-r6-failure-outcome\.json/u);
...
assert.ok(
  ...
  "R6 must check ambient configuration pre-authority, pre-build and post-build."
);
...
assert.match(executorSource, /Migration 011 does not match the fixed R6 candidate binding/u);
assert.match(executorSource, /Migration 012 does not match the fixed R6 candidate binding/u);
assert.match(
  readFileSync(join(import.meta.dirname, "execution-core.mjs"), "utf8"),
  /FOUNDER-AUTHORISED-STAGE-2-R6-SINGLE-ATTEMPT/u
);
assert.match(
  executorSource,
...
delete directInvocationEnvironment.ORACLE_STAGE2_R6_GOVERNED_WRAPPER;
...
assert.equal(contract.authority.stage3, "closed-not-authorised-by-r6");
assert.equal(contract.authority.stage4, "closed-not-authorised-by-r6");
assert.throws(
  () => assertRequiredTool("oracle-r6-tool-that-does-not-exist.exe"),
