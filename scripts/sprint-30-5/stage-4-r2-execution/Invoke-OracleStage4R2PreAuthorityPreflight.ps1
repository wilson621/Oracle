...
if([string]$contract.status -cne 'founder-authorised-execution-enabled'){throw 'Stage 4 R2 preparation is not admitted for an execution decision.'}
$output=[IO.Path]::GetFullPath($OutputPath);$approved=[IO.Path]::GetFullPath((Join-Path $root ([string]$contract.paths.preflightRoot)))
[void](Assert-OracleStage4R2NoReparseTraversal $output $approved)
$sidecar="$output.sha256.txt";[void](Assert-OracleStage4R2NoReparseTraversal $sidecar $approved)
