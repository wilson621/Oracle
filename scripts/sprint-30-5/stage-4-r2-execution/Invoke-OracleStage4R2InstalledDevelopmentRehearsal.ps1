...
$contract = Get-Content -Raw -LiteralPath (
  Join-Path $PSScriptRoot "Oracle.Stage4R2Contract.json"
) | ConvertFrom-Json
...
if (-not $principal.IsInRole(
  ...
  throw "Installed Stage 4 R2 development rehearsal requires elevation."
}
...
if ($exitCode -ne 0) {
  throw "Installed Stage 4 R2 development rehearsal failed with exit code $exitCode."
}
Write-Output $output
