...
function Assert-OracleStage4R2ApplicationActivationContract {
  [CmdletBinding()]
...
function Initialize-OracleStage4R2ActivationApi {
  if ($null -ne ("Oracle.Stage4R2.ApplicationActivation" -as [type])) {
    return
  }
...
function Test-OracleStage4R2ApplicationActivationApi {
  [CmdletBinding()]
  ...
  try {
    $native = if ($null -ne $ProbeRunner) {
      ...
      Initialize-OracleStage4R2ActivationApi
      [Oracle.Stage4R2.ApplicationActivation]::Probe()
    }
    if (
      $null -eq $native -or
...
function Invoke-OracleStage4R2ApplicationActivation {
  [CmdletBinding()]
  param(
  ...
  try {
    $native = if ($null -ne $ActivationRunner) {
      ...
      Initialize-OracleStage4R2ActivationApi
      [Oracle.Stage4R2.ApplicationActivation]::Activate(
        $AppUserModelId,
        $Arguments,
        [uint32]2
      )
...
function Assert-OracleStage4R2ApplicationActivationSucceeded {
  [CmdletBinding()]
