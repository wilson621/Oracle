...
if (principal.status !== 0) {
  throw new Error("Installed Stage 4 R2 development rehearsal requires an elevated process.");
}
...
const rehearsalIdentity = `stage4-r2-${timestamp}-${identitySuffix}`;
...
const result = spawnSync(process.execPath, [join(import.meta.dirname, "execute-live-environment.mjs")], {
  cwd: repositoryRoot,
  encoding: "utf8",
  shell: false,
  ...
  env: {
    ...
    ORACLE_STAGE4_R2_REHEARSAL_IDENTITY: rehearsalIdentity,
  },
});
...
try {
  ...
  throw new AggregateError(
    ...
    `Installed Stage 4 R2 development rehearsal failed; NON-QUALIFICATION diagnostics remain at ${root}`,
    { cause: error }
  );
}
