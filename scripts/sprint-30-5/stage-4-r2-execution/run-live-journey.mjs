...
const password = `Oracle-R2-${randomBytes(12).toString("hex")}!Aa1`;
const nonce = randomBytes(8).toString("hex");
const accounts = [`oracle-stage4-r2-a-${nonce}@example.invalid`,`oracle-stage4-r2-b-${nonce}@example.invalid`];
const journeys = [];
...
try {
  ...
  const apiResponse = await fetch(`${web}/api/oracle/conversation`, { method:"POST",headers:{cookie:cookieHeader,"content-type":"application/json"},body:JSON.stringify({requestId:`stage4-r2-${nonce}`,text:"qualification probe"}),redirect:"manual" });
  ...
  const record = { contract:developmentRehearsal?"oracle.sprint-30-5.stage-4-r2-development-rehearsal":"oracle.sprint-30-5.stage-4-r2-journey",result:"passed",classification:developmentRehearsal?["NON-QUALIFICATION","NON-AUTHORITY","NON-EVIDENCE","DEVELOPMENT REHEARSAL"]:"GOVERNED-STAGE-4-R2-QUALIFICATION",collectedAtUtc:new Date().toISOString(),provider:{classification:"disposable-local-non-production",productionEndpoint:false,externalEmail:false},journeys,rendering,isolation:{accountCount:2,crossAccountLeaks:0,distinctAuthenticatedPrincipals:true,distinctOperators:true,rlsBindingsPerPrincipal:[1,1]},secretsRetained:false };
