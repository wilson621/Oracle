...
$supabaseVersionRecord=Invoke-OracleStage4R2NativeProcess $approvedNode.path @($approvedSupabaseCli.path,'--version') $PSScriptRoot $supabaseOfflineEnvironment
...
$approvedGit=Resolve-OracleStage4R2BoundTool $contract.toolchain.approvedTools.git 'git'
...
function Assert-ToolIdentityRejected([object]$Specification,[string]$Fixture){try{[void](Resolve-OracleStage4R2BoundTool $Specification $Fixture);throw "Tool fixture accepted: $Fixture"}catch{if($_.Exception.Message -eq "Tool fixture accepted: $Fixture"){throw}}}
...
foreach($mutation in @('missing','duplicate','unexpected','failed','production','external-email','leak','rendering','secret','missing-nested')){
  ...
  try{[void](Assert-OracleStage4R2JourneyRecord $copy);throw "Mutation accepted: $mutation"}catch{if($_.Exception.Message -eq "Mutation accepted: $mutation"){throw}}
}
$optional=[pscustomobject]@{present='value'};if((Get-OracleStage4R2OptionalMember $optional 'absent') -ne $null){throw 'Absent optional member was not null.'};if((Get-OracleStage4R2OptionalMember $optional 'present') -cne 'value'){throw 'Present optional member changed.'}
try{[void](Get-OracleStage4R2MandatoryMember $optional 'absent' 'fixture');throw 'Missing mandatory member accepted.'}catch{if($_.Exception.Message -eq 'Missing mandatory member accepted.'){throw}}
$quoted=ConvertTo-OracleStage4R2WindowsArgument 'C:\Program Files\Oracle\value"quoted';if($quoted -notmatch '^".*"$'){throw 'Windows argument quoting failed.'}
$success=Invoke-OracleStage4R2NativeProcess (Join-Path $env:SystemRoot 'System32\cmd.exe') @('/d','/c','exit 0') $PSScriptRoot;if($success.exitCode -ne 0){throw 'Process success fixture failed.'}
try{[void](Invoke-OracleStage4R2NativeProcess (Join-Path $env:SystemRoot 'System32\cmd.exe') @('/d','/c','echo failure 1>&2 & exit 7') $PSScriptRoot);throw 'Nonzero process fixture accepted.'}catch{if($_.Exception.Message -eq 'Nonzero process fixture accepted.'){throw};$record=$_.Exception.Data['OracleStage4R2ProcessRecord'];if($null-eq$record -or $record.exitCode-ne 7 -or [string]::IsNullOrWhiteSpace([string]$record.stderr)){throw 'Process failure envelope was not preserved.'}}
$state=New-OracleStage4R2LifecycleState;foreach($phase in @(Get-OracleStage4R2LifecyclePhases)){[void](Move-OracleStage4R2Lifecycle $state $phase)};if(-not$state.terminal){throw 'Lifecycle did not terminate.'}
try{[void](Move-OracleStage4R2Lifecycle $state 'evidence-frozen');throw 'Repeat accepted.'}catch{if($_.Exception.Message -eq 'Repeat accepted.'){throw}}
$early=Get-OracleStage4R2TeardownObligations @('authority-consumed');if(-not$early.verifyZeroResidue -or -not$early.retryProhibited -or -not$early.preserveAttempt){throw 'Early teardown invariants failed.'}
Write-Output 'Stage 4 R2 PowerShell policy tests passed.'
