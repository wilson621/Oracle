...
function Assert-OracleStage4R2Transfer {
  param(
  ...
  [void](Assert-OracleStage4R2NoReparseTraversal $transfer $approvedRoot)
  if (-not (Test-Path -LiteralPath $transfer -PathType Container)) { throw "Governed R2 transfer is absent." }
  ...
  foreach ($path in @($manifestPath, $custodyPath, $manifestSidecar, $custodySidecar, $verificationPath, $verificationSidecar)) {
    [void](Assert-OracleStage4R2NoReparseTraversal $path $transfer)
  ...
  if (
    [string]$manifest.contract -cne "oracle.sprint-30-5.stage-4-r2-transfer-manifest" -or
  ...
  if (
    [string]$custody.contract -cne "oracle.sprint-30-5.stage-4-r2-transfer-custody" -or
  ...
  foreach ($entry in @($manifest.files)) {
    ...
    [void](Assert-OracleStage4R2NoReparseTraversal $path $payloadRoot)
