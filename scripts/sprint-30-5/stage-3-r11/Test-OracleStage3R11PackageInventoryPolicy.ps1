...
$fixtureRoot = Join-Path (
  [IO.Path]::GetTempPath()
) "oracle-stage3-r11-package-inventory-$([Guid]::NewGuid().ToString('N'))"
...
try {
  ...
  $first = @(Get-OracleStage3R11PackageZipInventory $validZip)
  $second = @(Get-OracleStage3R11PackageZipInventory $validZip)
  if (
    $first.Count -ne 3 -or
  ...
  try {
    Get-OracleStage3R11PackageZipInventory $duplicateZip | Out-Null
  ...
  try {
    Get-OracleStage3R11PackageZipInventory $traversalZip | Out-Null
  ...
  $contentMismatch = @(
    Get-OracleStage3R11PackageZipInventory $contentMismatchZip |
  ...
  $metadataMismatch = @(
    Get-OracleStage3R11PackageZipInventory $metadataMismatchZip |
  ...
  foreach ($entry in @($acceptedInventory.entries)) {
    ...
    if (
      ...
      (Test-OracleStage3R11ReservedPackageMetadata $canonical) -or
  ...
  $acceptedZip = @(
    Get-OracleStage3R11PackageZipInventory $AcceptedPackagePath
  )
