...
$bootstrapManifestPath = Join-Path $TransferRoot (
  "Oracle.Stage3R11TransferManifest.json"
)
if (
...
$bootstrapRequiredFiles = @(
  ...
  "Oracle.Stage3R11PackageInventoryPolicy.ps1",
  "Oracle.Stage3R11PreflightPolicy.ps1",
...
if (
  -not [StringComparer]::OrdinalIgnoreCase.Equals(
    ...
    [IO.Path]::GetFullPath((
      Join-Path $TransferRoot "payload/Invoke-OracleStage3R11Qualification.ps1"
    ))
  )
) { throw "Execution bootstrap script is outside the verified transfer." }
$contract = Get-Content -LiteralPath (
  Join-Path $scriptRoot "Oracle.Stage3R11Contract.json"
) -Raw | ConvertFrom-Json
. (Join-Path $scriptRoot "Oracle.Stage3R11IdentityPolicy.ps1")
. (Join-Path $scriptRoot "Oracle.Stage3R11ActivationPolicy.ps1")
. (Join-Path $scriptRoot "Oracle.Stage3R11CertificateTrustPolicy.ps1")
. (Join-Path $scriptRoot "Oracle.Stage3R11PackageInventoryPolicy.ps1")
. (Join-Path $scriptRoot "Oracle.Stage3R11InstalledSoftwarePolicy.ps1")
. (Join-Path $scriptRoot "Oracle.Stage3R11InstalledRuntimeConfigurationPolicy.ps1")
. (Join-Path $scriptRoot "Oracle.Stage3R11LifecyclePolicy.ps1")
. (Join-Path $scriptRoot "Oracle.Stage3R11ObservationPolicy.ps1")
. (Join-Path $scriptRoot "Oracle.Stage3R11PreflightPolicy.ps1")
. (Join-Path $scriptRoot "Oracle.Stage3R11ProcessPolicy.ps1")
. (Join-Path $scriptRoot "Oracle.Stage3R11WindowPolicy.ps1")
. (Join-Path $scriptRoot "Oracle.Stage3R11WindowsExecutablePolicy.ps1")
Assert-OracleStage3R11ApplicationActivationContract -Contract $contract
Assert-OracleStage3R11CertificateTrustContract -Contract $contract
$expectedToken = "FOUNDER-AUTHORISED-STAGE3-R11-EXECUTION"
...
$manifestPath = Join-Path $TransferRoot "Oracle.Stage3R11TransferManifest.json"
$custodyPath = Join-Path $TransferRoot "Oracle.Stage3R11TransferCustody.json"
...
$identityPolicyPath = Join-Path $payloadRoot "Oracle.Stage3R11IdentityPolicy.ps1"
$activationPolicyPath = Join-Path $payloadRoot "Oracle.Stage3R11ActivationPolicy.ps1"
...
$lifecyclePolicyPath = Join-Path $payloadRoot "Oracle.Stage3R11LifecyclePolicy.ps1"
$observationPolicyPath = Join-Path $payloadRoot (
  "Oracle.Stage3R11ObservationPolicy.ps1"
)
$preflightPolicyPath = Join-Path $payloadRoot "Oracle.Stage3R11PreflightPolicy.ps1"
$processPolicyPath = Join-Path $payloadRoot "Oracle.Stage3R11ProcessPolicy.ps1"
$windowPolicyPath = Join-Path $payloadRoot "Oracle.Stage3R11WindowPolicy.ps1"
...
$lifecycleState = New-OracleStage3R11LifecycleState
...
function New-GovernedRuntimeConfiguration {
  ...
  $sessionSecure = $null
  try {
    $anonValue = New-OracleStage3R11CryptographicValue
    $serviceValue = New-OracleStage3R11CryptographicValue
    $sessionValue = New-OracleStage3R11CryptographicValue
...
function Write-Lifecycle([string]$Phase, $Details) {
  ...
  Assert-OracleStage3R11NextLifecyclePhase -State $lifecycleState -Phase $Phase
  ...
  [void](Move-OracleStage3R11Lifecycle -State $lifecycleState -Phase $Phase)
}
...
function Invoke-GovernedProcess(
  ...
  Invoke-OracleStage3R11GovernedProcess `
    -Name $Name `
...
function Get-ExactCertificateMatches {
  @(Get-OracleStage3R11LogicalCertificateViews -Thumbprint $thumbprint)
}

function Get-PhysicalExactCertificateMatches {
  @(Get-OracleStage3R11PhysicalCertificateMatches -Thumbprint $thumbprint)
}
...
function Remove-ExactMachineTrust {
  ...
  [void](Invoke-GovernedProcess "exact-machine-trust-remove" $certutil @(
    Get-OracleStage3R11TrustRemovalArguments -Thumbprint $thumbprint
  ))
  Assert-OracleStage3R11NoCertificateResidue `
    -PhysicalMatches @(Get-PhysicalExactCertificateMatches) `
    -LogicalViews @(Get-ExactCertificateMatches)
}
...
function Stop-ExactPackageProcesses {
  ...
  foreach ($process in @(Get-Process -Name "Oracle" -ErrorAction SilentlyContinue)) {
    $resolution = Resolve-OracleStage3R11TeardownProcessOwnership `
    ...
    try {
      ...
      if (-not (Test-OracleStage3R11ProcessExists -ProcessId $process.Id)) {
        $teardownProcessEvents.Add([ordered]@{
          recordedAtUtc = [DateTime]::UtcNow.ToString("o")
          processId = [int]$process.Id
...
function Assert-IdentityAndTransfer {
  ...
  $attempt = [regex]::Match($AttemptId, '^stage3-r11-(\d{8}T\d{9}Z)-([0-9a-f]{8})$')
  $authority = [regex]::Match($AuthorityId, '^authority-stage3-r11-(\d{8}T\d{9}Z)-([0-9a-f]{8})$')
  ...
  $expectedFounderGrantId = $AttemptId -replace (
    '^stage3-r11-', 'founder-stage3-r11-grant-'
  )
  ...
  $expectedTransferRootEntries = @(
    "Oracle.Stage3R11TransferCustody.json",
    "Oracle.Stage3R11TransferCustody.json.sha256.txt",
    "Oracle.Stage3R11TransferManifest.json",
    "Oracle.Stage3R11TransferManifest.json.sha256.txt",
    "payload"
  ) | Sort-Object
  ...
  if (
    [string]$contract.programmeIdentity -cne
      "Sprint 30.5 Stage 3 Requalification R11" -or
    [string]$contract.revision -cne "R11" -or
    [string]$manifest.contract -cne
      "oracle.sprint-30-5.stage-3-r11-transfer" -or
    ...
    $manifest.transferId -cnotmatch
      '^transfer-stage3-r11-\d{8}T\d{9}Z-[0-9a-f]{8}$' -or
  ...
  $expectedPayload = @(
    "Get-OracleStage3R11HostContinuity.ps1",
    "Invoke-OracleStage3R11PreAuthorityPreflight.ps1",
    "Invoke-OracleStage3R11Qualification.ps1",
    ...
    "Oracle.Stage3R11LifecyclePolicy.ps1",
    "Oracle.Stage3R11ObservationPolicy.ps1",
    "Oracle.Stage3R11OptionalMemberAudit.json",
    "Oracle.Stage3R11PreflightPolicy.ps1",
    "Oracle.Stage3R11ProcessPolicy.ps1",
    "Oracle.Stage3R11WindowPolicy.ps1",
    "Oracle.Stage3R11WindowsExecutablePolicy.ps1",
    "Oracle.Stage3R11PhaseAudit.json",
    "Oracle.Stage3R11PackageInventoryPolicy.ps1",
  ...
  $runningHarnessEntry = @($manifest.payload | Where-Object {
    [string]$_.path -ceq "payload/Invoke-OracleStage3R11Qualification.ps1"
  })
  $runningContractEntry = @($manifest.payload | Where-Object {
    [string]$_.path -ceq "payload/Oracle.Stage3R11Contract.json"
  })
  $runningActivationPolicyEntry = @($manifest.payload | Where-Object {
    [string]$_.path -ceq "payload/Oracle.Stage3R11ActivationPolicy.ps1"
  })
  $runningCertificateTrustPolicyEntry = @($manifest.payload | Where-Object {
    [string]$_.path -ceq "payload/Oracle.Stage3R11CertificateTrustPolicy.ps1"
  })
  $runningIdentityPolicyEntry = @($manifest.payload | Where-Object {
    [string]$_.path -ceq "payload/Oracle.Stage3R11IdentityPolicy.ps1"
  })
  $runningPackageInventoryPolicyEntry = @($manifest.payload | Where-Object {
    [string]$_.path -ceq "payload/Oracle.Stage3R11PackageInventoryPolicy.ps1"
  })
  $runningInstalledSoftwarePolicyEntry = @($manifest.payload | Where-Object {
    [string]$_.path -ceq "payload/Oracle.Stage3R11InstalledSoftwarePolicy.ps1"
  })
  $runningInstalledRuntimeConfigurationPolicyEntry = @($manifest.payload | Where-Object {
    [string]$_.path -ceq
      "payload/Oracle.Stage3R11InstalledRuntimeConfigurationPolicy.ps1"
  })
  $runningLifecyclePolicyEntry = @($manifest.payload | Where-Object {
    [string]$_.path -ceq "payload/Oracle.Stage3R11LifecyclePolicy.ps1"
  })
  $runningObservationPolicyEntry = @($manifest.payload | Where-Object {
    [string]$_.path -ceq "payload/Oracle.Stage3R11ObservationPolicy.ps1"
  })
  $runningPreflightPolicyEntry = @($manifest.payload | Where-Object {
    [string]$_.path -ceq "payload/Oracle.Stage3R11PreflightPolicy.ps1"
  })
  $runningProcessPolicyEntry = @($manifest.payload | Where-Object {
    [string]$_.path -ceq "payload/Oracle.Stage3R11ProcessPolicy.ps1"
  })
  $runningWindowPolicyEntry = @($manifest.payload | Where-Object {
    [string]$_.path -ceq "payload/Oracle.Stage3R11WindowPolicy.ps1"
  })
  $runningWindowsExecutablePolicyEntry = @($manifest.payload | Where-Object {
    [string]$_.path -ceq "payload/Oracle.Stage3R11WindowsExecutablePolicy.ps1"
  })
  if (
    ...
    (Get-Sha256 (Join-Path $scriptRoot "Oracle.Stage3R11Contract.json")) -cne
    ...
    (Get-Sha256 (Join-Path $scriptRoot "Oracle.Stage3R11InstalledRuntimeConfigurationPolicy.ps1")) -cne
  ...
  $phaseAuditPath = Join-Path $payloadRoot "Oracle.Stage3R11PhaseAudit.json"
  ...
  if (
    ...
    @($auditedPhases).Count -ne
      @(Get-OracleStage3R11LifecyclePhases).Count
...
function Assert-PackageContent {
  ...
  foreach ($entry in @($packageInventory.entries)) {
    $canonicalExpectedPath =
      ConvertTo-OracleStage3R11CanonicalPackagePath ([string]$entry.path)
    if (
      $canonicalExpectedPath -cne [string]$entry.path -or
      (Test-OracleStage3R11ReservedPackageMetadata $canonicalExpectedPath) -or
  ...
  $zipEntries = @(Get-OracleStage3R11PackageZipInventory $msixPath)
...
function Invoke-NativeObservation([string]$Label) {
  ...
  while ([DateTime]::UtcNow -lt $deadline -and $null -eq $selected) {
    ...
    $windows = @(
      ConvertFrom-OracleStage3R11WindowDiscoveryJson -Json ([string]$result.stdout)
    )
    $candidates = @($windows | Where-Object {
      Test-OracleStage3R11QualifyingWindow $_
    })
  ...
  $ownership = Assert-OracleStage3R11ProcessPackageOwnership `
    -ProcessId ([int]$process.Id) `
  ...
  $stability = Invoke-OracleStage3R11CompleteStableObservation `
...
try {
  ...
  $preflight = Get-OracleStage3R11PreAuthorityObservation `
    -Contract $contract `
  ...
  Write-CreateOnlyJson $authorityPath ([ordered]@{
    contract = "oracle.sprint-30-5.stage-3-r11-authority"
    programmeIdentity = [string]$contract.programmeIdentity
    revision = [string]$contract.revision
    founderGrantId = $FounderGrantId
    authorityId = $AuthorityId
  ...
  $certificate = Get-OracleStage3R11AcceptedPublicCertificate `
  ...
  [void](Invoke-GovernedProcess "exact-machine-trust-import" $certutil @(
    Get-OracleStage3R11TrustImportArguments -CertificatePath $certificatePath
  ))
  $physicalMatches = @(Get-PhysicalExactCertificateMatches)
  $logicalViews = @(Get-ExactCertificateMatches)
  ...
  $initialActivation = Invoke-OracleStage3R11ApplicationActivation `
    -AppUserModelId $appUserModelId `
    -Arguments $initialActivationArguments
  Write-CreateOnlyJson (
    Join-Path $evidenceRoot "initial-activation.json"
  ) $initialActivation
  Assert-OracleStage3R11ApplicationActivationSucceeded -Result $initialActivation
  ...
  $repairActivation = Invoke-OracleStage3R11ApplicationActivation `
    -AppUserModelId $appUserModelId `
    -Arguments $repairActivationArguments
  Write-CreateOnlyJson (
    Join-Path $evidenceRoot "repair-activation.json"
  ) $repairActivation
  Assert-OracleStage3R11ApplicationActivationSucceeded -Result $repairActivation
