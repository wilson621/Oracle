...
function Get-OracleStage3R11RequiredWindowMember {
  [CmdletBinding()]
  param(
...
function ConvertFrom-OracleStage3R11WindowDiscoveryJson {
  [CmdletBinding()]
  ...
  foreach ($entry in $decoded) {
    ...
    foreach ($member in @(
      ...
      [void](Get-OracleStage3R11RequiredWindowMember $entry $member)
    }
    $entry
  }
}

function Test-OracleStage3R11QualifyingWindow {
  [CmdletBinding()]
  ...
  if (
    -not [Int64]::TryParse(
      [string](Get-OracleStage3R11RequiredWindowMember $Window "handle"),
      [Globalization.NumberStyles]::Integer,
      [Globalization.CultureInfo]::InvariantCulture,
      [ref]$handle
    ) -or
    -not [Int32]::TryParse(
      [string](Get-OracleStage3R11RequiredWindowMember $Window "processId"),
      [Globalization.NumberStyles]::Integer,
      [Globalization.CultureInfo]::InvariantCulture,
      [ref]$processId
    ) -or
    -not [Int32]::TryParse(
      [string](Get-OracleStage3R11RequiredWindowMember $Window "width"),
      [Globalization.NumberStyles]::Integer,
      [Globalization.CultureInfo]::InvariantCulture,
      [ref]$width
    ) -or
    -not [Int32]::TryParse(
      [string](Get-OracleStage3R11RequiredWindowMember $Window "height"),
      [Globalization.NumberStyles]::Integer,
      [Globalization.CultureInfo]::InvariantCulture,
      [ref]$height
    )
  ...
  (
    [string](Get-OracleStage3R11RequiredWindowMember $Window "processName") -ceq
      "Oracle" -and
    (Get-OracleStage3R11RequiredWindowMember $Window "visible") -eq $true -and
    (Get-OracleStage3R11RequiredWindowMember $Window "minimized") -eq $false -and
    $handle -gt 0 -and $processId -gt 0 -and $width -ge 1 -and $height -ge 1
  )
}

function Initialize-OracleStage3R11AppModelProcessIdentity {
  if ("Oracle.Stage3R11.AppModelProcessIdentity" -as [type]) { return }
...
function Get-OracleStage3R11ProcessOpenFailure {
  [CmdletBinding()]
  ...
  while ($null -ne $exception) {
    if (
      $exception.GetType().FullName -ceq
        "Oracle.Stage3R11.ProcessOpenException"
    ) { return $exception }
...
function Test-OracleStage3R11ProcessExists {
  [CmdletBinding()]
  param(
...
function Resolve-OracleStage3R11TeardownProcessOwnership {
  [CmdletBinding()]
  param(
    [Parameter(Mandatory = $true)][ValidateRange(1, [int]::MaxValue)]
    [int]$ProcessId,
  ...
  $exists = if ($null -eq $ProcessExists) {
    { param($Id) Test-OracleStage3R11ProcessExists -ProcessId $Id }
  } else {
    $ProcessExists
  }
  try {
    ...
    $queryFailure = $_
    $openFailure = Get-OracleStage3R11ProcessOpenFailure $queryFailure
...
function Get-OracleStage3R11ProcessPackageFamilyName {
  [CmdletBinding()]
  param(
    [Parameter(Mandatory = $true)][ValidateRange(1, [int]::MaxValue)]
    [int]$ProcessId,
  ...
  $familyName = if ($null -ne $Resolver) {
    ...
    [Oracle.Stage3R11.AppModelProcessIdentity]::QueryPackageFamilyName(
      [uint32]$ProcessId
    )
  }
...
function Assert-OracleStage3R11ProcessPackageOwnership {
  [CmdletBinding()]
  param(
    [Parameter(Mandatory = $true)][ValidateRange(1, [int]::MaxValue)]
    [int]$ProcessId,
  ...
  $observed = Get-OracleStage3R11ProcessPackageFamilyName `
