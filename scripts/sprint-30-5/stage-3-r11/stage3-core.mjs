...
const attemptPattern = new RegExp(`^stage3-r11-${compactTime}-${suffix}$`, "u");
const authorityPattern = new RegExp(
  `^authority-stage3-r11-${compactTime}-${suffix}$`,
  "u"
);
const transferPattern = new RegExp(`^transfer-stage3-r11-${compactTime}-${suffix}$`, "u");
...
function validateScopedId(value, timestampUtc, pattern, label) {
  ...
  if (!match) {
    throw new Error(`${label} has an invalid Stage 3 R11 format.`);
  }
...
export function validateProgrammeIdentity(value) {
  if (value !== canonicalProgrammeIdentity) {
    throw new Error("Stage 3 R11 programme identity differs.");
  }
  return value;
}
...
export function validateTransferIdentity({ transferId, timestampUtc }) {
  ...
  if (contract.rejectedTransfers.some((entry) => entry.transferId === transferId)) {
    throw new Error("Rejected Stage 3 R11 transfer identity cannot be reused.");
  }
}
