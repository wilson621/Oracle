...
function Assert-OracleStage3R11ApplicationActivationContract {
  [CmdletBinding()]
...
function Initialize-OracleStage3R11ActivationApi {
  if ($null -ne ("Oracle.Stage3R11.ApplicationActivation" -as [type])) {
    return
  }
...
function Test-OracleStage3R11ApplicationActivationApi {
  [CmdletBinding()]
  ...
  try {
    $native = if ($null -ne $ProbeRunner) {
      ...
      Initialize-OracleStage3R11ActivationApi
      [Oracle.Stage3R11.ApplicationActivation]::Probe()
    }
    if (
      $null -eq $native -or
...
function Invoke-OracleStage3R11ApplicationActivation {
  [CmdletBinding()]
  param(
  ...
  try {
    $native = if ($null -ne $ActivationRunner) {
      ...
      Initialize-OracleStage3R11ActivationApi
      [Oracle.Stage3R11.ApplicationActivation]::Activate(
        $AppUserModelId,
        $Arguments,
        [uint32]2
      )
...
function Assert-OracleStage3R11ApplicationActivationSucceeded {
  [CmdletBinding()]
