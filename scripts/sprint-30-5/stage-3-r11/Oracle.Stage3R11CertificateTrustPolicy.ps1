...
function Assert-OracleStage3R11CertificateTrustContract {
...
function Assert-OracleStage3R11Thumbprint {
...
function Get-OracleStage3R11CertificateStoreDefinitions {
  @(
...
function Get-OracleStage3R11PhysicalCertificateMatches {
  param([Parameter(Mandatory = $true)][string]$Thumbprint)

  Assert-OracleStage3R11Thumbprint -Thumbprint $Thumbprint
  $matches = @()
  foreach ($definition in @(Get-OracleStage3R11CertificateStoreDefinitions)) {
...
function Get-OracleStage3R11LogicalCertificateViews {
  param([Parameter(Mandatory = $true)][string]$Thumbprint)
...
function Test-OracleStage3R11CertificateIdentity {
  param(
...
function Assert-OracleStage3R11NoCertificateResidue {
  param(
...
function Assert-OracleStage3R11TemporaryTrustState {
  param(
    [Parameter(Mandatory = $true)]
    [AllowEmptyCollection()][object[]]$PhysicalMatches,
    [Parameter(Mandatory = $true)]
  ...
  if (
    ...
    -not (Test-OracleStage3R11CertificateIdentity `
...
function Assert-OracleStage3R11ExactRemovalTarget {
  param(
    [Parameter(Mandatory = $true)]
    [AllowEmptyCollection()][object[]]$PhysicalMatches,
    [Parameter(Mandatory = $true)]
...
function Test-OracleStage3R11ProcessIsElevated {
