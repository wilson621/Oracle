...
function Get-Classification(
  ...
  if (
    ...
    return [ordered]@{
      ...
      testCoverage = "Test-OracleStage3R11OptionalMemberAudit.ps1 and verify-preparation.mjs"
      disposition = "fail-closed-if-missing"
    }
  }
  if (
    $File -ceq "Oracle.Stage3R11PreflightPolicy.ps1" -and
    ...
    return [ordered]@{
      ...
      testCoverage = "Test-OracleStage3R11HostShapeFixtures.ps1"
      disposition = "fail-closed-if-missing"
    }
  }
  if ($Expression -match '\.PSObject\.Properties\[') {
    return [ordered]@{
      ...
      testCoverage = "Test-OracleStage3R11InstalledSoftwarePolicy.ps1"
      disposition = "safe"
    }
  }
  if ($File -ceq "Oracle.Stage3R11InstalledRuntimeConfigurationPolicy.ps1") {
    return [ordered]@{
      ...
      testCoverage = "Test-OracleStage3R11InstalledRuntimeConfigurationPolicy.ps1 and development rehearsal"
      disposition = "fail-closed-if-missing-or-mismatched"
    }
  }
  if ($File -ceq "Oracle.Stage3R11InstalledSoftwarePolicy.ps1") {
    return [ordered]@{
  ...
  if ($File -ceq "Oracle.Stage3R11LifecyclePolicy.ps1") {
    return [ordered]@{
      classification = "structurally-guaranteed-by-construction"
      dataSource = "policy-owned lifecycle state"
      mandatory = $true
      protection = "state shape is created only by New-OracleStage3R11LifecycleState"
      testCoverage = "Test-OracleStage3R11LifecyclePolicy.ps1"
      disposition = "safe"
    }
  }
  if ($File -ceq "Oracle.Stage3R11ObservationPolicy.ps1") {
    return [ordered]@{
      ...
      testCoverage = "Test-OracleStage3R11ObservationPolicy.ps1 and development rehearsal"
      disposition = "fail-closed-if-invalid"
    }
  }
  if ($File -ceq "Oracle.Stage3R11PreflightPolicy.ps1") {
    return [ordered]@{
      ...
      mandatory = $true
      protection = "typed producer or Get-OracleStage3R11RequiredPropertyValue"
      testCoverage = "host-shape fixtures and Founder-QA-01 pre-authority probe"
      disposition = "fail-closed-if-unavailable"
    }
  }
  if ($File -ceq "Oracle.Stage3R11ProcessPolicy.ps1") {
    return [ordered]@{
      ...
      mandatory = $true
      protection = "Get-OracleStage3R11RequiredProcessMember checks every member before use"
      testCoverage = "Test-OracleStage3R11ProcessPolicy.ps1"
      disposition = "fail-closed-if-missing"
    }
  }
  if ($File -ceq "Oracle.Stage3R11WindowPolicy.ps1") {
    return [ordered]@{
      ...
      testCoverage = "Test-OracleStage3R11WindowPolicy.ps1"
      disposition = "fail-closed-if-missing-or-mismatched"
    }
  }
  if ($File -ceq "Oracle.Stage3R11PackageInventoryPolicy.ps1") {
    return [ordered]@{
      ...
      testCoverage = "Test-OracleStage3R11PackageInventoryPolicy.ps1"
      disposition = "safe"
    }
  }
  if ($File -ceq "Oracle.Stage3R11IdentityPolicy.ps1") {
    return [ordered]@{
      classification = "structurally-guaranteed-by-platform-type"
      dataSource = "System.StringComparer"
      mandatory = $true
      protection = "typed .NET ordinal comparison"
      testCoverage = "Test-OracleStage3R11IdentityPolicy.ps1"
      disposition = "safe"
    }
  }
  if ($File -ceq "Oracle.Stage3R11CertificateTrustPolicy.ps1") {
    return [ordered]@{
      ...
      testCoverage = "Test-OracleStage3R11CertificateTrustPolicy.ps1"
      disposition = "fail-closed-if-missing-or-mismatched"
    }
  }
  if ($File -ceq "Oracle.Stage3R11ActivationPolicy.ps1") {
    return [ordered]@{
      ...
      testCoverage = "Test-OracleStage3R11ActivationPolicy.ps1 and elevated integration test"
      disposition = "fail-closed-if-missing-or-unsuccessful"
    }
  }
  if ($File -ceq "Oracle.Stage3R11WindowsExecutablePolicy.ps1") {
    return [ordered]@{
      ...
      testCoverage = "Test-OracleStage3R11WindowsExecutablePolicy.ps1"
      disposition = "fail-closed-if-unavailable"
    }
  }
  if (
    ...
    return [ordered]@{
      ...
      testCoverage = "Test-OracleStage3R11InstalledRuntimeConfigurationPolicy.ps1, verify-preparation.mjs and development rehearsal"
      disposition = "fail-closed-if-missing-or-mismatched"
    }
  }  if ($Expression -match '^\$contract(?:\.|$)') {
    return [ordered]@{
      classification = "mandatory-validated-contract-member"
      dataSource = "Oracle.Stage3R11Contract.json"
      mandatory = $true
...
$report = [ordered]@{
  schemaVersion = "1.0.0"
  contract = "oracle.sprint-30-5.stage-3-r11-optional-member-audit"
