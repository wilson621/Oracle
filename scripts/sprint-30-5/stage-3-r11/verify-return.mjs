...
assert.equal(manifest.contract, "oracle.sprint-30-5.stage-3-r11-archive-manifest");
assert.equal(validateProgrammeIdentity(manifest.programmeIdentity), canonicalProgrammeIdentity);
assert.equal(manifest.revision, "R11");
assert.match(manifest.attemptId, /^stage3-r11-\d{8}T\d{9}Z-[0-9a-f]{8}$/u);
assert.equal(
...
assert.equal(
  ...
  "oracle.sprint-30-5.stage-3-r11-evidence-manifest"
);
assert.equal(evidenceManifest.programmeIdentity, canonicalProgrammeIdentity);
assert.equal(evidenceManifest.revision, "R11");
...
assert.equal(completion.revision, "R11");
...
assert.equal(finalLifecycle.revision, "R11");
