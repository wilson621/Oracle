...
const sources = [
  ...
  join(import.meta.dirname, "Oracle.Stage3R11PhaseAudit.json"),
  join(import.meta.dirname, "Test-OracleStage3R11OptionalMemberAudit.ps1"),
  join(import.meta.dirname, "Test-OracleStage3R11InstalledRuntimeConfigurationPolicy.ps1"),
  join(import.meta.dirname, "Test-OracleStage3R11ObservationPolicy.ps1"),
  join(import.meta.dirname, "Test-OracleStage3R11ActivationPolicy.ps1"),
  join(import.meta.dirname, "Test-OracleStage3R11CertificateTrustPolicy.ps1"),
  join(import.meta.dirname, "Test-OracleStage3R11WindowPolicy.ps1"),
...
for (const source of sources) {
  if (!existsSync(source)) throw new Error(`Required R11 transfer source is missing: ${source}`);
}
...
if (
  ...
  throw new Error("R11 optional-member audit did not pass during transfer construction.");
}
...
const manifestPath = join(transferRoot, "Oracle.Stage3R11TransferManifest.json");
writeJsonAtomicCreateOnly(manifestPath, {
  schemaVersion: "1.0.0",
  contract: "oracle.sprint-30-5.stage-3-r11-transfer",
...
const custodyPath = join(transferRoot, "Oracle.Stage3R11TransferCustody.json");
writeJsonAtomicCreateOnly(custodyPath, {
  schemaVersion: "1.0.0",
  contract: "oracle.sprint-30-5.stage-3-r11-transfer-custody",
  programmeIdentity: contract.programmeIdentity,
  revision: contract.revision,
  transferId,
  ...
  purpose:
    "Create-only Stage 3 Requalification R11 transfer; R1-R9 and all historical transfers remain immutable",
  sourceRepository: {
    branch,
    harnessCommit,
    harnessTree,
