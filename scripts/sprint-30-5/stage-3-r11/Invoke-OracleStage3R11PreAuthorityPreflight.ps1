...
if (
  ...
  [string]$manifest.revision -cne "R11" -or
...
$runningScriptRelativePath = "payload/Invoke-OracleStage3R11PreAuthorityPreflight.ps1"
$runningScriptEntry = @($payloadEntries | Where-Object {
  [string]$_.path -ceq $runningScriptRelativePath
})
$contractRelativePath = "payload/Oracle.Stage3R11Contract.json"
...
) { throw "Pre-authority canonical R11 identity differs or is retired." }
if (
...
) { throw "Pre-authority accepted Stage 2 R6 binding differs." }
...
$getCertificateMatches = {
  @(
    @(Get-OracleStage3R11PhysicalCertificateMatches `
      -Thumbprint $contract.stage2.certificateThumbprint)
    @(Get-OracleStage3R11LogicalCertificateViews `
      -Thumbprint $contract.stage2.certificateThumbprint)
  )
}

$result = Get-OracleStage3R11PreAuthorityObservation `
  -Contract $contract `
