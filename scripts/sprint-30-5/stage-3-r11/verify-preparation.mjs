...
validateExecutionIdentity({
  authorityId: "authority-stage3-r11-20260728T223045123Z-a1b2c3d4",
  attemptId: "stage3-r11-20260728T223045123Z-a1b2c3d4",
  timestampUtc,
});
for (const invalid of [
  {
    authorityId: "authority-stage3-r11-20260728T223045123Z-a1b2c3d4",
    attemptId: "stage3-r11-20260728T223045123Z-ffffffff",
    timestampUtc,
  },
  {
    authorityId: "authority-stage3-r11-20260728T223045123Z-a1b2c3d4",
...
validateTransferIdentity({
  transferId: "transfer-stage3-r11-20260728T223045123Z-a1b2c3d4",
  timestampUtc,
});
assert.equal(
...
for (const invalidProgrammeIdentity of [
  "Sprint 30.5 Stage 3 Qualification R11",
  "sprint 30.5 Stage 3 Requalification R11",
  "Sprint 30.5  Stage 3 Requalification R11",
  "Sprint 30.5 Stage-3 Requalification R11",
  "Sprint 30.5 Stage 4 Requalification R11",
...
for (const rejectedTransfer of [
  ...
  assert.throws(
    ...
    /invalid Stage 3 R11 format/u
  );
}
assert.equal(
  validateTransferConstructionAuthority("FOUNDER-AUTHORISED-STAGE3-R11-TRANSFER"),
  "FOUNDER-AUTHORISED-STAGE3-R11-TRANSFER"
);
for (const invalidAuthority of [
  undefined,
  "",
  "Founder-authorised Stage 3 Qualification R11 corrective preparation",
  "FOUNDER-AUTHORISED-STAGE3-R11-EXECUTION",
]) {
  assert.throws(
...
try {
  ...
  const fixtureAttemptId = "stage3-r11-20260728T223045123Z-a1b2c3d4";
  ...
  writeFileSync(
    ...
    `${JSON.stringify({
      contract: "oracle.sprint-30-5.stage-3-r11-evidence-manifest",
  ...
  writeFileSync(
    ...
    `${JSON.stringify({
      contract: "oracle.sprint-30-5.stage-3-r11-lifecycle",
      programmeIdentity: canonicalProgrammeIdentity,
      revision: "R11",
      authorityId: fixtureAuthorityId,
      attemptId: fixtureAttemptId,
  ...
  const archiveCreation = spawnSync(
    ...
    [
      ...
      "-NonInteractive",
      "-Command",
      "Compress-Archive -LiteralPath $env:ORACLE_R11_FIXTURE_SOURCE -DestinationPath $env:ORACLE_R11_FIXTURE_DESTINATION",
    ],
    {
      encoding: "utf8",
      shell: false,
      env: {
        ...process.env,
        ORACLE_R11_FIXTURE_SOURCE: fixtureAttemptRoot,
        ORACLE_R11_FIXTURE_DESTINATION: returnedArchive,
      },
    }
  );
  ...
  writeFileSync(
    returnedManifest,
    `${JSON.stringify({
      contract: "oracle.sprint-30-5.stage-3-r11-archive-manifest",
      programmeIdentity: canonicalProgrammeIdentity,
      revision: "R11",
      authorityId: fixtureAuthorityId,
      attemptId: fixtureAttemptId,
  ...
  const parent = join(repositoryRoot, ".tmp-stage3-r11-validation");
  if (existsSync(parent)) rmSync(parent, { recursive: true, force: true });
}

const harness = readFileSync(
  join(import.meta.dirname, "Invoke-OracleStage3R11Qualification.ps1"),
  "utf8"
);
const preAuthorityPreflight = readFileSync(
  join(import.meta.dirname, "Invoke-OracleStage3R11PreAuthorityPreflight.ps1"),
  "utf8"
);
...
const continuityCollector = readFileSync(
  join(import.meta.dirname, "Get-OracleStage3R11HostContinuity.ps1"),
  "utf8"
);
const identityPolicy = readFileSync(
  join(import.meta.dirname, "Oracle.Stage3R11IdentityPolicy.ps1"),
  "utf8"
);
const activationPolicy = readFileSync(
  join(import.meta.dirname, "Oracle.Stage3R11ActivationPolicy.ps1"),
  "utf8"
);
const certificateTrustPolicy = readFileSync(
  join(import.meta.dirname, "Oracle.Stage3R11CertificateTrustPolicy.ps1"),
  "utf8"
);
const packageInventoryPolicy = readFileSync(
  join(import.meta.dirname, "Oracle.Stage3R11PackageInventoryPolicy.ps1"),
  "utf8"
);
const installedSoftwarePolicy = readFileSync(
  join(import.meta.dirname, "Oracle.Stage3R11InstalledSoftwarePolicy.ps1"),
  "utf8"
);
const installedRuntimeConfigurationPolicy = readFileSync(
  join(import.meta.dirname, "Oracle.Stage3R11InstalledRuntimeConfigurationPolicy.ps1"),
  "utf8"
);
const lifecyclePolicy = readFileSync(
  join(import.meta.dirname, "Oracle.Stage3R11LifecyclePolicy.ps1"),
  "utf8"
);
const observationPolicy = readFileSync(
  join(import.meta.dirname, "Oracle.Stage3R11ObservationPolicy.ps1"),
  "utf8"
);
const preflightPolicy = readFileSync(
  join(import.meta.dirname, "Oracle.Stage3R11PreflightPolicy.ps1"),
  "utf8"
);
const processPolicy = readFileSync(
  join(import.meta.dirname, "Oracle.Stage3R11ProcessPolicy.ps1"),
  "utf8"
);
const windowPolicy = readFileSync(
  join(import.meta.dirname, "Oracle.Stage3R11WindowPolicy.ps1"),
  "utf8"
);
const windowsExecutablePolicy = readFileSync(
  join(import.meta.dirname, "Oracle.Stage3R11WindowsExecutablePolicy.ps1"),
  "utf8"
);
...
const developmentRehearsalPath = join(
  import.meta.dirname,
  "Invoke-OracleStage3R11DevelopmentRehearsal.ps1"
);
const installedSoftwareFixturePath = join(
  import.meta.dirname,
  "Test-OracleStage3R11InstalledSoftwarePolicy.ps1"
);
const lifecycleFixturePath = join(
  import.meta.dirname,
  "Test-OracleStage3R11LifecyclePolicy.ps1"
);
const installedRuntimeConfigurationFixturePath = join(
  import.meta.dirname,
  "Test-OracleStage3R11InstalledRuntimeConfigurationPolicy.ps1"
);
const hostShapeFixturePath = join(
  import.meta.dirname,
  "Test-OracleStage3R11HostShapeFixtures.ps1"
);
const optionalMemberAuditPath = join(
  import.meta.dirname,
  "Test-OracleStage3R11OptionalMemberAudit.ps1"
);
const processFixturePath = join(
  import.meta.dirname,
  "Test-OracleStage3R11ProcessPolicy.ps1"
);
const windowFixturePath = join(
  import.meta.dirname,
  "Test-OracleStage3R11WindowPolicy.ps1"
);
const observationFixturePath = join(
  import.meta.dirname,
  "Test-OracleStage3R11ObservationPolicy.ps1"
);
const certificateTrustFixturePath = join(
  import.meta.dirname,
  "Test-OracleStage3R11CertificateTrustPolicy.ps1"
);
const activationFixturePath = join(
  import.meta.dirname,
  "Test-OracleStage3R11ActivationPolicy.ps1"
);
const windowsExecutableFixturePath = join(
  import.meta.dirname,
  "Test-OracleStage3R11WindowsExecutablePolicy.ps1"
);
const developmentPlatformFixturePath = join(
  import.meta.dirname,
  "Test-OracleStage3R11DevelopmentPlatformCompatibility.ps1"
);
const phaseAudit = JSON.parse(
  readFileSync(join(import.meta.dirname, "Oracle.Stage3R11PhaseAudit.json"), "utf8")
);
const hostShapeFixtures = JSON.parse(
  readFileSync(join(import.meta.dirname, "Oracle.Stage3R11HostShapeFixtures.json"), "utf8")
);
const harnessReadme = readFileSync(join(import.meta.dirname, "README.md"), "utf8");
const stage3R11Plan = readFileSync(
  join(repositoryRoot, "docs", "sprints", "SPRINT_30_5_STAGE_3_R11_PLAN.md"),
  "utf8"
);
const packageInventoryFixturePath = join(
  import.meta.dirname,
  "Test-OracleStage3R11PackageInventoryPolicy.ps1"
);
const scriptPathFixturePath = join(
  import.meta.dirname,
  "Test-OracleStage3R11ScriptPath.ps1"
);
const preExecutionGate = readFileSync(
  join(
    ...
    "docs",
    "sprints",
    "SPRINT_30_5_STAGE_3_R11_PRE_EXECUTION_GATE.md"
  ),
  "utf8"
);
const preparationValidationReport = readFileSync(
  join(
    ...
    "docs",
    "sprints",
    "SPRINT_30_5_STAGE_3_R11_PREPARATION_VALIDATION_REPORT.md"
  ),
  "utf8"
);
...
assert.equal(contract.revision, "R11");
...
for (const operationalSource of [
  ...
  assert.ok(
    !operationalSource.includes("Sprint 30.5 Stage 3 Qualification R11"),
    "Reachable R11 source retains the rejected programme identity."
  );
}
...
assert.match(transferBuilder, /Get-OracleStage3R11HostContinuity\.ps1/u);
assert.match(transferBuilder, /Oracle\.Stage3R11IdentityPolicy\.ps1/u);
assert.match(transferBuilder, /Oracle\.Stage3R11ActivationPolicy\.ps1/u);
assert.match(transferBuilder, /Oracle\.Stage3R11CertificateTrustPolicy\.ps1/u);
assert.match(transferBuilder, /Oracle\.Stage3R11InstalledSoftwarePolicy\.ps1/u);
assert.match(transferBuilder, /Oracle\.Stage3R11LifecyclePolicy\.ps1/u);
assert.match(transferBuilder, /Oracle\.Stage3R11ObservationPolicy\.ps1/u);
assert.match(transferBuilder, /Test-OracleStage3R11ObservationPolicy\.ps1/u);
assert.match(transferBuilder, /Oracle\.Stage3R11PackageInventoryPolicy\.ps1/u);
assert.match(transferBuilder, /Oracle\.Stage3R11PreflightPolicy\.ps1/u);
assert.match(transferBuilder, /Oracle\.Stage3R11ProcessPolicy\.ps1/u);
assert.match(transferBuilder, /Oracle\.Stage3R11WindowPolicy\.ps1/u);
assert.match(transferBuilder, /Test-OracleStage3R11WindowPolicy\.ps1/u);
assert.match(harness, /Oracle\.Stage3R11WindowPolicy\.ps1/u);
assert.match(harness, /Oracle\.Stage3R11ObservationPolicy\.ps1/u);
assert.match(harness, /Invoke-OracleStage3R11CompleteStableObservation/u);
...
assert.match(transferBuilder, /Oracle\.Stage3R11WindowsExecutablePolicy\.ps1/u);
assert.match(transferBuilder, /Invoke-OracleStage3R11PreAuthorityPreflight\.ps1/u);
assert.match(transferBuilder, /Oracle\.Stage3R11OptionalMemberAudit\.json/u);
assert.match(transferBuilder, /Test-OracleStage3R11ActivationPolicy\.ps1/u);
assert.match(harness, /"Test-OracleStage3R11ActivationPolicy\.ps1"/u);
assert.match(transferBuilder, /Oracle\.Stage3R11PhaseAudit\.json/u);
assert.match(transferBuilder, /Oracle\.Stage3R11TransferCustody\.json/u);
assert.match(transferBuilder, /authority:\s*founderAuthority,/u);
for (const admissionSource of [preAuthorityPreflight, harness]) {
  assert.match(
    admissionSource,
    /\[string\]\$custody\.authority[ \t]+-cne[ \t]*\r?\n[ \t]+"FOUNDER-AUTHORISED-STAGE3-R11-TRANSFER"/u
  );
}
assert.doesNotMatch(
  transferBuilder,
  /authority:\s*["']Founder-authorised Stage 3 Qualification R11 corrective preparation["']/u
);
...
for (const required of [
  ...
  "Get-OracleStage3R11InstalledSoftwareInventory",
  "Get-OracleStage3R11PreAuthorityObservation",
  "ConvertFrom-OracleStage3R11WindowDiscoveryJson",
  "Assert-OracleStage3R11ProcessPackageOwnership",
  "Invoke-OracleStage3R11ApplicationActivation",
  "Assert-OracleStage3R11ApplicationActivationSucceeded",
  "initial-activation.json",
  "repair-activation.json",
  "Get-OracleStage3R11TrustImportArguments",
  "Get-OracleStage3R11TrustRemovalArguments",
  "Assert-OracleStage3R11TemporaryTrustState",
  "Assert-OracleStage3R11ExactRemovalTarget",
  "Get-PhysicalExactCertificateMatches",
  "Move-OracleStage3R11Lifecycle",
  "package-content-reconciliation.json",
  "Get-OracleStage3R11PackageZipInventory",
  "ConvertTo-OracleStage3R11CanonicalPackagePath",
  "--remote-debugging",
  ...
  "oracle.sprint-30-5.stage-3-r11-transfer-custody",
...
assert.match(windowPolicy, /Test-OracleStage3R11ProcessExists/u);
...
assert.equal(
  packageScripts["sprint-30-5:stage-3:r11:validate"],
  "node scripts/sprint-30-5/stage-3-r11/verify-preparation.mjs"
);
assert.equal(
  packageScripts["sprint-30-5:stage-3:r11:prepare-transfer"],
  "node scripts/sprint-30-5/stage-3-r11/prepare-transfer.mjs"
);
assert.equal(
  packageScripts["sprint-30-5:stage-3:r11:verify-return"],
  "node scripts/sprint-30-5/stage-3-r11/verify-return.mjs"
);
assert.equal(packageScripts["sprint-30-5:stage-3:r11:execute"], undefined);
...
for (const required of [
  "Stage 3 Requalification R11 Pre-Execution Gate",
...
assert.match(
  transferBuilder,
  /Create-only Stage 3 Requalification R11 transfer; R1-R9 and all historical transfers remain immutable/u
);
for (const required of [
  "PSObject.Properties",
...
assert.match(harness, /Get-OracleStage3R11InstalledSoftwareInventory/u);
assert.match(harness, /Get-OracleStage3R11PreAuthorityObservation[\s\S]*New-Item/u);
assert.ok(
  harness.indexOf("Get-OracleStage3R11PreAuthorityObservation") <
...
assert.ok(
  harness.indexOf("$bootstrapManifestPath") <
    harness.indexOf(
      '. (Join-Path $scriptRoot "Oracle.Stage3R11IdentityPolicy.ps1")'
    ),
...
assert.match(processPolicy, /Get-OracleStage3R11RequiredProcessMember/u);
...
assert.match(preflightPolicy, /Test-OracleStage3R11ApplicationActivationApi/u);
assert.match(lifecyclePolicy, /Get-OracleStage3R11LifecyclePhases/u);
assert.match(preflightPolicy, /required Windows PowerShell 5\.1|Windows PowerShell 5\.1/u);
assert.match(preflightPolicy, /Test-OracleStage3R11ProcessIsElevated/u);
assert.match(preflightPolicy, /Machine-scoped AppX trust requires an elevated/u);
assert.match(preflightPolicy, /Get-OracleStage3R11InstalledSoftwareInventory/u);
...
const identityPolicyFixture = spawnSync(
  ...
  [
    ...
    "Bypass",
    "-File",
    join(import.meta.dirname, "Test-OracleStage3R11IdentityPolicy.ps1"),
  ],
  { encoding: "utf8", shell: false, windowsHide: true }
);
...
const packageInventoryFixture = spawnSync(
  ...
  [
    ...
    "-ContractPath",
    join(import.meta.dirname, "Oracle.Stage3R11Contract.json"),
  ],
  { encoding: "utf8", shell: false, windowsHide: true }
);
...
for (const required of [
  "oracle.sprint-30-5.stage-3-r11-host-continuity",
...
assert.equal(contract.authority.transfer, "founder-authorised-contingent-on-preparation-pass");
assert.equal(contract.authority.execution, "founder-authorised-contingent-on-transfer-and-pre-authority-pass");
assert.equal(contract.authority.stage4, "not-authorised");
assert.equal(contract.transferMedium.approvalState, "founder-authorised-after-r11-preparation-pass");
assert.deepEqual(contract.revisionLineage, {
  ...
  relationship:
    "R9 remains immutable historical qualification; R11 requalifies only the accepted Stage 2 R6 candidate",
});
assert.deepEqual(contract.correctionBasis, {
  basis: "accepted-stage-2-r6-product-baseline-change",
  historicalStage3Revision: "R9",
  historicalStage3Status: "founder-accepted-formally-closed-immutable",
  requiredSemantics:
    "repeat the complete clean Windows package lifecycle against the accepted corrected R6 MSIX, including attempt-scoped runtime configuration for initial and post-reset activation",
});
...
assert.doesNotThrow(() => assertOutsideHistoricalRoots(
  join(repositoryRoot, contract.output.transferBase, "transfer-stage3-r11-20260728T223045123Z-a1b2c3d4")
...
console.log("Stage 3 R11 preparation validation passed.");
