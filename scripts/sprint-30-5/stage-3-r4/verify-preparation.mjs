...
validateExecutionIdentity({
  authorityId: "authority-stage3-r4-20260728T223045123Z-a1b2c3d4",
  attemptId: "stage3-r4-20260728T223045123Z-a1b2c3d4",
  timestampUtc,
});
for (const invalid of [
  {
    authorityId: "authority-stage3-r4-20260728T223045123Z-a1b2c3d4",
    attemptId: "stage3-r4-20260728T223045123Z-ffffffff",
    timestampUtc,
  },
  {
    authorityId: "authority-stage3-r4-20260728T223045123Z-a1b2c3d4",
...
validateTransferIdentity({
  transferId: "transfer-stage3-r4-20260728T223045123Z-a1b2c3d4",
  timestampUtc,
});
...
try {
  ...
  const returnedArchive = join(temporaryRoot, "stage3-r4-fixture.zip");
  ...
  writeFileSync(
    returnedSidecar,
    `${returnedArchiveHash}  stage3-r4-fixture.zip\n`
  );
  writeFileSync(
    returnedManifest,
    `${JSON.stringify({
  ...
  const parent = join(repositoryRoot, ".tmp-stage3-r4-validation");
...
const packageInventoryFixturePath = join(
  import.meta.dirname,
  "Test-OracleStage3R4PackageInventoryPolicy.ps1"
);
const scriptPathFixturePath = join(
  import.meta.dirname,
  "Test-OracleStage3R4ScriptPath.ps1"
);
const preExecutionGate = readFileSync(
  join(
    ...
    "docs",
    "sprints",
    "SPRINT_30_5_STAGE_3_R4_PRE_EXECUTION_GATE.md"
  ),
  "utf8"
);
...
assert.match(transferBuilder, /Get-OracleStage3R4HostContinuity\.ps1/u);
assert.match(transferBuilder, /Oracle\.Stage3R4IdentityPolicy\.ps1/u);
assert.match(transferBuilder, /Oracle\.Stage3R4PackageInventoryPolicy\.ps1/u);
assert.match(transferBuilder, /Oracle\.Stage3R4TransferCustody\.json/u);
...
for (const required of [
  "FOUNDER-AUTHORISED-STAGE3-R4-EXECUTION",
  ...
  "Get-OracleStage3R4PackageZipInventory",
  "ConvertTo-OracleStage3R4CanonicalPackagePath",
  "--remote-debugging",
  ...
  "oracle.sprint-30-5.stage-3-r4-transfer-custody",
  "contract.transferMedium.hardwareSerial",
]) {
  assert.ok(harness.includes(required), `Harness contract missing: ${required}`);
}
assert.equal(
  packageScripts["sprint-30-5:stage-3:r4:validate"],
  "node scripts/sprint-30-5/stage-3-r4/verify-preparation.mjs"
);
assert.equal(
  packageScripts["sprint-30-5:stage-3:r4:prepare-transfer"],
  "node scripts/sprint-30-5/stage-3-r4/prepare-transfer.mjs"
);
assert.equal(
  packageScripts["sprint-30-5:stage-3:r4:verify-return"],
  "node scripts/sprint-30-5/stage-3-r4/verify-return.mjs"
);
assert.equal(packageScripts["sprint-30-5:stage-3:r4:execute"], undefined);
...
for (const required of [
  "Stage 3 Qualification R4 Pre-Execution Gate",
  "Execution:** Blocked and unauthorised",
  "ExpectedHarnessCommit",
  "ExpectedTransferManifestSha256",
  "ExpectedTransferCustodySha256",
  "ExpectedHostContinuitySha256",
  "FOUNDER-AUTHORISED-STAGE3-R4-TRANSFER",
  "FOUNDER-AUTHORISED-STAGE3-R4-EXECUTION",
...
assert.match(
  transferBuilder,
  /Create-only corrective Stage 3 R4 transfer; R1, failed R2 and failed R3 remain immutable/u
);
for (const required of [
...
const identityPolicyFixture = spawnSync(
  ...
  [
    ...
    "Bypass",
    "-File",
    join(import.meta.dirname, "Test-OracleStage3R4IdentityPolicy.ps1"),
  ],
...
for (const required of [
  "oracle.sprint-30-5.stage-3-r4-host-continuity",
...
for (const text of governance) {
  assert.match(text, /Stage 3\s+(?:Qualification\s+)?R4/iu);
...
console.log("Stage 3 R4 preparation validation passed.");
