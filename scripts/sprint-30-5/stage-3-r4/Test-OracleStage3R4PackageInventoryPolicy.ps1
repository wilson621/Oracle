...
$fixtureRoot = Join-Path (
  [IO.Path]::GetTempPath()
) "oracle-stage3-r4-package-inventory-$([Guid]::NewGuid().ToString('N'))"
...
try {
  ...
  $first = @(Get-OracleStage3R4PackageZipInventory $validZip)
  $second = @(Get-OracleStage3R4PackageZipInventory $validZip)
  if (
    $first.Count -ne 3 -or
  ...
  try {
    Get-OracleStage3R4PackageZipInventory $duplicateZip | Out-Null
  ...
  try {
    Get-OracleStage3R4PackageZipInventory $traversalZip | Out-Null
  ...
  $contentMismatch = @(
    Get-OracleStage3R4PackageZipInventory $contentMismatchZip |
  ...
  $metadataMismatch = @(
    Get-OracleStage3R4PackageZipInventory $metadataMismatchZip |
  ...
  foreach ($entry in @($acceptedInventory.entries)) {
    ...
    if (
      ...
      (Test-OracleStage3R4ReservedPackageMetadata $canonical) -or
  ...
  $acceptedZip = @(
    Get-OracleStage3R4PackageZipInventory $AcceptedPackagePath
  )
