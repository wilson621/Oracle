...
try {
  ...
  writeJsonAtomicCreateOnly(
    join(directories.evidence, "stage-2-requalification-r1-certification.json"),
    certification
  );
  const evidenceIndex = createEvidenceIndex();
  writeJsonAtomicCreateOnly(
    join(directories.evidence, "stage-2-requalification-r1-evidence-index.json"),
    evidenceIndex
  );
  ...
  writeJsonAtomicCreateOnly(
    join(
      ...
      "stage-2-requalification-r1-completion.json"
    ),
    {
      schemaVersion: "1.0.0",
      contract:
        "oracle.sprint-30-5.stage-2-requalification-r1-execution-completion",
      ...identity(),
  ...
  const finalManifestPath = join(
    directories.evidence,
    "Oracle.Stage2RequalificationR1EvidenceManifest.json"
  );
  writeJsonAtomicCreateOnly(finalManifestPath, finalManifest);
  const finalEvidenceHash = sha256File(finalManifestPath);
  writeFileAtomicCreateOnly(
    join(directories.evidence, "Oracle.Stage2RequalificationR1FinalEvidenceHash.sha256.txt"),
...
function assertExecutionContract() {
  assert.equal(contract.programmeIdentity, "Sprint 30.5 Stage 2 Requalification R1");
  assert.equal(contract.revision, "R1");
  assert.equal(contract.package.identity, "Oracle.Platform.LocalCertification");
  assert.equal(contract.package.version, "0.1.1.0");
  assert.equal(contract.package.architecture, "x64");
  assert.equal(contract.package.semanticVersion, "0.1.1");
  assert.equal(contract.package.runtimeManifestVersion, "1.7.0");
  assert.equal(contract.certificate.maximumLifetimeDays, 2);
...
function createQualificationCandidate() {
  ...
  return {
    schemaVersion: "1.0.0",
    contract:
      "oracle.sprint-30-5.stage-2-requalification-r1-qualification-candidate",
    ...identity(),
...
function runSourceBaseline() {
  ...
  runNpmLogged("web-build", ["run", "build"]);
...
function assertRepositoryAndCandidateUnchanged(candidate) {
  if (git(["branch", "--show-current"]) !== contract.requiredBranch) {
    throw new Error("Branch changed during R1 execution.");
  }
  if (git(["rev-parse", "HEAD"]) !== input.harnessCommit) {
    throw new Error("HEAD changed during R1 execution.");
  }
  if (git(["status", "--porcelain=v1", "--untracked-files=all"]) !== "") {
    throw new Error("Repository changed during R1 build.");
  }
...
async function constructPackageLayout() {
  ...
  writeJsonAtomicCreateOnly(join(stage, "package.json"), {
    name: "oracle-stage-2-requalification-r1-local-qualification",
    productName: "Oracle Stage 2 Requalification R1 Local Qualification",
...
function createTemporaryCertificate(layout) {
  pfxPath = join(directories.signing, "r1-local-test-signing.pfx");
  ...
  const script = [
    ...
    "$cert=[Security.Cryptography.X509Certificates.X509Certificate2]::new($env:ORACLE_R1_PFX,$env:ORACLE_R1_PASSWORD,$flags)",
  ...
  const result = JSON.parse(
    runLogged(
      ...
      [],
      { ORACLE_R1_PFX: pfxPath, ORACLE_R1_PASSWORD: password }
    ).stdout.trim()
  );
  ...
  if (lifetime <= 0 || lifetime > 2 * 24 * 60 * 60 * 1000 + 5 * 60 * 1000) {
    throw new Error("Generated certificate exceeds the two-day lifetime.");
  }
  return {
    exactThumbprint: result.thumbprint,
    certificateSubject: result.subject,
...
function recoverCertificateIdentityFromStoreDelta() {
  ...
  if (
    ...
    throw new Error("New certificate-store entries do not match the R1 identity.");
  }
...
function recoverCertificateIdentity() {
  ...
  const result = spawnSync(
    ...
    ["-NoProfile", "-NonInteractive", "-Command", script],
    {
      ...
      env: {
        ...process.env,
        ORACLE_R1_PFX: pfxPath,
        ORACLE_R1_PASSWORD: password,
      },
      encoding: "utf8",
      shell: false,
      windowsHide: true,
...
function buildSignedRelease(layout, candidate) {
  ...
  const sbomPath = join(directories.release, "oracle-0.1.1.cdx.json");
  ...
  writeJsonAtomicCreateOnly(
    ...
    {
      schemaVersion: "1.0.0",
      contract: "oracle.sprint-30-5.stage-2-requalification-r1-release-build",
      ...identity(),
...
function verifyRelease(release, candidate) {
  ...
  assert.match(manifestXml, /Version="0\.1\.1\.0"/u);
  ...
  writeJsonAtomicCreateOnly(
    join(directories.release, "package-content-inventory.json"),
    {
      schemaVersion: "1.0.0",
      contract:
        "oracle.sprint-30-5.stage-2-requalification-r1-package-content-inventory",
      ...identity(),
      packageSha256: sha256File(release.packagePath),
  ...
  try {
    ...
    if (exactStoreMatches.length === 0) {
      writeJsonAtomicCreateOnly(cleanupPath, {
        schemaVersion: "1.0.0",
        contract:
          "oracle.sprint-30-5.stage-2-requalification-r1-certificate-cleanup",
        programmeIdentity: contract.programmeIdentity,
        completedAt: new Date().toISOString(),
        status: "passed",
      ...
      runLogged("exact-certificate-teardown", "powershell.exe", [
        "-NoProfile",
        "-NonInteractive",
        "-ExecutionPolicy",
        "Bypass",
...
function createCertification(release, candidate, verification) {
  return {
    schemaVersion: "1.0.0",
    contract: "oracle.sprint-30-5.stage-2-requalification-r1-certification",
    ...identity(),
...
function createEvidenceIndex() {
  ...
  return {
    schemaVersion: "1.0.0",
    contract: "oracle.sprint-30-5.stage-2-requalification-r1-evidence-index",
    ...identity(),
    createdAt: new Date().toISOString(),
    status: "complete-awaiting-freeze",
    files: files.filter(
      (entry) => !entry.path.endsWith("stage-2-requalification-r1-evidence-index.json")
    ),
  };
}
...
function freezeEvidence(release, certification, evidenceIndex) {
  ...
  runLogged("evidence-archive", "tar.exe", [
    "-a",
    "-c",
    "-f",
    temporaryArchive,
  ...
  writeJsonAtomicCreateOnly(
    join(directories.evidence, "stage-2-requalification-r1-frozen-evidence.json"),
    {
      schemaVersion: "1.0.0",
      contract: "oracle.sprint-30-5.stage-2-requalification-r1-frozen-evidence",
      ...identity(),
      frozenAt: new Date().toISOString(),
...
function createFinalEvidenceManifest(archive) {
  return {
    schemaVersion: "1.0.0",
    contract: "oracle.sprint-30-5.stage-2-requalification-r1-final-evidence",
    ...identity(),
    createdAt: new Date().toISOString(),
    status: "execution-passed-awaiting-founder-review",
    archive: {
...
function publishRepositoryEvidence() {
  ...
  for (const name of [
    ...
    "stage-2-requalification-r1-certification.json",
    "stage-2-requalification-r1-evidence-index.json",
    "stage-2-requalification-r1-frozen-evidence.json",
    "stage-2-requalification-r1-completion.json",
...
function publishFinalRepositoryEvidence(target) {
  for (const name of [
    ...
    "Oracle.Stage2RequalificationR1EvidenceManifest.json",
    "Oracle.Stage2RequalificationR1FinalEvidenceHash.sha256.txt",
  ]) {
    writeFileAtomicCreateOnly(
      join(target, name),
...
function createFinalMachineCheckpoint() {
  ...
  if (git(["diff", "--cached", "--name-only"]) !== "") {
    throw new Error("The repository index changed during R1 execution.");
  }
  const machineCheckpoint = {
    schemaVersion: "1.0.0",
    contract:
      "oracle.sprint-30-5.stage-2-requalification-r1-machine-checkpoint",
    ...identity(),
...
function createFinalRepositoryCheckpoint(repositoryEvidenceTarget) {
  ...
  writeJsonAtomicCreateOnly(
    join(directories.evidence, "final-repository-checkpoint.json"),
    {
      schemaVersion: "1.0.0",
      contract:
        "oracle.sprint-30-5.stage-2-requalification-r1-repository-checkpoint",
      ...identity(),
      recordedAt: new Date().toISOString(),
      branch: git(["branch", "--show-current"]),
...
function createProvenance(packagePath, candidate) {
  return {
    ...
    predicate: {
      ...
      runDetails: {
        builder: {
          id: "oracle.local/sprint-30-5-stage-2-requalification-r1",
        },
...
function runLogged(label, command, args, redactions = [], environment = {}) {
  ...
  const record = {
    schemaVersion: "1.0.0",
    contract: "oracle.sprint-30-5.stage-2-requalification-r1-command",
    ...identity(),
    label,
    command,
