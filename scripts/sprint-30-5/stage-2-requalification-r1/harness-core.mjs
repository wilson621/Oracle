...
export const contractPath = join(
  import.meta.dirname,
  "Oracle.Stage2RequalificationR1Contract.json"
);
...
const attemptPattern = /^r1-(\d{8}T\d{9}Z)-([0-9a-f]{8})$/u;
...
export function assertOutsideHistoricalRoots(path) {
  ...
  for (const protectedRoot of contract.historicalProtectedRoots) {
    ...
    if (isSameOrDescendant(resolved, historical)) {
      throw new Error(
        `Refusing R1 output inside immutable historical root: ${protectedRoot}`
      );
    }
  }
  return resolved;
...
export function assertR1ArtifactPath(path, attemptId) {
  ...
  if (resolved !== requiredAttemptRoot) {
    throw new Error(
      `R1 output root must resolve exactly to ${relative(repositoryRoot, requiredAttemptRoot).replaceAll("\\", "/")}.`
    );
  }
  return resolved;
}
...
export function assertNoReparseTraversal(
  ...
  if (!isSameOrDescendant(resolved, repositoryRoot)) {
    throw new Error("Governed R1 output must remain inside the repository.");
  }
  ...
  for (const segment of segments) {
    ...
    if (
      ...
      throw new Error(`R1 output path traverses a symbolic link or junction: ${current}`);
    }
  }
}
...
export function validateAttemptIdentity({ attemptId, timestampUtc }) {
  ...
  if (!match) {
    throw new Error("Attempt ID must use r1-YYYYMMDDTHHMMSSmmmZ-xxxxxxxx.");
  }
...
export function validateRepositorySnapshot(snapshot, binding) {
  ...
  if (snapshot.status !== "") {
    throw new Error("R1 requires a clean repository and index.");
  }
}
...
export function validateMachineQualificationState(result) {
  ...
  if (result.certificates.length !== 0) {
    throw new Error(
      "An R1 temporary certificate identity already exists in a governed store."
    );
  }
}
...
export function createAttemptRecord(input) {
  ...
  const outputRoot = assertR1ArtifactPath(input.outputRoot, input.attemptId);
  return {
    schemaVersion: "1.0.0",
    contract: "oracle.sprint-30-5.stage-2-requalification-r1-attempt",
...
export function createAttemptDirectory(record) {
  const outputRoot = assertR1ArtifactPath(record.outputRoot, record.attemptId);
  ...
  writeJsonAtomicCreateOnly(
    join(outputRoot, "Oracle.Stage2RequalificationR1Attempt.json"),
    record
  );
  return outputRoot;
}
