...
function assertHistoricalEntryPointRetired() {
  throw new Error(
    "STAGE3_R3_ENTRY_POINT_RETIRED: historical transfer construction is permanently disabled."
  );
}
...
if (argumentsMap.get("founder-authority") !== "FOUNDER-AUTHORISED-STAGE3-R3-TRANSFER") {
...
const sources = [
  ...
  join(import.meta.dirname, "Oracle.Stage3R3Contract.json"),
...
const manifestPath = join(transferRoot, "Oracle.Stage3R3TransferManifest.json");
writeJsonAtomicCreateOnly(manifestPath, {
  schemaVersion: "1.0.0",
  contract: "oracle.sprint-30-5.stage-3-r3-transfer",
...
const custodyPath = join(transferRoot, "Oracle.Stage3R3TransferCustody.json");
writeJsonAtomicCreateOnly(custodyPath, {
  schemaVersion: "1.0.0",
  contract: "oracle.sprint-30-5.stage-3-r3-transfer-custody",
  transferId,
