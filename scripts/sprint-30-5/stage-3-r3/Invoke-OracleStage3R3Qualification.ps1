...
throw "STAGE3_R3_ENTRY_POINT_RETIRED: historical qualification execution is permanently disabled."
...
$expectedToken = "FOUNDER-AUTHORISED-STAGE3-R3-EXECUTION"
...
$manifestPath = Join-Path $TransferRoot "Oracle.Stage3R3TransferManifest.json"
$custodyPath = Join-Path $TransferRoot "Oracle.Stage3R3TransferCustody.json"
...
$identityPolicyPath = Join-Path $payloadRoot "Oracle.Stage3R3IdentityPolicy.ps1"
...
function Write-Lifecycle([string]$Phase, $Details) {
  ...
  Write-CreateOnlyJson (
    ...
    contract = "oracle.sprint-30-5.stage-3-r3-lifecycle"
...
function Assert-IdentityAndTransfer {
  ...
  $attempt = [regex]::Match($AttemptId, '^stage3-r3-(\d{8}T\d{9}Z)-([0-9a-f]{8})$')
  $authority = [regex]::Match($AuthorityId, '^authority-stage3-r3-(\d{8}T\d{9}Z)-([0-9a-f]{8})$')
  ...
  $expectedTransferRootEntries = @(
    "Oracle.Stage3R3TransferCustody.json",
    "Oracle.Stage3R3TransferCustody.json.sha256.txt",
    "Oracle.Stage3R3TransferManifest.json",
    "Oracle.Stage3R3TransferManifest.json.sha256.txt",
    "payload"
  ) | Sort-Object
  ...
  if ($manifest.transferId -cnotmatch '^transfer-stage3-r3-\d{8}T\d{9}Z-[0-9a-f]{8}$') {
  ...
  if (
    ...
    [string]$custody.contract -cne
      "oracle.sprint-30-5.stage-3-r3-transfer-custody" -or
  ...
  $expectedPayload = @(
    "Get-OracleStage3R3HostContinuity.ps1",
    "Invoke-OracleStage3R3Qualification.ps1",
    ...
    "Oracle.Stage3R3Contract.json",
    "Oracle.Stage3R3IdentityPolicy.ps1",
  ...
  $runningHarnessEntry = @($manifest.payload | Where-Object {
    [string]$_.path -ceq "payload/Invoke-OracleStage3R3Qualification.ps1"
  })
  $runningContractEntry = @($manifest.payload | Where-Object {
    [string]$_.path -ceq "payload/Oracle.Stage3R3Contract.json"
  })
  $runningIdentityPolicyEntry = @($manifest.payload | Where-Object {
    [string]$_.path -ceq "payload/Oracle.Stage3R3IdentityPolicy.ps1"
  })
  if (
    ...
    (Get-Sha256 (Join-Path $scriptRoot "Oracle.Stage3R3Contract.json")) -cne
...
try {
  ...
  if (
    [string]$hostContinuity.contract -cne
      "oracle.sprint-30-5.stage-3-r3-host-continuity" -or
  ...
  Write-CreateOnlyJson $authorityPath ([ordered]@{
    contract = "oracle.sprint-30-5.stage-3-r3-authority"
    authorityId = $AuthorityId
    attemptId = $AttemptId
  ...
  Write-CreateOnlyJson $evidenceManifestPath ([ordered]@{
    contract = "oracle.sprint-30-5.stage-3-r3-evidence-manifest"
    authorityId = $AuthorityId
    attemptId = $AttemptId
