...
$contract = Get-Content -LiteralPath (Join-Path $PSScriptRoot "Oracle.Stage5R2CleanHostContract.json") -Raw | ConvertFrom-Json
...
try {
  ...
  $record = [ordered]@{ contract = "oracle.sprint-30-5.stage-5-r2-clean-host-companion-transitions"; result = "passed"; classification = "STAGE5-CLEAN-HOST-INSTALLED-RENDERING"; fixture = [ordered]@{ classification = "DISPOSABLE-NON-PRODUCTION-WINDOW-DISCOVERY-FIXTURE"; sha256 = $fixtureHash }; transitions = @($records) }
