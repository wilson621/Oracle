...
$contract = Get-Content -Raw -LiteralPath (
  Join-Path $PSScriptRoot "Oracle.Stage5R2CleanHostContract.json"
) | ConvertFrom-Json
. (Join-Path $PSScriptRoot "Oracle.Stage5R2ObservationOwnershipPolicy.ps1")
$output = [IO.Path]::GetFullPath($OutputPath)
$rehearsalRoot = [IO.Path]::GetFullPath($ApprovedOutputRoot)
if (-not $output.StartsWith($rehearsalRoot.TrimEnd('\') + '\', [StringComparison]::OrdinalIgnoreCase)) {
  throw "Stage 5 observation output escapes the accepted rehearsal root."
}
...
$heldObservationMinimumSeconds = [int]$contract.developmentRehearsalProfile.heldObservationMinimumSeconds
$heldObservationPollStartMaximumSeconds = [int]$contract.developmentRehearsalProfile.heldObservationPollStartMaximumSeconds
$heldObservationCompletionMaximumSeconds = [int]$contract.developmentRehearsalProfile.heldObservationCompletionMaximumSeconds
$minimumHeldSamples = [int]$contract.developmentRehearsalProfile.minimumHeldObservationSamples
...
do {
  ...
  if ($currentGpu.Count -ne 1 -or
    ...
    Start-Sleep -Milliseconds ([int]$contract.developmentRehearsalProfile.sampleCadenceMilliseconds)
    continue
  }
  [void]$gpuPids.Add($gpuPid)
  $gpuUtilization = 0.0
  $pulses = @(Start-OwnedWindowRenderStimulus $runtime)
  try {
    Start-Sleep -Milliseconds 250
    $counter = Get-Counter '\GPU Engine(*)\Utilization Percentage' -SampleInterval 1 -MaxSamples 2 -ErrorAction Stop
