...
function Get-OracleStage3R9OptionalPropertyValue {
  [CmdletBinding()]
  param(
...
function ConvertTo-OracleStage3R9InstalledSoftwareInventory {
  [CmdletBinding()]
  param(
  ...
  foreach ($entry in $RegistryEntries) {
    ...
    $displayNameValue = Get-OracleStage3R9OptionalPropertyValue `
    ...
    $displayVersionValue = Get-OracleStage3R9OptionalPropertyValue `
      -InputObject $entry -Name "DisplayVersion"
    $publisherValue = Get-OracleStage3R9OptionalPropertyValue `
...
function Get-OracleStage3R9InstalledSoftwareInventory {
  [CmdletBinding()]
  param(
  ...
  ConvertTo-OracleStage3R9InstalledSoftwareInventory -RegistryEntries $entries
}
