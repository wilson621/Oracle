...
function Get-OracleStage3R9RequiredProcessMember {
  [CmdletBinding()]
  param(
...
function Invoke-OracleStage3R9GovernedProcess {
  [CmdletBinding()]
  param(
    [Parameter(Mandatory = $true)][string]$Name,
  ...
  foreach ($member in @(
    ...
    [void](Get-OracleStage3R9RequiredProcessMember $record $member)
  }
  ...
  $processError = Get-OracleStage3R9RequiredProcessMember $record "processError"
  $signal = Get-OracleStage3R9RequiredProcessMember $record "signal"
  $exitCode = Get-OracleStage3R9RequiredProcessMember $record "exitCode"
