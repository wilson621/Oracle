...
function Get-OracleStage3R9LifecyclePhases {
  @(
    "authority-consumed",
    "transfer-verified",
    "host-admitted",
...
function New-OracleStage3R9LifecycleState {
...
function Move-OracleStage3R9Lifecycle {
  [CmdletBinding()]
  param(
...
function Assert-OracleStage3R9NextLifecyclePhase {
  [CmdletBinding()]
  param(
    [Parameter(Mandatory = $true)][object]$State,
    [Parameter(Mandatory = $true)][string]$Phase
...
function Get-OracleStage3R9TeardownObligations {
  [CmdletBinding()]
  param(
