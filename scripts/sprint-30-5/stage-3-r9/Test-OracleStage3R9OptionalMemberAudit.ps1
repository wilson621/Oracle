...
function Get-Classification(
  ...
  if (
    ...
    return [ordered]@{
      ...
      testCoverage = "Test-OracleStage3R9OptionalMemberAudit.ps1 and verify-preparation.mjs"
      disposition = "fail-closed-if-missing"
    }
  }
  if (
    $File -ceq "Oracle.Stage3R9PreflightPolicy.ps1" -and
    ...
    return [ordered]@{
      ...
      testCoverage = "Test-OracleStage3R9HostShapeFixtures.ps1"
      disposition = "fail-closed-if-missing"
    }
  }
  if ($Expression -match '\.PSObject\.Properties\[') {
    return [ordered]@{
      ...
      testCoverage = "Test-OracleStage3R9InstalledSoftwarePolicy.ps1"
      disposition = "safe"
    }
  }
  if ($File -ceq "Oracle.Stage3R9InstalledSoftwarePolicy.ps1") {
    return [ordered]@{
  ...
  if ($File -ceq "Oracle.Stage3R9LifecyclePolicy.ps1") {
    return [ordered]@{
      classification = "structurally-guaranteed-by-construction"
      dataSource = "policy-owned lifecycle state"
      mandatory = $true
      protection = "state shape is created only by New-OracleStage3R9LifecycleState"
      testCoverage = "Test-OracleStage3R9LifecyclePolicy.ps1"
      disposition = "safe"
    }
  }
  if ($File -ceq "Oracle.Stage3R9ObservationPolicy.ps1") {
    return [ordered]@{
      ...
      testCoverage = "Test-OracleStage3R9ObservationPolicy.ps1 and development rehearsal"
      disposition = "fail-closed-if-invalid"
    }
  }
  if ($File -ceq "Oracle.Stage3R9PreflightPolicy.ps1") {
    return [ordered]@{
      ...
      mandatory = $true
      protection = "typed producer or Get-OracleStage3R9RequiredPropertyValue"
      testCoverage = "host-shape fixtures and Founder-QA-01 pre-authority probe"
      disposition = "fail-closed-if-unavailable"
    }
  }
  if ($File -ceq "Oracle.Stage3R9ProcessPolicy.ps1") {
    return [ordered]@{
      ...
      mandatory = $true
      protection = "Get-OracleStage3R9RequiredProcessMember checks every member before use"
      testCoverage = "Test-OracleStage3R9ProcessPolicy.ps1"
      disposition = "fail-closed-if-missing"
    }
  }
  if ($File -ceq "Oracle.Stage3R9WindowPolicy.ps1") {
    return [ordered]@{
      ...
      testCoverage = "Test-OracleStage3R9WindowPolicy.ps1"
      disposition = "fail-closed-if-missing-or-mismatched"
    }
  }
  if ($File -ceq "Oracle.Stage3R9PackageInventoryPolicy.ps1") {
    return [ordered]@{
      ...
      testCoverage = "Test-OracleStage3R9PackageInventoryPolicy.ps1"
      disposition = "safe"
    }
  }
  if ($File -ceq "Oracle.Stage3R9IdentityPolicy.ps1") {
    return [ordered]@{
      classification = "structurally-guaranteed-by-platform-type"
      dataSource = "System.StringComparer"
      mandatory = $true
      protection = "typed .NET ordinal comparison"
      testCoverage = "Test-OracleStage3R9IdentityPolicy.ps1"
      disposition = "safe"
    }
  }
  if ($File -ceq "Oracle.Stage3R9CertificateTrustPolicy.ps1") {
    return [ordered]@{
      ...
      testCoverage = "Test-OracleStage3R9CertificateTrustPolicy.ps1"
      disposition = "fail-closed-if-missing-or-mismatched"
    }
  }
  if ($File -ceq "Oracle.Stage3R9ActivationPolicy.ps1") {
    return [ordered]@{
      ...
      testCoverage = "Test-OracleStage3R9ActivationPolicy.ps1 and elevated integration test"
      disposition = "fail-closed-if-missing-or-unsuccessful"
    }
  }
  if ($File -ceq "Oracle.Stage3R9WindowsExecutablePolicy.ps1") {
    return [ordered]@{
      ...
      testCoverage = "Test-OracleStage3R9WindowsExecutablePolicy.ps1"
      disposition = "fail-closed-if-unavailable"
    }
  }
  if ($Expression -match '^\$contract(?:\.|$)') {
    return [ordered]@{
      ...
      dataSource = "Oracle.Stage3R9Contract.json"
      mandatory = $true
...
$report = [ordered]@{
  schemaVersion = "1.0.0"
  contract = "oracle.sprint-30-5.stage-3-r9-optional-member-audit"
