...
validateExecutionIdentity({
  authorityId: "authority-stage3-r9-20260728T223045123Z-a1b2c3d4",
  attemptId: "stage3-r9-20260728T223045123Z-a1b2c3d4",
  timestampUtc,
});
for (const invalid of [
  {
    authorityId: "authority-stage3-r9-20260728T223045123Z-a1b2c3d4",
    attemptId: "stage3-r9-20260728T223045123Z-ffffffff",
    timestampUtc,
  },
  {
    authorityId: "authority-stage3-r9-20260728T223045123Z-a1b2c3d4",
...
validateTransferIdentity({
  transferId: "transfer-stage3-r9-20260728T223045123Z-a1b2c3d4",
  timestampUtc,
});
...
try {
  ...
  const fixtureAttemptId = "stage3-r9-20260728T223045123Z-a1b2c3d4";
  ...
  writeFileSync(
    ...
    `${JSON.stringify({
      contract: "oracle.sprint-30-5.stage-3-r9-evidence-manifest",
  ...
  writeFileSync(
    ...
    `${JSON.stringify({
      contract: "oracle.sprint-30-5.stage-3-r9-lifecycle",
      authorityId: fixtureAuthorityId,
      attemptId: fixtureAttemptId,
  ...
  const archiveCreation = spawnSync(
    ...
    [
      ...
      "-NonInteractive",
      "-Command",
      "Compress-Archive -LiteralPath $env:ORACLE_R9_FIXTURE_SOURCE -DestinationPath $env:ORACLE_R9_FIXTURE_DESTINATION",
    ],
    {
      encoding: "utf8",
      shell: false,
      env: {
        ...process.env,
        ORACLE_R9_FIXTURE_SOURCE: fixtureAttemptRoot,
        ORACLE_R9_FIXTURE_DESTINATION: returnedArchive,
      },
    }
  );
  ...
  writeFileSync(
    returnedManifest,
    `${JSON.stringify({
      contract: "oracle.sprint-30-5.stage-3-r9-archive-manifest",
      authorityId: fixtureAuthorityId,
      attemptId: fixtureAttemptId,
  ...
  const parent = join(repositoryRoot, ".tmp-stage3-r9-validation");
...
const developmentRehearsalPath = join(
  import.meta.dirname,
  "Invoke-OracleStage3R9DevelopmentRehearsal.ps1"
);
const installedSoftwareFixturePath = join(
  import.meta.dirname,
  "Test-OracleStage3R9InstalledSoftwarePolicy.ps1"
);
const lifecycleFixturePath = join(
  import.meta.dirname,
  "Test-OracleStage3R9LifecyclePolicy.ps1"
);
const hostShapeFixturePath = join(
  import.meta.dirname,
  "Test-OracleStage3R9HostShapeFixtures.ps1"
);
const optionalMemberAuditPath = join(
  import.meta.dirname,
  "Test-OracleStage3R9OptionalMemberAudit.ps1"
);
const processFixturePath = join(
  import.meta.dirname,
  "Test-OracleStage3R9ProcessPolicy.ps1"
);
const windowFixturePath = join(
  import.meta.dirname,
  "Test-OracleStage3R9WindowPolicy.ps1"
);
const observationFixturePath = join(
  import.meta.dirname,
  "Test-OracleStage3R9ObservationPolicy.ps1"
);
const certificateTrustFixturePath = join(
  import.meta.dirname,
  "Test-OracleStage3R9CertificateTrustPolicy.ps1"
);
const activationFixturePath = join(
  import.meta.dirname,
  "Test-OracleStage3R9ActivationPolicy.ps1"
);
const windowsExecutableFixturePath = join(
  import.meta.dirname,
  "Test-OracleStage3R9WindowsExecutablePolicy.ps1"
);
const developmentPlatformFixturePath = join(
  import.meta.dirname,
  "Test-OracleStage3R9DevelopmentPlatformCompatibility.ps1"
);
const phaseAudit = JSON.parse(
  readFileSync(join(import.meta.dirname, "Oracle.Stage3R9PhaseAudit.json"), "utf8")
);
const packageInventoryFixturePath = join(
  import.meta.dirname,
  "Test-OracleStage3R9PackageInventoryPolicy.ps1"
);
const scriptPathFixturePath = join(
  import.meta.dirname,
  "Test-OracleStage3R9ScriptPath.ps1"
);
const preExecutionGate = readFileSync(
  join(
    ...
    "docs",
    "sprints",
    "SPRINT_30_5_STAGE_3_R9_PRE_EXECUTION_GATE.md"
  ),
  "utf8"
);
const preparationValidationReport = readFileSync(
  join(
    ...
    "docs",
    "sprints",
    "SPRINT_30_5_STAGE_3_R9_PREPARATION_VALIDATION_REPORT.md"
  ),
  "utf8"
);
...
assert.match(transferBuilder, /Get-OracleStage3R9HostContinuity\.ps1/u);
assert.match(transferBuilder, /Oracle\.Stage3R9IdentityPolicy\.ps1/u);
assert.match(transferBuilder, /Oracle\.Stage3R9ActivationPolicy\.ps1/u);
assert.match(transferBuilder, /Oracle\.Stage3R9CertificateTrustPolicy\.ps1/u);
assert.match(transferBuilder, /Oracle\.Stage3R9InstalledSoftwarePolicy\.ps1/u);
assert.match(transferBuilder, /Oracle\.Stage3R9LifecyclePolicy\.ps1/u);
assert.match(transferBuilder, /Oracle\.Stage3R9ObservationPolicy\.ps1/u);
assert.match(transferBuilder, /Test-OracleStage3R9ObservationPolicy\.ps1/u);
assert.match(transferBuilder, /Oracle\.Stage3R9PackageInventoryPolicy\.ps1/u);
assert.match(transferBuilder, /Oracle\.Stage3R9PreflightPolicy\.ps1/u);
assert.match(transferBuilder, /Oracle\.Stage3R9ProcessPolicy\.ps1/u);
assert.match(transferBuilder, /Oracle\.Stage3R9WindowPolicy\.ps1/u);
assert.match(transferBuilder, /Test-OracleStage3R9WindowPolicy\.ps1/u);
assert.match(harness, /Oracle\.Stage3R9WindowPolicy\.ps1/u);
assert.match(harness, /Oracle\.Stage3R9ObservationPolicy\.ps1/u);
assert.match(harness, /Invoke-OracleStage3R9CompleteStableObservation/u);
...
assert.match(transferBuilder, /Oracle\.Stage3R9WindowsExecutablePolicy\.ps1/u);
assert.match(transferBuilder, /Invoke-OracleStage3R9PreAuthorityPreflight\.ps1/u);
assert.match(transferBuilder, /Oracle\.Stage3R9OptionalMemberAudit\.json/u);
assert.match(transferBuilder, /Test-OracleStage3R9ActivationPolicy\.ps1/u);
assert.match(harness, /"Test-OracleStage3R9ActivationPolicy\.ps1"/u);
assert.match(transferBuilder, /Oracle\.Stage3R9PhaseAudit\.json/u);
assert.match(transferBuilder, /Oracle\.Stage3R9TransferCustody\.json/u);
...
assert.match(transferBuilder, /previousR8TransferModified: false/u);
...
for (const required of [
  "FOUNDER-AUTHORISED-STAGE3-R9-EXECUTION",
  ...
  "Get-OracleStage3R9InstalledSoftwareInventory",
  "Get-OracleStage3R9PreAuthorityObservation",
  "ConvertFrom-OracleStage3R9WindowDiscoveryJson",
  "Assert-OracleStage3R9ProcessPackageOwnership",
  "Invoke-OracleStage3R9ApplicationActivation",
  "Assert-OracleStage3R9ApplicationActivationSucceeded",
  "initial-activation.json",
  "repair-activation.json",
  "Get-OracleStage3R9TrustImportArguments",
  "Get-OracleStage3R9TrustRemovalArguments",
  "Assert-OracleStage3R9TemporaryTrustState",
  "Assert-OracleStage3R9ExactRemovalTarget",
  "Get-PhysicalExactCertificateMatches",
  "Move-OracleStage3R9Lifecycle",
  "package-content-reconciliation.json",
  "Get-OracleStage3R9PackageZipInventory",
  "ConvertTo-OracleStage3R9CanonicalPackagePath",
  "--remote-debugging",
  ...
  "oracle.sprint-30-5.stage-3-r9-transfer-custody",
...
assert.match(windowPolicy, /Test-OracleStage3R9ProcessExists/u);
...
assert.equal(
  packageScripts["sprint-30-5:stage-3:r9:validate"],
  "node scripts/sprint-30-5/stage-3-r9/verify-preparation.mjs"
);
assert.equal(
  packageScripts["sprint-30-5:stage-3:r9:prepare-transfer"],
  "node scripts/sprint-30-5/stage-3-r9/prepare-transfer.mjs"
);
assert.equal(
  packageScripts["sprint-30-5:stage-3:r9:verify-return"],
  "node scripts/sprint-30-5/stage-3-r9/verify-return.mjs"
);
assert.equal(packageScripts["sprint-30-5:stage-3:r9:execute"], undefined);
for (const revision of ["r6", "r7", "r8"]) {
...
for (const required of [
  "Stage 3 Qualification R9 Pre-Execution Gate",
  "Execution:** Blocked and unauthorised",
  "ExpectedHarnessCommit",
  "ExpectedTransferManifestSha256",
  "ExpectedTransferCustodySha256",
  "ExpectedHostContinuitySha256",
  "FOUNDER-AUTHORISED-STAGE3-R9-TRANSFER",
  "FOUNDER-AUTHORISED-STAGE3-R9-EXECUTION",
...
assert.match(
  transferBuilder,
  /Create-only corrected Stage 3 R9 transfer; R8 and all historical transfers remain immutable/u
);
for (const required of [
  "PSObject.Properties",
...
assert.match(harness, /Get-OracleStage3R9InstalledSoftwareInventory/u);
assert.match(harness, /Get-OracleStage3R9PreAuthorityObservation[\s\S]*New-Item/u);
assert.ok(
  harness.indexOf("Get-OracleStage3R9PreAuthorityObservation") <
...
assert.match(processPolicy, /Get-OracleStage3R9RequiredProcessMember/u);
...
assert.match(preflightPolicy, /Test-OracleStage3R9ApplicationActivationApi/u);
assert.match(lifecyclePolicy, /Get-OracleStage3R9LifecyclePhases/u);
assert.match(preflightPolicy, /required Windows PowerShell 5\.1|Windows PowerShell 5\.1/u);
assert.match(preflightPolicy, /Test-OracleStage3R9ProcessIsElevated/u);
assert.match(preflightPolicy, /Machine-scoped AppX trust requires an elevated/u);
assert.match(preflightPolicy, /Get-OracleStage3R9InstalledSoftwareInventory/u);
...
assert.ok(
  rehearsalResult.realPoliciesExercised.includes(
    "Oracle.Stage3R9WindowPolicy.ps1"
  )
);
assert.ok(
  rehearsalResult.realPoliciesExercised.includes(
    "Oracle.Stage3R9ObservationPolicy.ps1"
  )
);
...
const identityPolicyFixture = spawnSync(
  ...
  [
    ...
    "Bypass",
    "-File",
    join(import.meta.dirname, "Test-OracleStage3R9IdentityPolicy.ps1"),
  ],
...
for (const required of [
  "oracle.sprint-30-5.stage-3-r9-host-continuity",
...
assert.deepEqual(contract.correctionBasis, {
  ...
  predecessorTransferId:
    "transfer-stage3-r8-20260730T205703131Z-8a078ab9",
  ...
  failedPhase: "runtime-observed",
  invalidAssertion:
    "nominal-deadline-and-sample-count-used-instead-of-complete-measured-observation-duration",
  establishedFacts: [
    "machine-trust-succeeded",
    "package-installation-succeeded",
    "direct-activation-returned-s-ok",
    "activation-returned-process-id-14928",
    "native-discovery-found-expected-visible-window",
    "47-observations-were-valid-and-consistent",
    "measured-observation-span-was-59.929-seconds",
    "final-residue-verification-found-zero-residue",
  ],
  requiredSemantics:
    "monotonic evidence measurement from first valid sample through a final valid sample at or after sixty complete seconds; typed OpenProcess exit-race classification with live-process ownership remaining fail-closed",
});
...
console.log("Stage 3 R9 preparation validation passed.");
