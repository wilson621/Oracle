# Sprint 30.5 Stage 3 Qualification R9 Harness

This directory contains the prepared, R2-bound Stage 3 R9 transfer and clean
Windows qualification system.

Preparation and validation are authorised by the R9 observation and teardown
correction mission.
...
The shared `Oracle.Stage3R9IdentityPolicy.ps1` implements ordinal
...
The shared `Oracle.Stage3R9CertificateTrustPolicy.ps1` distinguishes physical
certificate-store entries from inherited logical provider projections. R9
...
R9 is the current attempt-scoped qualification revision, distinct from the
immutable historical `stage-3-revision-6-abandoned-design` record. Closed R1
through R8 repository transfer and qualification entry points are fail-closed;
their artifacts and evidence remain immutable.

The shared `Oracle.Stage3R9ActivationPolicy.ps1` uses the Windows
...
The shared `Oracle.Stage3R9PackageInventoryPolicy.ps1` defines the canonical
...
The shared `Oracle.Stage3R9InstalledSoftwarePolicy.ps1` enumerates native
...
`Invoke-OracleStage3R9PreAuthorityPreflight.ps1` is the only target-host
...
`Oracle.Stage3R9ProcessPolicy.ps1` is the single external-process envelope
...
`Oracle.Stage3R9ObservationPolicy.ps1` measures the interval from the first
valid captured observation to the final valid captured observation with a
monotonic stopwatch. Sampling continues until that evidence interval is at
least 60,000 milliseconds. Sample count, a nominal deadline, UTC rounding and
tolerance are not duration proof.

`Oracle.Stage3R9WindowPolicy.ps1` retains exact AppModel package-family
ownership for live processes. During teardown, only a typed `OpenProcess`
failure followed immediately by proof that the PID no longer exists is
classified as an ordinary exit race. Access denial for a live process,
identity mismatch and every other ownership failure remain fatal. Final
zero-process residue verification remains mandatory.

`Oracle.Stage3R9WindowsExecutablePolicy.ps1` maps only the System32-bound
...
`Oracle.Stage3R9LifecyclePolicy.ps1` is shared by production lifecycle
...
`Get-OracleStage3R9HostContinuity.ps1` creates the fresh, create-only,
...
`docs/sprints/SPRINT_30_5_STAGE_3_R9_PRE_EXECUTION_GATE.md`.
