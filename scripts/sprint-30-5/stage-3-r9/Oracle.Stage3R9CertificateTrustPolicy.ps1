...
function Assert-OracleStage3R9CertificateTrustContract {
...
function Assert-OracleStage3R9Thumbprint {
...
function Get-OracleStage3R9CertificateStoreDefinitions {
  @(
...
function Get-OracleStage3R9PhysicalCertificateMatches {
  param([Parameter(Mandatory = $true)][string]$Thumbprint)

  Assert-OracleStage3R9Thumbprint -Thumbprint $Thumbprint
  $matches = @()
  foreach ($definition in @(Get-OracleStage3R9CertificateStoreDefinitions)) {
...
function Get-OracleStage3R9LogicalCertificateViews {
  param([Parameter(Mandatory = $true)][string]$Thumbprint)
...
function Test-OracleStage3R9CertificateIdentity {
  param(
...
function Assert-OracleStage3R9NoCertificateResidue {
  param(
...
function Assert-OracleStage3R9TemporaryTrustState {
  param(
    [Parameter(Mandatory = $true)]
    [AllowEmptyCollection()][object[]]$PhysicalMatches,
    [Parameter(Mandatory = $true)]
  ...
  if (
    ...
    -not (Test-OracleStage3R9CertificateIdentity `
...
function Assert-OracleStage3R9ExactRemovalTarget {
  param(
    [Parameter(Mandatory = $true)]
    [AllowEmptyCollection()][object[]]$PhysicalMatches,
    [Parameter(Mandatory = $true)]
...
function Test-OracleStage3R9ProcessIsElevated {
