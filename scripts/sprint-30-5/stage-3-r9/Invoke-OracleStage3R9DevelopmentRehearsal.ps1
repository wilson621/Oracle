...
. (Join-Path $PSScriptRoot "Oracle.Stage3R9LifecyclePolicy.ps1")
. (Join-Path $PSScriptRoot "Oracle.Stage3R9IdentityPolicy.ps1")
. (Join-Path $PSScriptRoot "Oracle.Stage3R9ActivationPolicy.ps1")
. (Join-Path $PSScriptRoot "Oracle.Stage3R9CertificateTrustPolicy.ps1")
. (Join-Path $PSScriptRoot "Oracle.Stage3R9InstalledSoftwarePolicy.ps1")
. (Join-Path $PSScriptRoot "Oracle.Stage3R9ObservationPolicy.ps1")
. (Join-Path $PSScriptRoot "Oracle.Stage3R9ProcessPolicy.ps1")
. (Join-Path $PSScriptRoot "Oracle.Stage3R9WindowPolicy.ps1")
...
function Invoke-SimulatedPhase(
  ...
  Assert-OracleStage3R9NextLifecyclePhase -State $State -Phase $Phase
  ...
  [void](Move-OracleStage3R9Lifecycle -State $State -Phase $Phase)
}

function Invoke-SimulatedLifecycle([string]$Root, [string]$InjectedFailure) {
  $state = New-OracleStage3R9LifecycleState
  $failure = $null
  try {
    foreach ($phase in @(Get-OracleStage3R9LifecyclePhases)) {
  ...
  $teardown = Get-OracleStage3R9TeardownObligations `
...
$parent = Join-Path $repositoryRoot ".tmp-stage3-r9-development-rehearsal"
...
try {
  $software = @(ConvertTo-OracleStage3R9InstalledSoftwareInventory -RegistryEntries @(
  ...
  foreach ($name in @("certutil.exe", "reagentc.exe")) {
    ...
    [void](Get-OracleStage3R9WindowsExecutablePath `
      -Name $name -SystemDirectory $fixtureSystem32)
  }
  $fixtureActivation = Invoke-OracleStage3R9ApplicationActivation `
  ...
  $fixtureWindows = @(
    ConvertFrom-OracleStage3R9WindowDiscoveryJson -Json (
  ...
  if (
    ...
    -not (Test-OracleStage3R9QualifyingWindow $fixtureWindows[0])
  ) { throw "Shared window discovery policy failed inside rehearsal." }
  [void](Assert-OracleStage3R9ProcessPackageOwnership `
    -ProcessId 9808 `
  ...
  $observation = Invoke-OracleStage3R9CompleteStableObservation `
  ...
  $exitedProcess = Resolve-OracleStage3R9TeardownProcessOwnership `
    -ProcessId 9808 `
    -ExpectedPackageFamilyName "Oracle.Platform.LocalCertification_fixture" `
    -OwnershipResolver {
      param($ProcessId)
      throw [Oracle.Stage3R9.ProcessOpenException]::new(
        [uint32]$ProcessId, 87
      )
    } `
  ...
  $processCounts = @{}
  $processResult = Invoke-OracleStage3R9GovernedProcess `
