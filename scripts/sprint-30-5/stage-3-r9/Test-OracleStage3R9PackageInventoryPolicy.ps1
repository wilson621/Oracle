...
$fixtureRoot = Join-Path (
  [IO.Path]::GetTempPath()
) "oracle-stage3-r9-package-inventory-$([Guid]::NewGuid().ToString('N'))"
...
try {
  ...
  $first = @(Get-OracleStage3R9PackageZipInventory $validZip)
  $second = @(Get-OracleStage3R9PackageZipInventory $validZip)
  if (
    $first.Count -ne 3 -or
  ...
  try {
    Get-OracleStage3R9PackageZipInventory $duplicateZip | Out-Null
  ...
  try {
    Get-OracleStage3R9PackageZipInventory $traversalZip | Out-Null
  ...
  $contentMismatch = @(
    Get-OracleStage3R9PackageZipInventory $contentMismatchZip |
  ...
  $metadataMismatch = @(
    Get-OracleStage3R9PackageZipInventory $metadataMismatchZip |
  ...
  foreach ($entry in @($acceptedInventory.entries)) {
    ...
    if (
      ...
      (Test-OracleStage3R9ReservedPackageMetadata $canonical) -or
  ...
  $acceptedZip = @(
    Get-OracleStage3R9PackageZipInventory $AcceptedPackagePath
  )
