...
function Assert-OracleStage3R9ApplicationActivationContract {
  [CmdletBinding()]
...
function Initialize-OracleStage3R9ActivationApi {
  if ($null -ne ("Oracle.Stage3R9.ApplicationActivation" -as [type])) {
    return
  }
...
function Test-OracleStage3R9ApplicationActivationApi {
  [CmdletBinding()]
  ...
  try {
    $native = if ($null -ne $ProbeRunner) {
      ...
      Initialize-OracleStage3R9ActivationApi
      [Oracle.Stage3R9.ApplicationActivation]::Probe()
    }
    if (
      $null -eq $native -or
...
function Invoke-OracleStage3R9ApplicationActivation {
  [CmdletBinding()]
  param(
  ...
  try {
    $native = if ($null -ne $ActivationRunner) {
      ...
      Initialize-OracleStage3R9ActivationApi
      [Oracle.Stage3R9.ApplicationActivation]::Activate(
        $AppUserModelId,
        $Arguments,
        [uint32]2
      )
...
function Assert-OracleStage3R9ApplicationActivationSucceeded {
  [CmdletBinding()]
