...
$classification = @(
  "STAGE-4-R5-INSTALLED-PACKAGE-CONTROLLER",
  "SECRET-BEARING-NON-EVIDENCE-CONTROLLER"
)
$scriptRoot = $PSScriptRoot
$payloadRoot = [IO.Path]::GetFullPath((Join-Path $scriptRoot ".."))
$contractPath = Join-Path $scriptRoot "Oracle.Stage4R5ExecutionContract.json"
...
if ([string]$contract.executionSurface -cne "accepted-r8-installed-msix") {
  throw "R5 execution surface differs."
}

$attemptRoot = [IO.Path]::GetFullPath((Require-Environment "ORACLE_STAGE4_ATTEMPT_ROOT"))
$approvedAttemptRootSetting = if($developmentRehearsal){[string]$contract.paths.qualificationRehearsalRoot}else{[string]$contract.paths.qualificationAttemptRoot}
$approvedAttemptParent = [IO.Path]::GetFullPath($approvedAttemptRootSetting).TrimEnd("\") + "\"
if (-not $attemptRoot.StartsWith($approvedAttemptParent,[StringComparison]::OrdinalIgnoreCase)) { throw "Attempt root escapes the clean-host boundary." }
$logsRoot = Join-Path $attemptRoot "logs"
$journeyOutput = [IO.Path]::GetFullPath((Require-Environment "ORACLE_STAGE4_JOURNEY_OUTPUT"))
$transferRootSetting = Require-Environment "ORACLE_STAGE4_TRANSFER_ROOT"
$transferRootFull = [IO.Path]::GetFullPath($transferRootSetting)
$msixPath = [IO.Path]::GetFullPath((Join-Path $transferRootFull ("payload\release\" + [string]$contract.package.fileName)))
$certificatePath = [IO.Path]::GetFullPath((Join-Path $transferRootFull ("payload\release\" + [string]$contract.package.publicCertificateFileName)))
...
try {
  if ($TeardownOnly) {
    ...
    if ($developmentRehearsal) {
      ...
      $attemptId = Require-Environment "ORACLE_STAGE4_R5_REHEARSAL_IDENTITY"
      ...
      if (
        ...
        [string]$contract.executionAuthority.authorityCreationPermittedAfterAllPreAuthorityGates -ne "True" -or
        [string]$contract.executionAuthority.qualificationAttemptPermitted -ne "True"
      ) {
        throw "R5 contract does not authorise qualification execution."
      }
    ...
    if ((Get-FileHash -LiteralPath $certificatePath -Algorithm SHA256).Hash.ToLowerInvariant() -cne [string]$contract.package.publicCertificateSha256) {
      throw "Accepted R8 public certificate hash differs."
    }
    ...
    if (
      ...
      throw "Accepted R8 public certificate identity differs."
    }
    ...
    if ($import.Thumbprint -cne [string]$contract.stage2.certificateThumbprint) {
      throw "Exact R8 trust import differs."
    }
    ...
    if ($packages.Count -ne 1 -or [string]$packages[0].Version -cne [string]$contract.package.version) {
      throw "Exact R8 package registration differs."
    }
    ...
    if ($null -eq $applicationData -or [IO.Path]::GetFullPath([string]$applicationData.LocalFolder.Path) -cne [IO.Path]::GetFullPath((Join-Path $packageRoot "LocalState"))) {
      throw "Installed R5 package data initialization differs."
    }
    ...
    Assert-OracleStage3R13ApplicationActivationSucceeded $activation
    ...
    $mailpitUrl = Require-Environment "ORACLE_STAGE4_MAILPIT_URL"
    $journeyScript = Join-Path $scriptRoot "Invoke-OracleStage4R5CleanHostJourney.ps1"
    & $journeyScript -ProviderUrl $providerUrl -MailpitUrl $mailpitUrl -WebOrigin ([string]$server.origin) -AnonymousKey $anonKey -ServiceKey $serviceSecure -OutputPath $journeyOutput -DevelopmentRehearsal:$developmentRehearsal
    if (-not (Test-Path -LiteralPath $journeyOutput -PathType Leaf)) { throw "Installed R5 live journey output is absent." }
  ...
  try {
    ...
    if (@(Get-ExactCertificate).Count -ne 0) { throw "Exact R8 trust residue remains." }
...
$result = [pscustomobject][ordered]@{
  contract = "oracle.sprint-30-5.stage-4-r5-installed-package-result"
