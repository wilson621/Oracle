...
$hostAdmission=Get-OracleStage4R5CleanHostAdmission $contract;$admission=Get-Content -Raw -LiteralPath $admissionPath|ConvertFrom-Json;[void](Assert-OracleStage4R5ProviderAdmission $admission $contract);if([string]$admission.providerIdentity-cne[string]$request.providerIdentity-or[string]$admission.transferId-cne[string]$transfer.transferId){throw 'Rehearsal provider admission differs.'}
