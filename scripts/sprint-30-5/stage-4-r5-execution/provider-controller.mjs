...
if(mode==="start"){
  ...
  try{
    writeJsonCreateOnly(statePath,{schemaVersion:"1.0.0",contract:"oracle.sprint-30-5.stage-4-r5-provider-state",state:"starting",transferId:request.transferId,authorityId:authorityId,attemptId:runId,providerIdentity:request.providerIdentity,projectId,providerRoot,firewallNames,createdAtUtc:new Date().toISOString()});
    ...
    if(cleanup.length===0&&existsSync(statePath))rmSync(statePath,{force:false});
  ...
  const cleanup=[];if(existsSync(join(state.providerRoot,"supabase","config.toml")))try{run("provider-stop",node,[cli,"stop","--workdir",state.providerRoot,"--no-backup"]);}catch(error){cleanup.push(error.message)}
