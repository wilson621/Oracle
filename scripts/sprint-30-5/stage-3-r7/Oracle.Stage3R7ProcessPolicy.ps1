...
function Get-OracleStage3R7RequiredProcessMember {
  [CmdletBinding()]
  param(
...
function Invoke-OracleStage3R7GovernedProcess {
  [CmdletBinding()]
  param(
    [Parameter(Mandatory = $true)][string]$Name,
  ...
  foreach ($member in @(
    ...
    [void](Get-OracleStage3R7RequiredProcessMember $record $member)
  }
  ...
  $processError = Get-OracleStage3R7RequiredProcessMember $record "processError"
  $signal = Get-OracleStage3R7RequiredProcessMember $record "signal"
  $exitCode = Get-OracleStage3R7RequiredProcessMember $record "exitCode"
