...
function Get-OracleStage3R7OptionalPropertyValue {
  [CmdletBinding()]
  param(
...
function ConvertTo-OracleStage3R7InstalledSoftwareInventory {
  [CmdletBinding()]
  param(
  ...
  foreach ($entry in $RegistryEntries) {
    ...
    $displayNameValue = Get-OracleStage3R7OptionalPropertyValue `
    ...
    $displayVersionValue = Get-OracleStage3R7OptionalPropertyValue `
      -InputObject $entry -Name "DisplayVersion"
    $publisherValue = Get-OracleStage3R7OptionalPropertyValue `
...
function Get-OracleStage3R7InstalledSoftwareInventory {
  [CmdletBinding()]
  param(
  ...
  ConvertTo-OracleStage3R7InstalledSoftwareInventory -RegistryEntries $entries
}
