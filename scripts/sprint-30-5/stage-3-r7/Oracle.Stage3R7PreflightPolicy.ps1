...
function Get-OracleStage3R7RequiredPropertyValue {
  [CmdletBinding()]
  param(
...
function Assert-OracleStage3R7CommandSurface {
  [CmdletBinding()]
  param()
...
function Get-OracleStage3R7AcceptedPublicCertificate {
  [CmdletBinding()]
  param(
...
function Invoke-OracleStage3R7ReadOnlyProcess {
  [CmdletBinding()]
  param(
...
function Assert-OracleStage3R7NoReparseTraversal([string]$Path) {
...
function Get-OracleStage3R7PreAuthorityObservation {
  [CmdletBinding()]
  param(
    [Parameter(Mandatory = $true)][object]$Contract,
    [Parameter(Mandatory = $true)][string]$TransferRoot,
  ...
  if (
    ...
    -not (Test-OracleStage3R7ProcessIsElevated)
  ) {
  ...
  foreach ($path in @($TransferRoot, $EvidenceReturnRoot, $HostContinuityPath)) {
    Assert-OracleStage3R7NoReparseTraversal $path
  }
  if (
  ...
  $commands = @(Assert-OracleStage3R7CommandSurface)
  $windowsExecutables = [ordered]@{
    certutil = Get-OracleStage3R7WindowsExecutablePath -Name "certutil.exe"
    reagentc = Get-OracleStage3R7WindowsExecutablePath -Name "reagentc.exe"
  }
  $applicationActivation = Test-OracleStage3R7ApplicationActivationApi
  ...
  $recovery = Invoke-OracleStage3R7ReadOnlyProcess `
  ...
  $manufacturer = [string](Get-OracleStage3R7RequiredPropertyValue `
    $computerSystem "Manufacturer" "Win32_ComputerSystem")
  $model = [string](Get-OracleStage3R7RequiredPropertyValue `
  ...
  foreach ($member in @("TpmPresent", "TpmReady")) {
    [void](Get-OracleStage3R7RequiredPropertyValue $tpm $member "Get-Tpm")
  }
  foreach ($member in @("AntivirusEnabled", "RealTimeProtectionEnabled")) {
    [void](Get-OracleStage3R7RequiredPropertyValue $defender $member "Get-MpComputerStatus")
  }
  if (
    -not $secureBoot -or
    -not [bool](Get-OracleStage3R7RequiredPropertyValue $tpm "TpmPresent" "Get-Tpm") -or
    -not [bool](Get-OracleStage3R7RequiredPropertyValue $tpm "TpmReady" "Get-Tpm") -or
    -not [bool](Get-OracleStage3R7RequiredPropertyValue (
      $defender
  ...
  $activation = @(Get-CimInstance SoftwareLicensingProduct -ErrorAction Stop |
    Where-Object {
      $partialKey = Get-OracleStage3R7RequiredPropertyValue `
        $_ "PartialProductKey" "SoftwareLicensingProduct"
      $licenseStatus = Get-OracleStage3R7RequiredPropertyValue `
  ...
  $installedSoftware = @(Get-OracleStage3R7InstalledSoftwareInventory)
  ...
  foreach ($member in @("Status", "SignerCertificate")) {
    [void](Get-OracleStage3R7RequiredPropertyValue (
      $untrustedSignature
    ) $member "Get-AuthenticodeSignature")
  }
  $signerCertificate = Get-OracleStage3R7AcceptedPublicCertificate `
    -Contract $Contract -TransferRoot $TransferRoot
  $observedUntrustedSigner = Get-OracleStage3R7RequiredPropertyValue `
    $untrustedSignature "SignerCertificate" "Get-AuthenticodeSignature"
  if (
    [string](Get-OracleStage3R7RequiredPropertyValue (
      $untrustedSignature
  ...
  foreach ($member in @(
    ...
    [void](Get-OracleStage3R7RequiredPropertyValue $continuity $member "host continuity")
  }
  foreach ($member in @("deviceName", "manufacturer", "model")) {
    [void](Get-OracleStage3R7RequiredPropertyValue $continuity.host $member "host continuity host")
  }
  ...
  if (
    [string]$continuity.contract -cne
      "oracle.sprint-30-5.stage-3-r7-host-continuity" -or
  ...
  [pscustomobject][ordered]@{
    ...
    security = [ordered]@{
      ...
      tpmPresent = [bool](Get-OracleStage3R7RequiredPropertyValue (
        $tpm
      ) "TpmPresent" "Get-Tpm")
      tpmReady = [bool](Get-OracleStage3R7RequiredPropertyValue (
        $tpm
      ) "TpmReady" "Get-Tpm")
      defenderAntivirusEnabled = [bool](Get-OracleStage3R7RequiredPropertyValue (
        $defender
      ) "AntivirusEnabled" "Get-MpComputerStatus")
      defenderRealTimeProtectionEnabled = [bool](
        Get-OracleStage3R7RequiredPropertyValue (
          $defender
        ) "RealTimeProtectionEnabled" "Get-MpComputerStatus"
      )
