...
. (Join-Path $PSScriptRoot "Oracle.Stage3R7PreflightPolicy.ps1")
...
$inventory = @(Get-OracleStage3R7InstalledSoftwareInventory)
...
$contract = Get-Content -LiteralPath (
  Join-Path $PSScriptRoot "Oracle.Stage3R7Contract.json"
...
$fixtureRoot = Join-Path $env:TEMP (
  "oracle-stage3-r7-compatibility-" + [Guid]::NewGuid().ToString("N")
)
...
try {
  ...
  $detachedSigner = Get-OracleStage3R7AcceptedPublicCertificate `
