...
if (
  ...
  [string]$manifest.programmeIdentity -cne
    "Sprint 30.5 Stage 3 Qualification R7" -or
...
$runningScriptRelativePath = "payload/Invoke-OracleStage3R7PreAuthorityPreflight.ps1"
$runningScriptEntry = @($payloadEntries | Where-Object {
  [string]$_.path -ceq $runningScriptRelativePath
})
$contractRelativePath = "payload/Oracle.Stage3R7Contract.json"
...
$result = Get-OracleStage3R7PreAuthorityObservation `
  -Contract $contract `
