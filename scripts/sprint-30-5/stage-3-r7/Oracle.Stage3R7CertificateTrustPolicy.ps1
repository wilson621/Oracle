...
function Assert-OracleStage3R7CertificateTrustContract {
...
function Assert-OracleStage3R7Thumbprint {
...
function Get-OracleStage3R7CertificateStoreDefinitions {
  @(
...
function Get-OracleStage3R7PhysicalCertificateMatches {
  param([Parameter(Mandatory = $true)][string]$Thumbprint)

  Assert-OracleStage3R7Thumbprint -Thumbprint $Thumbprint
  $matches = @()
  foreach ($definition in @(Get-OracleStage3R7CertificateStoreDefinitions)) {
...
function Get-OracleStage3R7LogicalCertificateViews {
  param([Parameter(Mandatory = $true)][string]$Thumbprint)
...
function Test-OracleStage3R7CertificateIdentity {
  param(
...
function Assert-OracleStage3R7NoCertificateResidue {
  param(
...
function Assert-OracleStage3R7TemporaryTrustState {
  param(
    [Parameter(Mandatory = $true)]
    [AllowEmptyCollection()][object[]]$PhysicalMatches,
    [Parameter(Mandatory = $true)]
  ...
  if (
    ...
    -not (Test-OracleStage3R7CertificateIdentity `
...
function Assert-OracleStage3R7ExactRemovalTarget {
  param(
    [Parameter(Mandatory = $true)]
    [AllowEmptyCollection()][object[]]$PhysicalMatches,
    [Parameter(Mandatory = $true)]
...
function Test-OracleStage3R7ProcessIsElevated {
