# Sprint 30.5 Stage 3 Qualification R7 Harness

This directory contains the prepared, R2-bound Stage 3 R7 transfer and clean
Windows qualification system.

Preparation and validation are authorised by the R7 launch-correction mission.
...
The shared `Oracle.Stage3R7IdentityPolicy.ps1` implements ordinal
...
The shared `Oracle.Stage3R7CertificateTrustPolicy.ps1` distinguishes physical
certificate-store entries from inherited logical provider projections. R7
...
R7 is the current attempt-scoped qualification revision, distinct from the
...
The shared `Oracle.Stage3R7ActivationPolicy.ps1` uses the Windows
...
The shared `Oracle.Stage3R7PackageInventoryPolicy.ps1` defines the canonical
...
The shared `Oracle.Stage3R7InstalledSoftwarePolicy.ps1` enumerates native
...
`Invoke-OracleStage3R7PreAuthorityPreflight.ps1` is the only target-host
...
`Oracle.Stage3R7ProcessPolicy.ps1` is the single external-process envelope
...
`Oracle.Stage3R7WindowsExecutablePolicy.ps1` maps only the System32-bound
...
`Oracle.Stage3R7LifecyclePolicy.ps1` is shared by production lifecycle
...
`Get-OracleStage3R7HostContinuity.ps1` creates the fresh, create-only,
...
`docs/sprints/SPRINT_30_5_STAGE_3_R7_PRE_EXECUTION_GATE.md`.
