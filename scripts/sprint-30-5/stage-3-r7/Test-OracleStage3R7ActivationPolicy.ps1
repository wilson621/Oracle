...
Assert-Rejected {
  Assert-OracleStage3R7ApplicationActivationContract -Contract $invalidContract
} "Explorer exit-code contract"

$aumid = "Oracle.Platform.LocalCertification_test!Oracle"
$success = Invoke-OracleStage3R7ApplicationActivation `
...
Assert-OracleStage3R7ApplicationActivationSucceeded -Result $success

$availability = Test-OracleStage3R7ApplicationActivationApi -ProbeRunner {
...
Assert-Rejected {
  Invoke-OracleStage3R7ApplicationActivation -AppUserModelId "not-an-aumid"
} "malformed AUMID"
Assert-Rejected {
  $failure = Invoke-OracleStage3R7ApplicationActivation `
    -AppUserModelId $aumid -ActivationRunner {
      [pscustomobject]@{ HResult = "0x80004005"; ProcessId = 0 }
    }
  Assert-OracleStage3R7ApplicationActivationSucceeded -Result $failure
} "HRESULT failure"
Assert-Rejected {
  $zeroPid = Invoke-OracleStage3R7ApplicationActivation `
    -AppUserModelId $aumid -ActivationRunner {
      [pscustomobject]@{ HResult = "0x00000000"; ProcessId = 0 }
    }
  Assert-OracleStage3R7ApplicationActivationSucceeded -Result $zeroPid
} "zero process ID"
Assert-Rejected {
  $runnerError = Invoke-OracleStage3R7ApplicationActivation `
    -AppUserModelId $aumid -ActivationRunner {
      throw "simulated activation error"
    }
  Assert-OracleStage3R7ApplicationActivationSucceeded -Result $runnerError
} "runner error"
Assert-Rejected {
  $incomplete = Invoke-OracleStage3R7ApplicationActivation `
    -AppUserModelId $aumid -ActivationRunner {
      [pscustomobject]@{ HResult = "0x00000000" }
    }
  Assert-OracleStage3R7ApplicationActivationSucceeded -Result $incomplete
} "incomplete native result"
Assert-Rejected {
  $nullResult = Invoke-OracleStage3R7ApplicationActivation `
    -AppUserModelId $aumid -ActivationRunner { $null }
  Assert-OracleStage3R7ApplicationActivationSucceeded -Result $nullResult
} "null native result"
Assert-Rejected {
  $missing = $success | Select-Object * -ExcludeProperty hresult
  Assert-OracleStage3R7ApplicationActivationSucceeded -Result $missing
