...
function Get-OracleStage3R7LifecyclePhases {
  @(
    "authority-consumed",
    "transfer-verified",
    "host-admitted",
...
function New-OracleStage3R7LifecycleState {
...
function Move-OracleStage3R7Lifecycle {
  [CmdletBinding()]
  param(
...
function Assert-OracleStage3R7NextLifecyclePhase {
  [CmdletBinding()]
  param(
    [Parameter(Mandatory = $true)][object]$State,
    [Parameter(Mandatory = $true)][string]$Phase
...
function Get-OracleStage3R7TeardownObligations {
  [CmdletBinding()]
  param(
