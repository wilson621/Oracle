...
function Assert-OracleStage3R7ApplicationActivationContract {
  [CmdletBinding()]
...
function Initialize-OracleStage3R7ActivationApi {
  if ($null -ne ("Oracle.Stage3R7.ApplicationActivation" -as [type])) {
    return
  }
...
function Test-OracleStage3R7ApplicationActivationApi {
  [CmdletBinding()]
  ...
  try {
    $native = if ($null -ne $ProbeRunner) {
      ...
      Initialize-OracleStage3R7ActivationApi
      [Oracle.Stage3R7.ApplicationActivation]::Probe()
    }
    if (
      $null -eq $native -or
...
function Invoke-OracleStage3R7ApplicationActivation {
  [CmdletBinding()]
  param(
  ...
  try {
    $native = if ($null -ne $ActivationRunner) {
      ...
      Initialize-OracleStage3R7ActivationApi
      [Oracle.Stage3R7.ApplicationActivation]::Activate(
        $AppUserModelId,
        $Arguments,
        [uint32]2
      )
...
function Assert-OracleStage3R7ApplicationActivationSucceeded {
  [CmdletBinding()]
