...
try {
  Assert-OracleStage3R7CertificateTrustContract -Contract $invalidContract
} catch {
...
$pathWithSpaces = "C:\Stage 3 R7\attempt certificate.cer"
$importArguments = @(Get-OracleStage3R7TrustImportArguments $pathWithSpaces)
if (
...
$removalArguments = @(Get-OracleStage3R7TrustRemovalArguments $thumbprint)
if (
