...
export const R4_GOVERNED_WRAPPER_PROTOCOL =
  "oracle-stage2-r4-governed-wrapper-v1";
...
const attemptPattern = /^r4-(\d{8}T\d{9}Z)-([0-9a-f]{8})$/u;
...
export function assertOutsideHistoricalRoots(path) {
  ...
  for (const protectedRoot of contract.historicalProtectedRoots) {
    ...
    if (isSameOrDescendant(resolved, historical)) {
      throw new Error(
        `Refusing R4 output inside immutable historical root: ${protectedRoot}`
      );
    }
  }
  return resolved;
...
export function assertR4ArtifactPath(path, attemptId) {
  ...
  if (resolved !== requiredAttemptRoot) {
    throw new Error(
      `R4 output root must resolve exactly to ${relative(repositoryRoot, requiredAttemptRoot).replaceAll("\\", "/")}.`
    );
  }
  return resolved;
}
...
export function assertNoReparseTraversal(
  ...
  if (!isSameOrDescendant(resolved, repositoryRoot)) {
    throw new Error("Governed R4 output must remain inside the repository.");
  }
  ...
  for (const segment of segments) {
    ...
    if (
      ...
      throw new Error(`R4 output path traverses a symbolic link or junction: ${current}`);
    }
  }
}
...
export function validateGovernedWrapperInvocation({ observed, parentProcessId }) {
  ...
  const expected = `${R4_GOVERNED_WRAPPER_PROTOCOL}:${parentProcessId}`;
  if (observed !== expected) {
    throw new Error(
      "The R4 executor must be invoked by the governed invoke-attempt.ps1 wrapper."
    );
  }
  return Object.freeze({
    protocol: R4_GOVERNED_WRAPPER_PROTOCOL,
    parentProcessId,
  });
}
...
export function validateAttemptIdentity({ attemptId, timestampUtc }) {
  ...
  if (!match) {
    throw new Error("Attempt ID must use r4-YYYYMMDDTHHMMSSmmmZ-xxxxxxxx.");
  }
  if (match[2] === "00000000") {
...
export function validateBinding(binding) {
  ...
  if (binding.candidateCommit !== contract.candidate.commit) {
    throw new Error(`R4 candidate commit mismatch: expected ${contract.candidate.commit}, received ${binding.candidateCommit}.`);
  }
  if (
...
export function validateRepositorySnapshot(snapshot, binding) {
  ...
  if (snapshot.status !== "") {
    throw new Error("R4 requires a clean repository and index.");
  }
}
...
export function assertRepositoryPreflight(binding) {
  ...
  if (candidateTree !== contract.candidate.tree) {
    throw new Error(`R4 candidate tree mismatch: expected ${contract.candidate.tree}, found ${candidateTree}.`);
  }
...
export function validateMachineQualificationState(result) {
  ...
  if (result.certificates.length !== 0) {
    throw new Error(
      "An R4 temporary certificate identity already exists in a governed store."
    );
  }
}
...
export function createAttemptRecord(input) {
  ...
  const outputRoot = assertR4ArtifactPath(input.outputRoot, input.attemptId);
  return {
    schemaVersion: "1.0.0",
    contract: "oracle.sprint-30-5.stage-2-requalification-r4-attempt",
    ...
    authority: {
      ...
      stage3: "closed-not-authorised-by-r4",
      stage4: "closed-not-authorised-by-r4",
...
export function createAttemptDirectory(record) {
  const outputRoot = assertR4ArtifactPath(record.outputRoot, record.attemptId);
  ...
  writeJsonAtomicCreateOnly(
    join(outputRoot, "Oracle.Stage2RequalificationR4Attempt.json"),
    record
  );
  return outputRoot;
}
