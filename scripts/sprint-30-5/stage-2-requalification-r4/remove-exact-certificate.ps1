...
function Assert-CreateOnlyOutputPath {
  ...
  if (
    -not $relative.StartsWith(
      ".artifacts/sprint-30-5/stage-2-requalification-r4/",
      [StringComparison]::OrdinalIgnoreCase
    ) -or
    $relative.Contains("/../") -or
    $relative.EndsWith("/..", [StringComparison]::Ordinal)
  ) {
    throw "Cleanup evidence must be inside an R4 attempt artifact root."
  }

  $attemptRelative = $relative.Substring(
    ".artifacts/sprint-30-5/stage-2-requalification-r4/".Length
  )
  $attemptId = $attemptRelative.Split("/")[0]
  if ($attemptId -notmatch "^r4-\d{8}T\d{9}Z-[0-9a-f]{8}$") {
    throw "Cleanup evidence does not identify a valid immutable R4 attempt."
  }
...
if ($matches.Count -eq 0) {
  throw "The exact generated R4 certificate thumbprint was not found."
}
...
if ($rootMatches.Count -eq 1) {
  ...
  if ($PSCmdlet.ShouldProcess($rootTarget, "Remove exact R4 Root trust")) {
    $rootRemoval = Invoke-ExactRootRemoval
    [Console]::Error.WriteLine(
      "ORACLE_R4_ROOT_REMOVAL_PROCESS=" +
      (
        $rootRemoval |
...
if ($PSCmdlet.ShouldProcess($myTarget, "Remove exact R4 signing certificate")) {
  $myBeforeRemoval = @(
...
if ($remaining.Count -ne 0) {
  throw "Exact R4 certificate thumbprint remains after cleanup."
}

$evidence = [ordered]@{
  schemaVersion = "1.0.0"
  contract = "oracle.sprint-30-5.stage-2-requalification-r4-certificate-cleanup"
  programmeIdentity = "Sprint 30.5 Stage 2 Requalification R4"
...
if (-not (Test-Path -LiteralPath $parent -PathType Container)) {
  throw "The governed R4 attempt directory does not exist."
}
