...
$requiredAuthority = "FOUNDER-AUTHORISED-STAGE-2-R4-SINGLE-ATTEMPT"
if ($FounderAuthority -cne $requiredAuthority) {
  throw "The exact R4 Founder execution authority token is required."
}

$repositoryRoot = [IO.Path]::GetFullPath((Join-Path $PSScriptRoot "..\..\.."))
$contractPath = Join-Path $PSScriptRoot "Oracle.Stage2RequalificationR4Contract.json"
$executorPath = Join-Path $PSScriptRoot "execute-attempt.mjs"
if (-not (Test-Path -LiteralPath $contractPath -PathType Leaf)) { throw "R4 contract is missing." }
if (-not (Test-Path -LiteralPath $executorPath -PathType Leaf)) { throw "R4 executor is missing." }
...
try {
  ...
  $identity = New-OracleStage2R4ExecutionIdentity
  if ($identity.suffix -eq "00000000") { throw "The prohibited fixed suffix was generated." }
  $outputRoot = ".artifacts/sprint-30-5/stage-2-requalification-r4/$($identity.attemptId)"
  $authorityPath = ".artifacts/sprint-30-5/stage-2-requalification-r4/authorities/$($identity.authorityId).json"
  ...
  [pscustomobject]@{
    contract = "oracle.sprint-30-5.stage-2-requalification-r4-governed-invocation"
  ...
  if ($null -eq $executorExitCode) { throw "R4 executor returned a null exit code." }
