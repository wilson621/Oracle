...
const BUILD_CANARIES = Object.freeze({
  ORACLE_SUPABASE_URL: "https://r4-build-canary.invalid",
  ORACLE_SUPABASE_ANON_KEY: "oracle-r4-anon-canary-not-a-secret",
  ORACLE_SUPABASE_SERVICE_ROLE_KEY: "oracle-r4-service-canary-not-a-secret",
  ORACLE_WEB_SESSION_SECRET: "oracle-r4-session-canary-not-a-secret-32-bytes",
  NEXT_PUBLIC_SUPABASE_URL: "https://r4-build-canary.invalid",
  NEXT_PUBLIC_SUPABASE_ANON_KEY: "oracle-r4-anon-canary-not-a-secret",
});
...
export function assertNoAmbientRuntimeConfiguration(root, environment = process.env) {
  ...
  if (populated.length !== 0) {
    throw new Error(
      `Ambient runtime configuration is prohibited during R4 qualification: ${populated.join(", ")}`
    );
  }
  ...
  if (presentFiles.length !== 0) {
    throw new Error(
      `Ambient environment files are prohibited during R4 qualification: ${presentFiles.join(", ")}`
    );
  }
}
