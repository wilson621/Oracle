...
if (
  ...
  [string]$manifest.programmeIdentity -cne
    "Sprint 30.5 Stage 3 Qualification R6" -or
...
$runningScriptRelativePath = "payload/Invoke-OracleStage3R6PreAuthorityPreflight.ps1"
$runningScriptEntry = @($payloadEntries | Where-Object {
  [string]$_.path -ceq $runningScriptRelativePath
})
$contractRelativePath = "payload/Oracle.Stage3R6Contract.json"
...
$result = Get-OracleStage3R6PreAuthorityObservation `
  -Contract $contract `
