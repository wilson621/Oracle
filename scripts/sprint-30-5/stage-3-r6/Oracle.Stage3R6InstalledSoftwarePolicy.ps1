...
function Get-OracleStage3R6OptionalPropertyValue {
  [CmdletBinding()]
  param(
...
function ConvertTo-OracleStage3R6InstalledSoftwareInventory {
  [CmdletBinding()]
  param(
  ...
  foreach ($entry in $RegistryEntries) {
    ...
    $displayNameValue = Get-OracleStage3R6OptionalPropertyValue `
    ...
    $displayVersionValue = Get-OracleStage3R6OptionalPropertyValue `
      -InputObject $entry -Name "DisplayVersion"
    $publisherValue = Get-OracleStage3R6OptionalPropertyValue `
...
function Get-OracleStage3R6InstalledSoftwareInventory {
  [CmdletBinding()]
  param(
  ...
  ConvertTo-OracleStage3R6InstalledSoftwareInventory -RegistryEntries $entries
}
