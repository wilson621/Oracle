...
$recovery = Invoke-ReadOnlyProcess (
  Get-OracleStage3R6WindowsExecutablePath -Name "reagentc.exe"
) @("/info")
...
$physicalCertificateMatches = @(
  Get-OracleStage3R6PhysicalCertificateMatches `
    -Thumbprint $contract.stage2.certificateThumbprint
)
$logicalCertificateViews = @(
  Get-OracleStage3R6LogicalCertificateViews `
    -Thumbprint $contract.stage2.certificateThumbprint
)
$certificateMatches = @(
...
$record = [ordered]@{
  schemaVersion = "1.0.0"
  contract = "oracle.sprint-30-5.stage-3-r6-host-continuity"
