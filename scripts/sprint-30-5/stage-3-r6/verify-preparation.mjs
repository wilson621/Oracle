...
validateExecutionIdentity({
  authorityId: "authority-stage3-r6-20260728T223045123Z-a1b2c3d4",
  attemptId: "stage3-r6-20260728T223045123Z-a1b2c3d4",
  timestampUtc,
});
for (const invalid of [
  {
    authorityId: "authority-stage3-r6-20260728T223045123Z-a1b2c3d4",
    attemptId: "stage3-r6-20260728T223045123Z-ffffffff",
    timestampUtc,
  },
  {
    authorityId: "authority-stage3-r6-20260728T223045123Z-a1b2c3d4",
...
validateTransferIdentity({
  transferId: "transfer-stage3-r6-20260728T223045123Z-a1b2c3d4",
  timestampUtc,
});
...
try {
  ...
  const fixtureAttemptId = "stage3-r6-20260728T223045123Z-a1b2c3d4";
  ...
  const archiveCreation = spawnSync(
    ...
    [
      ...
      "-NonInteractive",
      "-Command",
      "Compress-Archive -LiteralPath $env:ORACLE_R6_FIXTURE_SOURCE -DestinationPath $env:ORACLE_R6_FIXTURE_DESTINATION",
    ],
    {
      encoding: "utf8",
      shell: false,
      env: {
        ...process.env,
        ORACLE_R6_FIXTURE_SOURCE: fixtureAttemptRoot,
        ORACLE_R6_FIXTURE_DESTINATION: returnedArchive,
      },
    }
  );
  ...
  const parent = join(repositoryRoot, ".tmp-stage3-r6-validation");
...
const developmentRehearsalPath = join(
  import.meta.dirname,
  "Invoke-OracleStage3R6DevelopmentRehearsal.ps1"
);
const installedSoftwareFixturePath = join(
  import.meta.dirname,
  "Test-OracleStage3R6InstalledSoftwarePolicy.ps1"
);
const lifecycleFixturePath = join(
  import.meta.dirname,
  "Test-OracleStage3R6LifecyclePolicy.ps1"
);
const hostShapeFixturePath = join(
  import.meta.dirname,
  "Test-OracleStage3R6HostShapeFixtures.ps1"
);
...
const processFixturePath = join(
  import.meta.dirname,
  "Test-OracleStage3R6ProcessPolicy.ps1"
);
...
const windowsExecutableFixturePath = join(
  import.meta.dirname,
  "Test-OracleStage3R6WindowsExecutablePolicy.ps1"
);
const developmentPlatformFixturePath = join(
  import.meta.dirname,
  "Test-OracleStage3R6DevelopmentPlatformCompatibility.ps1"
);
const phaseAudit = JSON.parse(
  readFileSync(join(import.meta.dirname, "Oracle.Stage3R6PhaseAudit.json"), "utf8")
);
const packageInventoryFixturePath = join(
  import.meta.dirname,
  "Test-OracleStage3R6PackageInventoryPolicy.ps1"
);
const scriptPathFixturePath = join(
  import.meta.dirname,
  "Test-OracleStage3R6ScriptPath.ps1"
);
const preExecutionGate = readFileSync(
  join(
    ...
    "docs",
    "sprints",
    "SPRINT_30_5_STAGE_3_R6_PRE_EXECUTION_GATE.md"
  ),
  "utf8"
);
const preparationValidationReport = readFileSync(
  join(
    ...
    "docs",
    "sprints",
    "SPRINT_30_5_STAGE_3_R6_PREPARATION_VALIDATION_REPORT.md"
  ),
  "utf8"
);
...
assert.match(transferBuilder, /Get-OracleStage3R6HostContinuity\.ps1/u);
assert.match(transferBuilder, /Oracle\.Stage3R6IdentityPolicy\.ps1/u);
assert.match(transferBuilder, /Oracle\.Stage3R6CertificateTrustPolicy\.ps1/u);
assert.match(transferBuilder, /Oracle\.Stage3R6InstalledSoftwarePolicy\.ps1/u);
assert.match(transferBuilder, /Oracle\.Stage3R6LifecyclePolicy\.ps1/u);
assert.match(transferBuilder, /Oracle\.Stage3R6PackageInventoryPolicy\.ps1/u);
assert.match(transferBuilder, /Oracle\.Stage3R6PreflightPolicy\.ps1/u);
assert.match(transferBuilder, /Oracle\.Stage3R6ProcessPolicy\.ps1/u);
assert.match(transferBuilder, /Oracle\.Stage3R6WindowsExecutablePolicy\.ps1/u);
assert.match(transferBuilder, /Invoke-OracleStage3R6PreAuthorityPreflight\.ps1/u);
assert.match(transferBuilder, /Oracle\.Stage3R6OptionalMemberAudit\.json/u);
assert.match(transferBuilder, /Oracle\.Stage3R6PhaseAudit\.json/u);
assert.match(transferBuilder, /Oracle\.Stage3R6TransferCustody\.json/u);
...
for (const required of [
  "FOUNDER-AUTHORISED-STAGE3-R6-EXECUTION",
  ...
  "Get-OracleStage3R6PreAuthorityObservation",
  "Get-OracleStage3R6WindowsExecutablePath",
  "Get-OracleStage3R6TrustImportArguments",
  "Get-OracleStage3R6TrustRemovalArguments",
  "Assert-OracleStage3R6TemporaryTrustState",
  "Assert-OracleStage3R6ExactRemovalTarget",
  "Get-PhysicalExactCertificateMatches",
  "Move-OracleStage3R6Lifecycle",
  "package-content-reconciliation.json",
  "Get-OracleStage3R6PackageZipInventory",
  "ConvertTo-OracleStage3R6CanonicalPackagePath",
  "--remote-debugging",
  ...
  "oracle.sprint-30-5.stage-3-r6-transfer-custody",
...
assert.equal(
  packageScripts["sprint-30-5:stage-3:r6:validate"],
  "node scripts/sprint-30-5/stage-3-r6/verify-preparation.mjs"
);
assert.equal(
  packageScripts["sprint-30-5:stage-3:r6:prepare-transfer"],
  "node scripts/sprint-30-5/stage-3-r6/prepare-transfer.mjs"
);
assert.equal(
  packageScripts["sprint-30-5:stage-3:r6:verify-return"],
  "node scripts/sprint-30-5/stage-3-r6/verify-return.mjs"
);
assert.equal(packageScripts["sprint-30-5:stage-3:r6:execute"], undefined);
...
for (const required of [
  "Stage 3 Qualification R6 Pre-Execution Gate",
  "Execution:** Blocked and unauthorised",
  "ExpectedHarnessCommit",
  "ExpectedTransferManifestSha256",
  "ExpectedTransferCustodySha256",
  "ExpectedHostContinuitySha256",
  "FOUNDER-AUTHORISED-STAGE3-R6-TRANSFER",
  "FOUNDER-AUTHORISED-STAGE3-R6-EXECUTION",
...
assert.match(
  transferBuilder,
  /Create-only corrected Stage 3 R6 transfer; prior R5 and historical transfers remain immutable/u
);
for (const required of [
  "PSObject.Properties",
...
assert.match(harness, /Get-OracleStage3R6InstalledSoftwareInventory/u);
assert.match(harness, /Get-OracleStage3R6PreAuthorityObservation[\s\S]*New-Item/u);
assert.ok(
  harness.indexOf("Get-OracleStage3R6PreAuthorityObservation") <
...
assert.match(processPolicy, /Get-OracleStage3R6RequiredProcessMember/u);
...
assert.match(
  preflightPolicy,
  /Get-OracleStage3R6WindowsExecutablePath -Name "explorer\.exe"/u
);
assert.equal(
  (
    harness.match(
      /Get-OracleStage3R6WindowsExecutablePath -Name "explorer\.exe"/gu
    ) ?? []
  ).length,
  2
);
assert.match(lifecyclePolicy, /Get-OracleStage3R6LifecyclePhases/u);
assert.match(preflightPolicy, /required Windows PowerShell 5\.1|Windows PowerShell 5\.1/u);
assert.match(preflightPolicy, /Test-OracleStage3R6ProcessIsElevated/u);
assert.match(preflightPolicy, /Machine-scoped AppX trust requires an elevated/u);
assert.match(preflightPolicy, /Get-OracleStage3R6InstalledSoftwareInventory/u);
...
const identityPolicyFixture = spawnSync(
  ...
  [
    ...
    "Bypass",
    "-File",
    join(import.meta.dirname, "Test-OracleStage3R6IdentityPolicy.ps1"),
  ],
...
for (const required of [
  "oracle.sprint-30-5.stage-3-r6-host-continuity",
...
for (const revision of ["r1", "r2", "r3", "r4", "r5"]) {
...
for (const text of governance) {
  assert.match(text, /(?:\bR4\b|R2[–-]R5)/u);
  assert.match(text, /\bR5\b/u);
  assert.match(text, /\bR6\b/u);
...
assert.deepEqual(contract.correctionBasis, {
  predecessorRevision: "R5",
  predecessorAttemptOrdinal: 1,
  acceptedFailureEvidence: "immutable-founder-accepted",
  appxDeploymentActivityId: "a549c0c7-1d3c-0004-cd5a-b7a63c1ddd01",
  hresult: "0x800B0109",
  rejectedTrustScope: "CurrentUser\\Root",
  requiredTrustScope: "LocalMachine\\TrustedPeople",
});
...
console.log("Stage 3 R6 preparation validation passed.");
