# Sprint 30.5 Stage 3 Qualification R6 Harness

This directory contains the prepared, R2-bound Stage 3 R6 transfer and clean
Windows qualification system.

Preparation and validation are authorised by the R6 trust-correction mission.
...
The shared `Oracle.Stage3R6IdentityPolicy.ps1` implements ordinal
...
The shared `Oracle.Stage3R6CertificateTrustPolicy.ps1` distinguishes physical
certificate-store entries from inherited logical provider projections. R6
...
R6 is the current attempt-scoped qualification revision, distinct from the
immutable historical `stage-3-revision-6-abandoned-design` record. Closed R1
through R5 repository transfer and qualification entry points are fail-closed;
their artifacts and evidence remain immutable.

The shared `Oracle.Stage3R6PackageInventoryPolicy.ps1` defines the canonical
...
The shared `Oracle.Stage3R6InstalledSoftwarePolicy.ps1` enumerates native
...
`Invoke-OracleStage3R6PreAuthorityPreflight.ps1` is the only target-host
...
`Oracle.Stage3R6ProcessPolicy.ps1` is the single external-process envelope
...
`Oracle.Stage3R6WindowsExecutablePolicy.ps1` is the single mapping for Windows
...
`Oracle.Stage3R6LifecyclePolicy.ps1` is shared by production lifecycle
...
`Get-OracleStage3R6HostContinuity.ps1` creates the fresh, create-only,
...
`docs/sprints/SPRINT_30_5_STAGE_3_R6_PRE_EXECUTION_GATE.md`.
