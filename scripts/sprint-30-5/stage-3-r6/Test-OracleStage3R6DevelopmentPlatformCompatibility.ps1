...
. (Join-Path $PSScriptRoot "Oracle.Stage3R6PreflightPolicy.ps1")
...
$inventory = @(Get-OracleStage3R6InstalledSoftwareInventory)
...
$contract = Get-Content -LiteralPath (
  Join-Path $PSScriptRoot "Oracle.Stage3R6Contract.json"
...
$fixtureRoot = Join-Path $env:TEMP (
  "oracle-stage3-r6-compatibility-" + [Guid]::NewGuid().ToString("N")
)
...
try {
  ...
  $detachedSigner = Get-OracleStage3R6AcceptedPublicCertificate `
