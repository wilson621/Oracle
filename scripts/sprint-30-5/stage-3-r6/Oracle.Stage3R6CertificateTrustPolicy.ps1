...
function Assert-OracleStage3R6CertificateTrustContract {
...
function Assert-OracleStage3R6Thumbprint {
...
function Get-OracleStage3R6CertificateStoreDefinitions {
  @(
...
function Get-OracleStage3R6PhysicalCertificateMatches {
  param([Parameter(Mandatory = $true)][string]$Thumbprint)

  Assert-OracleStage3R6Thumbprint -Thumbprint $Thumbprint
  $matches = @()
  foreach ($definition in @(Get-OracleStage3R6CertificateStoreDefinitions)) {
...
function Get-OracleStage3R6LogicalCertificateViews {
  param([Parameter(Mandatory = $true)][string]$Thumbprint)
...
function Test-OracleStage3R6CertificateIdentity {
  param(
...
function Assert-OracleStage3R6NoCertificateResidue {
  param(
...
function Assert-OracleStage3R6TemporaryTrustState {
  param(
    [Parameter(Mandatory = $true)]
    [AllowEmptyCollection()][object[]]$PhysicalMatches,
    [Parameter(Mandatory = $true)]
  ...
  if (
    ...
    -not (Test-OracleStage3R6CertificateIdentity `
...
function Assert-OracleStage3R6ExactRemovalTarget {
  param(
    [Parameter(Mandatory = $true)]
    [AllowEmptyCollection()][object[]]$PhysicalMatches,
    [Parameter(Mandatory = $true)]
...
function Test-OracleStage3R6ProcessIsElevated {
