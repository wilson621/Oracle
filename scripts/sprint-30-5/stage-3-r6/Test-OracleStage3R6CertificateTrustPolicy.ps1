...
try {
  Assert-OracleStage3R6CertificateTrustContract -Contract $invalidContract
} catch {
...
$pathWithSpaces = "C:\Stage 3 R6\attempt certificate.cer"
$importArguments = @(Get-OracleStage3R6TrustImportArguments $pathWithSpaces)
if (
...
$removalArguments = @(Get-OracleStage3R6TrustRemovalArguments $thumbprint)
if (
