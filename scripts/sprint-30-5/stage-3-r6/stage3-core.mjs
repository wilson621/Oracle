...
const attemptPattern = new RegExp(`^stage3-r6-${compactTime}-${suffix}$`, "u");
const authorityPattern = new RegExp(
  `^authority-stage3-r6-${compactTime}-${suffix}$`,
  "u"
);
const transferPattern = new RegExp(`^transfer-stage3-r6-${compactTime}-${suffix}$`, "u");
...
function validateScopedId(value, timestampUtc, pattern, label) {
  ...
  if (!match) {
    throw new Error(`${label} has an invalid Stage 3 R6 format.`);
  }
