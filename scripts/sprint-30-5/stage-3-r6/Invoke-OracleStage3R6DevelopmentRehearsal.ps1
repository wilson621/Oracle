...
. (Join-Path $PSScriptRoot "Oracle.Stage3R6LifecyclePolicy.ps1")
. (Join-Path $PSScriptRoot "Oracle.Stage3R6IdentityPolicy.ps1")
. (Join-Path $PSScriptRoot "Oracle.Stage3R6CertificateTrustPolicy.ps1")
. (Join-Path $PSScriptRoot "Oracle.Stage3R6InstalledSoftwarePolicy.ps1")
. (Join-Path $PSScriptRoot "Oracle.Stage3R6ProcessPolicy.ps1")
...
function Invoke-SimulatedPhase(
  ...
  [void](Move-OracleStage3R6Lifecycle -State $State -Phase $Phase)
}

function Invoke-SimulatedLifecycle([string]$Root, [string]$InjectedFailure) {
  $state = New-OracleStage3R6LifecycleState
  $failure = $null
  try {
    foreach ($phase in @(Get-OracleStage3R6LifecyclePhases)) {
  ...
  $teardown = Get-OracleStage3R6TeardownObligations `
...
$parent = Join-Path $repositoryRoot ".tmp-stage3-r6-development-rehearsal"
...
try {
  $software = @(ConvertTo-OracleStage3R6InstalledSoftwareInventory -RegistryEntries @(
  ...
  $fixtureExplorer = Get-OracleStage3R6WindowsExecutablePath `
    -Name "explorer.exe" `
  ...
  $processCounts = @{}
  $processResult = Invoke-OracleStage3R6GovernedProcess `
