...
function Get-OracleStage3R6RequiredPropertyValue {
  [CmdletBinding()]
  param(
...
function Assert-OracleStage3R6CommandSurface {
  [CmdletBinding()]
  param()
...
function Get-OracleStage3R6AcceptedPublicCertificate {
  [CmdletBinding()]
  param(
...
function Invoke-OracleStage3R6ReadOnlyProcess {
  [CmdletBinding()]
  param(
...
function Assert-OracleStage3R6NoReparseTraversal([string]$Path) {
...
function Get-OracleStage3R6PreAuthorityObservation {
  [CmdletBinding()]
  param(
    [Parameter(Mandatory = $true)][object]$Contract,
    [Parameter(Mandatory = $true)][string]$TransferRoot,
  ...
  if (
    ...
    -not (Test-OracleStage3R6ProcessIsElevated)
  ) {
  ...
  foreach ($path in @($TransferRoot, $EvidenceReturnRoot, $HostContinuityPath)) {
    Assert-OracleStage3R6NoReparseTraversal $path
  }
  if (
  ...
  $commands = @(Assert-OracleStage3R6CommandSurface)
  $windowsExecutables = [ordered]@{
    certutil = Get-OracleStage3R6WindowsExecutablePath -Name "certutil.exe"
    explorer = Get-OracleStage3R6WindowsExecutablePath -Name "explorer.exe"
    reagentc = Get-OracleStage3R6WindowsExecutablePath -Name "reagentc.exe"
  }
  Add-Type -AssemblyName System.IO.Compression
  Add-Type -AssemblyName System.IO.Compression.FileSystem
  $recovery = Invoke-OracleStage3R6ReadOnlyProcess `
  ...
  $manufacturer = [string](Get-OracleStage3R6RequiredPropertyValue `
    $computerSystem "Manufacturer" "Win32_ComputerSystem")
  $model = [string](Get-OracleStage3R6RequiredPropertyValue `
  ...
  foreach ($member in @("TpmPresent", "TpmReady")) {
    [void](Get-OracleStage3R6RequiredPropertyValue $tpm $member "Get-Tpm")
  }
  foreach ($member in @("AntivirusEnabled", "RealTimeProtectionEnabled")) {
    [void](Get-OracleStage3R6RequiredPropertyValue $defender $member "Get-MpComputerStatus")
  }
  if (
    -not $secureBoot -or
    -not [bool](Get-OracleStage3R6RequiredPropertyValue $tpm "TpmPresent" "Get-Tpm") -or
    -not [bool](Get-OracleStage3R6RequiredPropertyValue $tpm "TpmReady" "Get-Tpm") -or
    -not [bool](Get-OracleStage3R6RequiredPropertyValue (
      $defender
  ...
  $activation = @(Get-CimInstance SoftwareLicensingProduct -ErrorAction Stop |
    Where-Object {
      $partialKey = Get-OracleStage3R6RequiredPropertyValue `
        $_ "PartialProductKey" "SoftwareLicensingProduct"
      $licenseStatus = Get-OracleStage3R6RequiredPropertyValue `
  ...
  $installedSoftware = @(Get-OracleStage3R6InstalledSoftwareInventory)
  ...
  foreach ($member in @("Status", "SignerCertificate")) {
    [void](Get-OracleStage3R6RequiredPropertyValue (
      $untrustedSignature
    ) $member "Get-AuthenticodeSignature")
  }
  $signerCertificate = Get-OracleStage3R6AcceptedPublicCertificate `
    -Contract $Contract -TransferRoot $TransferRoot
  $observedUntrustedSigner = Get-OracleStage3R6RequiredPropertyValue `
    $untrustedSignature "SignerCertificate" "Get-AuthenticodeSignature"
  if (
    [string](Get-OracleStage3R6RequiredPropertyValue (
      $untrustedSignature
  ...
  foreach ($member in @(
    ...
    [void](Get-OracleStage3R6RequiredPropertyValue $continuity $member "host continuity")
  }
  foreach ($member in @("deviceName", "manufacturer", "model")) {
    [void](Get-OracleStage3R6RequiredPropertyValue $continuity.host $member "host continuity host")
  }
  ...
  if (
    [string]$continuity.contract -cne
      "oracle.sprint-30-5.stage-3-r6-host-continuity" -or
  ...
  [pscustomobject][ordered]@{
    ...
    security = [ordered]@{
      ...
      tpmPresent = [bool](Get-OracleStage3R6RequiredPropertyValue (
        $tpm
      ) "TpmPresent" "Get-Tpm")
      tpmReady = [bool](Get-OracleStage3R6RequiredPropertyValue (
        $tpm
      ) "TpmReady" "Get-Tpm")
      defenderAntivirusEnabled = [bool](Get-OracleStage3R6RequiredPropertyValue (
        $defender
      ) "AntivirusEnabled" "Get-MpComputerStatus")
      defenderRealTimeProtectionEnabled = [bool](
        Get-OracleStage3R6RequiredPropertyValue (
          $defender
        ) "RealTimeProtectionEnabled" "Get-MpComputerStatus"
      )
