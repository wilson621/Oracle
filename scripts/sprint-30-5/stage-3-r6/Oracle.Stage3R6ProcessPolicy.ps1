...
function Get-OracleStage3R6RequiredProcessMember {
  [CmdletBinding()]
  param(
...
function Invoke-OracleStage3R6GovernedProcess {
  [CmdletBinding()]
  param(
    [Parameter(Mandatory = $true)][string]$Name,
  ...
  foreach ($member in @(
    ...
    [void](Get-OracleStage3R6RequiredProcessMember $record $member)
  }
  ...
  $processError = Get-OracleStage3R6RequiredProcessMember $record "processError"
  $signal = Get-OracleStage3R6RequiredProcessMember $record "signal"
  $exitCode = Get-OracleStage3R6RequiredProcessMember $record "exitCode"
