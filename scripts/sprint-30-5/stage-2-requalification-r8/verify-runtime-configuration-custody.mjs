...
const root = mkdtempSync(join(tmpdir(), "oracle-r8-runtime-custody-"));
try {
  ...
  assert.equal(built.ORACLE_SUPABASE_URL, "https://r8-build-canary.invalid");
  ...
  writeFileSync(join(output, "leak.txt"), "oracle-r8-service-canary-not-a-secret", "utf8");
  assert.throws(
  ...
  console.log(
    JSON.stringify({
      contract: "oracle.sprint-30-5.stage-2-requalification-r8-runtime-custody-regression",
      status: "PASS",
      cases: 12,
    })
  );
