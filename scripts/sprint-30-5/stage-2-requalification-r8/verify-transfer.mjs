...
assert.equal(manifest.transferId,contract.executionMission.transferId);
assert.equal(manifest.founderGrantId,contract.executionMission.founderGrantId);
...
const verification={schemaVersion:"1.0.0",contract:"oracle.sprint-30-5.stage-2-r8-transfer-verification",result:"passed",classification:"INDEPENDENT TRANSFER VERIFICATION",verifiedAtUtc:new Date().toISOString(),transferId:manifest.transferId,founderGrantId:manifest.founderGrantId,manifestSha256:sha256(manifestPath),custodySha256:sha256(custodyPath),executionContractSha256:manifest.executionContractSha256,executionCommit:manifest.executionCommit,executionTree:manifest.executionTree,files:actual.length,bytes:custody.bytes,authorityCreated:false,attemptCreated:false};
