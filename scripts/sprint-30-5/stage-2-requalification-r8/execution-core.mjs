...
export const FOUNDER_EXECUTION_AUTHORITY =
  "FOUNDER-AUTHORISED-STAGE-2-R8-SINGLE-ATTEMPT";
...
export function claimSingleAttemptAuthority({
  ...
  const attemptRoot = assertR8ArtifactPath(outputRoot, attemptId);
  ...
  writeJsonAtomicCreateOnly(recordPath, {
    schemaVersion: "1.0.0",
    contract:
      "oracle.sprint-30-5.stage-2-requalification-r8-single-attempt-authority",
...
export function assertSingleAttemptAuthorityAvailable({
  ...
  if (
    ...
    throw new Error("R8 authority-consumption state is invalid.");
  }
  ...
  if (existingAttemptIds.some((value) => equals(value, attemptId))) {
    throw new Error("The immutable R8 attempt identity already exists.");
  }
}
...
export function createAttemptSubdirectory(attemptRoot, relativePath) {
  ...
  assertR8ArtifactPath(root, root.split(sep).at(-1));
  if (
  ...
  return Object.freeze({
    transition(phase, details = {}) {
      ...
      const record = {
        schemaVersion: "1.0.0",
        contract:
          "oracle.sprint-30-5.stage-2-requalification-r8-lifecycle-event",
        ...identity,
    ...
    fail(error, details = {}) {
      ...
      const failure = {
        schemaVersion: "1.0.0",
        contract:
          "oracle.sprint-30-5.stage-2-requalification-r8-lifecycle-failure",
        ...identity,
