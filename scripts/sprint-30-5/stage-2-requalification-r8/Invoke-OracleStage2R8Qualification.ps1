...
if (
  [string]$contract.status -cne 'founder-authorised-corrected-execution-enabled' -or
  ...
  [int]$contract.correctedMission.maximumTransfers -ne 1 -or
  [int]$contract.correctedMission.maximumAuthorities -ne 1 -or
  [int]$contract.correctedMission.maximumAttempts -ne 1 -or
  -not [bool]$contract.correctedMission.createAuthorityOnlyAfterAllPreAuthorityGates -or
  [bool]$contract.correctedMission.retryAfterConsumedAuthority -or
  [bool]$contract.correctedMission.stage3Authorised -or
  [string]$contract.correctedMission.founderGrantId -cne $FounderGrantId
) {
...
if ([string]$transfer.transferId -cne [string]$contract.correctedMission.transferId -or [string]$transfer.manifest.founderGrantId -cne $FounderGrantId -or [string]$transfer.manifest.replacesTransferId -cne [string]$contract.correctedMission.replacesTransferId -or [string]$transfer.manifest.preservesOriginalTransferId -cne [string]$contract.correctedMission.preservesOriginalTransferId) { throw 'R8 admitted transfer mission binding differs.' }
...
$continuity = [ordered]@{
  ...
  preservesOriginalTransferId=[string]$transfer.manifest.preservesOriginalTransferId
