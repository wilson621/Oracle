...
if (
  ...
  -not [bool]$contract.authority.qualificationExecutionPermitted
) {
  throw "R8 qualification execution is not authorised by this preparation baseline."
}
...
$continuity = [ordered]@{
  ...
  host=$hostAdmission
  localAttemptRootAvailable=$true
  returnAttemptRootAvailable=$true
...
$outcome = [ordered]@{
  ...
  result=if($null -eq $primary){'passed-awaiting-founder-review'}else{'failed'}
  authorityId=[string]$futureIdentity.authorityId
  attemptId=[string]$futureIdentity.attemptId
  transferId=[string]$transfer.transferId
  candidateVerification=$verification
  finalHostAdmission=$finalAdmission
  zeroResidue=([int]$finalAdmission.packagesPresent -eq 0 -and [int]$finalAdmission.certificatesPresent -eq 0)
...
if ($null -ne $primary) { throw "Stage 2 R8 qualification failed permanently: $($primary.Message)" }
