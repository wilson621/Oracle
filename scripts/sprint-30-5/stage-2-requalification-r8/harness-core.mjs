...
export const R8_GOVERNED_WRAPPER_PROTOCOL =
  "oracle-stage2-r8-governed-wrapper-v1";
export const contractPath = join(
  import.meta.dirname,
  "Oracle.Stage2RequalificationR8Contract.json"
);
...
const attemptPattern = /^r8-(\d{8}T\d{9}Z)-([0-9a-f]{8})$/u;
...
export function assertOutsideHistoricalRoots(path) {
  ...
  for (const protectedRoot of contract.historicalProtectedRoots) {
    ...
    if (isSameOrDescendant(resolved, historical)) {
      throw new Error(
        `Refusing R8 output inside immutable historical root: ${protectedRoot}`
      );
    }
  }
  return resolved;
...
export function assertR8ArtifactPath(path, attemptId) {
  ...
  if (resolved !== requiredAttemptRoot) {
    throw new Error(
      `R8 output root must resolve exactly to ${relative(repositoryRoot, requiredAttemptRoot).replaceAll("\\", "/")}.`
    );
  }
  return resolved;
}
...
export function assertNoReparseTraversal(
  ...
  if (!isSameOrDescendant(resolved, repositoryRoot)) {
    throw new Error("Governed R8 output must remain inside the repository.");
  }
  ...
  for (const segment of segments) {
    ...
    if (
      ...
      throw new Error(`R8 output path traverses a symbolic link or junction: ${current}`);
    }
  }
}
...
export function validateGovernedWrapperInvocation({ observed, parentProcessId }) {
  ...
  const expected = `${R8_GOVERNED_WRAPPER_PROTOCOL}:${parentProcessId}`;
  if (observed !== expected) {
    throw new Error(
      "The R8 executor must be invoked by the governed invoke-attempt.ps1 wrapper."
    );
  }
  return Object.freeze({
    protocol: R8_GOVERNED_WRAPPER_PROTOCOL,
    parentProcessId,
  });
}
...
export function validateAttemptIdentity({ attemptId, timestampUtc }) {
  ...
  if (!match) {
    throw new Error("Attempt ID must use r8-YYYYMMDDTHHMMSSmmmZ-xxxxxxxx.");
  }
  if (match[2] === "00000000") {
...
export function validateBinding(binding) {
  ...
  if (binding.candidateCommit !== contract.candidate.commit) {
    throw new Error(`R8 candidate commit mismatch: expected ${contract.candidate.commit}, received ${binding.candidateCommit}.`);
  }
  if (
...
export function validateRepositorySnapshot(snapshot, binding) {
  ...
  if (snapshot.status !== "") {
    throw new Error("R8 requires a clean repository and index.");
  }
}
...
export function assertRepositoryPreflight(binding) {
  ...
  if (candidateTree !== contract.candidate.tree) {
    throw new Error(`R8 candidate tree mismatch: expected ${contract.candidate.tree}, found ${candidateTree}.`);
  }
  const productDifference = spawnSync(
    ...
    [
      ...
      "scripts/verify-accessible-color-contract.mjs",
      "eslint.config.mjs",
...
export function validateMachineQualificationState(result) {
  ...
  if (result.certificates.length !== 0) {
    throw new Error(
      "An R8 temporary certificate identity already exists in a governed store."
    );
  }
}
...
export function createAttemptRecord(input) {
  ...
  const outputRoot = assertR8ArtifactPath(input.outputRoot, input.attemptId);
  return {
    schemaVersion: "1.0.0",
    contract: "oracle.sprint-30-5.stage-2-requalification-r8-attempt",
    ...
    authority: {
      ...
      stage3: "fresh-requalification-required-not-authorised-by-r8",
      stage4: "fresh-requalification-required-not-authorised-by-r8",
...
export function createAttemptDirectory(record) {
  const outputRoot = assertR8ArtifactPath(record.outputRoot, record.attemptId);
  ...
  writeJsonAtomicCreateOnly(
    join(outputRoot, "Oracle.Stage2RequalificationR8Attempt.json"),
    record
  );
  return outputRoot;
}
...
export function harnessFileInventory() {
  return [
    ...
    join(import.meta.dirname, "prepare-candidate.mjs"),
    join(import.meta.dirname, "prepare-transfer.mjs"),
    join(import.meta.dirname, "verify-transfer.mjs"),
    join(import.meta.dirname, "Oracle.Stage2R8CleanHostCore.ps1"),
    join(import.meta.dirname, "Invoke-OracleStage2R8Qualification.ps1"),
    join(import.meta.dirname, "Invoke-OracleStage2R8FounderHandoff.ps1"),
    ...
    join(import.meta.dirname, "verify-preparation.mjs"),
