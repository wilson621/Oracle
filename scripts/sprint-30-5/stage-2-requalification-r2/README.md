# Sprint 30.5 Stage 2 Requalification R2 Harness

This directory contains preparation infrastructure for the permanently named
`Sprint 30.5 Stage 2 Requalification R2` programme.

The preparation does not itself authorise an R2 build, package, signing,
...
Historical Stage 2, Stage 2 R1 and Stage 3 roots are deny-listed and cannot be
selected as R2 output. Historical build and verification scripts remain
available only as process history; the R2 entry points do not invoke them.
...
`npm.cmd run sprint-30-5:stage-2:r2:validate`

After a separate Founder single-attempt decision, the complete lifecycle is
entered only through:

`npm.cmd run sprint-30-5:stage-2:r2:execute -- <governed arguments>`
