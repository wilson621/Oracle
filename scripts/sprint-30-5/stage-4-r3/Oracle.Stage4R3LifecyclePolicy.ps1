...
function Get-OracleStage4R3LifecyclePhases {
  @(
...
function New-OracleStage4R3LifecycleState {
  [pscustomobject][ordered]@{ index=0; terminal=$false; completed=[Collections.Generic.List[string]]::new() }
}
function Assert-OracleStage4R3NextLifecyclePhase([object]$State,[string]$Phase) {
  $phases=@(Get-OracleStage4R3LifecyclePhases)
...
function Move-OracleStage4R3Lifecycle([object]$State,[string]$Phase) {
  Assert-OracleStage4R3NextLifecyclePhase $State $Phase
  $State.completed.Add($Phase)
  $State.index=[int]$State.index+1
  if([int]$State.index -eq @(Get-OracleStage4R3LifecyclePhases).Count){$State.terminal=$true}
  $State
}
function Get-OracleStage4R3TeardownObligations([string[]]$CompletedPhases) {
