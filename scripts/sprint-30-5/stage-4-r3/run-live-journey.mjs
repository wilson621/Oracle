...
const accounts = [`oracle-stage4-r3-a-${nonce}@example.invalid`,`oracle-stage4-r3-b-${nonce}@example.invalid`];
const journeys = [];
...
try {
  ...
  const apiResponse = await fetch(`${web}/api/oracle/conversation`, { method:"POST",headers:{cookie:cookieHeader,"content-type":"application/json"},body:JSON.stringify({requestId:`stage4-r3-${nonce}`,text:"qualification probe"}),redirect:"manual" });
  ...
  const record = { contract:developmentRehearsal?"oracle.sprint-30-5.stage-4-r3-development-rehearsal":"oracle.sprint-30-5.stage-4-r3-journey",result:"passed",classification:developmentRehearsal?["NON-QUALIFICATION","NON-AUTHORITY","NON-EVIDENCE","DEVELOPMENT REHEARSAL"]:"GOVERNED-STAGE-4-R3-QUALIFICATION",collectedAtUtc:new Date().toISOString(),provider:{classification:"disposable-local-non-production",productionEndpoint:false,externalEmail:false},journeys,rendering,isolation:{accountCount:2,crossAccountLeaks:0,distinctAuthenticatedPrincipals:true,distinctOperators:true,rlsBindingsPerPrincipal:[1,1]},secretsRetained:false };
