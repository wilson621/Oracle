...
function Get-OracleStage4R3OptionalMember([object]$Object,[string]$Name) {
  if($null -eq $Object){ return $null }
  $property=$Object.PSObject.Properties[$Name]
  if($null -eq $property){ return $null }
  $property.Value
}
function Get-OracleStage4R3MandatoryMember([object]$Object,[string]$Name,[string]$Context) {
...
function Get-OracleStage4R3RequiredJourneys {
...
function Assert-OracleStage4R3JourneyRecord([object]$Record,[switch]$DevelopmentRehearsal) {
  if($null -eq $Record){ throw "Journey record is absent." }
  foreach($name in @("contract","result","classification","provider","journeys","rendering","isolation","secretsRetained")){[void](Get-OracleStage4R3MandatoryMember $Record $name "Journey record")}
  $expectedContract=if($DevelopmentRehearsal){"oracle.sprint-30-5.stage-4-r3-development-rehearsal"}else{"oracle.sprint-30-5.stage-4-r3-journey"}
  ...
  } elseif([string]$Record.classification -cne "GOVERNED-STAGE-4-R3-QUALIFICATION"){throw "Journey classification mismatch."}
  if([bool]$Record.secretsRetained){throw "Journey evidence retains secrets."}
  $provider=$Record.provider
  foreach($name in @("classification","productionEndpoint","externalEmail")){[void](Get-OracleStage4R3MandatoryMember $provider $name "Provider record")}
  ...
  $journeys=@($Record.journeys);$expected=@(Get-OracleStage4R3RequiredJourneys)
  if($journeys.Count -ne $expected.Count){throw "Journey inventory count mismatch."}
  foreach($journey in $journeys){foreach($name in @("id","result")){[void](Get-OracleStage4R3MandatoryMember $journey $name "Journey result")}}
  ...
  foreach($name in @("accountCount","crossAccountLeaks","distinctAuthenticatedPrincipals","distinctOperators","rlsBindingsPerPrincipal")){[void](Get-OracleStage4R3MandatoryMember $isolation $name "Isolation record")}
  foreach($name in @("protectedStatus","mainLandmarks","levelOneHeadings","method")){[void](Get-OracleStage4R3MandatoryMember $rendering $name "Rendering record")}
...
function Assert-OracleStage4R3SecretFreeText([string]$Text,[string[]]$KnownSecrets) {
