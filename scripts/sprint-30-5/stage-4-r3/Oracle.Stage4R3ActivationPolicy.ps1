...
function Assert-OracleStage4R3ApplicationActivationContract {
  [CmdletBinding()]
  ...
  if (
    ...
    @($activation.runtimeProof).Count -ne 3 -or
    @($activation.runtimeProof) -cnotcontains "ownership-verified-installed-process-tree" -or
    @($activation.runtimeProof) -cnotcontains "package-owned-loopback-listener" -or
    @($activation.runtimeProof) -cnotcontains "ten-live-authentication-journeys"
  ) {
    throw "The application-activation success contract differs."
  }
}

function Initialize-OracleStage4R3ActivationApi {
  if ($null -ne ("Oracle.Stage4R3.ApplicationActivation" -as [type])) {
    return
  }
...
function Test-OracleStage4R3ApplicationActivationApi {
  [CmdletBinding()]
  ...
  try {
    $native = if ($null -ne $ProbeRunner) {
      ...
      Initialize-OracleStage4R3ActivationApi
      [Oracle.Stage4R3.ApplicationActivation]::Probe()
    }
    if (
      $null -eq $native -or
...
function Invoke-OracleStage4R3ApplicationActivation {
  [CmdletBinding()]
  param(
  ...
  try {
    $native = if ($null -ne $ActivationRunner) {
      ...
      Initialize-OracleStage4R3ActivationApi
      [Oracle.Stage4R3.ApplicationActivation]::Activate(
        $AppUserModelId,
        $Arguments,
        [uint32]2
      )
...
function Assert-OracleStage4R3ApplicationActivationSucceeded {
  [CmdletBinding()]
