...
$transferAdmission=Assert-OracleStage4R3Transfer -RepositoryRoot $root -Contract $contract -TransferRoot $TransferRoot -ExpectedManifestSha256 $TransferManifestSha256 -ExpectedCustodySha256 $TransferCustodySha256 -ExpectedVerificationSha256 $TransferVerificationSha256 -ExecutionCommit $PreparationCommit -ExecutionTree $PreparationTree
$preflightRoot=[IO.Path]::GetFullPath((Join-Path $root ([string]$contract.paths.preflightRoot)))
$preflightPath=[IO.Path]::GetFullPath($PreflightRecord);[void](Assert-OracleStage4R3NoReparseTraversal $preflightPath $preflightRoot)
...
$current=Invoke-OracleStage4R3PreAuthorityChecks $root $contract $PreparationCommit $PreparationTree
$transferAdmission=Assert-OracleStage4R3Transfer -RepositoryRoot $root -Contract $contract -TransferRoot $TransferRoot -ExpectedManifestSha256 $TransferManifestSha256 -ExpectedCustodySha256 $TransferCustodySha256 -ExpectedVerificationSha256 $TransferVerificationSha256 -ExecutionCommit $PreparationCommit -ExecutionTree $PreparationTree
...
$identity=$FounderGrantId.Substring('founder-stage4-r3-grant-'.Length);$attemptId="stage4-r3-$identity";$authorityId="authority-stage4-r3-$identity"
...
foreach($path in @($authorityRoot,$attemptRoot)){[void](Assert-OracleStage4R3NoReparseTraversal $path $artifactRoot)};[void](Assert-OracleStage4R3NoReparseTraversal $repositoryEvidence $evidenceRoot)
...
[IO.Directory]::CreateDirectory($authorityRoot)|Out-Null;[void](Assert-OracleStage4R3NoReparseTraversal $authorityRoot $artifactRoot)
...
$state=New-OracleStage4R3LifecycleState;$failure=$null;$index=0
function Publish-Lifecycle([string]$Phase,$Details){$script:index++;Write-CreateOnlyJson (Join-Path $attemptRoot ("lifecycle\{0:d3}-{1}.json" -f $script:index,$Phase)) ([ordered]@{contract='oracle.sprint-30-5.stage-4-r3-lifecycle';authorityId=$authorityId;attemptId=$attemptId;phase=$Phase;recordedAtUtc=(Get-Date).ToUniversalTime().ToString('o');details=$Details});[void](Move-OracleStage4R3Lifecycle $state $Phase)}
...
try{
  try{
    ...
    try{$controller=Invoke-OracleStage4R3NativeProcess ([string]$current.tools.node) @((Join-Path $PSScriptRoot 'execute-live-environment.mjs')) $root;Write-CreateOnlyJson $controllerLog $controller}catch{$record=$_.Exception.Data['OracleStage4R3ProcessRecord'];if($null-ne$record -and -not(Test-Path -LiteralPath $controllerLog)){Write-CreateOnlyJson $controllerLog $record};throw}
    ...
    $journeyText=Get-Content -Raw -LiteralPath $journeyPath;Assert-OracleStage4R3SecretFreeText -Text $journeyText -KnownSecrets @();$journey=$journeyText|ConvertFrom-Json;[void](Assert-OracleStage4R3JourneyRecord $journey)
    ...
    $payloadItems=@(Get-Inventory $payloadRoot);Write-CreateOnlyJson (Join-Path $payloadRoot 'payload-manifest.json') ([ordered]@{contract='oracle.sprint-30-5.stage-4-r3-payload-manifest';authorityId=$authorityId;attemptId=$attemptId;files=$payloadItems})
    $archive=Join-Path $attemptRoot 'Oracle.Sprint30.5.Stage4R3QualificationEvidence.zip';Add-Type -AssemblyName System.IO.Compression.FileSystem;[IO.Compression.ZipFile]::CreateFromDirectory($payloadRoot,$archive,[IO.Compression.CompressionLevel]::Optimal,$false)
    ...
    $finalItems=@(Get-Inventory $attemptRoot @('final-evidence-manifest.json'));Write-CreateOnlyJson (Join-Path $attemptRoot 'final-evidence-manifest.json') ([ordered]@{contract='oracle.sprint-30-5.stage-4-r3-final-evidence-manifest';result='passed-awaiting-founder-review';authorityId=$authorityId;attemptId=$attemptId;files=$finalItems})
