...
if (principal.status !== 0) {
  throw new Error("Installed Stage 4 R3 development rehearsal requires an elevated process.");
}
...
const rehearsalIdentity = `stage4-r3-${timestamp}-${identitySuffix}`;
...
try {
  ...
  throw new AggregateError(
    ...
    `Installed Stage 4 R3 development rehearsal failed; NON-QUALIFICATION diagnostics remain at ${root}`,
    { cause: error }
  );
}
