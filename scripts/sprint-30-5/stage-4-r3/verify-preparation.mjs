...
assert.equal(contract.acceptedPreparation.commit, "8fc782df9869bc3c0e85a0d6d01ee7ef0d866175");
assert.equal(contract.acceptedPreparation.tree, "911684539ef85f88e2092daacb896795097e0dd8");
...
assert.equal(sha256(join(repositoryRoot, "scripts/sprint-30-5/stage-4-r2-execution/Oracle.Stage4R2ExecutionManifest.json")), contract.acceptedPreparation.preparationManifestSha256);
...
assert.equal(contract.historicalEvidenceBindings.length, 20);
...
const preparationManifest = JSON.parse(readFileSync(join(harnessRoot, "Oracle.Stage4R3PreparationManifest.json"), "utf8"));
assert.equal(preparationManifest.contract, "oracle.sprint-30-5.stage-4-r3-preparation-manifest");
...
const inventory = readdirSync(harnessRoot)
  .filter(name => name !== "Oracle.Stage4R3PreparationManifest.json")
...
const rehearsal = JSON.parse(run(approvedTools.powershell.path, ["-NoLogo", "-NoProfile", "-NonInteractive", "-ExecutionPolicy", "Bypass", "-File", join(harnessRoot, "Invoke-OracleStage4R3DevelopmentRehearsal.ps1")]).stdout);
...
assert.notEqual(transferPreparation.status, 0, "Execution-barred R3 transfer preparation passed.");
...
assert.notEqual(qualification.status, 0, "Execution-barred R3 qualification passed.");
...
for (const [file, patterns] of [
  ["Invoke-OracleStage4R3PreAuthorityPreflight.ps1", [/Assert-OracleStage4R3Transfer/u, /TransferManifestSha256/u, /TransferCustodySha256/u, /TransferVerificationSha256/u]],
  ["Invoke-OracleStage4R3Qualification.ps1", [/Assert-OracleStage4R3Transfer/u, /transfer-admission\.json/u, /ORACLE_STAGE4_TRANSFER_ROOT/u]],
  ["Invoke-OracleStage4R3InstalledPackageJourney.ps1", [/transfer\.msixRelativePath/u, /transfer\.certificateRelativePath/u]],
...
const qualificationHarness = readFileSync(join(harnessRoot, "Invoke-OracleStage4R3Qualification.ps1"), "utf8");
const transferAdmissionCalls = qualificationHarness
  .split(/\r?\n/u)
  .filter((line) => line.includes("Assert-OracleStage4R3Transfer"));
...
console.log(JSON.stringify({
  result: "passed",
  classification: "STAGE-4-R3-ENGINEERING-PREPARATION-VALIDATION",
