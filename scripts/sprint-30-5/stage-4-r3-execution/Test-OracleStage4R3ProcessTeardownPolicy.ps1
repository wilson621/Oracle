...
. (Join-Path $PSScriptRoot "Oracle.Stage4R3ProcessTeardownPolicy.ps1")
...
$stopCalls = 0
$lookupCalls = 0
$stopped = Invoke-OracleStage4R3OwnedProcessStop -ObservedProcess $observed `
...
$ownershipChecks = 0
$alreadyExited = Invoke-OracleStage4R3OwnedProcessStop -ObservedProcess $observed `
  -OwnershipValidator { param($candidate) $script:ownershipChecks++; if ($candidate.ExecutablePath -cne $ownedPath) { throw "not owned" } } `
...
$ownershipChecks = 0
Assert-Throws {
  Invoke-OracleStage4R3OwnedProcessStop -ObservedProcess $observed `
    -OwnershipValidator { param($candidate) $script:ownershipChecks++; if ($candidate.ExecutablePath -cne $ownedPath) { throw "not owned" } } `
...
$stopCalls = 0
Assert-Throws {
  Invoke-OracleStage4R3OwnedProcessStop -ObservedProcess (New-ProcessFixture 1324 "C:\Temp\rogue.exe") `
...
[pscustomobject][ordered]@{
  result = "passed"
  classification = "STAGE-4-R3-PROCESS-TEARDOWN-CORRECTION-VALIDATION"
