...
$classification = @(
  "STAGE-4-R3-INSTALLED-PACKAGE-CONTROLLER",
  "SECRET-BEARING-NON-EVIDENCE-CONTROLLER"
)
$scriptRoot = $PSScriptRoot
$repositoryRoot = [IO.Path]::GetFullPath((Join-Path $scriptRoot "..\..\.."))
$contractPath = Join-Path $scriptRoot "Oracle.Stage4R3Contract.json"
...
function Assert-Administrator {
  ...
  if (-not $principal.IsInRole(
    ...
    throw "Installed Stage 4 R3 execution requires elevated Windows PowerShell."
  }
}
...
try {
  if ($TeardownOnly) {
    ...
    if ($developmentRehearsal) {
      ...
      $founderGrantId = $attemptId -replace '^stage4-r3-', 'founder-stage4-r3-grant-'
    } else {
      if (
    ...
    $activation = Invoke-OracleStage4R3ApplicationActivation `
      -AppUserModelId $appUserModelId -Arguments $arguments
    Assert-OracleStage4R3ApplicationActivationSucceeded $activation
    ...
    if ($process.ExitCode -ne 0 -or -not (Test-Path -LiteralPath $journeyOutput -PathType Leaf)) {
      throw "Installed R3 live journey failed with exit code $($process.ExitCode)."
    }
...
$result = [pscustomobject][ordered]@{
  contract = "oracle.sprint-30-5.stage-4-r3-installed-package-result"
...
if ($result.result -cne "passed" -or -not $result.zeroResidue) {
  throw "Installed Stage 4 R3 controller failed; teardown result was preserved."
}
