...
function Assert-OracleStage4R4ApplicationActivationContract {
  [CmdletBinding()]
...
function Initialize-OracleStage4R4ActivationApi {
  if ($null -ne ("Oracle.Stage4R4.ApplicationActivation" -as [type])) {
    return
  }
...
function Test-OracleStage4R4ApplicationActivationApi {
  [CmdletBinding()]
  ...
  try {
    $native = if ($null -ne $ProbeRunner) {
      ...
      Initialize-OracleStage4R4ActivationApi
      [Oracle.Stage4R4.ApplicationActivation]::Probe()
    }
    if (
      $null -eq $native -or
...
function Invoke-OracleStage4R4ApplicationActivation {
  [CmdletBinding()]
  param(
  ...
  try {
    $native = if ($null -ne $ActivationRunner) {
      ...
      Initialize-OracleStage4R4ActivationApi
      [Oracle.Stage4R4.ApplicationActivation]::Activate(
        $AppUserModelId,
        $Arguments,
        [uint32]2
      )
...
function Assert-OracleStage4R4ApplicationActivationSucceeded {
  [CmdletBinding()]
