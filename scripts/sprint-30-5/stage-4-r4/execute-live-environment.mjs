...
try {
  if (teardownOnly) {
    ...
    config = config.replace(/project_id = ".*"/u, 'project_id = "oracle-stage4-r4-disposable"')
    ...
    for (const expected of ['project_id = "oracle-stage4-r4-disposable"', 'site_url = "http://127.0.0.1:4314"', 'additional_redirect_urls = ["http://127.0.0.1:4314/auth/callback"]', "enable_confirmations = true", "minimum_password_length = 8"]) if (!config.includes(expected)) throw new Error(`Supabase configuration binding is absent: ${expected}`);
    ...
    if (mode === "qualification" || installedDevelopmentRehearsal) {
      ...
      const installedController = join(import.meta.dirname, "Invoke-OracleStage4R4InstalledPackageJourney.ps1");
...
async function performCleanup() {
  ...
  try {
    const current = run("provider-presence-before-stop", docker, ["ps", "-a", "--filter", "name=oracle-stage4-r4-disposable", "--format", "{{.ID}}"]);
  ...
  for (const [label, args] of [["container-residue", ["ps", "-a", "--filter", "name=oracle-stage4-r4-disposable", "--format", "{{.ID}}"]], ["volume-residue", ["volume", "ls", "--filter", "name=oracle-stage4-r4-disposable", "--format", "{{.Name}}"]], ["network-residue", ["network", "ls", "--filter", "name=oracle-stage4-r4-disposable", "--format", "{{.Name}}"]]]) {
...
function verifyProviderImagesAndRoute() {
  const ids = run("provider-container-list", docker, ["ps", "--filter", "name=oracle-stage4-r4-disposable", "--format", "{{.ID}}"] ).stdout.trim().split(/\r?\n/u).filter(Boolean);
