...
assert.equal(contract.acceptedPreparation.commit, "7905f8a4f6d469eaa48ba179deadcce408499e19");
assert.equal(contract.acceptedPreparation.tree, "69424ed208d6bbb6d549e9fab47cb9fe775ebd9c");
...
assert.equal(sha256(join(repositoryRoot, "scripts/sprint-30-5/stage-4-r3-execution/Oracle.Stage4R3ExecutionManifest.json")), contract.acceptedPreparation.preparationManifestSha256);
...
assert.equal(contract.historicalEvidenceBindings.length, 29);
...
const preparationManifest = JSON.parse(readFileSync(join(harnessRoot, "Oracle.Stage4R4PreparationManifest.json"), "utf8"));
assert.equal(preparationManifest.contract, "oracle.sprint-30-5.stage-4-r4-preparation-manifest");
assert.equal(preparationManifest.acceptedPreparationCommit, contract.acceptedPreparation.commit);
assert.equal(preparationManifest.acceptedPreparationTree, contract.acceptedPreparation.tree);
assert.equal(preparationManifest.transferPreparationPermitted, false);
assert.equal(preparationManifest.founderAuthorisedQualificationExecution, false);
assert.equal(preparationManifest.maximumAttempts, 0);
assert.equal(preparationManifest.retryAfterConsumedAuthorityPermitted, false);
const inventory = readdirSync(harnessRoot)
  .filter(name => name !== "Oracle.Stage4R4PreparationManifest.json")
  .filter(name => statSync(join(harnessRoot, name)).isFile())
  .sort((a, b) => a < b ? -1 : a > b ? 1 : 0);
assert.deepEqual(preparationManifest.files.map(entry => entry.path), inventory);
...
for (const name of ["Test-OracleStage4R4Policies.ps1", "Test-OracleStage4R4ActivationPolicy.ps1", "Test-OracleStage4R4InstalledRuntimeConfigurationPolicy.ps1", "Test-OracleStage4R4ProcessTeardownPolicy.ps1"]) {
...
const rehearsal = JSON.parse(run(approvedTools.powershell.path, ["-NoLogo", "-NoProfile", "-NonInteractive", "-ExecutionPolicy", "Bypass", "-File", join(harnessRoot, "Invoke-OracleStage4R4DevelopmentRehearsal.ps1")]).stdout);
...
const qualification = spawnSync(approvedTools.powershell.path, [
  "-NoLogo", "-NoProfile", "-NonInteractive", "-ExecutionPolicy", "Bypass", "-File", join(harnessRoot, "Invoke-OracleStage4R4Qualification.ps1"),
  "-FounderAuthorityToken", "UNAUTHORISED",
  "-FounderGrantId", "founder-stage4-r4-grant-20260804T120000123Z-a1b2c3d4",
...
assert.equal(existsSync(join(artifactRoot, "authorities", "authority-stage4-r4-20260804T120000123Z-a1b2c3d4.json")), false);
assert.equal(existsSync(join(artifactRoot, "stage4-r4-20260804T120000123Z-a1b2c3d4")), false);

for (const [file, patterns] of [
  ["Invoke-OracleStage4R4PreAuthorityPreflight.ps1", [/Assert-OracleStage4R4Transfer/u, /TransferManifestSha256/u, /TransferCustodySha256/u, /TransferVerificationSha256/u]],
  ["Invoke-OracleStage4R4Qualification.ps1", [/Assert-OracleStage4R4Transfer/u, /transfer-admission\.json/u, /ORACLE_STAGE4_TRANSFER_ROOT/u]],
  ["Invoke-OracleStage4R4InstalledPackageJourney.ps1", [/transfer\.msixRelativePath/u, /transfer\.certificateRelativePath/u]],
...
const qualificationHarness = readFileSync(join(harnessRoot, "Invoke-OracleStage4R4Qualification.ps1"), "utf8");
const transferAdmissionCalls = qualificationHarness
  .split(/\r?\n/u)
  .filter((line) => line.includes("Assert-OracleStage4R4Transfer"));
...
console.log(JSON.stringify({
  result: "passed",
  classification: "STAGE-4-R4-ENGINEERING-PREPARATION-VALIDATION",
  founderAuthorisedQualificationExecution: false,
  acceptedR3CorrectionPreserved: true,
