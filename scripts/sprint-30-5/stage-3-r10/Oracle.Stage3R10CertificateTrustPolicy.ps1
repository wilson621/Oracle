...
function Assert-OracleStage3R10CertificateTrustContract {
...
function Assert-OracleStage3R10Thumbprint {
...
function Get-OracleStage3R10CertificateStoreDefinitions {
  @(
...
function Get-OracleStage3R10PhysicalCertificateMatches {
  param([Parameter(Mandatory = $true)][string]$Thumbprint)

  Assert-OracleStage3R10Thumbprint -Thumbprint $Thumbprint
  $matches = @()
  foreach ($definition in @(Get-OracleStage3R10CertificateStoreDefinitions)) {
...
function Get-OracleStage3R10LogicalCertificateViews {
  param([Parameter(Mandatory = $true)][string]$Thumbprint)
...
function Test-OracleStage3R10CertificateIdentity {
  param(
...
function Assert-OracleStage3R10NoCertificateResidue {
  param(
...
function Assert-OracleStage3R10TemporaryTrustState {
  param(
    [Parameter(Mandatory = $true)]
    [AllowEmptyCollection()][object[]]$PhysicalMatches,
    [Parameter(Mandatory = $true)]
  ...
  if (
    ...
    -not (Test-OracleStage3R10CertificateIdentity `
...
function Assert-OracleStage3R10ExactRemovalTarget {
  param(
    [Parameter(Mandatory = $true)]
    [AllowEmptyCollection()][object[]]$PhysicalMatches,
    [Parameter(Mandatory = $true)]
...
function Test-OracleStage3R10ProcessIsElevated {
