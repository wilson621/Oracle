...
const sources = [
  ...
  join(releaseRoot, "oracle-0.1.2.cdx.json"),
  join(releaseRoot, "oracle-0.1.2.provenance.json"),
  ...
  join(evidenceRoot, "Oracle.Stage2RequalificationR4EvidenceManifest.json"),
  ...
  join(import.meta.dirname, "Oracle.Stage3R10PhaseAudit.json"),
  join(import.meta.dirname, "Test-OracleStage3R10OptionalMemberAudit.ps1"),
  join(import.meta.dirname, "Test-OracleStage3R10InstalledRuntimeConfigurationPolicy.ps1"),
  join(import.meta.dirname, "Test-OracleStage3R10ObservationPolicy.ps1"),
  join(import.meta.dirname, "Test-OracleStage3R10ActivationPolicy.ps1"),
  join(import.meta.dirname, "Test-OracleStage3R10CertificateTrustPolicy.ps1"),
  join(import.meta.dirname, "Test-OracleStage3R10WindowPolicy.ps1"),
...
for (const source of sources) {
  if (!existsSync(source)) throw new Error(`Required R10 transfer source is missing: ${source}`);
}
...
validateAcceptedBindings({
  ...
  sbomSha256: sha256(join(releaseRoot, "oracle-0.1.2.cdx.json")),
  provenanceSha256: sha256(join(releaseRoot, "oracle-0.1.2.provenance.json")),
...
if (
  ...
  throw new Error("R10 optional-member audit did not pass during transfer construction.");
}
...
const manifestPath = join(transferRoot, "Oracle.Stage3R10TransferManifest.json");
writeJsonAtomicCreateOnly(manifestPath, {
  schemaVersion: "1.0.0",
  contract: "oracle.sprint-30-5.stage-3-r10-transfer",
...
const custodyPath = join(transferRoot, "Oracle.Stage3R10TransferCustody.json");
