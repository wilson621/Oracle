...
$operationalFiles = @(
  "Get-OracleStage3R10HostContinuity.ps1",
  "Invoke-OracleStage3R10PreAuthorityPreflight.ps1",
  "Invoke-OracleStage3R10Qualification.ps1",
  "Oracle.Stage3R10ActivationPolicy.ps1",
  "Oracle.Stage3R10CertificateTrustPolicy.ps1",
  "Oracle.Stage3R10IdentityPolicy.ps1",
  "Oracle.Stage3R10InstalledSoftwarePolicy.ps1",
  "Oracle.Stage3R10InstalledRuntimeConfigurationPolicy.ps1",
  "Oracle.Stage3R10LifecyclePolicy.ps1",
  "Oracle.Stage3R10ObservationPolicy.ps1",
  "Oracle.Stage3R10PackageInventoryPolicy.ps1",
  "Oracle.Stage3R10PreflightPolicy.ps1",
  "Oracle.Stage3R10ProcessPolicy.ps1",
  "Oracle.Stage3R10WindowPolicy.ps1",
  "Oracle.Stage3R10WindowsExecutablePolicy.ps1"
)
...
function Get-Classification(
  ...
  if (
    ...
    return [ordered]@{
      ...
      testCoverage = "Test-OracleStage3R10OptionalMemberAudit.ps1 and verify-preparation.mjs"
      disposition = "fail-closed-if-missing"
    }
  }
  if (
    $File -ceq "Oracle.Stage3R10PreflightPolicy.ps1" -and
    ...
    return [ordered]@{
      ...
      testCoverage = "Test-OracleStage3R10HostShapeFixtures.ps1"
      disposition = "fail-closed-if-missing"
    }
  }
  if ($Expression -match '\.PSObject\.Properties\[') {
    return [ordered]@{
      ...
      testCoverage = "Test-OracleStage3R10InstalledSoftwarePolicy.ps1"
      disposition = "safe"
    }
  }
  if ($File -ceq "Oracle.Stage3R10InstalledRuntimeConfigurationPolicy.ps1") {
    return [ordered]@{
      classification = "explicitly-validated-runtime-configuration-member"
      dataSource = "typed filesystem, ACL, URI and policy-owned configuration record"
      mandatory = $true
      protection = "exact identity, path, binding, ACL, URI, hash and residue validation"
      testCoverage = "Test-OracleStage3R10InstalledRuntimeConfigurationPolicy.ps1 and development rehearsal"
      disposition = "fail-closed-if-missing-or-mismatched"
    }
  }
  if ($File -ceq "Oracle.Stage3R10InstalledSoftwarePolicy.ps1") {
    return [ordered]@{
  ...
  if ($File -ceq "Oracle.Stage3R10LifecyclePolicy.ps1") {
    return [ordered]@{
      classification = "structurally-guaranteed-by-construction"
      dataSource = "policy-owned lifecycle state"
      mandatory = $true
      protection = "state shape is created only by New-OracleStage3R10LifecycleState"
      testCoverage = "Test-OracleStage3R10LifecyclePolicy.ps1"
      disposition = "safe"
    }
  }
  if ($File -ceq "Oracle.Stage3R10ObservationPolicy.ps1") {
    return [ordered]@{
      classification = "observation-policy-owned-record-or-typed-clock-member"
      dataSource = "System.Diagnostics.Stopwatch, DateTime or policy-owned observation record"
      mandatory = $true
      protection = "typed monotonic clock plus explicit null, array, finite-value and backwards-time checks"
      testCoverage = "Test-OracleStage3R10ObservationPolicy.ps1 and development rehearsal"
      disposition = "fail-closed-if-invalid"
    }
  }
  if ($File -ceq "Oracle.Stage3R10PreflightPolicy.ps1") {
    return [ordered]@{
      ...
      mandatory = $true
      protection = "typed producer or Get-OracleStage3R10RequiredPropertyValue"
      testCoverage = "host-shape fixtures and Founder-QA-01 pre-authority probe"
      disposition = "fail-closed-if-unavailable"
    }
  }
  if ($File -ceq "Oracle.Stage3R10ProcessPolicy.ps1") {
    return [ordered]@{
      ...
      mandatory = $true
      protection = "Get-OracleStage3R10RequiredProcessMember checks every member before use"
      testCoverage = "Test-OracleStage3R10ProcessPolicy.ps1"
      disposition = "fail-closed-if-missing"
    }
  }
  if ($File -ceq "Oracle.Stage3R10WindowPolicy.ps1") {
    return [ordered]@{
      classification = "explicitly-validated-window-or-appmodel-member"
      dataSource = "native discovery JSON or typed AppModel policy record"
      mandatory = $true
      protection = "required members are checked through PSObject.Properties and AppModel identity is returned by a typed Win32 wrapper"
      testCoverage = "Test-OracleStage3R10WindowPolicy.ps1"
      disposition = "fail-closed-if-missing-or-mismatched"
    }
  }
  if ($File -ceq "Oracle.Stage3R10PackageInventoryPolicy.ps1") {
    return [ordered]@{
      ...
      testCoverage = "Test-OracleStage3R10PackageInventoryPolicy.ps1"
      disposition = "safe"
    }
  }
  if ($File -ceq "Oracle.Stage3R10IdentityPolicy.ps1") {
    return [ordered]@{
      classification = "structurally-guaranteed-by-platform-type"
      dataSource = "System.StringComparer"
      mandatory = $true
      protection = "typed .NET ordinal comparison"
      testCoverage = "Test-OracleStage3R10IdentityPolicy.ps1"
      disposition = "safe"
    }
  }
  if ($File -ceq "Oracle.Stage3R10CertificateTrustPolicy.ps1") {
    return [ordered]@{
      ...
      testCoverage = "Test-OracleStage3R10CertificateTrustPolicy.ps1"
      disposition = "fail-closed-if-missing-or-mismatched"
    }
  }
  if ($File -ceq "Oracle.Stage3R10ActivationPolicy.ps1") {
    return [ordered]@{
      classification = "activation-policy-owned-or-typed-platform-member"
      dataSource = "policy-owned activation result or typed COM interop result"
      mandatory = $true
      protection = "mandatory result-member checks, exact HRESULT and nonzero PID assertion"
      testCoverage = "Test-OracleStage3R10ActivationPolicy.ps1 and elevated integration test"
      disposition = "fail-closed-if-missing-or-unsuccessful"
    }
  }
  if ($File -ceq "Oracle.Stage3R10WindowsExecutablePolicy.ps1") {
    return [ordered]@{
      ...
      testCoverage = "Test-OracleStage3R10WindowsExecutablePolicy.ps1"
      disposition = "fail-closed-if-unavailable"
    }
  }
  if (
    $Expression -match '^\$(Record|initialRuntimeConfiguration|repairRuntimeConfiguration|initialRuntimeTeardown|repairRuntimeTeardown|runningInstalledRuntimeConfigurationPolicyEntry)(?:\[.*?\])?(?:\.|$)'
  ) {
    return [ordered]@{
      classification = "runtime-configuration-policy-owned-record"
      dataSource = "versioned runtime-configuration policy result or manifest entry"
      mandatory = $true
      protection = "producer validates and constructs exact result shape; transfer cardinality and hashes are checked before use"
      testCoverage = "Test-OracleStage3R10InstalledRuntimeConfigurationPolicy.ps1, verify-preparation.mjs and development rehearsal"
      disposition = "fail-closed-if-missing-or-mismatched"
    }
  }  if ($Expression -match '^\$contract(?:\.|$)') {
    return [ordered]@{
      classification = "mandatory-validated-contract-member"
      dataSource = "Oracle.Stage3R10Contract.json"
      mandatory = $true
  ...
  if ($Expression -match '^\$(MyInvocation|PSVersionTable|env:|phases|lifecycleState|processEvidenceCounts|teardownProcessEvents|actual|expected|samples|stability|resolution|developmentTools|certificateMatches|physicalCertificateMatches|logicalCertificateViews|physicalMatches|logicalViews|seen|issues|viewCalls|fixtures|allFiles|failureResults|teardownFailures|tamperedBytes|sidecarValue|custodySidecarValue|time|hostContinuityRecordedAt|deadline|scriptPath|attemptRoot|canonical|RawZipPath|actualTransferRootEntries|expectedTransferRootEntries|actualPayload|expectedPayload|actualPayloadPaths|payloadPaths|payloadEntries|runningScriptEntry|contractEntry|runningHarnessEntry|runningContractEntry|runningActivationPolicyEntry|runningCertificateTrustPolicyEntry|runningIdentityPolicyEntry|runningPackageInventoryPolicyEntry|runningInstalledSoftwarePolicyEntry|runningLifecyclePolicyEntry|runningObservationPolicyEntry|runningPreflightPolicyEntry|runningProcessPolicyEntry|runningWindowPolicyEntry|runningWindowsExecutablePolicyEntry|zipEntries|candidates|matches|initial|repair|initialActivation|repairActivation|applicationActivation|inventory|preflight|packageReconciliation|operation|definition|view)(?:\[.*?\])?(?:\.|$)|^@\(.+\)\.Count$|^\[DateTime\]::UtcNow\.|^\(\[DateTime\]::UtcNow\s+-') {
    return [ordered]@{
...
$report = [ordered]@{
  schemaVersion = "1.0.0"
  contract = "oracle.sprint-30-5.stage-3-r10-optional-member-audit"
