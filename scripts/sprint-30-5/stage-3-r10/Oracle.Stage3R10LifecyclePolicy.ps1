...
function Get-OracleStage3R10LifecyclePhases {
  @(
    "authority-consumed",
    "transfer-verified",
    "host-admitted",
...
function New-OracleStage3R10LifecycleState {
...
function Move-OracleStage3R10Lifecycle {
  [CmdletBinding()]
  param(
...
function Assert-OracleStage3R10NextLifecyclePhase {
  [CmdletBinding()]
  param(
    [Parameter(Mandatory = $true)][object]$State,
    [Parameter(Mandatory = $true)][string]$Phase
...
function Get-OracleStage3R10TeardownObligations {
  [CmdletBinding()]
  param(
