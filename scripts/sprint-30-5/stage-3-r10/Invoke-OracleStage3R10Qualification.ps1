...
if (
  -not [StringComparer]::OrdinalIgnoreCase.Equals(
    ...
    [IO.Path]::GetFullPath((
      Join-Path $TransferRoot "payload/Invoke-OracleStage3R10Qualification.ps1"
    ))
  )
) { throw "Execution bootstrap script is outside the verified transfer." }
$contract = Get-Content -LiteralPath (
  Join-Path $scriptRoot "Oracle.Stage3R10Contract.json"
...
. (Join-Path $scriptRoot "Oracle.Stage3R10PackageInventoryPolicy.ps1")
. (Join-Path $scriptRoot "Oracle.Stage3R10InstalledSoftwarePolicy.ps1")
. (Join-Path $scriptRoot "Oracle.Stage3R10InstalledRuntimeConfigurationPolicy.ps1")
. (Join-Path $scriptRoot "Oracle.Stage3R10LifecyclePolicy.ps1")
. (Join-Path $scriptRoot "Oracle.Stage3R10ObservationPolicy.ps1")
. (Join-Path $scriptRoot "Oracle.Stage3R10PreflightPolicy.ps1")
. (Join-Path $scriptRoot "Oracle.Stage3R10ProcessPolicy.ps1")
...
$expectedToken = "FOUNDER-AUTHORISED-STAGE3-R10-EXECUTION"
...
$identityPolicyPath = Join-Path $payloadRoot "Oracle.Stage3R10IdentityPolicy.ps1"
$activationPolicyPath = Join-Path $payloadRoot "Oracle.Stage3R10ActivationPolicy.ps1"
...
$lifecyclePolicyPath = Join-Path $payloadRoot "Oracle.Stage3R10LifecyclePolicy.ps1"
$observationPolicyPath = Join-Path $payloadRoot (
  "Oracle.Stage3R10ObservationPolicy.ps1"
)
$preflightPolicyPath = Join-Path $payloadRoot "Oracle.Stage3R10PreflightPolicy.ps1"
$processPolicyPath = Join-Path $payloadRoot "Oracle.Stage3R10ProcessPolicy.ps1"
$windowPolicyPath = Join-Path $payloadRoot "Oracle.Stage3R10WindowPolicy.ps1"
...
$lifecycleState = New-OracleStage3R10LifecycleState
...
function New-GovernedRuntimeConfiguration {
  ...
  $sessionSecure = $null
  try {
    $anonValue = New-OracleStage3R10CryptographicValue
    $serviceValue = New-OracleStage3R10CryptographicValue
    $sessionValue = New-OracleStage3R10CryptographicValue
...
function Write-Lifecycle([string]$Phase, $Details) {
  ...
  Assert-OracleStage3R10NextLifecyclePhase -State $lifecycleState -Phase $Phase
  ...
  [void](Move-OracleStage3R10Lifecycle -State $lifecycleState -Phase $Phase)
}
...
function Invoke-GovernedProcess(
  ...
  Invoke-OracleStage3R10GovernedProcess `
    -Name $Name `
...
function Get-ExactCertificateMatches {
  @(Get-OracleStage3R10LogicalCertificateViews -Thumbprint $thumbprint)
}

function Get-PhysicalExactCertificateMatches {
  @(Get-OracleStage3R10PhysicalCertificateMatches -Thumbprint $thumbprint)
}
...
function Remove-ExactMachineTrust {
  ...
  $certutil = Get-OracleStage3R10WindowsExecutablePath -Name "certutil.exe"
  [void](Invoke-GovernedProcess "exact-machine-trust-remove" $certutil @(
    Get-OracleStage3R10TrustRemovalArguments -Thumbprint $thumbprint
  ))
  Assert-OracleStage3R10NoCertificateResidue `
    -PhysicalMatches @(Get-PhysicalExactCertificateMatches) `
    -LogicalViews @(Get-ExactCertificateMatches)
}
...
function Stop-ExactPackageProcesses {
  ...
  foreach ($process in @(Get-Process -Name "Oracle" -ErrorAction SilentlyContinue)) {
    $resolution = Resolve-OracleStage3R10TeardownProcessOwnership `
    ...
    try {
      ...
      if (-not (Test-OracleStage3R10ProcessExists -ProcessId $process.Id)) {
        $teardownProcessEvents.Add([ordered]@{
          recordedAtUtc = [DateTime]::UtcNow.ToString("o")
          processId = [int]$process.Id
...
function Assert-IdentityAndTransfer {
  ...
  $attempt = [regex]::Match($AttemptId, '^stage3-r10-(\d{8}T\d{9}Z)-([0-9a-f]{8})$')
  $authority = [regex]::Match($AuthorityId, '^authority-stage3-r10-(\d{8}T\d{9}Z)-([0-9a-f]{8})$')
  ...
  $expectedFounderGrantId = $AttemptId -replace (
    '^stage3-r10-', 'founder-stage3-r10-grant-'
  )
  ...
  ) { throw "Transfer is not bound to accepted Stage 2 R4." }
  $expectedPayload = @(
    ...
    "SPRINT_30_5_STAGE_2_REQUALIFICATION_R4_CLOSURE.md",
    ...
    "Oracle.Stage3R10LifecyclePolicy.ps1",
    "Oracle.Stage3R10ObservationPolicy.ps1",
    "Oracle.Stage3R10OptionalMemberAudit.json",
    "Oracle.Stage3R10PreflightPolicy.ps1",
    "Oracle.Stage3R10ProcessPolicy.ps1",
    "Oracle.Stage3R10WindowPolicy.ps1",
    "Oracle.Stage3R10WindowsExecutablePolicy.ps1",
    "Oracle.Stage3R10PhaseAudit.json",
    "Oracle.Stage3R10PackageInventoryPolicy.ps1",
  ...
  $runningHarnessEntry = @($manifest.payload | Where-Object {
    [string]$_.path -ceq "payload/Invoke-OracleStage3R10Qualification.ps1"
  })
  $runningContractEntry = @($manifest.payload | Where-Object {
    [string]$_.path -ceq "payload/Oracle.Stage3R10Contract.json"
  })
  $runningActivationPolicyEntry = @($manifest.payload | Where-Object {
    [string]$_.path -ceq "payload/Oracle.Stage3R10ActivationPolicy.ps1"
  })
  $runningCertificateTrustPolicyEntry = @($manifest.payload | Where-Object {
    [string]$_.path -ceq "payload/Oracle.Stage3R10CertificateTrustPolicy.ps1"
  })
  $runningIdentityPolicyEntry = @($manifest.payload | Where-Object {
    [string]$_.path -ceq "payload/Oracle.Stage3R10IdentityPolicy.ps1"
  })
  $runningPackageInventoryPolicyEntry = @($manifest.payload | Where-Object {
    [string]$_.path -ceq "payload/Oracle.Stage3R10PackageInventoryPolicy.ps1"
  })
  $runningInstalledSoftwarePolicyEntry = @($manifest.payload | Where-Object {
    [string]$_.path -ceq "payload/Oracle.Stage3R10InstalledSoftwarePolicy.ps1"
  })
  $runningInstalledRuntimeConfigurationPolicyEntry = @($manifest.payload | Where-Object {
    [string]$_.path -ceq
      "payload/Oracle.Stage3R10InstalledRuntimeConfigurationPolicy.ps1"
  })
  $runningLifecyclePolicyEntry = @($manifest.payload | Where-Object {
    [string]$_.path -ceq "payload/Oracle.Stage3R10LifecyclePolicy.ps1"
  })
  $runningObservationPolicyEntry = @($manifest.payload | Where-Object {
    [string]$_.path -ceq "payload/Oracle.Stage3R10ObservationPolicy.ps1"
  })
  $runningPreflightPolicyEntry = @($manifest.payload | Where-Object {
    [string]$_.path -ceq "payload/Oracle.Stage3R10PreflightPolicy.ps1"
  })
  $runningProcessPolicyEntry = @($manifest.payload | Where-Object {
    [string]$_.path -ceq "payload/Oracle.Stage3R10ProcessPolicy.ps1"
  })
  $runningWindowPolicyEntry = @($manifest.payload | Where-Object {
    [string]$_.path -ceq "payload/Oracle.Stage3R10WindowPolicy.ps1"
  })
  $runningWindowsExecutablePolicyEntry = @($manifest.payload | Where-Object {
    [string]$_.path -ceq "payload/Oracle.Stage3R10WindowsExecutablePolicy.ps1"
  })
  if (
    ...
    (Get-Sha256 (Join-Path $scriptRoot "Oracle.Stage3R10Contract.json")) -cne
    ...
    (Get-Sha256 (Join-Path $scriptRoot "Oracle.Stage3R10InstalledRuntimeConfigurationPolicy.ps1")) -cne
  ...
  $sbomPath = Join-Path $payloadRoot "oracle-0.1.2.cdx.json"
  $provenancePath = Join-Path $payloadRoot "oracle-0.1.2.provenance.json"
  ...
  $phaseAuditPath = Join-Path $payloadRoot "Oracle.Stage3R10PhaseAudit.json"
  ...
  if (
    ...
    @($auditedPhases).Count -ne
      @(Get-OracleStage3R10LifecyclePhases).Count
  ...
  ) { throw "Accepted R4 signature or teardown evidence differs." }
}
...
function Assert-PackageContent {
  ...
  if (
    [string]$packageInventory.contract -cne
      "oracle.sprint-30-5.stage-2-requalification-r4-package-content-inventory" -or
  ...
  foreach ($entry in @($packageInventory.entries)) {
    $canonicalExpectedPath =
      ConvertTo-OracleStage3R10CanonicalPackagePath ([string]$entry.path)
    if (
      $canonicalExpectedPath -cne [string]$entry.path -or
      (Test-OracleStage3R10ReservedPackageMetadata $canonicalExpectedPath) -or
  ...
  $zipEntries = @(Get-OracleStage3R10PackageZipInventory $msixPath)
...
function Invoke-NativeObservation([string]$Label) {
  ...
  while ([DateTime]::UtcNow -lt $deadline -and $null -eq $selected) {
    ...
    $windows = @(
      ConvertFrom-OracleStage3R10WindowDiscoveryJson -Json ([string]$result.stdout)
    )
    $candidates = @($windows | Where-Object {
      Test-OracleStage3R10QualifyingWindow $_
    })
  ...
  $ownership = Assert-OracleStage3R10ProcessPackageOwnership `
    -ProcessId ([int]$process.Id) `
  ...
  $stability = Invoke-OracleStage3R10CompleteStableObservation `
...
try {
  ...
  $preflight = Get-OracleStage3R10PreAuthorityObservation `
    -Contract $contract `
  ...
  $certificate = Get-OracleStage3R10AcceptedPublicCertificate `
  ...
  [void](Invoke-GovernedProcess "exact-machine-trust-import" $certutil @(
    Get-OracleStage3R10TrustImportArguments -CertificatePath $certificatePath
  ))
  $physicalMatches = @(Get-PhysicalExactCertificateMatches)
  $logicalViews = @(Get-ExactCertificateMatches)
  ...
  $initialActivation = Invoke-OracleStage3R10ApplicationActivation `
    -AppUserModelId $appUserModelId `
    -Arguments $initialActivationArguments
  Write-CreateOnlyJson (
    Join-Path $evidenceRoot "initial-activation.json"
  ) $initialActivation
  Assert-OracleStage3R10ApplicationActivationSucceeded -Result $initialActivation
  ...
  $repairActivation = Invoke-OracleStage3R10ApplicationActivation `
    -AppUserModelId $appUserModelId `
    -Arguments $repairActivationArguments
  Write-CreateOnlyJson (
    Join-Path $evidenceRoot "repair-activation.json"
  ) $repairActivation
  Assert-OracleStage3R10ApplicationActivationSucceeded -Result $repairActivation
