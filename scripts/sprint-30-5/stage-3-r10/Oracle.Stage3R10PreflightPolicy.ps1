...
function Get-OracleStage3R10RequiredPropertyValue {
  [CmdletBinding()]
  param(
...
function Assert-OracleStage3R10CommandSurface {
  [CmdletBinding()]
  param()
...
function Get-OracleStage3R10AcceptedPublicCertificate {
  [CmdletBinding()]
  param(
...
function Invoke-OracleStage3R10ReadOnlyProcess {
  [CmdletBinding()]
  param(
...
function Assert-OracleStage3R10NoReparseTraversal([string]$Path) {
...
function Get-OracleStage3R10PreAuthorityObservation {
  [CmdletBinding()]
  param(
    [Parameter(Mandatory = $true)][object]$Contract,
    [Parameter(Mandatory = $true)][string]$TransferRoot,
  ...
  if (
    ...
    -not (Test-OracleStage3R10ProcessIsElevated)
  ) {
  ...
  foreach ($path in @($TransferRoot, $EvidenceReturnRoot, $HostContinuityPath)) {
    Assert-OracleStage3R10NoReparseTraversal $path
  }
  if (
  ...
  $commands = @(Assert-OracleStage3R10CommandSurface)
  $windowsExecutables = [ordered]@{
    certutil = Get-OracleStage3R10WindowsExecutablePath -Name "certutil.exe"
    reagentc = Get-OracleStage3R10WindowsExecutablePath -Name "reagentc.exe"
  }
  $applicationActivation = Test-OracleStage3R10ApplicationActivationApi
  ...
  $recovery = Invoke-OracleStage3R10ReadOnlyProcess `
  ...
  $manufacturer = [string](Get-OracleStage3R10RequiredPropertyValue `
    $computerSystem "Manufacturer" "Win32_ComputerSystem")
  $model = [string](Get-OracleStage3R10RequiredPropertyValue `
  ...
  foreach ($member in @("TpmPresent", "TpmReady")) {
    [void](Get-OracleStage3R10RequiredPropertyValue $tpm $member "Get-Tpm")
  }
  foreach ($member in @("AntivirusEnabled", "RealTimeProtectionEnabled")) {
    [void](Get-OracleStage3R10RequiredPropertyValue $defender $member "Get-MpComputerStatus")
  }
  if (
    -not $secureBoot -or
    -not [bool](Get-OracleStage3R10RequiredPropertyValue $tpm "TpmPresent" "Get-Tpm") -or
    -not [bool](Get-OracleStage3R10RequiredPropertyValue $tpm "TpmReady" "Get-Tpm") -or
    -not [bool](Get-OracleStage3R10RequiredPropertyValue (
      $defender
  ...
  $activation = @(Get-CimInstance SoftwareLicensingProduct -ErrorAction Stop |
    Where-Object {
      $partialKey = Get-OracleStage3R10RequiredPropertyValue `
        $_ "PartialProductKey" "SoftwareLicensingProduct"
      $licenseStatus = Get-OracleStage3R10RequiredPropertyValue `
  ...
  $installedSoftware = @(Get-OracleStage3R10InstalledSoftwareInventory)
  ...
  if ($certificateMatches.Count -ne 0) {
    throw "Governed Stage 2 R4 certificate is already present."
  }
  ...
  foreach ($member in @("Status", "SignerCertificate")) {
    [void](Get-OracleStage3R10RequiredPropertyValue (
      $untrustedSignature
    ) $member "Get-AuthenticodeSignature")
  }
  $signerCertificate = Get-OracleStage3R10AcceptedPublicCertificate `
    -Contract $Contract -TransferRoot $TransferRoot
  $observedUntrustedSigner = Get-OracleStage3R10RequiredPropertyValue `
    $untrustedSignature "SignerCertificate" "Get-AuthenticodeSignature"
  if (
    [string](Get-OracleStage3R10RequiredPropertyValue (
      $untrustedSignature
  ...
  foreach ($member in @(
    ...
    [void](Get-OracleStage3R10RequiredPropertyValue $continuity $member "host continuity")
  }
  foreach ($member in @("deviceName", "manufacturer", "model")) {
    [void](Get-OracleStage3R10RequiredPropertyValue $continuity.host $member "host continuity host")
  }
  ...
  if (
    [string]$continuity.contract -cne
      "oracle.sprint-30-5.stage-3-r10-host-continuity" -or
  ...
  [pscustomobject][ordered]@{
    ...
    security = [ordered]@{
      ...
      tpmPresent = [bool](Get-OracleStage3R10RequiredPropertyValue (
        $tpm
      ) "TpmPresent" "Get-Tpm")
      tpmReady = [bool](Get-OracleStage3R10RequiredPropertyValue (
        $tpm
      ) "TpmReady" "Get-Tpm")
      defenderAntivirusEnabled = [bool](Get-OracleStage3R10RequiredPropertyValue (
        $defender
      ) "AntivirusEnabled" "Get-MpComputerStatus")
      defenderRealTimeProtectionEnabled = [bool](
        Get-OracleStage3R10RequiredPropertyValue (
          $defender
        ) "RealTimeProtectionEnabled" "Get-MpComputerStatus"
      )
