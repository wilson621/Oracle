...
function Assert-OracleStage3R10ApplicationActivationContract {
  [CmdletBinding()]
...
function Initialize-OracleStage3R10ActivationApi {
  if ($null -ne ("Oracle.Stage3R10.ApplicationActivation" -as [type])) {
    return
  }
...
function Test-OracleStage3R10ApplicationActivationApi {
  [CmdletBinding()]
  ...
  try {
    $native = if ($null -ne $ProbeRunner) {
      ...
      Initialize-OracleStage3R10ActivationApi
      [Oracle.Stage3R10.ApplicationActivation]::Probe()
    }
    if (
      $null -eq $native -or
...
function Invoke-OracleStage3R10ApplicationActivation {
  [CmdletBinding()]
  param(
  ...
  try {
    $native = if ($null -ne $ActivationRunner) {
      ...
      Initialize-OracleStage3R10ActivationApi
      [Oracle.Stage3R10.ApplicationActivation]::Activate(
        $AppUserModelId,
        $Arguments,
        [uint32]2
      )
...
function Assert-OracleStage3R10ApplicationActivationSucceeded {
  [CmdletBinding()]
