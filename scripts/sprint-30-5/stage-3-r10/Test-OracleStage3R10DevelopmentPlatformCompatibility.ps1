...
. (Join-Path $PSScriptRoot "Oracle.Stage3R10PreflightPolicy.ps1")
...
$inventory = @(Get-OracleStage3R10InstalledSoftwareInventory)
...
$contract = Get-Content -LiteralPath (
  Join-Path $PSScriptRoot "Oracle.Stage3R10Contract.json"
...
$fixtureRoot = Join-Path $env:TEMP (
  "oracle-stage3-r10-compatibility-" + [Guid]::NewGuid().ToString("N")
)
...
try {
  ...
  $detachedSigner = Get-OracleStage3R10AcceptedPublicCertificate `
