...
function Get-OracleStage3R10RequiredProcessMember {
  [CmdletBinding()]
  param(
...
function Invoke-OracleStage3R10GovernedProcess {
  [CmdletBinding()]
  param(
    [Parameter(Mandatory = $true)][string]$Name,
  ...
  foreach ($member in @(
    ...
    [void](Get-OracleStage3R10RequiredProcessMember $record $member)
  }
  ...
  $processError = Get-OracleStage3R10RequiredProcessMember $record "processError"
  $signal = Get-OracleStage3R10RequiredProcessMember $record "signal"
  $exitCode = Get-OracleStage3R10RequiredProcessMember $record "exitCode"
