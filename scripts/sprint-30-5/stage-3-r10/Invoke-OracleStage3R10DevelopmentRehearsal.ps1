...
. (Join-Path $PSScriptRoot "Oracle.Stage3R10LifecyclePolicy.ps1")
. (Join-Path $PSScriptRoot "Oracle.Stage3R10IdentityPolicy.ps1")
. (Join-Path $PSScriptRoot "Oracle.Stage3R10ActivationPolicy.ps1")
. (Join-Path $PSScriptRoot "Oracle.Stage3R10CertificateTrustPolicy.ps1")
. (Join-Path $PSScriptRoot "Oracle.Stage3R10InstalledSoftwarePolicy.ps1")
. (Join-Path $PSScriptRoot "Oracle.Stage3R10InstalledRuntimeConfigurationPolicy.ps1")
. (Join-Path $PSScriptRoot "Oracle.Stage3R10ObservationPolicy.ps1")
. (Join-Path $PSScriptRoot "Oracle.Stage3R10ProcessPolicy.ps1")
. (Join-Path $PSScriptRoot "Oracle.Stage3R10WindowPolicy.ps1")
. (Join-Path $PSScriptRoot "Oracle.Stage3R10WindowsExecutablePolicy.ps1")
...
function Invoke-SimulatedPhase(
  ...
  Assert-OracleStage3R10NextLifecyclePhase -State $State -Phase $Phase
  ...
  [void](Move-OracleStage3R10Lifecycle -State $State -Phase $Phase)
}

function Invoke-SimulatedLifecycle([string]$Root, [string]$InjectedFailure) {
  $state = New-OracleStage3R10LifecycleState
  $failure = $null
  try {
    foreach ($phase in @(Get-OracleStage3R10LifecyclePhases)) {
  ...
  $teardown = Get-OracleStage3R10TeardownObligations `
...
$parent = Join-Path $repositoryRoot ".tmp-r10"
...
try {
  $software = @(ConvertTo-OracleStage3R10InstalledSoftwareInventory -RegistryEntries @(
  ...
  $fixtureThumbprint = "03BEFBF303751D3DC14DF3FA224EB6BC5A6E4222"
  $fixtureSubject = "CN=Oracle Stage 2 Requalification R4 Local Test Signing - NOT PRODUCTION"
  ...
  foreach ($name in @("certutil.exe", "reagentc.exe")) {
    ...
    [void](Get-OracleStage3R10WindowsExecutablePath `
      -Name $name -SystemDirectory $fixtureSystem32)
  }
  $fixtureActivation = Invoke-OracleStage3R10ApplicationActivation `
  ...
  $fixtureWindows = @(
    ConvertFrom-OracleStage3R10WindowDiscoveryJson -Json (
  ...
  if (
    ...
    -not (Test-OracleStage3R10QualifyingWindow $fixtureWindows[0])
  ) { throw "Shared window discovery policy failed inside rehearsal." }
  [void](Assert-OracleStage3R10ProcessPackageOwnership `
    -ProcessId 9808 `
  ...
  $observation = Invoke-OracleStage3R10CompleteStableObservation `
  ...
  $exitedProcess = Resolve-OracleStage3R10TeardownProcessOwnership `
    -ProcessId 9808 `
    -ExpectedPackageFamilyName "Oracle.Platform.LocalCertification_fixture" `
    -OwnershipResolver {
      param($ProcessId)
      throw [Oracle.Stage3R10.ProcessOpenException]::new(
        [uint32]$ProcessId, 87
      )
    } `
  ...
  $processCounts = @{}
  $processResult = Invoke-OracleStage3R10GovernedProcess `
  ...
  $runtimeAttempt = "stage3-r10-20260803T120000000Z-a1b2c3d4"
  $runtimeParameters = @{
    PackageFamilyName = $runtimeFamily
    ExpectedPackageFamilyName = $runtimeFamily
    ConfigurationId = "runtime-$runtimeAttempt"
    FounderGrantId = "founder-stage3-r10-grant-20260803T120000000Z-a1b2c3d4"
