...
function Get-OracleStage3R13RequiredProcessMember {
  [CmdletBinding()]
  param(
...
function Invoke-OracleStage3R13GovernedProcess {
  [CmdletBinding()]
  param(
    [Parameter(Mandatory = $true)][string]$Name,
  ...
  foreach ($member in @(
    ...
    [void](Get-OracleStage3R13RequiredProcessMember $record $member)
  }
  ...
  $processError = Get-OracleStage3R13RequiredProcessMember $record "processError"
  $signal = Get-OracleStage3R13RequiredProcessMember $record "signal"
  $exitCode = Get-OracleStage3R13RequiredProcessMember $record "exitCode"
