...
function Get-Classification(
  ...
  if (
    ...
    return [ordered]@{
      ...
      testCoverage = "Test-OracleStage3R13OptionalMemberAudit.ps1 and verify-preparation.mjs"
      disposition = "fail-closed-if-missing"
    }
  }
  if (
    $File -ceq "Oracle.Stage3R13PreflightPolicy.ps1" -and
    ...
    return [ordered]@{
      ...
      testCoverage = "Test-OracleStage3R13HostShapeFixtures.ps1"
      disposition = "fail-closed-if-missing"
    }
  }
  if ($Expression -match '\.PSObject\.Properties\[') {
    return [ordered]@{
      ...
      testCoverage = "Test-OracleStage3R13InstalledSoftwarePolicy.ps1"
      disposition = "safe"
    }
  }
  if ($File -ceq "Oracle.Stage3R13InstalledRuntimeConfigurationPolicy.ps1") {
    return [ordered]@{
      ...
      testCoverage = "Test-OracleStage3R13InstalledRuntimeConfigurationPolicy.ps1 and development rehearsal"
      disposition = "fail-closed-if-missing-or-mismatched"
    }
  }
  if ($File -ceq "Oracle.Stage3R13InstalledSoftwarePolicy.ps1") {
    return [ordered]@{
  ...
  if ($File -ceq "Oracle.Stage3R13LifecyclePolicy.ps1") {
    return [ordered]@{
      classification = "structurally-guaranteed-by-construction"
      dataSource = "policy-owned lifecycle state"
      mandatory = $true
      protection = "state shape is created only by New-OracleStage3R13LifecycleState"
      testCoverage = "Test-OracleStage3R13LifecyclePolicy.ps1"
      disposition = "safe"
    }
  }
  if ($File -ceq "Oracle.Stage3R13ObservationPolicy.ps1") {
    return [ordered]@{
      ...
      testCoverage = "Test-OracleStage3R13ObservationPolicy.ps1 and development rehearsal"
      disposition = "fail-closed-if-invalid"
    }
  }
  if ($File -ceq "Oracle.Stage3R13PreflightPolicy.ps1") {
    return [ordered]@{
      ...
      mandatory = $true
      protection = "typed producer or Get-OracleStage3R13RequiredPropertyValue"
      testCoverage = "host-shape fixtures and Founder-QA-01 pre-authority probe"
      disposition = "fail-closed-if-unavailable"
    }
  }
  if ($File -ceq "Oracle.Stage3R13ProcessPolicy.ps1") {
    return [ordered]@{
      ...
      mandatory = $true
      protection = "Get-OracleStage3R13RequiredProcessMember checks every member before use"
      testCoverage = "Test-OracleStage3R13ProcessPolicy.ps1"
      disposition = "fail-closed-if-missing"
    }
  }
  if ($File -ceq "Oracle.Stage3R13WindowPolicy.ps1") {
    return [ordered]@{
      ...
      testCoverage = "Test-OracleStage3R13WindowPolicy.ps1"
      disposition = "fail-closed-if-missing-or-mismatched"
    }
  }
  if ($File -ceq "Oracle.Stage3R13PackageInventoryPolicy.ps1") {
    return [ordered]@{
      ...
      testCoverage = "Test-OracleStage3R13PackageInventoryPolicy.ps1"
      disposition = "safe"
    }
  }
  if ($File -ceq "Oracle.Stage3R13IdentityPolicy.ps1") {
    return [ordered]@{
      classification = "structurally-guaranteed-by-platform-type"
      dataSource = "System.StringComparer"
      mandatory = $true
      protection = "typed .NET ordinal comparison"
      testCoverage = "Test-OracleStage3R13IdentityPolicy.ps1"
      disposition = "safe"
    }
  }
  if ($File -ceq "Oracle.Stage3R13CertificateTrustPolicy.ps1") {
    return [ordered]@{
      ...
      testCoverage = "Test-OracleStage3R13CertificateTrustPolicy.ps1"
      disposition = "fail-closed-if-missing-or-mismatched"
    }
  }
  if ($File -ceq "Oracle.Stage3R13ActivationPolicy.ps1") {
    return [ordered]@{
      ...
      testCoverage = "Test-OracleStage3R13ActivationPolicy.ps1 and elevated integration test"
      disposition = "fail-closed-if-missing-or-unsuccessful"
    }
  }
  if ($File -ceq "Oracle.Stage3R13WindowsExecutablePolicy.ps1") {
    return [ordered]@{
      ...
      testCoverage = "Test-OracleStage3R13WindowsExecutablePolicy.ps1"
      disposition = "fail-closed-if-unavailable"
    }
  }
  if (
    ...
    return [ordered]@{
      ...
      testCoverage = "Test-OracleStage3R13InstalledRuntimeConfigurationPolicy.ps1, verify-preparation.mjs and development rehearsal"
      disposition = "fail-closed-if-missing-or-mismatched"
    }
  }  if ($Expression -match '^\$contract(?:\.|$)') {
    return [ordered]@{
      classification = "mandatory-validated-contract-member"
      dataSource = "Oracle.Stage3R13Contract.json"
      mandatory = $true
...
$report = [ordered]@{
  schemaVersion = "1.0.0"
  contract = "oracle.sprint-30-5.stage-3-r13-optional-member-audit"
