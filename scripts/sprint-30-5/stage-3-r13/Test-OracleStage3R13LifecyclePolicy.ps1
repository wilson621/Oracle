...
$phases = @(Get-OracleStage3R13LifecyclePhases)
$success = New-OracleStage3R13LifecycleState
foreach ($phase in $phases) {
  [void](Move-OracleStage3R13Lifecycle -State $success -Phase $phase)
}
...
try {
  ...
  [void](Move-OracleStage3R13Lifecycle -State $state -Phase $phases[1])
...
try {
  $state = New-OracleStage3R13LifecycleState
  [void](Move-OracleStage3R13Lifecycle -State $state -Phase $phases[0])
  [void](Move-OracleStage3R13Lifecycle -State $state -Phase $phases[0])
...
for ($failureIndex = 0; $failureIndex -lt $phases.Count; $failureIndex++) {
  ...
  for ($index = 0; $index -lt $failureIndex; $index++) {
    [void](Move-OracleStage3R13Lifecycle -State $state -Phase $phases[$index])
  }
  $obligations = Get-OracleStage3R13TeardownObligations `
