...
function Get-OracleStage3R13LifecyclePhases {
  @(
    "authority-consumed",
    "transfer-verified",
    "host-admitted",
...
function New-OracleStage3R13LifecycleState {
...
function Move-OracleStage3R13Lifecycle {
  [CmdletBinding()]
  param(
...
function Assert-OracleStage3R13NextLifecyclePhase {
  [CmdletBinding()]
  param(
    [Parameter(Mandatory = $true)][object]$State,
    [Parameter(Mandatory = $true)][string]$Phase
...
function Get-OracleStage3R13TeardownObligations {
  [CmdletBinding()]
  param(
