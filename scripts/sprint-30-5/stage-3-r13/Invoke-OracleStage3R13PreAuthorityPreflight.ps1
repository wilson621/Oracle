...
if (
  ...
  [string]$manifest.revision -cne "R13" -or
...
$runningScriptRelativePath = "payload/Invoke-OracleStage3R13PreAuthorityPreflight.ps1"
$runningScriptEntry = @($payloadEntries | Where-Object {
  [string]$_.path -ceq $runningScriptRelativePath
})
$contractRelativePath = "payload/Oracle.Stage3R13Contract.json"
...
) { throw "Pre-authority canonical R13 identity differs or is retired." }
if (
...
) { throw "Pre-authority accepted Stage 2 R8 binding differs." }
...
$getCertificateMatches = {
  @(
    @(Get-OracleStage3R13PhysicalCertificateMatches `
      -Thumbprint $contract.stage2.certificateThumbprint)
    @(Get-OracleStage3R13LogicalCertificateViews `
      -Thumbprint $contract.stage2.certificateThumbprint)
  )
}

$result = Get-OracleStage3R13PreAuthorityObservation `
  -Contract $contract `
