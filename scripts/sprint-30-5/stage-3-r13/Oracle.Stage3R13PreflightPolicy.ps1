...
function Get-OracleStage3R13RequiredPropertyValue {
  [CmdletBinding()]
  param(
...
function Assert-OracleStage3R13CommandSurface {
  [CmdletBinding()]
  param()
...
function Get-OracleStage3R13AcceptedPublicCertificate {
  [CmdletBinding()]
  param(
  ...
  $publicCertificatePath = Join-Path $payloadRoot (
    [string]$Contract.package.publicCertificateFileName
  )
  if (
    -not (Test-Path -LiteralPath $publicCertificatePath -PathType Leaf) -or
    (Get-FileHash -LiteralPath $publicCertificatePath -Algorithm SHA256).Hash.ToLowerInvariant() -cne
      [string]$Contract.stage2.publicCertificateSha256
  ) { throw "Accepted public-certificate bytes differ." }
  $payloadCertificate = [Security.Cryptography.X509Certificates.X509Certificate2]::new(
    [IO.File]::ReadAllBytes($publicCertificatePath)
  )
  if (
    $payloadCertificate.Thumbprint -cne $certificate.Thumbprint -or
    $payloadCertificate.Subject -cne $certificate.Subject -or
    $payloadCertificate.HasPrivateKey -or
    [Convert]::ToBase64String($payloadCertificate.RawData) -cne
      [Convert]::ToBase64String($certificate.RawData)
  ) { throw "Detached signer and transferred public certificate differ." }
  if (
...
function Invoke-OracleStage3R13ReadOnlyProcess {
  [CmdletBinding()]
  param(
...
function Assert-OracleStage3R13NoReparseTraversal([string]$Path) {
...
function Get-OracleStage3R13PreAuthorityObservation {
  [CmdletBinding()]
  param(
    [Parameter(Mandatory = $true)][object]$Contract,
    [Parameter(Mandatory = $true)][string]$TransferRoot,
  ...
  if (
    ...
    -not (Test-OracleStage3R13ProcessIsElevated)
  ) {
  ...
  foreach ($path in @($TransferRoot, $EvidenceReturnRoot, $HostContinuityPath)) {
    Assert-OracleStage3R13NoReparseTraversal $path
  }
  if (
  ...
  $recovery = Invoke-OracleStage3R13ReadOnlyProcess `
  ...
  $manufacturer = [string](Get-OracleStage3R13RequiredPropertyValue `
    $computerSystem "Manufacturer" "Win32_ComputerSystem")
  $model = [string](Get-OracleStage3R13RequiredPropertyValue `
  ...
  foreach ($member in @("TpmPresent", "TpmReady")) {
    [void](Get-OracleStage3R13RequiredPropertyValue $tpm $member "Get-Tpm")
  }
  foreach ($member in @("AntivirusEnabled", "RealTimeProtectionEnabled")) {
    [void](Get-OracleStage3R13RequiredPropertyValue $defender $member "Get-MpComputerStatus")
  }
  if (
    -not $secureBoot -or
    -not [bool](Get-OracleStage3R13RequiredPropertyValue $tpm "TpmPresent" "Get-Tpm") -or
    -not [bool](Get-OracleStage3R13RequiredPropertyValue $tpm "TpmReady" "Get-Tpm") -or
    -not [bool](Get-OracleStage3R13RequiredPropertyValue (
      $defender
  ...
  $activation = @(Get-CimInstance SoftwareLicensingProduct -ErrorAction Stop |
    Where-Object {
      $partialKey = Get-OracleStage3R13RequiredPropertyValue `
        $_ "PartialProductKey" "SoftwareLicensingProduct"
      $licenseStatus = Get-OracleStage3R13RequiredPropertyValue `
  ...
  foreach ($name in $runtimeEnvironmentNames) {
    foreach ($target in @(
      ...
      if ($null -ne [Environment]::GetEnvironmentVariable($name, $target)) {
        throw "Ambient installed runtime configuration is present: $name/$target"
      }
    }
  }
  if (
    [string]$Contract.runtimeConfiguration.contract -cne
      "oracle.installed-runtime-configuration" -or
    [int]$Contract.runtimeConfiguration.version -ne 1 -or
    [int]$Contract.runtimeConfiguration.maximumLifetimeSeconds -ne 900 -or
    [string]$Contract.runtimeConfiguration.providerOrigin -cne
      "http://127.0.0.1:54321" -or
    $Contract.runtimeConfiguration.secretsOutsidePackageAndEvidence -ne $true -or
    $Contract.runtimeConfiguration.finalZeroResidueRequired -ne $true
  ) { throw "Installed runtime-configuration contract is invalid." }
  $installedSoftware = @(Get-OracleStage3R13InstalledSoftwareInventory)
  ...
  if ($certificateMatches.Count -ne 0) {
    throw "Governed Stage 2 R8 certificate is already present."
  }
  ...
  $signerCertificate = Get-OracleStage3R13AcceptedPublicCertificate `
    -Contract $Contract -TransferRoot $TransferRoot
  $observedUntrustedSigner = Get-OracleStage3R13RequiredPropertyValue `
    $untrustedSignature "SignerCertificate" "Get-AuthenticodeSignature"
  if (
    [string](Get-OracleStage3R13RequiredPropertyValue (
      $untrustedSignature
  ...
  foreach ($member in @(
    ...
    [void](Get-OracleStage3R13RequiredPropertyValue $continuity $member "host continuity")
  }
  foreach ($member in @("deviceName", "manufacturer", "model")) {
    [void](Get-OracleStage3R13RequiredPropertyValue $continuity.host $member "host continuity host")
  }
  ...
  if (
    [string]$continuity.contract -cne
      "oracle.sprint-30-5.stage-3-r13-host-continuity" -or
  ...
  $developmentTools = @($Contract.host.prohibitedDependencies) |
    ForEach-Object {
  ...
  $repositoryPaths = @($Contract.host.forbiddenRepositoryPaths | ForEach-Object {
    [pscustomobject][ordered]@{
      path = [string]$_
      present = Test-Path -LiteralPath ([string]$_)
    }
  })
  if (@($repositoryPaths | Where-Object { $_.present }).Count -ne 0) {
    throw "Pre-authority host contains the engineering repository."
  }
  ...
  [pscustomobject][ordered]@{
    ...
    security = [ordered]@{
      ...
      tpmPresent = [bool](Get-OracleStage3R13RequiredPropertyValue (
        $tpm
      ) "TpmPresent" "Get-Tpm")
      tpmReady = [bool](Get-OracleStage3R13RequiredPropertyValue (
        $tpm
      ) "TpmReady" "Get-Tpm")
      defenderAntivirusEnabled = [bool](Get-OracleStage3R13RequiredPropertyValue (
        $defender
      ) "AntivirusEnabled" "Get-MpComputerStatus")
      defenderRealTimeProtectionEnabled = [bool](
        Get-OracleStage3R13RequiredPropertyValue (
          $defender
        ) "RealTimeProtectionEnabled" "Get-MpComputerStatus"
      )
    ...
    runtimeConfiguration = [ordered]@{
      contract = [string]$Contract.runtimeConfiguration.contract
      ambientVariableMatches = 0
      providerOrigin = [string]$Contract.runtimeConfiguration.providerOrigin
      maximumLifetimeSeconds =
        [int]$Contract.runtimeConfiguration.maximumLifetimeSeconds
