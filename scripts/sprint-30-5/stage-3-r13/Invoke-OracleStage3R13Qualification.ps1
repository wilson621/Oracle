...
if (
  -not [StringComparer]::OrdinalIgnoreCase.Equals(
    ...
    [IO.Path]::GetFullPath((
      Join-Path $TransferRoot "payload/Invoke-OracleStage3R13Qualification.ps1"
    ))
  )
...
. (Join-Path $scriptRoot "Oracle.Stage3R13IdentityPolicy.ps1")
. (Join-Path $scriptRoot "Oracle.Stage3R13ActivationPolicy.ps1")
. (Join-Path $scriptRoot "Oracle.Stage3R13CertificateTrustPolicy.ps1")
. (Join-Path $scriptRoot "Oracle.Stage3R13PackageInventoryPolicy.ps1")
. (Join-Path $scriptRoot "Oracle.Stage3R13InstalledSoftwarePolicy.ps1")
. (Join-Path $scriptRoot "Oracle.Stage3R13InstalledRuntimeConfigurationPolicy.ps1")
. (Join-Path $scriptRoot "Oracle.Stage3R13LifecyclePolicy.ps1")
. (Join-Path $scriptRoot "Oracle.Stage3R13ObservationPolicy.ps1")
. (Join-Path $scriptRoot "Oracle.Stage3R13PreflightPolicy.ps1")
. (Join-Path $scriptRoot "Oracle.Stage3R13ProcessPolicy.ps1")
. (Join-Path $scriptRoot "Oracle.Stage3R13TransferInventoryPolicy.ps1")
. (Join-Path $scriptRoot "Oracle.Stage3R13WindowPolicy.ps1")
. (Join-Path $scriptRoot "Oracle.Stage3R13WindowsExecutablePolicy.ps1")
...
if ([string]$contract.authority.execution -cne "founder-authorised") {
  throw "Stage 3 R13 qualification execution is not authorised by the contract."
}
$expectedToken = "FOUNDER-AUTHORISED-STAGE3-R13-EXECUTION"
...
$manifestPath = Join-Path $TransferRoot "Oracle.Stage3R13TransferManifest.json"
$custodyPath = Join-Path $TransferRoot "Oracle.Stage3R13TransferCustody.json"
...
$identityPolicyPath = Join-Path $payloadRoot "Oracle.Stage3R13IdentityPolicy.ps1"
$activationPolicyPath = Join-Path $payloadRoot "Oracle.Stage3R13ActivationPolicy.ps1"
...
$lifecyclePolicyPath = Join-Path $payloadRoot "Oracle.Stage3R13LifecyclePolicy.ps1"
$observationPolicyPath = Join-Path $payloadRoot (
  "Oracle.Stage3R13ObservationPolicy.ps1"
)
$preflightPolicyPath = Join-Path $payloadRoot "Oracle.Stage3R13PreflightPolicy.ps1"
$processPolicyPath = Join-Path $payloadRoot "Oracle.Stage3R13ProcessPolicy.ps1"
$windowPolicyPath = Join-Path $payloadRoot "Oracle.Stage3R13WindowPolicy.ps1"
...
$lifecycleState = New-OracleStage3R13LifecycleState
...
function Get-InstalledSoftwareInventory {
  @(Get-OracleStage3R13InstalledSoftwareInventory)
}
...
function New-GovernedRuntimeConfiguration {
  ...
  $sessionSecure = $null
  try {
    $anonValue = New-OracleStage3R13CryptographicValue
    $serviceValue = New-OracleStage3R13CryptographicValue
    $sessionValue = New-OracleStage3R13CryptographicValue
...
function Write-Lifecycle([string]$Phase, $Details) {
  ...
  Assert-OracleStage3R13NextLifecyclePhase -State $lifecycleState -Phase $Phase
  Write-CreateOnlyJson (
    Join-Path $lifecycleRoot ("{0:D2}-{1}.json" -f ($phaseIndex + 1), $Phase)
  ) ([ordered]@{
    contract = "oracle.sprint-30-5.stage-3-r13-lifecycle"
  ...
  [void](Move-OracleStage3R13Lifecycle -State $lifecycleState -Phase $Phase)
}
...
function Invoke-GovernedProcess(
  ...
  Invoke-OracleStage3R13GovernedProcess `
    -Name $Name `
...
function Get-ExactCertificateMatches {
  @(Get-OracleStage3R13LogicalCertificateViews -Thumbprint $thumbprint)
}

function Get-PhysicalExactCertificateMatches {
  @(Get-OracleStage3R13PhysicalCertificateMatches -Thumbprint $thumbprint)
}
...
function Remove-ExactMachineTrust {
  ...
  [void](Invoke-GovernedProcess "exact-machine-trust-remove" $certutil @(
    Get-OracleStage3R13TrustRemovalArguments -Thumbprint $thumbprint
  ))
...
function Stop-ExactPackageProcesses {
  ...
  foreach ($process in @(Get-Process -Name "Oracle" -ErrorAction SilentlyContinue)) {
    $resolution = Resolve-OracleStage3R13TeardownProcessOwnership `
    ...
    try {
      ...
      if (-not (Test-OracleStage3R13ProcessExists -ProcessId $process.Id)) {
        $teardownProcessEvents.Add([ordered]@{
          recordedAtUtc = [DateTime]::UtcNow.ToString("o")
          processId = [int]$process.Id
...
function Assert-IdentityAndTransfer {
  if ([string]$contract.authority.execution -cne "founder-authorised") {
    throw "Stage 3 R13 qualification execution is not authorised."
  }
  if ($FounderAuthorityToken -cne $expectedToken) {
    throw "Separate Founder Stage 3 execution authority is required."
  }
  $attempt = [regex]::Match($AttemptId, '^stage3-r13-(\d{8}T\d{9}Z)-([0-9a-f]{8})$')
  $authority = [regex]::Match($AuthorityId, '^authority-stage3-r13-(\d{8}T\d{9}Z)-([0-9a-f]{8})$')
  ...
  $expectedFounderGrantId = $AttemptId -replace (
    '^stage3-r13-', 'founder-stage3-r13-grant-'
  )
  ...
  if (
    [string]$contract.programmeIdentity -cne
      "Sprint 30.5 Stage 3 Requalification R13" -or
    [string]$contract.revision -cne "R13" -or
    [string]$manifest.contract -cne
      "oracle.sprint-30-5.stage-3-r13-transfer" -or
    ...
    $manifest.transferId -cnotmatch
      '^transfer-stage3-r13-\d{8}T\d{9}Z-[0-9a-f]{8}$' -or
  ...
  if (
    $manifest.acceptedStage2.revision -cne $contract.stage2.revision -or
    ...
    $manifest.acceptedStage2.preparationId -cne $contract.stage2.preparationId -or
    $manifest.acceptedStage2.packageInventoryAttemptId -cne
      $contract.stage2.packageInventoryAttemptId -or
    $manifest.acceptedStage2.packageInventoryAuthorityId -cne
      $contract.stage2.packageInventoryAuthorityId -or
    $manifest.acceptedStage2.candidateCommit -cne $contract.stage2.candidateCommit -or
    ...
    $manifest.acceptedStage2.qualificationOutcomeSha256 -cne
      $contract.stage2.qualificationOutcomeSha256 -or
    $manifest.acceptedStage2.publicCertificateSha256 -cne
      $contract.stage2.publicCertificateSha256 -or
    ...
    $manifest.acceptedStage2.provenanceSha256 -cne $contract.stage2.provenanceSha256 -or
    $manifest.acceptedStage2.certificateThumbprint -cne
      $contract.stage2.certificateThumbprint -or
    $manifest.acceptedStage2.archiveRequired -ne $false -or
    $manifest.acceptedStage2.archiveProduced -ne $false -or
    (Get-Sha256 $msixPath) -cne $contract.stage2.msixSha256
  ) { throw "Transfer is not bound to accepted Stage 2 R8." }
  $transferPayloadInventory = Assert-OracleStage3R13TransferPayloadInventory `
    -Manifest $manifest `
    -Contract $contract `
    -TransferRoot $TransferRoot `
    -GetSha256 ${function:Get-Sha256}
  $runningHarnessEntry = @($manifest.payload | Where-Object {
    [string]$_.path -ceq "payload/Invoke-OracleStage3R13Qualification.ps1"
  })
  $runningContractEntry = @($manifest.payload | Where-Object {
    [string]$_.path -ceq "payload/Oracle.Stage3R13Contract.json"
  })
  $runningActivationPolicyEntry = @($manifest.payload | Where-Object {
    [string]$_.path -ceq "payload/Oracle.Stage3R13ActivationPolicy.ps1"
  })
  $runningCertificateTrustPolicyEntry = @($manifest.payload | Where-Object {
    [string]$_.path -ceq "payload/Oracle.Stage3R13CertificateTrustPolicy.ps1"
  })
  $runningIdentityPolicyEntry = @($manifest.payload | Where-Object {
    [string]$_.path -ceq "payload/Oracle.Stage3R13IdentityPolicy.ps1"
  })
  $runningPackageInventoryPolicyEntry = @($manifest.payload | Where-Object {
    [string]$_.path -ceq "payload/Oracle.Stage3R13PackageInventoryPolicy.ps1"
  })
  $runningInstalledSoftwarePolicyEntry = @($manifest.payload | Where-Object {
    [string]$_.path -ceq "payload/Oracle.Stage3R13InstalledSoftwarePolicy.ps1"
  })
  $runningInstalledRuntimeConfigurationPolicyEntry = @($manifest.payload | Where-Object {
    [string]$_.path -ceq
      "payload/Oracle.Stage3R13InstalledRuntimeConfigurationPolicy.ps1"
  })
  $runningLifecyclePolicyEntry = @($manifest.payload | Where-Object {
    [string]$_.path -ceq "payload/Oracle.Stage3R13LifecyclePolicy.ps1"
  })
  $runningObservationPolicyEntry = @($manifest.payload | Where-Object {
    [string]$_.path -ceq "payload/Oracle.Stage3R13ObservationPolicy.ps1"
  })
  $runningPreflightPolicyEntry = @($manifest.payload | Where-Object {
    [string]$_.path -ceq "payload/Oracle.Stage3R13PreflightPolicy.ps1"
  })
  $runningProcessPolicyEntry = @($manifest.payload | Where-Object {
    [string]$_.path -ceq "payload/Oracle.Stage3R13ProcessPolicy.ps1"
  })
  $runningWindowPolicyEntry = @($manifest.payload | Where-Object {
    [string]$_.path -ceq "payload/Oracle.Stage3R13WindowPolicy.ps1"
  })
  $runningWindowsExecutablePolicyEntry = @($manifest.payload | Where-Object {
    [string]$_.path -ceq "payload/Oracle.Stage3R13WindowsExecutablePolicy.ps1"
  })
  if (
    ...
    (Get-Sha256 (Join-Path $scriptRoot "Oracle.Stage3R13Contract.json")) -cne
    ...
    (Get-Sha256 (Join-Path $scriptRoot "Oracle.Stage3R13InstalledRuntimeConfigurationPolicy.ps1")) -cne
  ...
  $sbomPath = Join-Path $payloadRoot "oracle-0.1.6.cdx.json"
  $provenancePath = Join-Path $payloadRoot "oracle-0.1.6.provenance.json"
  ...
  $optionalMemberAuditPath = Join-Path $payloadRoot (
    "Oracle.Stage3R13OptionalMemberAudit.json"
  )
  $phaseAuditPath = Join-Path $payloadRoot "Oracle.Stage3R13PhaseAudit.json"
  ...
  if (
    ...
    @($auditedPhases).Count -ne
      @(Get-OracleStage3R13LifecyclePhases).Count
  ...
  ) { throw "Accepted R8 signature or teardown evidence differs." }
}
...
function Assert-PackageContent {
  ...
  if (
    [string]$packageInventory.contract -cne
      "oracle.sprint-30-5.stage-2-requalification-r8-package-content-inventory" -or
    [string]$packageInventory.attemptId -cne $contract.stage2.packageInventoryAttemptId -or
    [string]$packageInventory.authorityId -cne $contract.stage2.packageInventoryAuthorityId -or
  ...
  foreach ($entry in @($packageInventory.entries)) {
    $canonicalExpectedPath =
      ConvertTo-OracleStage3R13CanonicalPackagePath ([string]$entry.path)
    if (
      $canonicalExpectedPath -cne [string]$entry.path -or
      (Test-OracleStage3R13ReservedPackageMetadata $canonicalExpectedPath) -or
  ...
  $zipEntries = @(Get-OracleStage3R13PackageZipInventory $msixPath)
...
function Invoke-NativeObservation([string]$Label) {
  ...
  while ([DateTime]::UtcNow -lt $deadline -and $null -eq $selected) {
    ...
    $windows = @(
      ConvertFrom-OracleStage3R13WindowDiscoveryJson -Json ([string]$result.stdout)
    )
    $candidates = @($windows | Where-Object {
      Test-OracleStage3R13QualifyingWindow $_
    })
  ...
  $stability = Invoke-OracleStage3R13CompleteStableObservation `
...
try {
  ...
  $preflight = Get-OracleStage3R13PreAuthorityObservation `
    -Contract $contract `
    -TransferRoot $TransferRoot `
  ...
  Write-CreateOnlyJson $authorityPath ([ordered]@{
    contract = "oracle.sprint-30-5.stage-3-r13-authority"
    programmeIdentity = [string]$contract.programmeIdentity
    revision = [string]$contract.revision
    founderGrantId = $FounderGrantId
    authorityId = $AuthorityId
  ...
  $certificate = Get-OracleStage3R13AcceptedPublicCertificate `
  ...
  ) { throw "MSIX signer certificate validity differs from accepted R8 evidence." }
  ...
  [void](Invoke-GovernedProcess "exact-machine-trust-import" $certutil @(
    Get-OracleStage3R13TrustImportArguments -CertificatePath $certificatePath
  ))
  $physicalMatches = @(Get-PhysicalExactCertificateMatches)
  $logicalViews = @(Get-ExactCertificateMatches)
  ...
  $initialActivation = Invoke-OracleStage3R13ApplicationActivation `
    -AppUserModelId $appUserModelId `
    -Arguments $initialActivationArguments
  Write-CreateOnlyJson (
    Join-Path $evidenceRoot "initial-activation.json"
  ) $initialActivation
  Assert-OracleStage3R13ApplicationActivationSucceeded -Result $initialActivation
  ...
  $repairActivation = Invoke-OracleStage3R13ApplicationActivation `
    -AppUserModelId $appUserModelId `
    -Arguments $repairActivationArguments
  Write-CreateOnlyJson (
    Join-Path $evidenceRoot "repair-activation.json"
  ) $repairActivation
  Assert-OracleStage3R13ApplicationActivationSucceeded -Result $repairActivation
  ...
  Write-CreateOnlyJson $evidenceManifestPath ([ordered]@{
    contract = "oracle.sprint-30-5.stage-3-r13-evidence-manifest"
    programmeIdentity = [string]$contract.programmeIdentity
    revision = [string]$contract.revision
    founderGrantId = $FounderGrantId
    authorityId = $AuthorityId
  ...
  Write-CreateOnlyJson "$archivePath.manifest.json" ([ordered]@{
    contract = "oracle.sprint-30-5.stage-3-r13-archive-manifest"
    programmeIdentity = [string]$contract.programmeIdentity
    revision = [string]$contract.revision
    founderGrantId = $FounderGrantId
    authorityId = $AuthorityId
