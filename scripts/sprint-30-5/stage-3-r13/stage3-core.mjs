...
const attemptPattern = new RegExp(`^stage3-r13-${compactTime}-${suffix}$`, "u");
const authorityPattern = new RegExp(
  `^authority-stage3-r13-${compactTime}-${suffix}$`,
  "u"
);
const transferPattern = new RegExp(`^transfer-stage3-r13-${compactTime}-${suffix}$`, "u");
export const canonicalProgrammeIdentity =
  "Sprint 30.5 Stage 3 Requalification R13";
...
function validateScopedId(value, timestampUtc, pattern, label) {
  ...
  if (!match) {
    throw new Error(`${label} has an invalid Stage 3 R13 format.`);
  }
...
export function validateProgrammeIdentity(value) {
  if (value !== canonicalProgrammeIdentity) {
    throw new Error("Stage 3 R13 programme identity differs.");
  }
  return value;
}
...
export function validateTransferIdentity({ transferId, timestampUtc }) {
  ...
  if (contract.rejectedTransfers.some((entry) => entry.transferId === transferId)) {
    throw new Error("Rejected Stage 3 R13 transfer identity cannot be reused.");
  }
}

export function validateTransferConstructionAuthority(value) {
  if (contract.authority.transfer !== "founder-authorised") {
    throw new Error("Stage 3 R13 transfer construction is not authorised.");
  }
...
export function validateAcceptedBindings(candidate) {
  ...
  const identityFields = [
    ...
    "authorityId",
    "preparationId",
    "packageInventoryAttemptId",
    "packageInventoryAuthorityId",
    "candidateCommit",
    "candidateTree",
    "harnessCommit",
    "harnessTree",
  ...
  const hashFields = [
    ...
    "qualificationOutcomeSha256",
    "authoritySha256",
    "hostContinuitySha256",
    "completionSha256",
    "engineeringFreezeSha256",
    "msixSha256",
    ...
    "runtimeConfigurationBuildSecrecySha256",
  ];
  for (const field of [...identityFields, ...hashFields]) {
    if (candidate[field] !== expected[field]) {
      throw new Error(`Accepted Stage 2 R8 binding mismatch: ${field}`);
    }
  }
  for (const field of hashFields) assert.match(candidate[field], shaPattern);
  assert.equal(candidate.archiveRequired, false);
  assert.equal(candidate.archiveProduced, false);
}
