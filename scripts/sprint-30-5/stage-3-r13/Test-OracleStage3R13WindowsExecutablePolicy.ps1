...
$root = Join-Path $env:TEMP (
  "oracle-stage3-r13-windows-executable-" + [Guid]::NewGuid().ToString("N")
)
...
try {
  ...
  $certutil = Get-OracleStage3R13WindowsExecutablePath `
    -Name "certutil.exe" `
    -SystemDirectory $systemDirectory
  $reagentc = Get-OracleStage3R13WindowsExecutablePath `
    -Name "reagentc.exe" `
    -SystemDirectory $systemDirectory
