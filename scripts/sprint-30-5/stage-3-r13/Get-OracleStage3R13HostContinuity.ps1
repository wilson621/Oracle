...
$recovery = Invoke-ReadOnlyProcess (
  Get-OracleStage3R13WindowsExecutablePath -Name "reagentc.exe"
) @("/info")
...
$physicalCertificateMatches = @(
  Get-OracleStage3R13PhysicalCertificateMatches `
    -Thumbprint $contract.stage2.certificateThumbprint
)
$logicalCertificateViews = @(
  Get-OracleStage3R13LogicalCertificateViews `
    -Thumbprint $contract.stage2.certificateThumbprint
)
$certificateMatches = @(
  @($physicalCertificateMatches | ForEach-Object {
    "physical:$($_.Location)\$($_.Store)"
  })
  @($logicalCertificateViews | ForEach-Object {
    "logical:$($_.Location)\$($_.Store)"
  })
)
$issues = @()
if (
...
$record = [ordered]@{
  schemaVersion = "1.0.0"
  contract = "oracle.sprint-30-5.stage-3-r13-host-continuity"
  ...
  repositoryPresent = @($repositoryPaths | Where-Object { $_.present }).Count -ne 0
