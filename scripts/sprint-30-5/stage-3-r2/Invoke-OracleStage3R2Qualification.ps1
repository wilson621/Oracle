...
throw "STAGE3_R2_ENTRY_POINT_RETIRED: historical qualification execution is permanently disabled."
...
$contract = Get-Content -LiteralPath (
  Join-Path $scriptRoot "Oracle.Stage3R2Contract.json"
) -Raw | ConvertFrom-Json
. (Join-Path $scriptRoot "Oracle.Stage3R2IdentityPolicy.ps1")
$expectedToken = "FOUNDER-AUTHORISED-STAGE3-R2-EXECUTION"
...
$manifestPath = Join-Path $TransferRoot "Oracle.Stage3R2TransferManifest.json"
$custodyPath = Join-Path $TransferRoot "Oracle.Stage3R2TransferCustody.json"
...
$identityPolicyPath = Join-Path $payloadRoot "Oracle.Stage3R2IdentityPolicy.ps1"
...
function Write-Lifecycle([string]$Phase, $Details) {
  ...
  Write-CreateOnlyJson (
    ...
    contract = "oracle.sprint-30-5.stage-3-r2-lifecycle"
...
function Assert-IdentityAndTransfer {
  ...
  $attempt = [regex]::Match($AttemptId, '^stage3-r2-(\d{8}T\d{9}Z)-([0-9a-f]{8})$')
  $authority = [regex]::Match($AuthorityId, '^authority-stage3-r2-(\d{8}T\d{9}Z)-([0-9a-f]{8})$')
  ...
  $expectedTransferRootEntries = @(
    "Oracle.Stage3R2TransferCustody.json",
    "Oracle.Stage3R2TransferCustody.json.sha256.txt",
    "Oracle.Stage3R2TransferManifest.json",
    "Oracle.Stage3R2TransferManifest.json.sha256.txt",
    "payload"
  ) | Sort-Object
  ...
  if ($manifest.transferId -cnotmatch '^transfer-stage3-r2-\d{8}T\d{9}Z-[0-9a-f]{8}$') {
  ...
  if (
    ...
    [string]$custody.contract -cne
      "oracle.sprint-30-5.stage-3-r2-transfer-custody" -or
  ...
  $expectedPayload = @(
    "Get-OracleStage3R2HostContinuity.ps1",
    "Invoke-OracleStage3R2Qualification.ps1",
    ...
    "Oracle.Stage3R2Contract.json",
    "Oracle.Stage3R2IdentityPolicy.ps1",
  ...
  $runningHarnessEntry = @($manifest.payload | Where-Object {
    [string]$_.path -ceq "payload/Invoke-OracleStage3R2Qualification.ps1"
  })
  $runningContractEntry = @($manifest.payload | Where-Object {
    [string]$_.path -ceq "payload/Oracle.Stage3R2Contract.json"
  })
  $runningIdentityPolicyEntry = @($manifest.payload | Where-Object {
    [string]$_.path -ceq "payload/Oracle.Stage3R2IdentityPolicy.ps1"
  })
  if (
    ...
    (Get-Sha256 (Join-Path $scriptRoot "Oracle.Stage3R2Contract.json")) -cne
      [string]$runningContractEntry[0].sha256 -or
    (Get-Sha256 (
      Join-Path $scriptRoot "Oracle.Stage3R2IdentityPolicy.ps1"
    )) -cne [string]$runningIdentityPolicyEntry[0].sha256
  ) { throw "Executing harness, contract or identity-policy bytes differ from the transfer." }
...
try {
  ...
  if (
    [string]$hostContinuity.contract -cne
      "oracle.sprint-30-5.stage-3-r2-host-continuity" -or
  ...
  Write-CreateOnlyJson $authorityPath ([ordered]@{
    contract = "oracle.sprint-30-5.stage-3-r2-authority"
    authorityId = $AuthorityId
    attemptId = $AttemptId
  ...
  Write-CreateOnlyJson $evidenceManifestPath ([ordered]@{
    contract = "oracle.sprint-30-5.stage-3-r2-evidence-manifest"
    authorityId = $AuthorityId
    attemptId = $AttemptId
  ...
  Write-CreateOnlyJson "$archivePath.manifest.json" ([ordered]@{
    contract = "oracle.sprint-30-5.stage-3-r2-archive-manifest"
    authorityId = $AuthorityId
    attemptId = $AttemptId
