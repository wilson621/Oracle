...
validateExecutionIdentity({
  authorityId: "authority-stage3-r2-20260728T223045123Z-a1b2c3d4",
  attemptId: "stage3-r2-20260728T223045123Z-a1b2c3d4",
  timestampUtc,
});
for (const invalid of [
  {
    authorityId: "authority-stage3-r2-20260728T223045123Z-a1b2c3d4",
    attemptId: "stage3-r2-20260728T223045123Z-ffffffff",
    timestampUtc,
  },
  {
    authorityId: "authority-stage3-r2-20260728T223045123Z-a1b2c3d4",
...
validateTransferIdentity({
  transferId: "transfer-stage3-r2-20260728T223045123Z-a1b2c3d4",
  timestampUtc,
});
...
try {
  ...
  const returnedArchive = join(temporaryRoot, "stage3-r2-fixture.zip");
  ...
  writeFileSync(
    returnedSidecar,
    `${returnedArchiveHash}  stage3-r2-fixture.zip\n`
  );
  writeFileSync(
    returnedManifest,
    `${JSON.stringify({
  ...
  const parent = join(repositoryRoot, ".tmp-stage3-r2-validation");
  if (existsSync(parent)) rmSync(parent, { recursive: true, force: true });
}

const harness = readFileSync(
  join(import.meta.dirname, "Invoke-OracleStage3R2Qualification.ps1"),
  "utf8"
);
...
const continuityCollector = readFileSync(
  join(import.meta.dirname, "Get-OracleStage3R2HostContinuity.ps1"),
  "utf8"
);
const identityPolicy = readFileSync(
  join(import.meta.dirname, "Oracle.Stage3R2IdentityPolicy.ps1"),
  "utf8"
);
const preExecutionGate = readFileSync(
  join(
    ...
    "docs",
    "sprints",
    "SPRINT_30_5_STAGE_3_R2_PRE_EXECUTION_GATE.md"
  ),
  "utf8"
);
...
assert.match(transferBuilder, /Get-OracleStage3R2HostContinuity\.ps1/u);
assert.match(transferBuilder, /Oracle\.Stage3R2IdentityPolicy\.ps1/u);
assert.match(transferBuilder, /Oracle\.Stage3R2TransferCustody\.json/u);
...
for (const required of [
  "FOUNDER-AUTHORISED-STAGE3-R2-EXECUTION",
  ...
  "oracle.sprint-30-5.stage-3-r2-transfer-custody",
  "contract.transferMedium.hardwareSerial",
]) {
  assert.ok(harness.includes(required), `Harness contract missing: ${required}`);
}
assert.equal(
  packageScripts["sprint-30-5:stage-3:r2:validate"],
  "node scripts/sprint-30-5/stage-3-r2/verify-preparation.mjs"
);
assert.equal(
  packageScripts["sprint-30-5:stage-3:r2:prepare-transfer"],
  "node scripts/sprint-30-5/stage-3-r2/prepare-transfer.mjs"
);
assert.equal(
  packageScripts["sprint-30-5:stage-3:r2:verify-return"],
  "node scripts/sprint-30-5/stage-3-r2/verify-return.mjs"
);
assert.equal(packageScripts["sprint-30-5:stage-3:r2:execute"], undefined);
...
for (const required of [
  "Stage 3 Qualification R2 Pre-Execution Gate",
  "Execution:** Blocked and unauthorised",
  "ExpectedHarnessCommit",
  "ExpectedTransferManifestSha256",
  "ExpectedTransferCustodySha256",
  "ExpectedHostContinuitySha256",
  "FOUNDER-AUTHORISED-STAGE3-R2-TRANSFER",
  "FOUNDER-AUTHORISED-STAGE3-R2-EXECUTION",
...
const identityPolicyFixture = spawnSync(
  ...
  [
    ...
    "Bypass",
    "-File",
    join(import.meta.dirname, "Test-OracleStage3R2IdentityPolicy.ps1"),
  ],
...
for (const required of [
  "oracle.sprint-30-5.stage-3-r2-host-continuity",
...
for (const text of governance) {
  assert.match(text, /Stage 3\s+(?:Qualification\s+)?R2/iu);
...
console.log("Stage 3 R2 preparation validation passed.");
