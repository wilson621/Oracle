...
function assertHistoricalEntryPointRetired() {
  throw new Error(
    "STAGE3_R2_ENTRY_POINT_RETIRED: historical transfer construction is permanently disabled."
  );
}
...
if (argumentsMap.get("founder-authority") !== "FOUNDER-AUTHORISED-STAGE3-R2-TRANSFER") {
...
const sources = [
  ...
  join(import.meta.dirname, "Oracle.Stage3R2Contract.json"),
...
const manifestPath = join(transferRoot, "Oracle.Stage3R2TransferManifest.json");
writeJsonAtomicCreateOnly(manifestPath, {
  schemaVersion: "1.0.0",
  contract: "oracle.sprint-30-5.stage-3-r2-transfer",
...
const custodyPath = join(transferRoot, "Oracle.Stage3R2TransferCustody.json");
writeJsonAtomicCreateOnly(custodyPath, {
  schemaVersion: "1.0.0",
  contract: "oracle.sprint-30-5.stage-3-r2-transfer-custody",
  transferId,
  recordedAtUtc: timestampUtc,
  operator: "Codex",
  founder: "Lee Wilson",
  authority: "Founder-authorised Stage 3 Qualification R2 corrective preparation",
  purpose: "Create-only corrective Stage 3 R2 transfer; R1 remains immutable",
  sourceRepository: {
    branch,
    harnessCommit,
    harnessTree,
