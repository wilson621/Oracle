...
export const FOUNDER_EXECUTION_AUTHORITY =
  "FOUNDER-AUTHORISED-STAGE-2-R5-SINGLE-ATTEMPT";
...
export function claimSingleAttemptAuthority({
  ...
  const attemptRoot = assertR5ArtifactPath(outputRoot, attemptId);
  ...
  writeJsonAtomicCreateOnly(recordPath, {
    schemaVersion: "1.0.0",
    contract:
      "oracle.sprint-30-5.stage-2-requalification-r5-single-attempt-authority",
...
export function assertSingleAttemptAuthorityAvailable({
  ...
  if (
    ...
    throw new Error("R5 authority-consumption state is invalid.");
  }
  ...
  if (existingAttemptIds.some((value) => equals(value, attemptId))) {
    throw new Error("The immutable R5 attempt identity already exists.");
  }
}
...
export function createAttemptSubdirectory(attemptRoot, relativePath) {
  ...
  assertR5ArtifactPath(root, root.split(sep).at(-1));
  if (
  ...
  return Object.freeze({
    transition(phase, details = {}) {
      ...
      const record = {
        schemaVersion: "1.0.0",
        contract:
          "oracle.sprint-30-5.stage-2-requalification-r5-lifecycle-event",
        ...identity,
    ...
    fail(error, details = {}) {
      ...
      const failure = {
        schemaVersion: "1.0.0",
        contract:
          "oracle.sprint-30-5.stage-2-requalification-r5-lifecycle-failure",
        ...identity,
