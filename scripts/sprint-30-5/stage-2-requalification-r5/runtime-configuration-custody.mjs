...
const BUILD_CANARIES = Object.freeze({
  ORACLE_SUPABASE_URL: "https://r5-build-canary.invalid",
  ORACLE_SUPABASE_ANON_KEY: "oracle-r5-anon-canary-not-a-secret",
  ORACLE_SUPABASE_SERVICE_ROLE_KEY: "oracle-r5-service-canary-not-a-secret",
  ORACLE_WEB_SESSION_SECRET: "oracle-r5-session-canary-not-a-secret-32-bytes",
  NEXT_PUBLIC_SUPABASE_URL: "https://r5-build-canary.invalid",
  NEXT_PUBLIC_SUPABASE_ANON_KEY: "oracle-r5-anon-canary-not-a-secret",
});
...
export function assertNoAmbientRuntimeConfiguration(root, environment = process.env) {
  ...
  if (populated.length !== 0) {
    throw new Error(
      `Ambient runtime configuration is prohibited during R5 qualification: ${populated.join(", ")}`
    );
  }
  ...
  if (presentFiles.length !== 0) {
    throw new Error(
      `Ambient environment files are prohibited during R5 qualification: ${presentFiles.join(", ")}`
    );
  }
}
