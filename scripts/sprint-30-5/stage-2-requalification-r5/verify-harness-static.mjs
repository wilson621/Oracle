...
const fixtureAttemptId = "r5-20260728T131415678Z-deadbeef";
...
const fixtureBinding = {
  ...
  wrapperProtocol: "oracle-stage2-r5-governed-wrapper-v1",
...
assert.deepEqual(
  validateGovernedWrapperInvocation({
    observed: "oracle-stage2-r5-governed-wrapper-v1:4242",
    parentProcessId: 4242,
  }),
  {
    protocol: "oracle-stage2-r5-governed-wrapper-v1",
    parentProcessId: 4242,
  }
);
assert.throws(
...
assert.throws(
  () =>
    validateGovernedWrapperInvocation({
      observed: "oracle-stage2-r5-governed-wrapper-v1:4243",
      parentProcessId: 4242,
    }),
  /must be invoked by the governed/
);
assert.throws(
  () =>
    validateGovernedWrapperInvocation({
      observed: "oracle-stage2-r5-governed-wrapper-v1:0",
      parentProcessId: 0,
    }),
...
assert.throws(
  () =>
    validateAttemptIdentity({
      attemptId: "r5-20260728T131415678Z-00000000",
      timestampUtc: fixtureTimestamp,
    }),
...
const expectedHarnessFiles = [
  "Oracle.Stage2RequalificationR5Contract.json",
  "execute-attempt.mjs",
  "execution-core.mjs",
...
].map((name) => `scripts/sprint-30-5/stage-2-requalification-r5/${name}`).sort();
assert.deepEqual(
...
const attemptCreationRecord = createAttemptRecord({
  ...
  attemptId: fixtureAttemptId,
  timestampUtc: fixtureTimestamp,
  outputRoot: join(".artifacts", "sprint-30-5", "stage-2-requalification-r5", fixtureAttemptId),
});
...
const r5ValidatorSource = readFileSync(import.meta.filename, "utf8");
...
const r5FixtureLabels = new Set(
  [...r5ValidatorSource.matchAll(fixtureLabelPattern)].map((match) => match[1])
);
assert.ok(r2FixtureLabels.length > 0, "R2 regression labels were not discovered.");
for (const label of r2FixtureLabels) {
  assert.ok(r5FixtureLabels.has(label), `R5 lost inherited R2 fixture: ${label}`);
}
assert.throws(
...
assert.equal(
  isSameOrDescendant(
    join(
      ".artifacts",
      "sprint-30-5",
      "stage-2-requalification-r5",
      fixtureAttemptId
    ),
    join(".artifacts", "sprint-30-5", "stage-2")
  ),
  false,
  "Segment-aware protection must distinguish the R5 and historical roots."
);
...
assert.doesNotThrow(() => assertR5ArtifactPath(governedRoot, fixtureAttemptId));
assert.doesNotThrow(() =>
  assertNoReparseTraversal(
    join(".artifacts", "sprint-30-5", "stage-2-requalification-r5")
  )
);
assert.throws(
  () =>
...
assert.throws(
  () => assertR5ArtifactPath(`${governedRoot}-other`, fixtureAttemptId),
...
for (const [base, prefix] of [
  ...
  for (const entry of readdirSync(base, { withFileTypes: true })) {
    if (
      ...
      entry.name !== "stage-2-requalification-r5"
    ) {
...
const temporaryRoot = mkdtempSync(join(os.tmpdir(), "oracle-stage2-r5-static-"));
try {
...
assert.doesNotThrow(() =>
  validateFinalIdentity({
    ...certificateBound,
    evidence: {
      manifest: "Oracle.Stage2RequalificationR5EvidenceManifest.json",
...
assert.match(
  cleanupScript,
  /exact generated R5 certificate thumbprint was not found/u
);
assert.match(
  cleanupScript,
...
assert.match(
  cleanupScript,
  /ORACLE_R5_ROOT_REMOVAL_PROCESS=[\s\S]*ConvertTo-Json -Compress -Depth 5/u
);
assert.match(
  cleanupScript,
...
const teardownExpectedSubject =
  "CN=Oracle Stage 2 Requalification R5 Local Test Signing - NOT PRODUCTION";
const syntheticMy = {
...
const r5PackageEntryPoints = Object.keys(packageJson.scripts).filter((name) =>
  name.startsWith("sprint-30-5:stage-2:r5:")
);
assert.deepEqual(
  r5PackageEntryPoints,
  [],
  "R5 must not modify the candidate package.json or expose independently runnable phases."
);
assert.equal(
...
assert.ok(
  ...
  "R5 must not expose a standalone attempt-preparation entry point."
);
...
assert.throws(
  () =>
    validateAuthorityIdentity({
      authorityId: "authority-r5-other",
      attemptId: fixtureAttemptId,
    }),
  /must exactly equal/
);
const previousAttemptId = "r5-20260728T120000000Z-01234567";
...
assert.match(executorSource, /stage-2-requalification-r5-failure-outcome\.json/u);
...
assert.ok(
  ...
  "R5 must check ambient configuration pre-authority, pre-build and post-build."
);
...
assert.match(executorSource, /Migration 011 does not match the fixed R5 candidate binding/u);
assert.match(executorSource, /Migration 012 does not match the fixed R5 candidate binding/u);
assert.match(
  readFileSync(join(import.meta.dirname, "execution-core.mjs"), "utf8"),
  /FOUNDER-AUTHORISED-STAGE-2-R5-SINGLE-ATTEMPT/u
);
assert.match(
  executorSource,
...
delete directInvocationEnvironment.ORACLE_STAGE2_R5_GOVERNED_WRAPPER;
...
assert.equal(contract.authority.stage3, "closed-not-authorised-by-r5");
assert.equal(contract.authority.stage4, "closed-not-authorised-by-r5");
assert.throws(
  () => assertRequiredTool("oracle-r5-tool-that-does-not-exist.exe"),
