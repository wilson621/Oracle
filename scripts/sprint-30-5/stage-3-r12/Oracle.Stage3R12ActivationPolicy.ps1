...
function Assert-OracleStage3R12ApplicationActivationContract {
  [CmdletBinding()]
...
function Initialize-OracleStage3R12ActivationApi {
  if ($null -ne ("Oracle.Stage3R12.ApplicationActivation" -as [type])) {
    return
  }
...
function Test-OracleStage3R12ApplicationActivationApi {
  [CmdletBinding()]
  ...
  try {
    $native = if ($null -ne $ProbeRunner) {
      ...
      Initialize-OracleStage3R12ActivationApi
      [Oracle.Stage3R12.ApplicationActivation]::Probe()
    }
    if (
      $null -eq $native -or
...
function Invoke-OracleStage3R12ApplicationActivation {
  [CmdletBinding()]
  param(
  ...
  try {
    $native = if ($null -ne $ActivationRunner) {
      ...
      Initialize-OracleStage3R12ActivationApi
      [Oracle.Stage3R12.ApplicationActivation]::Activate(
        $AppUserModelId,
        $Arguments,
        [uint32]2
      )
...
function Assert-OracleStage3R12ApplicationActivationSucceeded {
  [CmdletBinding()]
