...
$fixtureRoot = Join-Path (
  [IO.Path]::GetTempPath()
) "oracle-stage3-r12-package-inventory-$([Guid]::NewGuid().ToString('N'))"
...
try {
  ...
  $first = @(Get-OracleStage3R12PackageZipInventory $validZip)
  $second = @(Get-OracleStage3R12PackageZipInventory $validZip)
  if (
    $first.Count -ne 3 -or
  ...
  try {
    Get-OracleStage3R12PackageZipInventory $duplicateZip | Out-Null
  ...
  try {
    Get-OracleStage3R12PackageZipInventory $traversalZip | Out-Null
  ...
  $contentMismatch = @(
    Get-OracleStage3R12PackageZipInventory $contentMismatchZip |
  ...
  $metadataMismatch = @(
    Get-OracleStage3R12PackageZipInventory $metadataMismatchZip |
  ...
  foreach ($entry in @($acceptedInventory.entries)) {
    ...
    if (
      ...
      (Test-OracleStage3R12ReservedPackageMetadata $canonical) -or
  ...
  $acceptedZip = @(
    Get-OracleStage3R12PackageZipInventory $AcceptedPackagePath
  )
