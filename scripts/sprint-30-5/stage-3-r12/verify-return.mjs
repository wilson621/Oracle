...
assert.equal(manifest.contract, "oracle.sprint-30-5.stage-3-r12-archive-manifest");
assert.equal(validateProgrammeIdentity(manifest.programmeIdentity), canonicalProgrammeIdentity);
assert.equal(manifest.revision, "R12");
assert.match(manifest.attemptId, /^stage3-r12-\d{8}T\d{9}Z-[0-9a-f]{8}$/u);
assert.equal(
...
assert.equal(
  ...
  "oracle.sprint-30-5.stage-3-r12-evidence-manifest"
);
assert.equal(evidenceManifest.programmeIdentity, canonicalProgrammeIdentity);
assert.equal(evidenceManifest.revision, "R12");
...
assert.equal(completion.revision, "R12");
...
assert.equal(finalLifecycle.revision, "R12");
