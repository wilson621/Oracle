...
. (Join-Path $PSScriptRoot "Oracle.Stage3R12TransferInventoryPolicy.ps1")

$root = Join-Path $env:TEMP (
  "oracle-stage3-r12-transfer-inventory-" + [Guid]::NewGuid().ToString("N")
)
...
try {
  ...
  $passing = Assert-OracleStage3R12TransferPayloadInventory `
    -Manifest $manifest `
    -Contract $contract `
    -TransferRoot $root `
    -GetSha256 $getSha256
  ...
  try {
    Assert-OracleStage3R12TransferPayloadInventory `
  ...
  [pscustomobject][ordered]@{
    contract = "oracle.sprint-30-5.stage-3-r12-transfer-inventory-policy-test"
