...
const attemptPattern = new RegExp(`^stage3-r12-${compactTime}-${suffix}$`, "u");
const authorityPattern = new RegExp(
  `^authority-stage3-r12-${compactTime}-${suffix}$`,
  "u"
);
const transferPattern = new RegExp(`^transfer-stage3-r12-${compactTime}-${suffix}$`, "u");
...
function validateScopedId(value, timestampUtc, pattern, label) {
  ...
  if (!match) {
    throw new Error(`${label} has an invalid Stage 3 R12 format.`);
  }
...
export function validateProgrammeIdentity(value) {
  if (value !== canonicalProgrammeIdentity) {
    throw new Error("Stage 3 R12 programme identity differs.");
  }
  return value;
}
...
export function validateTransferIdentity({ transferId, timestampUtc }) {
  ...
  if (contract.rejectedTransfers.some((entry) => entry.transferId === transferId)) {
    throw new Error("Rejected Stage 3 R12 transfer identity cannot be reused.");
  }
}

export function validateTransferConstructionAuthority(value) {
  if (contract.authority.transfer !== "founder-authorised") {
    throw new Error("Stage 3 R12 transfer construction is not authorised.");
  }
...
export function validateAcceptedBindings(candidate) {
  ...
  for (const field of [
    ...
    if (candidate[field] !== expected[field]) {
      throw new Error(`Accepted Stage 2 R6 binding mismatch: ${field}`);
    }
  }
  for (const field of [
    "acceptedEvidenceIndexSha256",
