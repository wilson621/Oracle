...
function Get-OracleStage3R12RequiredWindowMember {
  [CmdletBinding()]
  param(
...
function ConvertFrom-OracleStage3R12WindowDiscoveryJson {
  [CmdletBinding()]
  ...
  foreach ($entry in $decoded) {
    ...
    foreach ($member in @(
      ...
      [void](Get-OracleStage3R12RequiredWindowMember $entry $member)
    }
    $entry
  }
}

function Test-OracleStage3R12QualifyingWindow {
  [CmdletBinding()]
  ...
  if (
    -not [Int64]::TryParse(
      [string](Get-OracleStage3R12RequiredWindowMember $Window "handle"),
      [Globalization.NumberStyles]::Integer,
      [Globalization.CultureInfo]::InvariantCulture,
      [ref]$handle
    ) -or
    -not [Int32]::TryParse(
      [string](Get-OracleStage3R12RequiredWindowMember $Window "processId"),
      [Globalization.NumberStyles]::Integer,
      [Globalization.CultureInfo]::InvariantCulture,
      [ref]$processId
    ) -or
    -not [Int32]::TryParse(
      [string](Get-OracleStage3R12RequiredWindowMember $Window "width"),
      [Globalization.NumberStyles]::Integer,
      [Globalization.CultureInfo]::InvariantCulture,
      [ref]$width
    ) -or
    -not [Int32]::TryParse(
      [string](Get-OracleStage3R12RequiredWindowMember $Window "height"),
      [Globalization.NumberStyles]::Integer,
      [Globalization.CultureInfo]::InvariantCulture,
      [ref]$height
    )
  ...
  (
    [string](Get-OracleStage3R12RequiredWindowMember $Window "processName") -ceq
      "Oracle" -and
    (Get-OracleStage3R12RequiredWindowMember $Window "visible") -eq $true -and
    (Get-OracleStage3R12RequiredWindowMember $Window "minimized") -eq $false -and
    $handle -gt 0 -and $processId -gt 0 -and $width -ge 1 -and $height -ge 1
  )
}

function Initialize-OracleStage3R12AppModelProcessIdentity {
  if ("Oracle.Stage3R12.AppModelProcessIdentity" -as [type]) { return }
...
function Get-OracleStage3R12ProcessOpenFailure {
  [CmdletBinding()]
  ...
  while ($null -ne $exception) {
    if (
      $exception.GetType().FullName -ceq
        "Oracle.Stage3R12.ProcessOpenException"
    ) { return $exception }
...
function Test-OracleStage3R12ProcessExists {
  [CmdletBinding()]
  param(
...
function Resolve-OracleStage3R12TeardownProcessOwnership {
  [CmdletBinding()]
  param(
    [Parameter(Mandatory = $true)][ValidateRange(1, [int]::MaxValue)]
    [int]$ProcessId,
  ...
  $exists = if ($null -eq $ProcessExists) {
    { param($Id) Test-OracleStage3R12ProcessExists -ProcessId $Id }
  } else {
    $ProcessExists
  }
  try {
    ...
    $queryFailure = $_
    $openFailure = Get-OracleStage3R12ProcessOpenFailure $queryFailure
...
function Get-OracleStage3R12ProcessPackageFamilyName {
  [CmdletBinding()]
  param(
    [Parameter(Mandatory = $true)][ValidateRange(1, [int]::MaxValue)]
    [int]$ProcessId,
  ...
  $familyName = if ($null -ne $Resolver) {
    ...
    [Oracle.Stage3R12.AppModelProcessIdentity]::QueryPackageFamilyName(
      [uint32]$ProcessId
    )
  }
...
function Assert-OracleStage3R12ProcessPackageOwnership {
  [CmdletBinding()]
  param(
    [Parameter(Mandatory = $true)][ValidateRange(1, [int]::MaxValue)]
    [int]$ProcessId,
  ...
  $observed = Get-OracleStage3R12ProcessPackageFamilyName `
