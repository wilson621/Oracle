...
'@
$windows = @(ConvertFrom-OracleStage3R12WindowDiscoveryJson -Json $json)
if (
  $windows.Count -ne 2 -or
  $windows[0] -is [Array] -or
  -not (Test-OracleStage3R12QualifyingWindow $windows[0]) -or
  (Test-OracleStage3R12QualifyingWindow $windows[1])
) { throw "Multi-window discovery JSON was not normalized deterministically." }

$empty = @(ConvertFrom-OracleStage3R12WindowDiscoveryJson -Json "[]")
...
foreach ($invalid in @(
  ...
  $failed = $false
  try {
    $values = @(ConvertFrom-OracleStage3R12WindowDiscoveryJson -Json $invalid)
    foreach ($value in $values) {
      [void](Test-OracleStage3R12QualifyingWindow $value)
    }
  } catch { $failed = $true }
...
$exitedResolution = Resolve-OracleStage3R12TeardownProcessOwnership `
  -ProcessId 9808 -ExpectedPackageFamilyName $expectedFamily `
  -OwnershipResolver {
    param($ProcessId)
...
try {
  [void](Resolve-OracleStage3R12TeardownProcessOwnership `
    -ProcessId 9808 -ExpectedPackageFamilyName $expectedFamily `
    -OwnershipResolver {
      param($ProcessId)
      throw [Oracle.Stage3R12.ProcessOpenException]::new(
...
$livePidConfirmed = Test-OracleStage3R12ProcessExists -ProcessId $PID
if (-not $livePidConfirmed) {
  throw "The current live PowerShell PID was not affirmatively detected."
}
$absentPidConfirmed = -not (
  Test-OracleStage3R12ProcessExists -ProcessId ([int]::MaxValue)
)
...
try {
  [void](Test-OracleStage3R12ProcessExists -ProcessId 9808 -ProcessQuery {
    param($ProcessId)
