...
function Assert-OracleStage3R12CertificateTrustContract {
...
function Assert-OracleStage3R12Thumbprint {
...
function Get-OracleStage3R12CertificateStoreDefinitions {
  @(
...
function Get-OracleStage3R12PhysicalCertificateMatches {
  param([Parameter(Mandatory = $true)][string]$Thumbprint)

  Assert-OracleStage3R12Thumbprint -Thumbprint $Thumbprint
  $matches = @()
  foreach ($definition in @(Get-OracleStage3R12CertificateStoreDefinitions)) {
...
function Get-OracleStage3R12LogicalCertificateViews {
  param([Parameter(Mandatory = $true)][string]$Thumbprint)
...
function Test-OracleStage3R12CertificateIdentity {
  param(
...
function Assert-OracleStage3R12NoCertificateResidue {
  param(
...
function Assert-OracleStage3R12TemporaryTrustState {
  param(
    [Parameter(Mandatory = $true)]
    [AllowEmptyCollection()][object[]]$PhysicalMatches,
    [Parameter(Mandatory = $true)]
  ...
  if (
    ...
    -not (Test-OracleStage3R12CertificateIdentity `
...
function Assert-OracleStage3R12ExactRemovalTarget {
  param(
    [Parameter(Mandatory = $true)]
    [AllowEmptyCollection()][object[]]$PhysicalMatches,
    [Parameter(Mandatory = $true)]
...
function Test-OracleStage3R12ProcessIsElevated {
