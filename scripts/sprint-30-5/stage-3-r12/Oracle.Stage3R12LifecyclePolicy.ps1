...
function Get-OracleStage3R12LifecyclePhases {
  @(
    "authority-consumed",
    "transfer-verified",
    "host-admitted",
...
function New-OracleStage3R12LifecycleState {
...
function Move-OracleStage3R12Lifecycle {
  [CmdletBinding()]
  param(
...
function Assert-OracleStage3R12NextLifecyclePhase {
  [CmdletBinding()]
  param(
    [Parameter(Mandatory = $true)][object]$State,
    [Parameter(Mandatory = $true)][string]$Phase
...
function Get-OracleStage3R12TeardownObligations {
  [CmdletBinding()]
  param(
