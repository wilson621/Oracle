...
No R12 transfer, return or execution entry point is exposed through `package.json`. Direct token-gated construction of one fresh execution-enabled, create-only transfer is authorised. One qualification authority and attempt are Founder-authorised only after independent transfer verification, fresh continuity, elevated pre-authority admission and every zero-state, security, trust and return-root gate passes. The transferred harness fails closed unless its manifest-bound contract explicitly records `founder-authorised` execution.
