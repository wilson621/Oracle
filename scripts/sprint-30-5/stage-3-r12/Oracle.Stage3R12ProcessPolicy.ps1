...
function Get-OracleStage3R12RequiredProcessMember {
  [CmdletBinding()]
  param(
...
function Invoke-OracleStage3R12GovernedProcess {
  [CmdletBinding()]
  param(
    [Parameter(Mandatory = $true)][string]$Name,
  ...
  foreach ($member in @(
    ...
    [void](Get-OracleStage3R12RequiredProcessMember $record $member)
  }
  ...
  $processError = Get-OracleStage3R12RequiredProcessMember $record "processError"
  $signal = Get-OracleStage3R12RequiredProcessMember $record "signal"
  $exitCode = Get-OracleStage3R12RequiredProcessMember $record "exitCode"
