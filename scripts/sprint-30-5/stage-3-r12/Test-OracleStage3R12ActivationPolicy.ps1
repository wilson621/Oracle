...
Assert-Rejected {
  Assert-OracleStage3R12ApplicationActivationContract -Contract $invalidContract
} "Explorer exit-code contract"

$aumid = "Oracle.Platform.LocalCertification_test!Oracle"
$success = Invoke-OracleStage3R12ApplicationActivation `
...
Assert-OracleStage3R12ApplicationActivationSucceeded -Result $success

$availability = Test-OracleStage3R12ApplicationActivationApi -ProbeRunner {
...
Assert-Rejected {
  Invoke-OracleStage3R12ApplicationActivation -AppUserModelId "not-an-aumid"
} "malformed AUMID"
Assert-Rejected {
  $failure = Invoke-OracleStage3R12ApplicationActivation `
    -AppUserModelId $aumid -ActivationRunner {
      [pscustomobject]@{ HResult = "0x80004005"; ProcessId = 0 }
    }
  Assert-OracleStage3R12ApplicationActivationSucceeded -Result $failure
} "HRESULT failure"
Assert-Rejected {
  $zeroPid = Invoke-OracleStage3R12ApplicationActivation `
    -AppUserModelId $aumid -ActivationRunner {
      [pscustomobject]@{ HResult = "0x00000000"; ProcessId = 0 }
    }
  Assert-OracleStage3R12ApplicationActivationSucceeded -Result $zeroPid
} "zero process ID"
Assert-Rejected {
  $runnerError = Invoke-OracleStage3R12ApplicationActivation `
    -AppUserModelId $aumid -ActivationRunner {
      throw "simulated activation error"
    }
  Assert-OracleStage3R12ApplicationActivationSucceeded -Result $runnerError
} "runner error"
Assert-Rejected {
  $incomplete = Invoke-OracleStage3R12ApplicationActivation `
    -AppUserModelId $aumid -ActivationRunner {
      [pscustomobject]@{ HResult = "0x00000000" }
    }
  Assert-OracleStage3R12ApplicationActivationSucceeded -Result $incomplete
} "incomplete native result"
Assert-Rejected {
  $nullResult = Invoke-OracleStage3R12ApplicationActivation `
    -AppUserModelId $aumid -ActivationRunner { $null }
  Assert-OracleStage3R12ApplicationActivationSucceeded -Result $nullResult
} "null native result"
Assert-Rejected {
  $missing = $success | Select-Object * -ExcludeProperty hresult
  Assert-OracleStage3R12ApplicationActivationSucceeded -Result $missing
