...
if (-not $principal.IsInRole([Security.Principal.WindowsBuiltInRole]::Administrator)) {
  throw "R12 engineering integration requires an elevated Windows PowerShell process."
}
...
function Assert-ZeroPreState {
  ...
  if ($existingPackages.Count -ne 0 -or $existingCertificates.Count -ne 0) {
    throw (
      "R12 engineering integration requires zero Oracle package and trust pre-state; " +
...
try {
  ...
  if (
    ...
    throw "Accepted R6 MSIX binding differs."
  }
  $certificate = [Security.Cryptography.X509Certificates.X509Certificate2]::new(
    $certificatePath
  )
  if ($certificate.Thumbprint -cne $thumbprint -or $certificate.HasPrivateKey) {
    throw "R11-preserved public certificate binding differs."
  }
  ...
  if (
    ...
    throw (
      "R12 engineering integration cleanup failed: " +
...
if ($null -eq $result) {
  throw "R12 engineering integration produced no passing result."
}
