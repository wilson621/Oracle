...
. (Join-Path $PSScriptRoot "Oracle.Stage3R12LifecyclePolicy.ps1")
. (Join-Path $PSScriptRoot "Oracle.Stage3R12IdentityPolicy.ps1")
. (Join-Path $PSScriptRoot "Oracle.Stage3R12ActivationPolicy.ps1")
. (Join-Path $PSScriptRoot "Oracle.Stage3R12CertificateTrustPolicy.ps1")
. (Join-Path $PSScriptRoot "Oracle.Stage3R12InstalledSoftwarePolicy.ps1")
. (Join-Path $PSScriptRoot "Oracle.Stage3R12InstalledRuntimeConfigurationPolicy.ps1")
. (Join-Path $PSScriptRoot "Oracle.Stage3R12ObservationPolicy.ps1")
. (Join-Path $PSScriptRoot "Oracle.Stage3R12ProcessPolicy.ps1")
. (Join-Path $PSScriptRoot "Oracle.Stage3R12WindowPolicy.ps1")
...
function Invoke-SimulatedPhase(
  ...
  Assert-OracleStage3R12NextLifecyclePhase -State $State -Phase $Phase
  ...
  [void](Move-OracleStage3R12Lifecycle -State $State -Phase $Phase)
}

function Invoke-SimulatedLifecycle([string]$Root, [string]$InjectedFailure) {
  $state = New-OracleStage3R12LifecycleState
  $failure = $null
  try {
    foreach ($phase in @(Get-OracleStage3R12LifecyclePhases)) {
  ...
  $teardown = Get-OracleStage3R12TeardownObligations `
...
$parent = Join-Path $repositoryRoot ".tmp-r12"
...
try {
  ...
  $fixtureFullName = "Oracle.Platform.LocalCertification_0.1.4.0_x64__fw69ec0wxwzn4"
  ...
  $software = @(ConvertTo-OracleStage3R12InstalledSoftwareInventory -RegistryEntries @(
  ...
  foreach ($name in @("certutil.exe", "reagentc.exe")) {
    ...
    [void](Get-OracleStage3R12WindowsExecutablePath `
      -Name $name -SystemDirectory $fixtureSystem32)
  }
  $fixtureActivation = Invoke-OracleStage3R12ApplicationActivation `
  ...
  $fixtureWindows = @(
    ConvertFrom-OracleStage3R12WindowDiscoveryJson -Json (
  ...
  if (
    ...
    -not (Test-OracleStage3R12QualifyingWindow $fixtureWindows[0])
  ) { throw "Shared window discovery policy failed inside rehearsal." }
  [void](Assert-OracleStage3R12ProcessPackageOwnership `
    -ProcessId 9808 `
  ...
  $observation = Invoke-OracleStage3R12CompleteStableObservation `
  ...
  Initialize-OracleStage3R12AppModelProcessIdentity
  $exitedProcess = Resolve-OracleStage3R12TeardownProcessOwnership `
    -ProcessId 9808 `
    -ExpectedPackageFamilyName "Oracle.Platform.LocalCertification_fixture" `
    -OwnershipResolver {
      param($ProcessId)
      throw [Oracle.Stage3R12.ProcessOpenException]::new(
        [uint32]$ProcessId, 87
      )
    } `
  ...
  $processCounts = @{}
  $processResult = Invoke-OracleStage3R12GovernedProcess `
  ...
  $runtimeAttempt = "stage3-r12-20260803T120000000Z-a1b2c3d4"
  $runtimeParameters = @{
    PackageFamilyName = $runtimeFamily
    ExpectedPackageFamilyName = $runtimeFamily
    ConfigurationId = "runtime-$runtimeAttempt"
    FounderGrantId = "founder-stage3-r12-grant-20260803T120000000Z-a1b2c3d4"
