...
if (
  ...
  [string]$manifest.revision -cne "R12" -or
...
$runningScriptRelativePath = "payload/Invoke-OracleStage3R12PreAuthorityPreflight.ps1"
$runningScriptEntry = @($payloadEntries | Where-Object {
  [string]$_.path -ceq $runningScriptRelativePath
})
$contractRelativePath = "payload/Oracle.Stage3R12Contract.json"
...
) { throw "Pre-authority canonical R12 identity differs or is retired." }
if (
...
$getCertificateMatches = {
  @(
    @(Get-OracleStage3R12PhysicalCertificateMatches `
      -Thumbprint $contract.stage2.certificateThumbprint)
    @(Get-OracleStage3R12LogicalCertificateViews `
      -Thumbprint $contract.stage2.certificateThumbprint)
  )
}

$result = Get-OracleStage3R12PreAuthorityObservation `
  -Contract $contract `
