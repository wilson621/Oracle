...
function Assert-OracleStage5R1Transfer {
  param(
  ...
  [void](Assert-OracleStage4R4NoReparseTraversal $transfer $approvedRoot)
  if (-not (Test-Path -LiteralPath $transfer -PathType Container)) { throw "Governed Stage 5 R1 transfer is absent." }
  ...
  foreach ($path in @($manifestPath, $custodyPath, $manifestSidecar, $custodySidecar, $verificationPath, $verificationSidecar)) {
    [void](Assert-OracleStage4R4NoReparseTraversal $path $transfer)
  ...
  foreach ($entry in @($manifest.files)) {
    ...
    [void](Assert-OracleStage4R4NoReparseTraversal $path $payloadRoot)
