...
assert.notEqual(manifest.transferId, failed.transferId);
assert.equal(manifest.replacementTransfer, true);
assert.equal(manifest.replacesFailedTransferId, failed.transferId);
assert.equal(manifest.failedTransferManifestSha256, failed.manifestSha256);
assert.equal(manifest.failedTransferCustodySha256, failed.custodySha256);
assert.equal(manifest.failedTransferVerificationSha256, failed.verificationSha256);
assert.equal(manifest.acceptedCorrectedEngineeringCommit, contract.acceptedPreAuthorityCorrection.engineeringCommit);
assert.equal(manifest.acceptedCorrectionClosureHead, contract.acceptedPreAuthorityCorrection.closureHead);
...
assert.equal(custody.replacementTransfer, true);
assert.equal(custody.replacesFailedTransferId, failed.transferId);
assert.equal(custody.failedTransferAdmissionProhibited, true);
