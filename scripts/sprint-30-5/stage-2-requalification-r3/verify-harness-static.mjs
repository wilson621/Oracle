...
const fixtureAttemptId = "r3-20260728T131415678Z-deadbeef";
...
assert.equal(contract.candidate.commit, "a7fc67f207d9c95407c70812828fa66bd487285d");
assert.equal(contract.candidate.tree, "356f6d52f1bf70065692e892af8bf916acc8727a");
assert.equal(
...
const r3ValidatorSource = readFileSync(import.meta.filename, "utf8");
...
const r3FixtureLabels = new Set(
  [...r3ValidatorSource.matchAll(fixtureLabelPattern)].map((match) => match[1])
);
assert.ok(r2FixtureLabels.length > 0, "R2 regression labels were not discovered.");
for (const label of r2FixtureLabels) {
  assert.ok(r3FixtureLabels.has(label), `R3 lost inherited R2 fixture: ${label}`);
}
assert.throws(
...
assert.equal(
  isSameOrDescendant(
    join(
      ".artifacts",
      "sprint-30-5",
      "stage-2-requalification-r3",
      fixtureAttemptId
    ),
    join(".artifacts", "sprint-30-5", "stage-2")
  ),
  false,
  "Segment-aware protection must distinguish the R3 and historical roots."
);
...
assert.doesNotThrow(() => assertR3ArtifactPath(governedRoot, fixtureAttemptId));
assert.doesNotThrow(() =>
  assertNoReparseTraversal(
    join(".artifacts", "sprint-30-5", "stage-2-requalification-r3")
  )
);
assert.throws(
  () =>
...
assert.throws(
  () => assertR3ArtifactPath(`${governedRoot}-other`, fixtureAttemptId),
...
for (const [base, prefix] of [
  ...
  for (const entry of readdirSync(base, { withFileTypes: true })) {
    if (
      ...
      entry.name !== "stage-2-requalification-r3"
    ) {
...
const temporaryRoot = mkdtempSync(join(os.tmpdir(), "oracle-stage2-r3-static-"));
try {
...
assert.doesNotThrow(() =>
  validateFinalIdentity({
    ...certificateBound,
    evidence: {
      manifest: "Oracle.Stage2RequalificationR3EvidenceManifest.json",
...
assert.match(
  cleanupScript,
  /exact generated R3 certificate thumbprint was not found/u
);
assert.match(
  cleanupScript,
...
assert.match(
  cleanupScript,
  /ORACLE_R3_ROOT_REMOVAL_PROCESS=[\s\S]*ConvertTo-Json -Compress -Depth 5/u
);
assert.match(
  cleanupScript,
...
const r3PackageEntryPoints = Object.keys(packageJson.scripts).filter((name) =>
  name.startsWith("sprint-30-5:stage-2:r3:")
);
assert.deepEqual(
  r3PackageEntryPoints,
  [],
  "R3 must not modify the candidate package.json or expose independently runnable phases."
);
assert.equal(
...
assert.ok(
  ...
  "R3 must not expose a standalone attempt-preparation entry point."
);
...
assert.throws(
  () =>
    validateAuthorityIdentity({
      authorityId: "authority-r3-other",
      attemptId: fixtureAttemptId,
    }),
  /must exactly equal/
);
const previousAttemptId = "r3-20260728T120000000Z-01234567";
...
assert.match(executorSource, /Migration 011 does not match the fixed R3 candidate binding/u);
assert.match(executorSource, /Migration 012 does not match the fixed R3 candidate binding/u);
assert.match(
  readFileSync(join(import.meta.dirname, "execution-core.mjs"), "utf8"),
  /FOUNDER-AUTHORISED-STAGE-2-R3-SINGLE-ATTEMPT/u
);
assert.match(
  executorSource,
...
assert.equal(contract.authority.stage3, "closed-not-authorised-by-r3");
assert.equal(contract.authority.stage4, "not-authorised-by-r3");
assert.throws(
  () => assertRequiredTool("oracle-r3-tool-that-does-not-exist.exe"),
