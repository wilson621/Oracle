...
function Assert-CreateOnlyOutputPath {
  ...
  if (
    -not $relative.StartsWith(
      ".artifacts/sprint-30-5/stage-2-requalification-r3/",
      [StringComparison]::OrdinalIgnoreCase
    ) -or
    $relative.Contains("/../") -or
    $relative.EndsWith("/..", [StringComparison]::Ordinal)
  ) {
    throw "Cleanup evidence must be inside an R3 attempt artifact root."
  }

  $attemptRelative = $relative.Substring(
    ".artifacts/sprint-30-5/stage-2-requalification-r3/".Length
  )
  $attemptId = $attemptRelative.Split("/")[0]
  if ($attemptId -notmatch "^r3-\d{8}T\d{9}Z-[0-9a-f]{8}$") {
    throw "Cleanup evidence does not identify a valid immutable R3 attempt."
  }
...
if ($matches.Count -eq 0) {
  throw "The exact generated R3 certificate thumbprint was not found."
}
...
if ($rootMatches.Count -eq 1) {
  ...
  if ($PSCmdlet.ShouldProcess($rootTarget, "Remove exact R3 Root trust")) {
    $rootRemoval = Invoke-ExactRootRemoval
    [Console]::Error.WriteLine(
      "ORACLE_R3_ROOT_REMOVAL_PROCESS=" +
      (
        $rootRemoval |
...
if ($PSCmdlet.ShouldProcess($myTarget, "Remove exact R3 signing certificate")) {
  $myBeforeRemoval = @(
...
if ($remaining.Count -ne 0) {
  throw "Exact R3 certificate thumbprint remains after cleanup."
}

$evidence = [ordered]@{
  schemaVersion = "1.0.0"
  contract = "oracle.sprint-30-5.stage-2-requalification-r3-certificate-cleanup"
  programmeIdentity = "Sprint 30.5 Stage 2 Requalification R3"
...
if (-not (Test-Path -LiteralPath $parent -PathType Container)) {
  throw "The governed R3 attempt directory does not exist."
}
