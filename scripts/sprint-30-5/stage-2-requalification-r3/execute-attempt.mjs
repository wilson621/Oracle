...
try {
  ...
  writeJsonAtomicCreateOnly(
    join(directories.evidence, "stage-2-requalification-r3-certification.json"),
    certification
  );
  const evidenceIndex = createEvidenceIndex();
  writeJsonAtomicCreateOnly(
    join(directories.evidence, "stage-2-requalification-r3-evidence-index.json"),
    evidenceIndex
  );
  ...
  writeJsonAtomicCreateOnly(
    join(
      ...
      "stage-2-requalification-r3-completion.json"
    ),
    {
      schemaVersion: "1.0.0",
      contract:
        "oracle.sprint-30-5.stage-2-requalification-r3-execution-completion",
      ...identity(),
  ...
  const finalManifestPath = join(
    directories.evidence,
    "Oracle.Stage2RequalificationR3EvidenceManifest.json"
  );
  writeJsonAtomicCreateOnly(finalManifestPath, finalManifest);
  const finalEvidenceHash = sha256File(finalManifestPath);
  writeFileAtomicCreateOnly(
    join(directories.evidence, "Oracle.Stage2RequalificationR3FinalEvidenceHash.sha256.txt"),
...
function assertExecutionContract() {
  assert.equal(contract.programmeIdentity, "Sprint 30.5 Stage 2 Requalification R3");
  ...
  assert.equal(
    ...
    "Migration 011 does not match the fixed R3 candidate binding."
  );
  assert.equal(
    sha256File(join(repositoryRoot, "database", "012_operator_identity_lifecycle.sql")),
    contract.candidate.migration012Sha256,
    "Migration 012 does not match the fixed R3 candidate binding."
  );
}
...
function createQualificationCandidate() {
  ...
  return {
    schemaVersion: "1.0.0",
    contract:
      "oracle.sprint-30-5.stage-2-requalification-r3-qualification-candidate",
    ...identity(),
...
function assertRepositoryAndCandidateUnchanged(candidate) {
  if (git(["branch", "--show-current"]) !== contract.requiredBranch) {
    throw new Error("Branch changed during R3 execution.");
  }
  if (git(["rev-parse", "HEAD"]) !== input.harnessCommit) {
    throw new Error("HEAD changed during R3 execution.");
  }
  if (git(["status", "--porcelain=v1", "--untracked-files=all"]) !== "") {
    throw new Error("Repository changed during R3 build.");
  }
...
async function constructPackageLayout() {
  ...
  writeJsonAtomicCreateOnly(join(stage, "package.json"), {
    name: "oracle-stage-2-requalification-r3-local-qualification",
    productName: "Oracle Stage 2 Requalification R3 Local Qualification",
...
function createTemporaryCertificate(layout) {
  pfxPath = join(directories.signing, "r3-local-test-signing.pfx");
  ...
  const script = [
    ...
    "$cert=[Security.Cryptography.X509Certificates.X509Certificate2]::new($env:ORACLE_R3_PFX,$env:ORACLE_R3_PASSWORD,$flags)",
  ...
  const result = JSON.parse(
    runLogged(
      ...
      [],
      { ORACLE_R3_PFX: pfxPath, ORACLE_R3_PASSWORD: password }
    ).stdout.trim()
  );
...
function recoverCertificateIdentityFromStoreDelta() {
  ...
  if (
    ...
    throw new Error("New certificate-store entries do not match the R3 identity.");
  }
...
function recoverCertificateIdentity() {
  ...
  const result = spawnSync(
    ...
    ["-NoProfile", "-NonInteractive", "-Command", script],
    {
      ...
      env: {
        ...process.env,
        ORACLE_R3_PFX: pfxPath,
        ORACLE_R3_PASSWORD: password,
      },
      encoding: "utf8",
      shell: false,
      windowsHide: true,
...
function buildSignedRelease(layout, candidate) {
  ...
  writeJsonAtomicCreateOnly(
    ...
    {
      schemaVersion: "1.0.0",
      contract: "oracle.sprint-30-5.stage-2-requalification-r3-release-build",
      ...identity(),
...
function verifyRelease(release, candidate) {
  ...
  writeJsonAtomicCreateOnly(
    join(directories.release, "package-content-inventory.json"),
    {
      schemaVersion: "1.0.0",
      contract:
        "oracle.sprint-30-5.stage-2-requalification-r3-package-content-inventory",
      ...identity(),
      packageSha256: sha256File(release.packagePath),
  ...
  try {
    ...
    if (exactStoreMatches.length === 0) {
      writeJsonAtomicCreateOnly(cleanupPath, {
        schemaVersion: "1.0.0",
        contract:
          "oracle.sprint-30-5.stage-2-requalification-r3-certificate-cleanup",
        programmeIdentity: contract.programmeIdentity,
        completedAt: new Date().toISOString(),
        status: "passed",
...
function createCertification(release, candidate, verification) {
  return {
    schemaVersion: "1.0.0",
    contract: "oracle.sprint-30-5.stage-2-requalification-r3-certification",
    ...identity(),
...
function createEvidenceIndex() {
  ...
  return {
    schemaVersion: "1.0.0",
    contract: "oracle.sprint-30-5.stage-2-requalification-r3-evidence-index",
    ...identity(),
    createdAt: new Date().toISOString(),
    status: "complete-awaiting-freeze",
    files: files.filter(
      (entry) => !entry.path.endsWith("stage-2-requalification-r3-evidence-index.json")
    ),
  };
}
...
function freezeEvidence(release, certification, evidenceIndex) {
  ...
  writeJsonAtomicCreateOnly(
    join(directories.evidence, "stage-2-requalification-r3-frozen-evidence.json"),
    {
      schemaVersion: "1.0.0",
      contract: "oracle.sprint-30-5.stage-2-requalification-r3-frozen-evidence",
      ...identity(),
      frozenAt: new Date().toISOString(),
...
function createFinalEvidenceManifest(archive) {
  return {
    schemaVersion: "1.0.0",
    contract: "oracle.sprint-30-5.stage-2-requalification-r3-final-evidence",
    ...identity(),
    createdAt: new Date().toISOString(),
    status: "execution-passed-awaiting-founder-review",
    archive: {
...
function publishRepositoryEvidence() {
  ...
  for (const name of [
    ...
    "single-attempt-authority.json",
    "stage-2-requalification-r3-certification.json",
    "stage-2-requalification-r3-evidence-index.json",
    "stage-2-requalification-r3-frozen-evidence.json",
    "stage-2-requalification-r3-completion.json",
...
function publishFinalRepositoryEvidence(target) {
  for (const name of [
    ...
    "Oracle.Stage2RequalificationR3EvidenceManifest.json",
    "Oracle.Stage2RequalificationR3FinalEvidenceHash.sha256.txt",
  ]) {
    writeFileAtomicCreateOnly(
      join(target, name),
...
function createFinalMachineCheckpoint() {
  ...
  if (git(["diff", "--cached", "--name-only"]) !== "") {
    throw new Error("The repository index changed during R3 execution.");
  }
  const machineCheckpoint = {
    schemaVersion: "1.0.0",
    contract:
      "oracle.sprint-30-5.stage-2-requalification-r3-machine-checkpoint",
    ...identity(),
    recordedAt: new Date().toISOString(),
...
function createFinalRepositoryCheckpoint(repositoryEvidenceTarget) {
  ...
  writeJsonAtomicCreateOnly(
    join(directories.evidence, "final-repository-checkpoint.json"),
    {
      schemaVersion: "1.0.0",
      contract:
        "oracle.sprint-30-5.stage-2-requalification-r3-repository-checkpoint",
      ...identity(),
      recordedAt: new Date().toISOString(),
      branch: git(["branch", "--show-current"]),
...
function createProvenance(packagePath, candidate) {
  return {
    ...
    predicate: {
      ...
      runDetails: {
        builder: {
          id: "oracle.local/sprint-30-5-stage-2-requalification-r3",
        },
...
function runLogged(label, command, args, redactions = [], environment = {}) {
  ...
  const record = {
    schemaVersion: "1.0.0",
    contract: "oracle.sprint-30-5.stage-2-requalification-r3-command",
    ...identity(),
    label,
    command,
