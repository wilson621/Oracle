...
$requiredAuthority = "FOUNDER-AUTHORISED-STAGE-2-R7-SINGLE-ATTEMPT"
if ($FounderAuthority -cne $requiredAuthority) {
  throw "The exact R7 Founder execution authority token is required."
}

$repositoryRoot = [IO.Path]::GetFullPath((Join-Path $PSScriptRoot "..\..\.."))
$contractPath = Join-Path $PSScriptRoot "Oracle.Stage2RequalificationR7Contract.json"
$executorPath = Join-Path $PSScriptRoot "execute-attempt.mjs"
if (-not (Test-Path -LiteralPath $contractPath -PathType Leaf)) { throw "R7 contract is missing." }
if (-not (Test-Path -LiteralPath $executorPath -PathType Leaf)) { throw "R7 executor is missing." }
...
try {
  ...
  $identity = New-OracleStage2R7ExecutionIdentity
  if ($identity.suffix -eq "00000000") { throw "The prohibited fixed suffix was generated." }
  $outputRoot = ".artifacts/sprint-30-5/stage-2-requalification-r7/$($identity.attemptId)"
  $authorityPath = ".artifacts/sprint-30-5/stage-2-requalification-r7/authorities/$($identity.authorityId).json"
  ...
  [pscustomobject]@{
    contract = "oracle.sprint-30-5.stage-2-requalification-r7-governed-invocation"
  ...
  if ($null -eq $executorExitCode) { throw "R7 executor returned a null exit code." }
