...
function Assert-CreateOnlyOutputPath {
  ...
  if (
    -not $relative.StartsWith(
      ".artifacts/sprint-30-5/stage-2-requalification-r7/",
      [StringComparison]::OrdinalIgnoreCase
    ) -or
    $relative.Contains("/../") -or
    $relative.EndsWith("/..", [StringComparison]::Ordinal)
  ) {
    throw "Cleanup evidence must be inside an R7 attempt artifact root."
  }

  $attemptRelative = $relative.Substring(
    ".artifacts/sprint-30-5/stage-2-requalification-r7/".Length
  )
  $attemptId = $attemptRelative.Split("/")[0]
  if ($attemptId -notmatch "^r7-\d{8}T\d{9}Z-[0-9a-f]{8}$") {
    throw "Cleanup evidence does not identify a valid immutable R7 attempt."
  }
...
if ($matches.Count -eq 0) {
  throw "The exact generated R7 certificate thumbprint was not found."
}
...
if ($rootMatches.Count -eq 1) {
  ...
  if ($PSCmdlet.ShouldProcess($rootTarget, "Remove exact R7 Root trust")) {
    $rootRemoval = Invoke-ExactRootRemoval
    [Console]::Error.WriteLine(
      "ORACLE_R7_ROOT_REMOVAL_PROCESS=" +
      (
        $rootRemoval |
...
if ($PSCmdlet.ShouldProcess($myTarget, "Remove exact R7 signing certificate")) {
  $myBeforeRemoval = @(
...
if ($remaining.Count -ne 0) {
  throw "Exact R7 certificate thumbprint remains after cleanup."
}

$evidence = [ordered]@{
  schemaVersion = "1.0.0"
  contract = "oracle.sprint-30-5.stage-2-requalification-r7-certificate-cleanup"
...
if (-not (Test-Path -LiteralPath $parent -PathType Container)) {
  throw "The governed R7 attempt directory does not exist."
}
