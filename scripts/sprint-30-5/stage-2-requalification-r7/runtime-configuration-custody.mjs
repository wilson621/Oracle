...
const BUILD_CANARIES = Object.freeze({
  ORACLE_SUPABASE_URL: "https://r7-build-canary.invalid",
  ORACLE_SUPABASE_ANON_KEY: "oracle-r7-anon-canary-not-a-secret",
  ORACLE_SUPABASE_SERVICE_ROLE_KEY: "oracle-r7-service-canary-not-a-secret",
  ORACLE_WEB_SESSION_SECRET: "oracle-r7-session-canary-not-a-secret-32-bytes",
  NEXT_PUBLIC_SUPABASE_URL: "https://r7-build-canary.invalid",
  NEXT_PUBLIC_SUPABASE_ANON_KEY: "oracle-r7-anon-canary-not-a-secret",
});
...
export function assertNoAmbientRuntimeConfiguration(root, environment = process.env) {
  ...
  if (populated.length !== 0) {
    throw new Error(
      `Ambient runtime configuration is prohibited during R7 qualification: ${populated.join(", ")}`
    );
  }
  ...
  if (presentFiles.length !== 0) {
    throw new Error(
      `Ambient environment files are prohibited during R7 qualification: ${presentFiles.join(", ")}`
    );
  }
}
