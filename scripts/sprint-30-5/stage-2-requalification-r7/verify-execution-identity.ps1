...
if ($identity.attemptId -ne "r7-20260731T170102345Z-deadbeef") { throw "Attempt mismatch." }
if ($identity.authorityId -ne "authority-$($identity.attemptId)") { throw "Authority mismatch." }
$secondIdentity = New-OracleStage2R7ExecutionIdentity `
  -UtcNowProvider { $utc } `
...
Assert-Throws {
  New-OracleStage2R7ExecutionIdentity -UtcNowProvider { $utc } -EntropyProvider { throw "fixture entropy failure" }
} "fixture entropy failure"
Assert-Throws {
  New-OracleStage2R7ExecutionIdentity -UtcNowProvider { $utc } -EntropyProvider { $null }
} "returned null"
Assert-Throws {
  New-OracleStage2R7ExecutionIdentity -UtcNowProvider { $utc } -EntropyProvider { [byte[]] @(1, 2, 3) }
} "exactly four bytes"
Assert-Throws {
  New-OracleStage2R7ExecutionIdentity -UtcNowProvider { $utc } -EntropyProvider { [byte[]] @(0, 0, 0, 0) }
} "prohibited all-zero"
