...
const fixtureAttemptId = "r7-20260728T131415678Z-deadbeef";
...
const fixtureBinding = {
  ...
  wrapperProtocol: "oracle-stage2-r7-governed-wrapper-v1",
...
assert.deepEqual(
  validateGovernedWrapperInvocation({
    observed: "oracle-stage2-r7-governed-wrapper-v1:4242",
    parentProcessId: 4242,
  }),
  {
    protocol: "oracle-stage2-r7-governed-wrapper-v1",
    parentProcessId: 4242,
  }
);
assert.throws(
...
assert.throws(
  () =>
    validateGovernedWrapperInvocation({
      observed: "oracle-stage2-r7-governed-wrapper-v1:4243",
      parentProcessId: 4242,
    }),
  /must be invoked by the governed/
);
assert.throws(
  () =>
    validateGovernedWrapperInvocation({
      observed: "oracle-stage2-r7-governed-wrapper-v1:0",
      parentProcessId: 0,
    }),
...
assert.throws(
  () =>
    validateAttemptIdentity({
      attemptId: "r7-20260728T131415678Z-00000000",
      timestampUtc: fixtureTimestamp,
    }),
...
const expectedHarnessFiles = [
  "Oracle.Stage2RequalificationR7Contract.json",
  "execute-attempt.mjs",
  "execution-core.mjs",
...
].map((name) => `scripts/sprint-30-5/stage-2-requalification-r7/${name}`).sort();
assert.deepEqual(
...
const attemptCreationRecord = createAttemptRecord({
  ...
  attemptId: fixtureAttemptId,
  timestampUtc: fixtureTimestamp,
  outputRoot: join(".artifacts", "sprint-30-5", "stage-2-requalification-r7", fixtureAttemptId),
});
...
const r7ValidatorSource = readFileSync(import.meta.filename, "utf8");
...
const r7FixtureLabels = new Set(
  [...r7ValidatorSource.matchAll(fixtureLabelPattern)].map((match) => match[1])
);
assert.ok(r2FixtureLabels.length > 0, "R2 regression labels were not discovered.");
for (const label of r2FixtureLabels) {
  assert.ok(r7FixtureLabels.has(label), `R7 lost inherited R2 fixture: ${label}`);
}
assert.throws(
...
assert.equal(
  isSameOrDescendant(
    join(
      ".artifacts",
      "sprint-30-5",
      "stage-2-requalification-r7",
      fixtureAttemptId
    ),
    join(".artifacts", "sprint-30-5", "stage-2")
  ),
  false,
  "Segment-aware protection must distinguish the R7 and historical roots."
);
...
assert.doesNotThrow(() => assertR7ArtifactPath(governedRoot, fixtureAttemptId));
assert.doesNotThrow(() =>
  assertNoReparseTraversal(
    join(".artifacts", "sprint-30-5", "stage-2-requalification-r7")
  )
);
assert.throws(
  () =>
...
assert.throws(
  () => assertR7ArtifactPath(`${governedRoot}-other`, fixtureAttemptId),
...
for (const [base, prefix] of [
  ...
  for (const entry of readdirSync(base, { withFileTypes: true })) {
    if (
      ...
      entry.name !== "stage-2-requalification-r7"
    ) {
...
const temporaryRoot = mkdtempSync(join(os.tmpdir(), "oracle-stage2-r7-static-"));
try {
...
assert.doesNotThrow(() =>
  validateFinalIdentity({
    ...certificateBound,
    evidence: {
      manifest: "Oracle.Stage2RequalificationR7EvidenceManifest.json",
...
assert.match(
  cleanupScript,
  /exact generated R7 certificate thumbprint was not found/u
);
assert.match(
  cleanupScript,
...
assert.match(
  cleanupScript,
  /ORACLE_R7_ROOT_REMOVAL_PROCESS=[\s\S]*ConvertTo-Json -Compress -Depth 5/u
);
assert.match(
  cleanupScript,
...
const teardownExpectedSubject =
  "CN=Oracle Stage 2 Requalification R7 Local Test Signing - NOT PRODUCTION";
const syntheticMy = {
...
const r7PackageEntryPoints = Object.keys(packageJson.scripts).filter((name) =>
  name.startsWith("sprint-30-5:stage-2:r7:")
);
assert.deepEqual(
  r7PackageEntryPoints,
  [],
  "R7 must not modify the candidate package.json or expose independently runnable phases."
);
assert.equal(
...
assert.ok(
  ...
  "R7 must not expose a standalone attempt-preparation entry point."
);
...
assert.throws(
  () =>
    validateAuthorityIdentity({
      authorityId: "authority-r7-other",
      attemptId: fixtureAttemptId,
    }),
  /must exactly equal/
);
const previousAttemptId = "r7-20260728T120000000Z-01234567";
...
assert.match(harnessCore, /scripts\/verify-accessible-color-contract\.mjs/u);
assert.match(harnessCore, /eslint\.config\.mjs/u);
assert.match(executorSource, /stage-2-requalification-r7-failure-outcome\.json/u);
assert.match(
  executorSource,
  /runNpmLogged\("accessible-color-contract", \[[\s\S]*?"accessibility:color:verify"/u
);
...
assert.ok(
  ...
  "R7 must check ambient configuration pre-authority, pre-build and post-build."
);
...
assert.match(executorSource, /Migration 011 does not match the fixed R7 candidate binding/u);
assert.match(executorSource, /Migration 012 does not match the fixed R7 candidate binding/u);
assert.match(
  readFileSync(join(import.meta.dirname, "execution-core.mjs"), "utf8"),
  /FOUNDER-AUTHORISED-STAGE-2-R7-SINGLE-ATTEMPT/u
);
assert.match(
  executorSource,
...
delete directInvocationEnvironment.ORACLE_STAGE2_R7_GOVERNED_WRAPPER;
...
assert.equal(contract.authority.stage3, "fresh-requalification-required-not-authorised-by-r7");
assert.equal(contract.authority.stage4, "fresh-requalification-required-not-authorised-by-r7");
assert.throws(
  () => assertRequiredTool("oracle-r7-tool-that-does-not-exist.exe"),
...
assert.match(executorSource, /Version="0\\\.1\\\.4\\\.0"/u);
assert.doesNotMatch(executorSource, /Version="0\\\.1\\\.2\\\.0"/u);
console.log(
  JSON.stringify(
    {
      result: "PASS",
