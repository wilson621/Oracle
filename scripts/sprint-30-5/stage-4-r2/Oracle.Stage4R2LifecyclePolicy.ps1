...
function Get-OracleStage4R2LifecyclePhases {
  @(
...
function New-OracleStage4R2LifecycleState {
  [pscustomobject][ordered]@{ index=0; terminal=$false; completed=[Collections.Generic.List[string]]::new() }
}
function Assert-OracleStage4R2NextLifecyclePhase([object]$State,[string]$Phase) {
  $phases=@(Get-OracleStage4R2LifecyclePhases)
...
function Move-OracleStage4R2Lifecycle([object]$State,[string]$Phase) {
  Assert-OracleStage4R2NextLifecyclePhase $State $Phase
  $State.completed.Add($Phase)
  $State.index=[int]$State.index+1
  if([int]$State.index -eq @(Get-OracleStage4R2LifecyclePhases).Count){$State.terminal=$true}
  $State
}
function Get-OracleStage4R2TeardownObligations([string[]]$CompletedPhases) {
