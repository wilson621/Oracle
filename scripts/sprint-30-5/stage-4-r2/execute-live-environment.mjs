...
if (mode === "qualification") {
  assert.match(basename(attemptRoot), new RegExp(contract.identity.attemptPattern, "u"));
  ...
  if (authority.attemptId !== basename(attemptRoot) || authority.consumed !== true) throw new Error("Qualification controller authority binding failed.");
...
try {
  if (teardownOnly) {
    ...
    config = config.replace(/project_id = ".*"/u, 'project_id = "oracle-stage4-r2-disposable"')
    ...
    for (const expected of ['project_id = "oracle-stage4-r2-disposable"', 'site_url = "http://127.0.0.1:4314"', 'additional_redirect_urls = ["http://127.0.0.1:4314/auth/callback"]', "enable_confirmations = true", "minimum_password_length = 8"]) if (!config.includes(expected)) throw new Error(`Supabase configuration binding is absent: ${expected}`);
    writeFileSync(configPath, config);
    const migrations = join(providerRoot, "supabase", "migrations"); mkdirSync(migrations, { recursive: false });
    const migrationNames = readdirSync(join(repositoryRoot, "database")).filter(name => /^\d{3}_.+\.sql$/u.test(name)).sort(codePointCompare);
    ...
    const sharedEnvironment = { ...governedEnvironment, SUPABASE_SECRET_KEY: service, ORACLE_WEB_SESSION_SECRET: webSessionSecret, NODE_ENV: "production", ORACLE_STAGE4_EXECUTION_MODE: mode, ORACLE_STAGE4_PROVIDER_URL: api, ORACLE_STAGE4_MAILPIT_URL: mail, ORACLE_STAGE4_ANON_KEY: anon, ORACLE_STAGE4_OUTPUT: output, ORACLE_STAGE4_JOURNEY_OUTPUT: output, ORACLE_STAGE4_ATTEMPT_ROOT: attemptRoot, ORACLE_STAGE4_NODE_PATH: node };
    if (mode === "qualification" || installedDevelopmentRehearsal) {
      ...
      const installedController = join(import.meta.dirname, "Invoke-OracleStage4R2InstalledPackageJourney.ps1");
...
async function performCleanup() {
  ...
  try {
    const current = run("provider-presence-before-stop", docker, ["ps", "-a", "--filter", "name=oracle-stage4-r2-disposable", "--format", "{{.ID}}"]);
  ...
  for (const [label, args] of [["container-residue", ["ps", "-a", "--filter", "name=oracle-stage4-r2-disposable", "--format", "{{.ID}}"]], ["volume-residue", ["volume", "ls", "--filter", "name=oracle-stage4-r2-disposable", "--format", "{{.Name}}"]], ["network-residue", ["network", "ls", "--filter", "name=oracle-stage4-r2-disposable", "--format", "{{.Name}}"]]]) {
...
function verifyProviderImagesAndRoute() {
  const ids = run("provider-container-list", docker, ["ps", "--filter", "name=oracle-stage4-r2-disposable", "--format", "{{.ID}}"] ).stdout.trim().split(/\r?\n/u).filter(Boolean);
