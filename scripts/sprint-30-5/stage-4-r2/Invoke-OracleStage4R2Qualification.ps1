...
$preflightPath=[IO.Path]::GetFullPath($PreflightRecord);[void](Assert-OracleStage4R2NoReparseTraversal $preflightPath $preflightRoot)
...
$current=Invoke-OracleStage4R2PreAuthorityChecks $root $contract $PreparationCommit $PreparationTree
...
$identity=$FounderGrantId.Substring('founder-stage4-r2-grant-'.Length);$attemptId="stage4-r2-$identity";$authorityId="authority-stage4-r2-$identity"
...
foreach($path in @($authorityRoot,$attemptRoot)){[void](Assert-OracleStage4R2NoReparseTraversal $path $artifactRoot)};[void](Assert-OracleStage4R2NoReparseTraversal $repositoryEvidence $evidenceRoot)
...
[IO.Directory]::CreateDirectory($authorityRoot)|Out-Null;[void](Assert-OracleStage4R2NoReparseTraversal $authorityRoot $artifactRoot)
...
try{
  try{
    ...
    try{$controller=Invoke-OracleStage4R2NativeProcess ([string]$current.tools.node) @((Join-Path $PSScriptRoot 'execute-live-environment.mjs')) $root;Write-CreateOnlyJson $controllerLog $controller}catch{$record=$_.Exception.Data['OracleStage4R2ProcessRecord'];if($null-ne$record -and -not(Test-Path -LiteralPath $controllerLog)){Write-CreateOnlyJson $controllerLog $record};throw}
    ...
    $journeyText=Get-Content -Raw -LiteralPath $journeyPath;Assert-OracleStage4R2SecretFreeText -Text $journeyText -KnownSecrets @();$journey=$journeyText|ConvertFrom-Json;[void](Assert-OracleStage4R2JourneyRecord $journey)
    ...
    $payloadItems=@(Get-Inventory $payloadRoot);Write-CreateOnlyJson (Join-Path $payloadRoot 'payload-manifest.json') ([ordered]@{contract='oracle.sprint-30-5.stage-4-r2-payload-manifest';authorityId=$authorityId;attemptId=$attemptId;files=$payloadItems})
    $archive=Join-Path $attemptRoot 'Oracle.Sprint30.5.Stage4R2QualificationEvidence.zip';Add-Type -AssemblyName System.IO.Compression.FileSystem;[IO.Compression.ZipFile]::CreateFromDirectory($payloadRoot,$archive,[IO.Compression.CompressionLevel]::Optimal,$false)
    ...
    $finalItems=@(Get-Inventory $attemptRoot @('final-evidence-manifest.json'));Write-CreateOnlyJson (Join-Path $attemptRoot 'final-evidence-manifest.json') ([ordered]@{contract='oracle.sprint-30-5.stage-4-r2-final-evidence-manifest';result='passed-awaiting-founder-review';authorityId=$authorityId;attemptId=$attemptId;files=$finalItems})
