...
. (Join-Path $scriptRoot "Oracle.Stage4R4ActivationPolicy.ps1")
...
$contract = Get-Content -LiteralPath (
  Join-Path $scriptRoot "Oracle.Stage4R4Contract.json"
) -Raw | ConvertFrom-Json
Assert-OracleStage4R4ApplicationActivationContract -Contract $contract
...
Assert-Rejected {
  Assert-OracleStage4R4ApplicationActivationContract -Contract $invalidContract
} "Explorer exit-code contract"

$aumid = "Oracle.Platform.LocalCertification_test!Oracle"
$success = Invoke-OracleStage4R4ApplicationActivation `
...
Assert-OracleStage4R4ApplicationActivationSucceeded -Result $success

$availability = Test-OracleStage4R4ApplicationActivationApi -ProbeRunner {
...
Assert-Rejected {
  Invoke-OracleStage4R4ApplicationActivation -AppUserModelId "not-an-aumid"
} "malformed AUMID"
Assert-Rejected {
  $failure = Invoke-OracleStage4R4ApplicationActivation `
    -AppUserModelId $aumid -ActivationRunner {
      [pscustomobject]@{ HResult = "0x80004005"; ProcessId = 0 }
    }
  Assert-OracleStage4R4ApplicationActivationSucceeded -Result $failure
} "HRESULT failure"
Assert-Rejected {
  $zeroPid = Invoke-OracleStage4R4ApplicationActivation `
    -AppUserModelId $aumid -ActivationRunner {
      [pscustomobject]@{ HResult = "0x00000000"; ProcessId = 0 }
    }
  Assert-OracleStage4R4ApplicationActivationSucceeded -Result $zeroPid
} "zero process ID"
Assert-Rejected {
  $runnerError = Invoke-OracleStage4R4ApplicationActivation `
    -AppUserModelId $aumid -ActivationRunner {
      throw "simulated activation error"
    }
  Assert-OracleStage4R4ApplicationActivationSucceeded -Result $runnerError
} "runner error"
Assert-Rejected {
  $incomplete = Invoke-OracleStage4R4ApplicationActivation `
    -AppUserModelId $aumid -ActivationRunner {
      [pscustomobject]@{ HResult = "0x00000000" }
    }
  Assert-OracleStage4R4ApplicationActivationSucceeded -Result $incomplete
} "incomplete native result"
Assert-Rejected {
  $nullResult = Invoke-OracleStage4R4ApplicationActivation `
    -AppUserModelId $aumid -ActivationRunner { $null }
  Assert-OracleStage4R4ApplicationActivationSucceeded -Result $nullResult
} "null native result"
Assert-Rejected {
  $missing = $success | Select-Object * -ExcludeProperty hresult
  Assert-OracleStage4R4ApplicationActivationSucceeded -Result $missing
