...
const contract = JSON.parse(fs.readFileSync(path.join(harnessRoot, "Oracle.Stage4R4Contract.json"), "utf8"));
if (contract.status !== "founder-authorised-execution-enabled" || contract.transfer.executionAuthorised !== true || contract.executionAuthority.founderAuthorisedQualificationExecution !== true || contract.executionAuthority.authorityCreationPermitted !== true || contract.executionAuthority.qualificationAttemptPermitted !== true) {
  throw new Error("Stage 4 R4 transfer preparation is not Founder-authorised by the bound contract.");
}
const args = new Map();
...
const identity = transferId.slice("transfer-stage4-r4-".length);
...
const executionManifestPath = path.join(harnessRoot, "Oracle.Stage4R4ExecutionManifest.json");
const manifest = {
...
const custody = {
  ...
  source: "accepted-r4-execution-baseline",
