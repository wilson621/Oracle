...
assert.equal(contract.acceptedPreparation.commit, "48ddf5fcd448c79026d8cde6a245ec0ba4d37e8e");
assert.equal(contract.acceptedPreparation.tree, "8d67dcb677099add358eec62a08dd475a4badc1a");
...
assert.equal(sha256(join(repositoryRoot, "scripts/sprint-30-5/stage-4-r4/Oracle.Stage4R4PreparationManifest.json")), contract.acceptedPreparation.preparationManifestSha256);
...
assert.equal(contract.historicalEvidenceBindings.length, 31);
...
const executionManifest = JSON.parse(readFileSync(join(harnessRoot, "Oracle.Stage4R4ExecutionManifest.json"), "utf8"));
assert.equal(executionManifest.contract, "oracle.sprint-30-5.stage-4-r4-execution-manifest");
...
const inventory = readdirSync(harnessRoot)
  .filter(name => name !== "Oracle.Stage4R4ExecutionManifest.json")
...
console.log(JSON.stringify({
  result: "passed",
  classification: "STAGE-4-R4-EXECUTION-OVERLAY-VALIDATION",
