...
try {
  if (teardownOnly) {
    ...
    config = config.replace(/project_id = ".*"/u, 'project_id = "oracle-stage4-r1-disposable"')
    ...
    for (const expected of ['project_id = "oracle-stage4-r1-disposable"', 'site_url = "http://127.0.0.1:4314"', 'additional_redirect_urls = ["http://127.0.0.1:4314/auth/callback"]', "enable_confirmations = true", "minimum_password_length = 8"]) if (!config.includes(expected)) throw new Error(`Supabase configuration binding is absent: ${expected}`);
    ...
    await waitForUrl("http://127.0.0.1:4314/auth", server); mark("web-server-ready", { endpoint: "http://127.0.0.1:4314" });
    const journeyArgs = [join(import.meta.dirname, "run-live-journey.mjs")];
    const journey = spawnSync(node, journeyArgs, { cwd: repositoryRoot, encoding: "utf8", shell: false, maxBuffer: 64 * 1024 * 1024, env: { ...env, ORACLE_STAGE4_EXECUTION_MODE: mode, ORACLE_STAGE4_PROVIDER_URL: api, ORACLE_STAGE4_WEB_ORIGIN: "http://127.0.0.1:4314", ORACLE_STAGE4_MAILPIT_URL: mail, ORACLE_STAGE4_ANON_KEY: anon, ORACLE_STAGE4_OUTPUT: output } });
...
async function performCleanup() {
  ...
  try {
    const current = run("provider-presence-before-stop", docker, ["ps", "-a", "--filter", "name=oracle-stage4-r1-disposable", "--format", "{{.ID}}"]);
  ...
  for (const [label, args] of [["container-residue", ["ps", "-a", "--filter", "name=oracle-stage4-r1-disposable", "--format", "{{.ID}}"]], ["volume-residue", ["volume", "ls", "--filter", "name=oracle-stage4-r1-disposable", "--format", "{{.Name}}"]], ["network-residue", ["network", "ls", "--filter", "name=oracle-stage4-r1-disposable", "--format", "{{.Name}}"]]]) {
...
function verifyProviderImagesAndRoute() {
  const ids = run("provider-container-list", docker, ["ps", "--filter", "name=oracle-stage4-r1-disposable", "--format", "{{.ID}}"] ).stdout.trim().split(/\r?\n/u).filter(Boolean);
