...
import { assertSafeCreateOnly, contract, redactEvidence, repositoryRoot, sha256, validateAcceptedBindings, validateExecutionIdentity, validateProcessEnvelope, validateProviderEndpoint, writeJsonAtomicCreateOnly } from "./stage4-core.mjs";
...
assert.equal(run("git",["diff","--name-only",contract.repository.acceptedCandidateCommit,"HEAD","--",...contract.repository.productPaths]).stdout.trim(),"","Accepted R3 product drifted.");
for(const f of ["stage4-core.mjs","run-live-journey.mjs","run-live-development-rehearsal.mjs","execute-live-environment.mjs","verify-preparation.mjs"])run(process.execPath,["--check",join(import.meta.dirname,f)]);
...
assert.doesNotMatch(ex,/Import-Certificate|Add-AppxPackage|certutil\.exe/u);assert.match(pre,/Invoke-OracleStage4R1PreAuthorityChecks/u);assert.match(shared,/historicalEvidenceBindings/u);assert.match(shared,/DestinationPrefix -in @\('0\.0\.0\.0\/0','::\/0'\)/u);assert.match(ctl,/ORACLE_STAGE4_EXECUTION_MODE/u);assert.match(ctl,/ORACLE_STAGE4_AUTHORITY_RECORD/u);assert.match(ctl,/teardown-only/u);assert.match(ctl,/environment-result\.json/u);
