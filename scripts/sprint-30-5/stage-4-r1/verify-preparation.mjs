...
import { assertSafeCreateOnly, assertSupabaseOfflineEnvironment, buildSupabaseJsonArguments, contract, createGovernedEnvironment, parseSupabaseJsonObject, redactEvidence, repositoryRoot, sha256, validateAcceptedBindings, validateExecutionIdentity, validateApprovedTool, validateProcessEnvelope, validateProviderEndpoint, validateSupabaseMachineReadableOutputPolicy, validateSupabaseOfflinePolicy, writeJsonAtomicCreateOnly } from "./stage4-core.mjs";
...
const supabaseOutputPolicy=validateSupabaseMachineReadableOutputPolicy();assert.deepEqual(supabaseOutputPolicy.commands,["start","status"]);assert.deepEqual(buildSupabaseJsonArguments("start",["--workdir","fixture"]),["start","--workdir","fixture","--output","json"]);assert.deepEqual(parseSupabaseJsonObject('{"result":"passed"}',"fixture"),{result:"passed"});
...
for(const f of ["stage4-core.mjs","run-live-journey.mjs","run-live-development-rehearsal.mjs","execute-live-environment.mjs","verify-provider-output-contract.mjs","verify-preparation.mjs"])run(approvedTools.node.path,["--check",join(import.meta.dirname,f)]);
run(approvedTools.node.path,[join(import.meta.dirname,"verify-provider-output-contract.mjs")],governedEnvironment);
...
for(const command of ["init","stop"])assert.match(ctl,new RegExp(`supabaseCli, "${command}"`,"u"));assert.match(shared,/supabaseCli,'--version'/u);
for(const command of ["start","status"])assert.match(ctl,new RegExp(`buildSupabaseJsonArguments\\("${command}"`,"u"));assert.match(ctl,/parseSupabaseJsonObject\(started\.stdout/u);assert.match(ctl,/parseSupabaseJsonObject\(status\.stdout/u);assert.doesNotMatch(ctl,/JSON\.parse\((?:started|status)\.stdout\)/u);
