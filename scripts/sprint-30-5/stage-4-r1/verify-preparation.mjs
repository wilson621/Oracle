...
assert.doesNotMatch(ex,/Import-Certificate|Add-AppxPackage|certutil\.exe/u);assert.match(pre,/Invoke-OracleStage4R1PreAuthorityChecks/u);assert.match(shared,/historicalEvidenceBindings/u);assert.doesNotMatch(shared,/Get-Command/u);assert.match(shared,/Resolve-OracleStage4R1BoundTool/u);assert.match(shared,/ancestryReparseFree/u);assert.match(shared,/ConvertTo-OracleStage4R1ProcessEvidenceArray/u);assert.doesNotMatch(shared,/processEvidence\s*=\s*@\(\$processEvidence\)/u);assert.match(shared,/DestinationPrefix -in @\('0\.0\.0\.0\/0','::\/0'\)/u);assert.match(ctl,/ORACLE_STAGE4_EXECUTION_MODE/u);assert.match(ctl,/ORACLE_STAGE4_AUTHORITY_RECORD/u);assert.match(ctl,/ORACLE_STAGE4_POWERSHELL_PATH/u);assert.match(ctl,/ORACLE_STAGE4_TASKKILL_PATH/u);assert.match(ctl,/governedEnvironment/u);assert.match(ctl,/filter\(\(\[key\]\) => key\.toLowerCase\(\) !== "path"\)/u);assert.match(ctl,/teardown-only/u);assert.match(ctl,/environment-result\.json/u);
