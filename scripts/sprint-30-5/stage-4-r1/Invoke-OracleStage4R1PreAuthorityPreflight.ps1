...
$record=[ordered]@{schemaVersion='1.1.0';contract='oracle.sprint-30-5.stage-4-r1-pre-authority-preflight';classification=@($contract.preflight.classification);result='passed';collectedAtUtc=(Get-Date).ToUniversalTime().ToString('o');branch=$checks.branch;preparationCommit=$checks.preparationCommit;preparationTree=$checks.preparationTree;acceptedCandidateCommit=$checks.acceptedCandidateCommit;acceptedCandidateTree=$checks.acceptedCandidateTree;productDrift=@($checks.productDrift);executionRoute='loopback-only-runtime-verified';tools=$checks.tools;versions=$checks.versions;docker=$checks.docker;historicalBindingsVerified=$checks.historicalBindingsVerified;providerImagesVerified=$checks.providerImagesVerified;portsAvailable=$checks.portsAvailable;providerResidue=$checks.providerResidue;processEvidence=@($checks.processEvidence);authorityCreated=$false;attemptCreated=$false;hostMutation=$false;qualificationEvidence=$false}
