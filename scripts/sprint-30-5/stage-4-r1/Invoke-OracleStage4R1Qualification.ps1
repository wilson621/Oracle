...
if([string]$contract.status -cne 'ready-for-founder-execution-decision'){throw 'Stage 4 R1 preparation is not admitted for execution.'}
if($FounderAuthorityToken -cne [string]$contract.founderAuthorityToken){throw 'Exact Founder execution authority token is absent.'}
...
$preflightPath=[IO.Path]::GetFullPath($PreflightRecord);[void](Assert-OracleStage4R1NoReparseTraversal $preflightPath $preflightRoot)
...
$current=Invoke-OracleStage4R1PreAuthorityChecks $root $contract $PreparationCommit $PreparationTree
if([string]$current.tools.node -cne [string]$preflight.tools.node -or [string]$current.tools.npmCli -cne [string]$preflight.tools.npmCli -or [string]$current.tools.docker -cne [string]$preflight.tools.docker -or [string]$current.acceptedCandidateCommit -cne [string]$preflight.acceptedCandidateCommit){throw 'Current pre-authority state differs from the bound preflight.'}
$identity=$FounderGrantId.Substring('founder-stage4-r1-grant-'.Length);$attemptId="stage4-r1-$identity";$authorityId="authority-stage4-r1-$identity"
...
foreach($path in @($authorityRoot,$attemptRoot)){[void](Assert-OracleStage4R1NoReparseTraversal $path $artifactRoot)};[void](Assert-OracleStage4R1NoReparseTraversal $repositoryEvidence $evidenceRoot)
...
[IO.Directory]::CreateDirectory($authorityRoot)|Out-Null;[void](Assert-OracleStage4R1NoReparseTraversal $authorityRoot $artifactRoot)
...
Write-CreateOnlyJson $authorityPath ([ordered]@{contract='oracle.sprint-30-5.stage-4-r1-authority';founderGrantId=$FounderGrantId;authorityId=$authorityId;attemptId=$attemptId;timestampUtc=$timestamp;preflightSha256=$observedPreflight;preparationCommit=$PreparationCommit;preparationTree=$PreparationTree;consumed=$true})
New-Item -ItemType Directory -Path $attemptRoot -ErrorAction Stop|Out-Null
$state=New-OracleStage4R1LifecycleState;$failure=$null;$index=0
function Publish-Lifecycle([string]$Phase,$Details){$script:index++;Write-CreateOnlyJson (Join-Path $attemptRoot ("lifecycle\{0:d3}-{1}.json" -f $script:index,$Phase)) ([ordered]@{contract='oracle.sprint-30-5.stage-4-r1-lifecycle';authorityId=$authorityId;attemptId=$attemptId;phase=$Phase;recordedAtUtc=(Get-Date).ToUniversalTime().ToString('o');details=$Details});[void](Move-OracleStage4R1Lifecycle $state $Phase)}
...
$env:ORACLE_STAGE4_EXECUTION_MODE='qualification';$env:ORACLE_STAGE4_ATTEMPT_ROOT=$attemptRoot;$env:ORACLE_STAGE4_JOURNEY_OUTPUT=$journeyPath;$env:ORACLE_STAGE4_DOCKER_PATH=[string]$current.tools.docker;$env:ORACLE_STAGE4_NPM_CLI_PATH=[string]$current.tools.npmCli;$env:ORACLE_STAGE4_NODE_PATH=[string]$current.tools.node;$env:ORACLE_STAGE4_AUTHORITY_RECORD=$authorityPath;$env:ORACLE_STAGE4_NETWORK_ISOLATION_ADMITTED=$(if([bool]$current.networkIsolated){'1'}else{throw 'Current host network isolation was not admitted.'})
try{
  try{
    Publish-Lifecycle 'authority-consumed' @{authorityId=$authorityId;founderGrantId=$FounderGrantId}
    Publish-Lifecycle 'baseline-verified' @{commit=$PreparationCommit;tree=$PreparationTree;preflightSha256=$observedPreflight;acceptedCandidateCommit=$contract.repository.acceptedCandidateCommit;acceptedCandidateTree=$contract.repository.acceptedCandidateTree}
    try{$controller=Invoke-OracleStage4R1NativeProcess ([string]$current.tools.node) @((Join-Path $PSScriptRoot 'execute-live-environment.mjs')) $root;Write-CreateOnlyJson $controllerLog $controller}catch{$record=$_.Exception.Data['OracleStage4R1ProcessRecord'];if($null-ne$record -and -not(Test-Path -LiteralPath $controllerLog)){Write-CreateOnlyJson $controllerLog $record};throw}
    ...
    $journeyText=Get-Content -Raw -LiteralPath $journeyPath;Assert-OracleStage4R1SecretFreeText -Text $journeyText -KnownSecrets @();$journey=$journeyText|ConvertFrom-Json;[void](Assert-OracleStage4R1JourneyRecord $journey)
    ...
    Publish-Lifecycle 'source-built' @{candidateCommit=$contract.repository.acceptedCandidateCommit;candidateTree=$contract.repository.acceptedCandidateTree}
    ...
    $payloadItems=@(Get-Inventory $payloadRoot);Write-CreateOnlyJson (Join-Path $payloadRoot 'payload-manifest.json') ([ordered]@{contract='oracle.sprint-30-5.stage-4-r1-payload-manifest';authorityId=$authorityId;attemptId=$attemptId;files=$payloadItems})
    $archive=Join-Path $attemptRoot 'Oracle.Sprint30.5.Stage4R1QualificationEvidence.zip';Add-Type -AssemblyName System.IO.Compression.FileSystem;[IO.Compression.ZipFile]::CreateFromDirectory($payloadRoot,$archive,[IO.Compression.CompressionLevel]::Optimal,$false)
    ...
    $finalItems=@(Get-Inventory $attemptRoot @('final-evidence-manifest.json'));Write-CreateOnlyJson (Join-Path $attemptRoot 'final-evidence-manifest.json') ([ordered]@{contract='oracle.sprint-30-5.stage-4-r1-final-evidence-manifest';result='passed-awaiting-founder-review';authorityId=$authorityId;attemptId=$attemptId;files=$finalItems})
...
}finally{foreach($name in @('ORACLE_STAGE4_EXECUTION_MODE','ORACLE_STAGE4_ATTEMPT_ROOT','ORACLE_STAGE4_JOURNEY_OUTPUT','ORACLE_STAGE4_DOCKER_PATH','ORACLE_STAGE4_NPM_CLI_PATH','ORACLE_STAGE4_NODE_PATH','ORACLE_STAGE4_AUTHORITY_RECORD','ORACLE_STAGE4_NETWORK_ISOLATION_ADMITTED')){Remove-Item "Env:$name" -ErrorAction SilentlyContinue}}
...
[pscustomobject]@{result='passed-awaiting-founder-review';authorityId=$authorityId;attemptId=$attemptId;attemptRoot=$attemptRoot;repositoryEvidence=$repositoryEvidence;archiveSha256=$archiveHash;finalEvidenceManifestSha256=$finalManifestHash}|ConvertTo-Json -Depth 8